...
- `npm run build` produces `dist/drora.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
drora plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.drora/cli/plugins`:
...
- `marketplaces/drora-plugins-official/`: bundled and CDN partitions plus the merged metadata for the single official marketplace.

This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and Drora Guide content plugins are default-enabled and appear as `browser-use@drora-plugins-official`, `document-skills@drora-plugins-official`, `skill-creator@drora-plugins-official`, and `drora-guide@drora-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@drora-plugins-official`, `android-emulator@drora-plugins-official`, and `restore-legacy-sessions@drora-plugins-official`, are discovered by drora but stay disabled until the user enables them.

```sh
drora plugins list
drora plugins enable ios-simulator
drora plugins disable browser-use
drora plugins enable restore-legacy-sessions
drora plugins disable ios-simulator
```
...
Supported fields in the current drora plugin surface:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${DRORA_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${DRORA_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${DRORA_PLUGIN_DATA}",
...
Only environment variables with the `DRORA_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting drora-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

drora reads MCP servers from the main JSON config. The default user config path is `~/.drora/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits drora's environment plus any `env` overrides.
...
drora reads hooks from the same main JSON config file as MCP, usually `~/.drora/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
