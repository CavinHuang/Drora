import type { HttpClientPort, HttpClientRunOptions, TraceContext } from "@drora/contracts";
import { resolveBigModelApiOrigin } from "@drora/shared";

const ZAI_API_HOST = "https://api.z.ai";
const JSON_CONTENT_TYPE = "application/json";
const DRORA_API_KEY_NAME = "drora-api-key";
...
async function resolveBizApiKey(
  ...
  const keyEntry =
    keys.find((item) => item.name === DRORA_API_KEY_NAME) ??
    (await requestRemoteData<RemoteApiKeySummary>(
      input.httpClient,
      {
        body: new TextEncoder().encode(JSON.stringify({ name: DRORA_API_KEY_NAME })),
        headers: createBizAuthHeaders(input.authorization),
        method: "POST",
        trace: input.trace,
        url: listUrl,
