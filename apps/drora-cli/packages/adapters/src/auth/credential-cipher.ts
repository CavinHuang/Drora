...
const CREDENTIAL_SECRET_ENV_KEY = "DRORA_CREDENTIAL_SECRET";

export interface DroraCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface DroraCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createDroraCredentialCipher(
  options: DroraCredentialCipherOptions = {},
): DroraCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedDroraCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedDroraCredentialValue(value: string): boolean {
...
function resolveCredentialSecret(env: Record<string, string | undefined>): string {
  ...
  return `drora-credential-fallback:${platform()}:${homedir()}:${username}`;
}
