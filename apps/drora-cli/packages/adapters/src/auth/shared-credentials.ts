...
import { atomicWritePrivateTextFile, backupCorruptFile, withFileLock } from "@drora/shared/node";
import { createDroraCredentialCipher, type DroraCredentialCipher } from "./credential-cipher.js";

const DRORA_DATA_BASE_DIR_ENV_KEY = "DRORA_DATA_BASE_DIR";
...
export const SHARED_DRORA_CREDENTIAL_KEYS = {
  ...
  droraJwtToken: "drorajwttoken",
} as const;

export interface SharedDroraCredentialStoreOptions {
  baseDir?: string;
  cipher?: DroraCredentialCipher;
...
export interface SharedDroraCredentialStore {
...
export function createSharedDroraCredentialStore(
  options: SharedDroraCredentialStoreOptions = {},
): SharedDroraCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedDroraCredentialsPath(options);
  const cipher = options.cipher ?? createDroraCredentialCipher({ env });
  ...
  return {
    ...
    async clearZaiLoginCredentials(): Promise<void> {
      await mutateRawCredentialRecord(filePath, async (rawCredentials) => {
        const activeProviderRaw = rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.activeProvider];
        const activeProvider = activeProviderRaw ? cipher.decrypt(activeProviderRaw) : null;
        delete rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.zaiAccessToken];
        delete rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.zaiRefreshToken];
        delete rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.zaiUserInfo];
        delete rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.droraJwtToken];
        if (activeProvider === ZAI_PROVIDER_ID) {
          delete rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.activeProvider];
        }
      });
    },
    ...
    async saveZaiLoginCredentials(payload: ZaiLoginCredentialPayload): Promise<void> {
      ...
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
        rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.activeProvider] =
          encryptedCredentials.activeProvider;
        rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.zaiAccessToken] =
          encryptedCredentials.accessToken;
        rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.droraJwtToken] = encryptedCredentials.jwtToken;
        rawCredentials[SHARED_DRORA_CREDENTIAL_KEYS.zaiUserInfo] = encryptedCredentials.userInfo;
      });
    },
  };
}

export function loadSharedDroraCredentialSync(
  key: string,
  options: SharedDroraCredentialStoreOptions = {},
): string | undefined {
  const filePath = resolveSharedDroraCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createDroraCredentialCipher({ env: options.env });
...
export function resolveSharedDroraCredentialsPath(
  options: SharedDroraCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[DRORA_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".drora", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared Drora credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared Drora credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}
