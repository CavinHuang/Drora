// Env Config Adapter - Parse the intentionally small DRORA_* environment surface.
...
const DEFAULT_PREFIX = "DRORA_";

/**
 * Parse DRORA_* environment variables into config
 */
...
export function getToolConcurrencyConfig(): { maxConcurrency: number } {
  return {
    maxConcurrency: normalizeNumber(process.env.DRORA_MAX_TOOL_CONCURRENCY ?? "10"),
  };
}
