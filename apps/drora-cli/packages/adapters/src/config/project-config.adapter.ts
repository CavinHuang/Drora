...
import type { McpServerConfig, RuntimeConfigPatch } from "@drora/contracts";
import {
    ...
    ...(hooks
      ? {
          hookCandidate: createWorkspaceHookSourceInput({
            ...
            // hooks 字段已由 loadFileConfig 经 DroraConfigFileSchema（shared 单源
...
function getProjectConfigBaseDir(path: string): string {
  ...
  return basename(configDirectory) === ".drora" ? dirname(configDirectory) : configDirectory;
}
