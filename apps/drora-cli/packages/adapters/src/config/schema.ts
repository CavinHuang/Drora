/* eslint-disable max-lines -- drora-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
...
export const LEGACY_CUA_PLUGIN_ID = "drora-cua@drora-plugins-official";
export const CANONICAL_CUA_PLUGIN_ID = "computer-use@drora-plugins-official";
...
export const DroraConfigFileSchema = z
  .object({
...
export type DroraConfigFile = z.infer<typeof DroraConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. Drora does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；Drora 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = DroraConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: DroraConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: DroraConfigFile["plugins"],
