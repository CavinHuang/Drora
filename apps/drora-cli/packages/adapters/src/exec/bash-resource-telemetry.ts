...
interface BashResourceTelemetryOptions {
  ...
  onComplete: (sample: DroraToolExecResource) => void;
  readContext?: () => Pick<DroraToolExecResource, "cliRssKb" | "systemFreeMemoryKb">;
}

/** 每条命令独占状态与探针失败预算；finish 同步封口，绝不等待遥测 IO。 */
export function createBashResourceTelemetry(options: BashResourceTelemetryOptions): {
  finish(exitKind: DroraToolExecResource["exitKind"]): void;
} {
