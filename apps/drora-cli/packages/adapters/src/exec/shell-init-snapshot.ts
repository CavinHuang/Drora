...
import { windowsPathToGitBashPath } from "@drora/contracts";
import type { ExecutionShellDialect } from "@drora/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_DRORA_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_DRORA_SHELL_INIT_SNAPSHOT",
  ];
}
