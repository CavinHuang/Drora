...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@drora/contracts";
import { LogLevel, LogLevelName } from "@drora/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.DRORA_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.DRORA_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".drora", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeDroraRuntimeEnv(env[DRORA_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `drora-${formatLocalLogDate(new Date())}.jsonl`;
}
