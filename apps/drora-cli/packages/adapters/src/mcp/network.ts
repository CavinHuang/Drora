...
import { sanitizeDroraRuntimeEnv } from "@drora/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeDroraRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.drora/server/node 启动，但登录环境 PATH 不含该目录，
