...
import { acquireFileLock } from "@drora/shared/node";
import { DRORA_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@drora/shared";
import type { SharedDroraCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === DRORA_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;
