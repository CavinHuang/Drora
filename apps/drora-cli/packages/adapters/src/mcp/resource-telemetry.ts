...
import { DRORA_MCP_RESOURCE_SAMPLE_INTERVAL_MS, type DroraMcpResourceSample } from "@drora/shared";
import {
  createProcessProbe,
  type ProcessProbe,
...
export interface McpResourceTelemetryOptions {
  arch: DroraMcpResourceSample["arch"];
  platform: DroraMcpResourceSample["platform"];
  now(): number;
  getProcesses(): McpResourceProcess[];
  onResourceSamples?(samples: DroraMcpResourceSample[]): void;
...
export function createMcpResourceTelemetry(options: McpResourceTelemetryOptions) {
  ...
  const sampleNow = async (): Promise<void> => {
    ...
    try {
      ...
      const intervalMs =
        previousAt === undefined
          ? DRORA_MCP_RESOURCE_SAMPLE_INTERVAL_MS
          : Math.max(1, sampledAt - previousAt);
      const next = new Map<string, Map<number, number>>();
      const groups = new Map<string, DroraMcpResourceSample>();
