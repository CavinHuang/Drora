...
import type { DroraMcpTelemetryEvent } from "@drora/shared";
import {
...
type McpTelemetryEvent = DroraMcpTelemetryEvent;
...
interface CreateMcpTelemetryTrackerOptions {
  arch?: DroraMcpTelemetryEvent["arch"];
  idSalt: string;
  now?: () => number;
  onEvent(event: McpTelemetryEvent): void;
  platform?: DroraMcpTelemetryEvent["platform"];
  randomId?: () => string;
...
export function createMcpTelemetryTracker(
  ...
  const arch = options.arch ?? (process.arch as DroraMcpTelemetryEvent["arch"]);
  const now = options.now ?? Date.now;
  const platform = options.platform ?? (process.platform as DroraMcpTelemetryEvent["platform"]);
