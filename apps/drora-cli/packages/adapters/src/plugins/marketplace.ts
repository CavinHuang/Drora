...
const DRORA_MANIFEST_PATH = join(".zcode-plugin", "plugin.json");
...
function buildMarketplaceGitEnv(
  ...
  const env = sanitizeDroraRuntimeEnv(sourceEnv);
  // marketplace 安装会启动 Git 子进程，不能只依赖父进程继承的 shell 代理。
  // 这里统一从 Drora 显式网络环境恢复 HTTP(S)/NO_PROXY/CA，避免安装按钮卡到协议超时。
...
export async function uninstallMarketplacePlugin(input: {
  ...
  storageRoot: string;
  removeCache?: boolean;
  /** `drora plugins uninstall --keep-data`：删安装缓存但保留 data/<plugin-id> 用户数据目录。 */
  keepData?: boolean;
...
async function clonePluginSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "drora-plugin-src-"));
  const args = ["clone"];
...
async function cloneMarketplaceSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "drora-marketplace-src-"));
...
async function execGitCommand(args: string[], signal?: AbortSignal): Promise<void> {
  ...
  try {
    ...
    const gitBinary = process.env.DRORA_GIT_BINARY?.trim() || "git";
...
function findPluginManifestPath(rootPath: string): string | null {
  for (const candidate of [DRORA_MANIFEST_PATH, CLAUDE_MANIFEST_PATH, CODEX_MANIFEST_PATH]) {
  ...
  for (const key of UNSUPPORTED_MANIFEST_FIELDS) {
    if (key in input.manifest) {
      input.diagnostics.push({
        ...
        message: `Plugin component is diagnostic-only in this Drora runtime: ${key}`,
