/*
 * Plugin `.mcp.json` 里 `auth: { type: "drora_official" }` 的严格解析与 provenance 生成。
 *
 * 单独成文件而不是留在 mcp.ts：官方鉴权的解析规则覆盖 http 与 stdio 两种传输方式，
 * 且与模板变量解析、传输层字段解析没有耦合——放一起只会让 mcp.ts 继续膨胀（它已到 max-lines 上限）。
 */
import type { McpOfficialProvenance, DroraOfficialMcpAuthConfig } from "@drora/contracts";
 ...
 */
export function parseDroraOfficialAuth(
  value: unknown,
  mcpKey: string,
): DroraOfficialMcpAuthConfig | undefined {
  if (value === undefined) return undefined;
  if (!isRecord(value)) {
    throw new Error(`MCP server ${mcpKey}: auth must be an object`);
  }
  // 精确值匹配，区分大小写；drora-official、drora_official_auth 等别名一律拒绝。
  if (value.type !== "drora_official") {
  ...
  return { type: "drora_official", provider: "jwt_token" };
}
