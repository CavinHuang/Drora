...
import type { SkillRoot, SkillSource } from "@drora/contracts";
...
export async function resolveDefaultSkillRoots(
  ...
  const includeDrora = options.includeDroraSkills ?? true;
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.drora` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.drora` 优先，后续同名按 root 顺序解析。
  return [
    root(join(baseDirectory, DRORA_DIR, SKILLS_DIR), scope, "drora", nextPriority()),
