...
Drora plugin for model-driven Android app development.

The package root is the plugin root:

```bash
drora --plugin-dir /absolute/path/to/android-emulator-plugin
```
...
Drora bundles this package as the official
`android-emulator@drora-plugins-official` plugin. Enable the plugin to project
its skill, slash command, and MCP server into the session.
At startup Drora copies the packaged plugin files into
`~/.drora/cli/plugins/cache/drora-plugins-official/android-emulator/0.1.0/`
and runs the MCP server from that cache directory. SEA builds rewrite the
official plugin's Drora manifest to launch the cached MCP server through Drora's
...
```json
{
  "plugins": {
    "enabledPlugins": {
      "android-emulator@drora-plugins-official": true
    },
    "options": {
      "android-emulator@drora-plugins-official": {
...
The plugin MCP server name is `android-emulator`. Drora normalizes that name for
...
implements the raw MCP tool names such as `android_preflight`; Drora maps
...
```bash
pnpm install
pnpm --filter @drora/android-emulator-plugin typecheck
pnpm --filter @drora/android-emulator-plugin test
pnpm --filter @drora/android-emulator-plugin build
```
