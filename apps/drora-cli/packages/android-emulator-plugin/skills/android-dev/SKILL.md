...
This skill assumes the MCP server is configured in drora as `android_emulator`. In drora, MCP tools are exposed to the model as `mcp__android_emulator__<tool>`.

If the server is configured with a different name, use the corresponding visible `mcp__<server>__...` tool names from the active drora tool list.
