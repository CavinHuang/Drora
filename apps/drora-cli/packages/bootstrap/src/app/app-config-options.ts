...
import { detectLocale, resolveLocale } from "@drora/i18n";
import type { RuntimeConfigPatch, SupportedLocale, UiLocale } from "@drora/contracts";
import type { DroraAppOptions } from "./types.js";

export function isMessageEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.DRORA_MESSAGE_ENABLED === "1" || env.DRORA_MESSAGE_ENABLED === "true";
}

export function createConfigCliOverrides(options: DroraAppOptions): RuntimeConfigPatch | undefined {
...
function detectAppLocale(options: DroraAppOptions): SupportedLocale | undefined {
