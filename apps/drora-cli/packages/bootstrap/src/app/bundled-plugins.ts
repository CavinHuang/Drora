...
import { writeBundledOfficialMarketplacePartitionSync } from "@drora/adapters";
import { DRORA_OFFICIAL_PLUGIN_MARKETPLACE, type Logger } from "@drora/contracts";
import { isDroraCuaInternalFeatureEnabled, DRORA_CUA_OFFICIAL_PLUGIN_ID } from "@drora/shared";
import {
    ...
    if (plugin.missingSeedPaths.length > 0) {
      warnCacheDegraded(input.logger, {
        error: Object.assign(
          ...
          { code: "DRORA_PLUGIN_SEED_INCOMPLETE" },
        ),
...
export function resolveOfficialPluginRoots(input: {
  ...
  // drora-cua 内置 plugin 默认不启用，由 feature flag 控制加载。在 seed/discovery 层门控
  // （而非只隐藏某个 UI 面），这样开关关闭时用户无法经 plugin 列表/marketplace/MCP 设置/CLI 命令看到它。
  if (!isDroraCuaInternalFeatureEnabled(input.env ?? process.env)) {
    suppressedBuiltins.add(DRORA_CUA_OFFICIAL_PLUGIN_ID);
  }
  ...
  const fallbackRoots = failedSeeds.flatMap((definition) => {
    ...
    if (`${definition.name}@${OFFICIAL_PLUGIN_MARKETPLACE}` === DRORA_CUA_OFFICIAL_PLUGIN_ID) {
      return [];
    }
