...
} from "@drora/adapters/exec";
import { createNodeFileSystemAdapter } from "@drora/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@drora/adapters/http";
import { createJimpImageProcessorAdapter } from "@drora/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@drora/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@drora/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@drora/adapters/context";
import { createNodeSkillAdapter } from "@drora/adapters/skills";
import { createMcpAdapter } from "@drora/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@drora/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveDroraRuntimeEnv } from "@drora/shared";
import {
...
import { resolveDroraCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveDroraBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadDroraAgentProfiles } from "../subagents.js";
...
export async function createDroraApp(options: DroraAppOptions): Promise<DroraApp> {
  if (!options?.providerRegistry) {
    throw new Error("createDroraApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("drora").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveDroraRuntimeEnv(options.env ?? process.env) === "development",
    );
    const droraSubagentProfileOutcome = await loadDroraAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const droraSubagentProfiles = droraSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: droraSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: droraSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...droraSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          droraSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("Drora runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("Drora runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).DRORA_MAILBOX_ROOT ?? "~/.drora/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveDroraBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveDroraCustomCommandPrompt(text, {
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("drora-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("drora-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new DroraAttachmentFaultError(DRORA_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new DroraAttachmentFaultError(DRORA_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("Drora app startup failed", error, {
