...
import type { PluginLoadOutcome } from "@drora/contracts";
...
import type {
  DroraApp,
  DroraAppOptions,
  DroraPluginSetResult,
  DroraPluginUninstallResult,
} from "./types.js";
...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    listPlugins: async (): Promise<PluginLoadOutcome> => listDroraPlugins(commonOptions()),
    setPluginEnabled: async (
      plugin: string,
      enabled: boolean,
    ): Promise<DroraPluginSetResult> => {
    ...
    uninstallPlugin: async (plugin: string): Promise<DroraPluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === DRORA_OFFICIAL_PLUGIN_MARKETPLACE) {
