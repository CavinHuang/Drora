...
export interface ProcessProviderRegistryRuntimeOptions {
  ...
  readonly standalone?: {
    ...
    readonly onBuiltinRefreshResult?: (event: DroraBuiltinRefreshEvent) => void;
  };
}
...
export async function startProcessProviderRegistryRuntime(
  ...
  if (!paths) {
    throw new Error("缺少进程 Provider Registry 的 Drora Built-in / Personal Config 路径");
  }

  const accountSource = new MutableAccountProviderConfigSource();
  const credentialStore = options.standalone
    ? (options.standalone.credentialStore ?? createSharedDroraCredentialStore({ env: { ...env } }))
    : undefined;
  let standaloneAccount: AccountProviderService | undefined;
  const bundledFile = options.standalone
    ? env[DRORA_BUILTIN_PROVIDER_BUNDLED_CONFIG_FILE_ENV]?.trim()
    : undefined;
  const runtime = new NodeProviderRegistryRuntime({
    ...paths,
    ...(bundledFile
      ? {
          droraBuiltinFilePath: bundledFile,
          droraBuiltinActiveFilePath: paths.droraBuiltinFilePath,
          droraBuiltinRemote: {
            controlFilePath: join(
              dirname(paths.droraBuiltinFilePath),
              "drora-builtin-refresh.json",
            ),
            resolveEndpointKey: () => resolveRuntimeDroraEndpointOrigin(env),
            fetchRelease: (endpointOrigin, signal) =>
              downloadDroraBuiltinRelease({
                endpointOrigin,
                signal,
                appVersion: DRORA_VERSION,
                platform: resolveDroraBuiltinClientPlatform(),
      ...
      : {}),
    onDroraBuiltinRefreshError: options.standalone?.onBuiltinRefreshError,
    accountSource,
    ...(credentialStore
      ? {
  ...
  const disposeRecovery = standaloneAccount
    ? runtime.onDidCheckDroraBuiltin(async () => {
        const [config, account] = await Promise.all([
          runtime.configService.read(),
          standaloneAccount!.read(),
        ]);
        if (config.droraBuiltinRevision !== account.basedOnDroraBuiltinRevision)
...
export function parseProcessAccountProviderConfigSnapshot(input: {
  ...
  const basedOnDroraBuiltinRevision = input.basedOnDroraBuiltinRevision.trim();
  if (!basedOnDroraBuiltinRevision) {
