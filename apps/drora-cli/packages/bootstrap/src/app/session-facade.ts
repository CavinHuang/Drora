import { updateUiLocaleInFileConfig, type ConfigResult } from "@drora/adapters/config";
import type { AgentRuntime } from "@drora/core";
import { resolveLocale } from "@drora/i18n";
import { normalizeModelSelection, type ModelSelection } from "@drora/provider";
import {
...
import type { PrepareUserExecutionBoundary, DroraApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<DroraApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<DroraApp["updateTargetStatus"]>
      >,
