...
import type { SharedDroraCredentialStore } from "@drora/adapters/auth";
import type { ProviderRuntimeHeadersPort } from "@drora/core";
import {
...
async function readStandaloneCodingPlanCatalog(
  ...
  if (config)
    return {
      droraBuiltinRevision: config.revision,
  ...
  const filePath = env[DRORA_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  if (!filePath) {
    throw new Error(`${DRORA_BUILTIN_PROVIDER_CONFIG_FILE_ENV} is required for login`);
  }
...
export async function resolveStandaloneCodingPlanProvider(
  ...
  if (matches.length !== 1) {
    throw new Error(
      `Drora Built-in Config 必须为 ${family} 声明唯一 Individual Coding Plan Provider`,
    );
  }
  return matches[0]!;
}
...
export async function readStandaloneAccountProviderConfigSnapshot(
  credentialStore: Pick<SharedDroraCredentialStore, "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
  config?: Pick<ProviderConfigLayerSnapshot, "revision" | "providers">,
  ...
  return createAccountProviderConfigSnapshot(catalog.droraBuiltinRevision, providers);
}
...
export function createStandaloneProviderRuntimeHeadersPort(
  credentialStore: Pick<SharedDroraCredentialStore, "load" | "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
