import type { DroraToolExecResource, BackgroundBashOutputResult } from "@drora/shared";
import type { AiSdkModelAdapter } from "@drora/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { DroraModelOption } from "@drora/shared";
import type { EffectiveModelSelectionResult } from "@drora/shared/model-selection";
export type { DroraModelOption } from "@drora/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { DroraInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@drora/contracts";
...
export type DroraAppRuntimeConfigInput = AgentRuntimeConfig;

export interface DroraAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: DroraAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: DroraToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@drora/contracts").TurnBackgroundAttribution;
...
export interface DroraPluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface DroraPluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: DroraInstalledPluginData | null;
}
...
export interface DroraApp {
  ...
  getCurrentModelOption?(): DroraModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): DroraModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@drora/shared").DroraSessionSubagentsResult>;
  ...
  listModels(): DroraModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<DroraPluginSetResult>;
  uninstallPlugin(plugin: string): Promise<DroraPluginUninstallResult>;
  /**
...
export interface RunDroraProtocolAgentOptions {
...
export interface ListDroraSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
