...
function createShellExpansionEnv(input: {
  ...
  set.DRORA_PROJECT_DIR = input.workingDirectory;
  if (input.sessionId) {
    set.CLAUDE_CODE_SESSION_ID = input.sessionId;
    set.CLAUDE_SESSION_ID = input.sessionId;
    set.DRORA_SESSION_ID = input.sessionId;
  }
  if (input.plugin) {
    set.CLAUDE_PLUGIN_DATA = input.plugin.dataPath;
    set.CLAUDE_PLUGIN_ROOT = input.plugin.rootPath;
    set.DRORA_PLUGIN_DATA = input.plugin.dataPath;
    set.DRORA_PLUGIN_ID = input.plugin.id;
    set.DRORA_PLUGIN_NAME = input.plugin.name;
    set.DRORA_PLUGIN_ROOT = input.plugin.rootPath;
  }
  ...
  for (const match of input.shellCommand.matchAll(SHELL_CONTEXT_VARIABLE_PATTERN)) {
    ...
    if (
      ...
      (name === "CLAUDE_PLUGIN_DATA" ||
        ...
        name === "DRORA_PLUGIN_DATA" ||
        name === "DRORA_PLUGIN_ROOT")
    ) {
      throw new Error(
