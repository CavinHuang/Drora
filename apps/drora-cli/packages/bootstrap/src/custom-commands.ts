...
import { createNodeCustomCommandAdapter } from "@drora/adapters/commands";
import type {
...
export interface ListDroraCustomCommandsOptions {
...
export interface InspectDroraCustomCommandOptions extends ListDroraCustomCommandsOptions {
  name: string;
}

export interface DroraCustomCommandInspection {
  command: CustomCommandContent;
  diagnostics: CustomCommandDiagnostic[];
}

export async function listDroraCustomCommands(
...
export async function inspectDroraCustomCommand(
  options: InspectDroraCustomCommandOptions,
): Promise<DroraCustomCommandInspection> {
  const discovery = createCustomCommandDiscovery(options);
...
export async function loadDroraCustomCommand(
  options: InspectDroraCustomCommandOptions,
...
function createCustomCommandDiscovery(options: ListDroraCustomCommandsOptions) {
