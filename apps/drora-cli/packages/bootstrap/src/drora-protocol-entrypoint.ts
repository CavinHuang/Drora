...
import { createNodeModelSelectionFacade } from "@drora/provider-node";
import { createNodeLoggerFactory } from "@drora/adapters/logging";
import {
  createMcpAdapterConnectionPool,
  createMcpTelemetryTracker,
  type McpConnectionPool,
  type McpTelemetryTracker,
} from "@drora/adapters/mcp";
import {
  droraProtocolNotifications,
  type DroraMcpResourceSample,
  type DroraMcpTelemetryEvent,
} from "@drora/shared";
import type { SqliteSessionStore } from "@drora/adapters/storage";
import { traceContextToLogContext, createRootTraceContext } from "@drora/contracts";
import type { McpPort, ModelSelection } from "@drora/contracts";
import type { PresentationSurface } from "@drora/core";
import type { RunDroraProtocolAgentOptions, DroraAppOptions } from "./app/types.js";
import { createDroraApp } from "./app/create-app.js";
import {
...
} from "./drora-protocol/storage-startup.js";
...
import { installDroraProtocolAiSdkWarningLogger } from "./drora-protocol/ai-sdk-warning-logger.js";
import {
  createOfficialMcpAuthHeadersPort,
  type OfficialMcpAuthRequestContext,
} from "./drora-protocol/official-mcp-auth-port.js";
import {
  createOfficialMcpTrustedOriginRegistry,
  OFFICIAL_MCP_DEV_TRUSTED_ORIGINS_ENV,
  DRORA_WORKSPACE_IDENTITY_ENV,
  resolveRuntimeDroraEndpointOrigin,
} from "@drora/shared";
import { DroraProtocolAgentServer } from "./drora-protocol/server.js";
import { DroraProtocolNdjsonConnection } from "./drora-protocol/transport.js";
import { cleanupProtocolRuntime } from "./drora-protocol/runtime-cleanup.js";
import { startProtocolResourceSampler } from "./drora-protocol/resource-sampler.js";
...
): Omit<DroraAppOptions, "providerRegistry"> {
  return {
    ...options,
    runtimeConfig: {
...
function applyProtocolProviderRegistry(
  options: Omit<DroraAppOptions, "providerRegistry">,
  providerRegistry: DroraAppOptions["providerRegistry"],
...
export async function runDroraProtocolAgent(
  options: RunDroraProtocolAgentOptions = {},
): Promise<void> {
  ...
  const traceContext = createRootTraceContext({
    attributes: {
      entrypoint: "drora_protocol",
    },
  });
  const logger = loggerFactory.createLogger("drora").child({
    ...traceContextToLogContext(traceContext),
    module: "bootstrap.drora_protocol",
  });
  installDroraProtocolAiSdkWarningLogger(logger);
  const startupTimer = new StartupTimer(
    logger,
    {
      ...traceContextToLogContext(traceContext),
      module: "bootstrap.drora_protocol",
      startupKind: "drora_protocol_agent",
    },
    startupStartedAt,
  );
  startupTimer.start("Drora Protocol agent startup started", {
    context: { version: options.version },
    event: "drora_protocol.startup.started",
    stage: "start",
  });

  let sessionStore: SqliteSessionStore | undefined;
  let serverForCleanup: DroraProtocolAgentServer | undefined;
  ...
  let mcpResourceSink: ((samples: DroraMcpResourceSample[]) => void) | undefined;
  let mcpTelemetrySink: ((event: DroraMcpTelemetryEvent) => void) | undefined;
  let processResourceSampler: DroraProcessResourceSampler | undefined;
  ...
  try {
        ...
        openProtocolStartupStorage({
          ...
          onProgress: (progress) =>
            logger.info("SQLite startup state", {
              event: "drora_protocol.startup.storage_state",
              ...progress,
            }),
        }),
    });
    ...
    logger.info("Worker Provider Registry 已就绪", {
      ...
      event: "drora_protocol.provider_registry.ready",
      module: "bootstrap.drora_protocol",
    ...
    const telemetryEnv = await acquireProtocolStartupResource({
      signal: options.lifecycle?.signal,
      logger,
      disposeLate: () => shutdownDroraTelemetry(),
      create: () =>
        prepareDroraTelemetryEnv(runtimeEnv, {
          cliVersion: options.version,
          productVersion: options.env?.DRORA_APP_VERSION,
          runtimeSurface,
        }),
    });
    const telemetryDeviceMid = telemetryEnv.DRORA_TELEMETRY_DEVICE_MID;
    mcpTelemetryTracker =
    ...
    // 与下面 trustedOrigins 的 resolveDroraApiOrigin 必须是同一个表达式，否则两侧判定分叉。
    const resolveDroraApiOrigin = (): string =>
      resolveRuntimeDroraEndpointOrigin(options.env ?? process.env);
    const workspaceIdentity = (options.env ?? process.env)[DRORA_WORKSPACE_IDENTITY_ENV]?.trim();
    const officialMcpAuth = {
      ...
      resolveDroraApiOrigin,
      ...(workspaceIdentity ? { workspaceIdentity } : {}),
      // 信任判定只看一条：目标 origin 等于当前 Drora API origin（https）。pluginId 不参与。
    ...
    options.lifecycle?.signal.throwIfAborted();
    const server = (serverForCleanup = new DroraProtocolAgentServer({
      createDroraApp: (appOptions = {}) =>
        createDroraApp({
          ...
          env: {
            ...telemetryEnv,
            ...appOptions.env,
            ...(telemetryDeviceMid ? { DRORA_TELEMETRY_DEVICE_MID: telemetryDeviceMid } : {}),
          },
          ...
          onToolExecResource: (params) =>
            connection.send({ method: droraProtocolNotifications.toolExecResource, params }),
        }),
      cwd: options.cwd,
      env: options.env,
      loggerFactory,
    ...
    const connection = new DroraProtocolNdjsonConnection({
      signal: options.lifecycle?.signal,
    ...
    mcpResourceSink = (samples) =>
      connection.send({
        method: droraProtocolNotifications.mcpResourceSamples,
        params: samples,
      });
    mcpTelemetrySink = (event) => {
      ...
      connection.send({
        method: droraProtocolNotifications.mcpTelemetry,
        params: event,
      });
    };
    connection.start();
    ...
    startupTimer.complete("Drora Protocol agent startup completed", {
      event: "drora_protocol.startup.completed",
      stage: "total",
    });
    ...
    startupTimer.fail("Drora Protocol agent startup failed", error, {
      event: "drora_protocol.startup.failed",
      stage: "total",
    });
    throw error;
  } finally {
    ...
    logger.info("Drora Protocol agent shutdown completed", {
      ...traceContextToLogContext(traceContext),
      event: "drora_protocol.shutdown.completed",
      module: "bootstrap.drora_protocol",
      status: "completed",
    });
  }
...
function resolveProtocolRuntimeSurface(
  ...
  return env.DRORA_TELEMETRY_RUNTIME_SURFACE?.trim() === "remote_workspace_host"
