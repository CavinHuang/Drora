...
import { getModelUsageTotalTokens, SessionEventType } from "@drora/contracts";
import { parseAutomationRunId } from "@drora/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const drora = recordValue(meta.drora);
  return (
    ...
    optionalString(drora.parentToolCallId) ??
    optionalString(drora.parentToolUseId)
  );
}
