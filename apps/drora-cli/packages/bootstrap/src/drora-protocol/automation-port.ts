...
import {
  ...
  droraAutomationCheckTaskBindingResultSchema,
  droraAutomationCreateResultSchema,
  droraAutomationDeleteResultSchema,
  droraAutomationListResultSchema,
  droraAutomationUpdateResultSchema,
  droraProtocolMethods,
  type DroraAutomationProtocol,
} from "@drora/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => DroraProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              droraProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              droraProtocolMethods.automationList,
              {},
              droraAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          droraProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        droraProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        droraProtocolMethods.automationDelete,
        { automationId: input.id },
        droraAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: DroraAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: DroraAutomationProtocol): CronAutomation {
  return {
