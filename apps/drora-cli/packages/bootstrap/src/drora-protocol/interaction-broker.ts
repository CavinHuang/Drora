...
import {
  ...
  droraUserInputResponseSchema,
  type DroraPermissionOption,
  type DroraPermissionResponse,
  type DroraUserInputQuestion,
...
import type { DroraProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        droraProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → DroraPermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: DroraPermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 droraPermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        droraProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): DroraUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): DroraUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): DroraUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): DroraUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "drora_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
