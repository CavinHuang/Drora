...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@drora/contracts";
import {
  droraMcpListParamsSchema,
  droraMcpListResultSchema,
  type DroraMcpListResult,
} from "@drora/shared";
import {
...
export async function listMcpServers(
  ...
  rawParams: unknown,
): Promise<DroraMcpListResult> {
  const params = parseParams(droraMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.drora_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.drora` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return droraMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return droraMcpListResultSchema.parse({
