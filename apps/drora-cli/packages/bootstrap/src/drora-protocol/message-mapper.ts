import type { DroraMessagePart, DroraMessageWithParts, DroraToolState } from "@drora/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@drora/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): DroraMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): DroraMessagePart {
  const base = {
...
function mapToolState(state: ToolState): DroraToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // Drora protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
