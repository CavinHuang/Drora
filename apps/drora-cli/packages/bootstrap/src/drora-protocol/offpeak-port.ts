import type { OffPeakPort, OffPeakTaskSummary } from "@drora/contracts";
import {
  droraOffPeakCreateResultSchema,
  droraOffPeakListResultSchema,
  droraProtocolMethods,
  type DroraOffPeakTaskProtocolSnapshot,
} from "@drora/shared";
import type {
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            droraProtocolMethods.offPeakList,
            {},
            droraOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        droraProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: DroraOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
