...
import {
  ...
  droraPluginsReferenceCatalogParamsSchema,
  droraPluginsResolveSuggestedReferenceParamsSchema,
  type DroraPluginReferenceCatalogEntry,
  type DroraPluginDiagnostic as SharedPluginDiagnostic,
  type DroraPluginsReferenceCatalogResult,
  type DroraPluginsResolveSuggestedReferenceResult,
} from "@drora/shared";
...
const SUGGESTED_PLUGIN_MARKETPLACE = DRORA_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<DroraPluginsResolveSuggestedReferenceResult> {
  const params = parseParams(droraPluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  if (
    ...
    return unavailable(
      ...
      "推荐插件不是受信任的官方 drora-plugins-official 来源",
    );
  }

  const workingDirectory = params.workspace.workspacePath;
  const readState = () => {
    const outcome = resolveDroraPlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: droraProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateDroraPluginMarketplace({
    ...
    const refreshTimeoutRequest = new Promise<never>((_, reject) => {
      ...
      refreshTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("刷新 drora-plugins-official 超时（10000 ms）");
  ...
  const overview = getDroraPluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getDroraPluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): DroraPluginReferenceCatalogEntry {
