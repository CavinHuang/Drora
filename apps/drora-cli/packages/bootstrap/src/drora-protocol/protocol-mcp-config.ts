import type { McpServerConfig } from "@drora/contracts";
import type { DroraProtocolMcpServer } from "@drora/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: DroraProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // Drora Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
