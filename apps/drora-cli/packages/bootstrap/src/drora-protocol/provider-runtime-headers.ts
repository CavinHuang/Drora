...
import {
  ...
  droraProviderRuntimeHeadersResponseSchema,
  type DroraWorkspaceRef,
} from "@drora/shared";
import type { DroraAppOptions } from "../app/types.js";
import {
...
): NonNullable<DroraAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          droraProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: droraProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
