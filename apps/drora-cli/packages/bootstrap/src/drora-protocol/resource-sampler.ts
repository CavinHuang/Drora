...
import {
  createDroraProcessResourceSampler,
  type DroraProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: DroraProtocolAgentServer,
  send: (message: DroraProtocolNotification) => void,
  logger: Logger,
): DroraProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createDroraProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: droraProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "drora_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
