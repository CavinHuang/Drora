import type { Logger, McpPort } from "@drora/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@drora/adapters/mcp";
import type { SqliteSessionStore } from "@drora/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@drora/telemetry";
...
import type { DroraProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<DroraProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<DroraProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`Drora Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `drora_protocol.${resource}.shutdown.failed`,
