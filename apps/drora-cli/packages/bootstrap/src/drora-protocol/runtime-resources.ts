...
import type { DroraProtocolAgentDependencies } from "./server-types.js";

/** 持有 app 资源而非业务 session；临时 workspace app、尚未登记的 app 也归此 owner。 */
export class ProtocolRuntimeResources {
  private stopping = false;
  private readonly apps = new Set<DroraApp>();
  private closePromise?: Promise<void>;

  constructor(private readonly factory: DroraProtocolAgentDependencies["createDroraApp"]) {}

  readonly create: DroraProtocolAgentDependencies["createDroraApp"] = async (options) => {
    this.assertServing();
  ...
  assertServing(): void {
    if (this.stopping) throw new Error("Drora Protocol runtime is stopping");
  }
  ...
  private async stopApp(app: DroraApp): Promise<void> {
    try {
