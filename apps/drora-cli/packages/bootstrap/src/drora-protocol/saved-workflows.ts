...
// 解析器与序列化器只从 @drora/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.drora/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@drora/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  DRORA_WORKFLOWS_RUNS_MAX_LIMIT,
  droraWorkflowsDeleteParamsSchema,
  droraWorkflowsGetParamsSchema,
  droraWorkflowsListParamsSchema,
  droraWorkflowsMoveParamsSchema,
  droraWorkflowsRunsParamsSchema,
  droraWorkflowsUpdateMetaParamsSchema,
  type DroraSavedWorkflowRun,
  type DroraSavedWorkflowScope,
  type DroraWorkflowsDeleteResult,
  type DroraWorkflowsGetResult,
  type DroraWorkflowsListResult,
  type DroraWorkflowsMoveResult,
  type DroraWorkflowsRunsResult,
  type DroraWorkflowsUpdateMetaResult,
} from "@drora/shared";
import type { JournalStorePort } from "@drora/dynamic-workflow";
...
function scopeOf(params: { scope?: DroraSavedWorkflowScope }): DroraSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  rawParams: unknown,
): Promise<DroraWorkflowsListResult> {
  const params = parseParams(droraWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: DroraProtocolAgentServerContext,
  rawParams: unknown,
): Promise<DroraWorkflowsGetResult> {
  const params = parseParams(droraWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(droraWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(droraWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(droraWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(DRORA_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: DroraSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, DroraSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, DroraSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: DroraProtocolAgentServerContext,
  rawParams: unknown,
): Promise<DroraWorkflowsMoveResult> {
  const params = parseParams(droraWorkflowsMoveParamsSchema, rawParams);
