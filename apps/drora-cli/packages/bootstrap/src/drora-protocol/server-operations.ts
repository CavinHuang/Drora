/* eslint-disable max-lines -- Drora Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../drora-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@drora/adapters/exec";
import { inputIntentMetadata } from "../drora-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@drora/core";
import {
  CoreErrorType,
...
import {
  ...
  droraProtocolErrorCodes,
  droraProtocolMethods,
  droraSessionCancelBackgroundTaskParamsSchema,
  droraSessionCompactParamsSchema,
  droraSessionCloseParamsSchema,
  droraSessionCreateParamsSchema,
  droraSessionEventsParamsSchema,
  droraSessionForkParamsSchema,
  droraSessionGoalParamsSchema,
  droraSessionListParamsSchema,
  droraSessionMessagesParamsSchema,
  droraSessionReadParamsSchema,
  droraSessionRuntimePreferencesResultSchema,
  droraSessionResumeParamsSchema,
  droraSessionSendParamsSchema,
  droraSessionSetModeParamsSchema,
  droraSessionSetModelParamsSchema,
  droraSessionSetThoughtLevelParamsSchema,
  droraSessionStopParamsSchema,
  droraSessionSubscribeParamsSchema,
  droraSessionSubagentsParamsSchema,
  droraTaskTokenUsageParamsSchema,
  droraUsageStatsParamsSchema,
  droraWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type DroraAutomationBotDeliveryTarget,
  type DroraSessionCreateParams,
  type DroraDeliveryKind,
  type IntegratedTerminalShellSelection,
  type DroraSessionRuntimePreferencesScope,
  type DroraSessionRuntimePreferencesResult,
  type DroraModelContextBudgetStrategy,
  type DroraProtocolTrace,
  type DroraSessionEvent,
  type DroraSessionHistoryTarget,
  type DroraSessionResumeParams,
...
import {
  ...
  parseParams,
  requireSession,
  type DroraProtocolAgentServerContext,
  type DroraProtocolSessionRecord,
  type DroraProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceDroraApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
import { runSessionModelConfigMutation } from "../drora-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<DroraProtocolSessionRecord["app"]["readTarget"]>>>
>;

type DroraSessionRecordParams = (
  | DroraSessionCreateParams
  | (DroraSessionResumeParams & {
      mode?: DroraSessionCreateParams["mode"];
      model?: DroraSessionCreateParams["model"];
      parentSessionId?: DroraSessionCreateParams["parentSessionId"];
      thoughtLevel?: DroraSessionCreateParams["thoughtLevel"];
...
interface SessionStartupPreferences {
  ...
  modelContextBudgetStrategy: DroraModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: DroraProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<DroraProtocolSessionRecord["app"], "listThoughtLevels">,
...
function getProtocolToolInputTransmissionState(
  record: DroraProtocolSessionRecord,
  deliveryKind?: DroraDeliveryKind,
): DroraProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: DroraProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const drora = asRecord(meta.drora);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(drora.parentToolUseId) ??
    stringValue(drora.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: DroraSessionEvent,
  toolInputTransmissions: DroraProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
  context: DroraProtocolAgentServerContext,
  record: DroraProtocolSessionRecord,
  event: SessionEvent,
  deliveryKind?: DroraDeliveryKind,
  options: {
...
): DroraSessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("Drora Protocol session event filtered", {
      event: "drora_protocol.session_event.filtered",
      eventId: String(event.id),
      module: "bootstrap.drora_protocol",
  ...
  // tool_input_delta 等高频模型中间态。Drora Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<DroraSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: DroraSessionEvent[] = [];
  const toolInputTransmissions: DroraProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("Drora Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "drora_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<DroraSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<DroraProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<DroraSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: DroraProtocolSessionRecord;
  sessionId: SessionId;
  createParams: DroraSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("Drora Protocol imported history persisted", {
    event: "drora_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: DroraProtocolAgentServerContext,
  rawParams: unknown,
  trace: DroraProtocolTrace | undefined,
  project: (
    record: DroraProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(droraSessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("Drora Protocol session/create started", {
    event: "drora_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("Drora Protocol session/create skipped unsupported thought level", {
          event: "drora_protocol.session_create.thought_level_skipped",
          module: "bootstrap.drora_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 Drora session。
    ...
    context.logger?.info("Drora Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "drora_protocol.session_create.completed",
    ...
    context.logger?.warn("Drora Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "drora_protocol.session_create.failed",
      module: "bootstrap.drora_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: DroraProtocolAgentServerContext,
  params: DroraSessionResumeParams,
  ...
  context.logger?.debug("Drora Protocol session resume started", {
    activeBeforeWait,
    event: "drora_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("Drora Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "drora_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.drora_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      droraProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("Drora Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "drora_protocol.session.resume_completed",
    module: "bootstrap.drora_protocol",
...
export async function resumeSession(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "drora_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "drora_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.drora_protocol",
      sessionId: session.id,
...
export async function listSessions(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(droraSessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("Drora Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "drora_protocol.session.persisted_missing",
      module: "bootstrap.drora_protocol",
...
export async function getUsageStats(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionReadParamsSchema, rawParams);
...
export async function readMessages(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(droraSessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("Drora Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("Drora Protocol session/send accepted", {
...
export async function compactSession(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("Drora Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("Drora Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("Drora Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("Drora Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("Drora Protocol goal paused"));
    }
...
export async function forkSession(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: DroraProtocolAgentServerContext,
  record: DroraProtocolSessionRecord,
  fork: Awaited<ReturnType<DroraProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: DroraProtocolAgentServerContext,
  parentRecord: DroraProtocolSessionRecord,
  forkRecord: DroraProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("Drora Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("Drora Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("Drora Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("Drora Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("Drora Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("Drora Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("Drora Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("Drora Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(droraSessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("Drora Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(droraSessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: DroraProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(droraSessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: DroraSessionPersistence,
  expectedPersistence?: DroraSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(droraWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceDroraApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: DroraProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: DroraProtocolAgentServerContext,
  _record: DroraProtocolSessionRecord,
...
async function resolveForkTarget(
  context: DroraProtocolAgentServerContext,
  record: DroraProtocolSessionRecord,
  target: DroraSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(droraTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: DroraProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: droraProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "drora_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "drora_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // DroraProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("Drora Protocol runtime preferences request started", {
    event: "drora_protocol.runtime_preferences.request_started",
    module: "bootstrap.drora_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      droraProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      droraSessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("Drora Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "drora_protocol.runtime_preferences.response_received",
      module: "bootstrap.drora_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "drora_protocol.runtime_preferences.request_failed",
      module: "bootstrap.drora_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("Drora Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("Drora Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "Drora Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_DRORA_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: DroraProtocolAgentServerContext,
  params: DroraSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: DroraProtocolTrace,
): Promise<DroraProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("Drora Protocol createRecord MCP config", {
    event: "drora_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: DroraProtocolSessionRecord | undefined;
  const app = await createWorkspaceDroraApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.drora/.agents 的 enabled MCP，
      ...
      // Drora Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // Drora Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: DroraProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): DroraSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isDroraSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isDroraSessionMode(
  value: unknown,
): value is NonNullable<DroraSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("Drora Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "drora_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "drora_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "drora_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "drora_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "drora_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: DroraStateUpdatedNotification = {
    patch,
    reason,
