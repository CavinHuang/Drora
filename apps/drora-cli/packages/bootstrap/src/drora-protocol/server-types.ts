...
import type { McpTelemetryTracker } from "@drora/adapters";
import type { WorkspaceHookPolicyProvider } from "@drora/core";
import type { AccountProviderConfigSnapshot } from "@drora/provider";
import {
  ...
  type DroraProtocolMessage,
  type DroraProtocolMethod,
  type DroraProtocolNotification,
  type DroraProtocolRequest,
  type DroraProtocolRequestId,
...
import type { ConversationV4Gateway } from "../drora-protocol-v4/v4-gateway.js";
...
export interface DroraProtocolAgentDependencies {
  createDroraApp(options?: Omit<DroraAppOptions, "providerRegistry">): DroraApp | Promise<DroraApp>;
  /**
...
export interface DroraProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface DroraProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface DroraProtocolSessionRecord {
  app: DroraApp;
...
export interface DroraProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: DroraProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: DroraProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: DroraProtocolMessage): message is DroraProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: DroraProtocolMessage,
): message is DroraProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: DroraProtocolMessage,
): message is { id: DroraProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: DroraProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): DroraProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: DroraDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("Drora Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "drora_protocol.session.require_missing",
...
export function createProtocolLogger(deps: DroraProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("drora").child({
    module: "bootstrap.drora_protocol",
  });
}
