...
import {
  droraPluginsCancelOperationParamsSchema,
  droraProtocolMethods,
  droraWorkspaceCancelGenerateTextParamsSchema,
  droraWorkspaceHookTrustGrantParamsSchema,
} from "@drora/shared";
import type { BrowserControlPort } from "@drora/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@drora/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  DroraProtocolError,
  DroraProtocolMessage,
  DroraProtocolMethod,
  DroraProtocolNotification,
  DroraProtocolRequest,
  DroraProtocolRequestId,
  DroraProtocolResponse,
} from "@drora/shared";
import {
...
} from "../drora-protocol-v4/interaction-registry.js";
...
import {
  ...
  parseParams,
  toProtocolError,
  type DroraProtocolClientRequestOptions,
  type DroraProtocolAgentDependencies,
  type DroraProtocolAgentServerContext,
  type DroraProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@drora/contracts";

export type { DroraProtocolAgentDependencies, DroraProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type DroraProtocolOutboundMessage = DroraProtocolNotification | DroraProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, DroraProtocolSessionRecord>;
}): Promise<void> {
...
interface DroraProtocolPostResponseBatch {
  readonly messages: readonly DroraProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class DroraProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<DroraProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: DroraProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: DroraProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    DroraProtocolRequestId,
    DroraProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: DroraProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createDroraApp);
    const resolvedDeps = {
      ...deps,
      createDroraApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, DroraProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: DroraProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("Drora Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "drora_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: DroraProtocolRequestId): DroraProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: DroraProtocolRequestId): DroraProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: DroraProtocolMessage,
  ): Promise<DroraProtocolError | DroraProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("Drora Protocol notification ignored", {
        event: "drora_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: DroraProtocolRequest,
  ): Promise<DroraProtocolError | DroraProtocolResponse> {
  ...
  private async dispatchRequest(request: DroraProtocolRequest) {
      ...
      case droraProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: droraWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? droraWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(droraPluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(droraWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: DroraProtocolRequestId, result: unknown): DroraProtocolResponse {
    return { id, result };
  }

  private fail(
    id: DroraProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): DroraProtocolError {
  ...
  private requestClient<T>(
    method: DroraProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: DroraProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No Drora Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: DroraProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: DroraProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
