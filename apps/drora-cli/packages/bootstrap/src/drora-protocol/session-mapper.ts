import {
  ...
  type DroraActiveToolCall,
  type DroraContextUsageBreakdownItem,
  type DroraDeliveryKind,
  type DroraPendingPermission,
  type DroraSessionContextUsage,
  type DroraSessionEvent,
  type DroraSessionGoal,
  type DroraSessionGoalVerification,
  type DroraSessionGoalVerificationTimeline,
  type DroraSessionGoalStats,
  type DroraSessionInfo,
  type DroraSessionKind,
  type DroraSessionProjection,
  type DroraSessionRuntimeState,
  type DroraSessionSettingsState,
  type DroraSessionStateSnapshot,
  type DroraSessionTodoGroup,
...
import type { DroraApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: DRORA_PROTOCOL_NAME,
      version: DRORA_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<DroraApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 drora-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("drora-artifact://") ? artifactUri : undefined;
}
...
): Promise<DroraSessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<DroraApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as DroraSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is DroraSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = droraApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const droraMeta = asRecord(meta.drora);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = droraApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): DroraSessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<DroraApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: DroraDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): DroraSessionContextUsage | undefined {
...
): DroraSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: DroraSessionContextUsage["cache"] | undefined,
): DroraSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): DroraSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): DroraPendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 droraPendingPermissionSchema（shared/src/drora-protocol/index.ts:1139）和
  // droraPermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 droraPermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): DroraActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): DroraSessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): DroraSessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): DroraSessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, DroraSessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, DroraSessionTodoGroup>,
  input: {
  ...
  const group: DroraSessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): DroraSessionEvent["type"] {
  switch (type) {
