...
import {
  droraSkillsReferenceCatalogParamsSchema,
  type DroraSkillReferenceCatalogEntry,
  type DroraSkillsReferenceCatalogResult,
} from "@drora/shared";
import type { SkillLoadOutcome, SkillMetadata } from "@drora/contracts";
import { listDroraSkills } from "../skills.js";
import {
  parseParams,
  requireSession,
...
export async function getSkillReferenceCatalog(
  ...
  const params = parseParams(droraSkillsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
  ...
  const outcome = await listDroraSkills({
...
function toResult(
  authority: DroraSkillsReferenceCatalogResult["authority"],
...
function toReferenceCatalogEntry(skill: SkillMetadata): DroraSkillReferenceCatalogEntry {
  const scope =
