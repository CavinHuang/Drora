import { BUILTIN_DRORA_SLASH_COMMAND_HELP_ENTRIES, type DroraSlashCommand } from "@drora/shared";
import {
  listDroraCustomCommands,
  type ListDroraCustomCommandsOptions,
} from "../custom-commands.js";
import {
  APP_PROTOCOL_APP_ONLY_BUILTIN_SLASH_COMMANDS,
  APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES,
  isReservedDroraSlashCommandName,
} from "../slash-command-surface.js";

/**
 * `workflow` 是 drora-guide 内置插件的自定义命令，随 CLI 打包，不受用户 commandOverrides
 * 影响；灰度关闭时只能在装配目录时按名剔除。
 */
const DYNAMIC_WORKFLOW_SLASH_COMMAND_NAME = "workflow";

export interface ListProtocolSlashCommandsOptions extends ListDroraCustomCommandsOptions {
  /**
   * 动态工作流灰度门。**只有显式 false
...
export async function listProtocolSlashCommands(
...
): Promise<DroraSlashCommand[]> {
  const builtins = listAppProtocolBuiltinSlashCommands();
  let customCommands: Awaited<ReturnType<typeof listDroraCustomCommands>>["commands"] = [];
  try {
    const outcome = await listDroraCustomCommands(options);
  ...
  return pinWorkflowAfterGoal([
    ...
    ...customCommands
      ...
      .filter((command) => !isReservedDroraSlashCommandName(command.name))
 ...
 * `workflow` 是 drora-guide 内置插件的自定义命令，
 * 按发现顺序会沉在 custom 段末尾；产品要求它与 `goal` 一样作为「开启一段工作」的入口，
 * 紧随 goal 之后。
 * 只调顺序：来源、去重与 reserved 规则不变；任一方缺席时保持原序。
 */
function pinWorkflowAfterGoal(commands: DroraSlashCommand[]): DroraSlashCommand[] {
...
function listAppProtocolBuiltinSlashCommands(): DroraSlashCommand[] {
  const sharedBuiltins = APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES.flatMap((name) => {
    const command = BUILTIN_DRORA_SLASH_COMMAND_HELP_ENTRIES.find((entry) => entry.name === name);
    if (!command) return [];
    return [
      {
