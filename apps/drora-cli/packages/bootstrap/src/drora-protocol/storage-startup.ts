...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@drora/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  droraProtocolNotifications,
  droraStorageStartupStateSchema,
  type DroraStorageStartupState,
} from "@drora/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: DroraStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = droraStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: droraProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { droraStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@drora/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = droraStoragePathReadySchema.parse(JSON.parse(line));
