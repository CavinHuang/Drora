...
import type { DroraSessionEndedSubagent, DroraSessionRunningSubagent } from "@drora/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: DroraSessionRunningSubagent[];
  ended: DroraSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): DroraSessionRunningSubagent["status"] | undefined {
...
): DroraSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): DroraSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: DroraSessionRunningSubagent[] = [];
  const ended: DroraSessionEndedSubagent[] = [];
...
function encodeCursor(item: DroraSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly DroraSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: DroraSessionEndedSubagent[]; nextCursor?: string } {
