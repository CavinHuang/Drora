...
import { droraProtocolMessageSchema, droraProtocolMethods } from "@drora/shared";
import type { Logger } from "@drora/contracts";

type DroraProtocolOutgoingMessage =
  | DroraProtocolError
  | DroraProtocolNotification
  | DroraProtocolRequest
  | DroraProtocolResponse;

type DroraProtocolMessageHandler = (
  message: DroraProtocolMessage,
) => Promise<DroraProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface DroraProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: DroraProtocolMessageHandler;
  ...
  ) => readonly DroraProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: DroraProtocolRequestId) => {
    readonly messages: readonly DroraProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class DroraProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: DroraProtocolNdjsonConnectionOptions) {
  ...
  send(message: DroraProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("Drora Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("Drora Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): DroraProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("Drora Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "drora_protocol.parse.failed",
    ...
    const parsed = droraProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid Drora Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: DroraProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === droraProtocolMethods.sessionStop ||
        message.method === droraProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: DroraProtocolMessage): Promise<void> {
    if (this.terminal) return;
