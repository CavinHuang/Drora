...
import { workspaceHookPolicySchema } from "@drora/contracts";
import type { WorkspaceHookPolicyProvider } from "@drora/core";
import {
  droraWorkspaceHookTrustGrantParamsSchema,
  droraWorkspaceHookTrustGrantReasonCodeSchema,
  droraWorkspaceHookTrustGrantResultSchema,
  type DroraWorkspaceHookTrustGrantReasonCode,
  type DroraWorkspaceHookTrustGrantResult,
} from "@drora/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = droraWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return droraWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): DroraWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): DroraWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = droraWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
