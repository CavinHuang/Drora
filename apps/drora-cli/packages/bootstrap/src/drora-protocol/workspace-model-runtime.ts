...
import { createInMemorySessionEventStore } from "@drora/adapters/storage";
import type { ModelSelection } from "@drora/contracts";
import {
  droraProviderTestModelConnectivityParamsSchema,
  droraWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(droraWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(droraProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function createWorkspaceDroraApp(
  context: DroraProtocolAgentServerContext,
  ...
  return context.deps.createDroraApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createDroraApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<DroraProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: DroraProtocolAgentServerContext,
  record: Pick<DroraProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {
