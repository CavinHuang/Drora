...
import {
  ...
  DRORA_CUA_OFFICIAL_PLUGIN_ID,
  DRORA_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  DRORA_PLUGIN_ID_ENV_KEY,
} from "@drora/shared";

export { DRORA_CUA_BROKER_SOCKET_ENV_KEY as DRORA_CUA_BROKER_SOCKET_ENV } from "@drora/shared";
// CLI 入口会先清理 broker 凭据；shared node_repl 的可信配置随后从进程内捕获快照恢复它们。
function resolveDroraCuaBrokerSocket(): string | undefined {
  // captured 优先；运行时残留的 stale socket 不能覆盖可信快照。
  return (
    getCapturedDroraCuaBrokerCredentials().socket ||
    process.env[DRORA_CUA_BROKER_SOCKET_ENV_KEY]?.trim()
  );
}
    ...
    Object.entries(pluginServers)
      .filter(
        ([name, config]) =>
          ...
          config.type === "stdio" &&
          config.env?.[DRORA_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() ===
            DRORA_CUA_OFFICIAL_PLUGIN_ID,
      )
      .map(([name]) => name),
  );
...
export async function listMcpServerStatuses(
  ...
  for (const [name, config] of Object.entries(servers)) {
    // 迁移后旧的 drora-cua MCP 只是历史配置，不再进入状态投影；CUA
...
function injectCuaCredentialsIntoNodeRepl(
  ...
  return {
    ...
    env: {
      ...config.env,
      [DRORA_CUA_BROKER_SOCKET_ENV_KEY]: socketPath,
      ...(refreshMarker ? { [REFRESH_MARKER_ENV]: refreshMarker } : {}),
      ...(pluginAuthority ? { [DRORA_CUA_PLUGIN_AUTHORITY_ENV_KEY]: pluginAuthority } : {}),
      [DRORA_CUA_NODE_REPL_HOST_ENV_KEY]: "1",
      [DRORA_PLUGIN_ID_ENV_KEY]: DRORA_CUA_OFFICIAL_PLUGIN_ID,
    },
  };
}

function isDroraCuaStdioServer(
  name: string,
  config: McpServerConfig,
): config is McpStdioServerConfig {
  if (config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 内置 official drora-cua plugin 的 MCP server 走 __drora-plugin-host，command 是 Helper
  // (非 drora-cua)、args 是 [drora.cjs, __drora-plugin-host, server.js]（非 drora-cua package arg），
  // 上面的 name/command/args 三条都匹配不到。_plugin id 由 adapters resolver 权威写入 env
  // （manifest/user env 不可覆盖），用它识别 official plugin server。
  if (
    config.env?.[DRORA_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() === DRORA_CUA_OFFICIAL_PLUGIN_ID
  ) {
    return true;
  }
  // 判定与 desktop/services 共用 @drora/shared 的单一事实源，避免两条注入入口漂移。
  if (isDroraCuaMcpCommand(config.command)) return true;
  return (config.args ?? []).some(isDroraCuaMcpPackageArg);
}
...
function isRetiredCuaMcpServer(name: string, config: McpServerConfig | undefined): boolean {
  ...
  return (
    name !== NODE_REPL_SERVER_NAME && config !== undefined && isDroraCuaStdioServer(name, config)
  );
}
