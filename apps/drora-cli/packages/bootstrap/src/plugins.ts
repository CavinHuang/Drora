...
import { DRORA_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@drora/contracts";
import { DRORA_CUA_OFFICIAL_PLUGIN_ID, isDroraCuaInternalFeatureEnabled } from "@drora/shared";
...
export interface ResolveDroraPluginsOptions {
...
export interface ListDroraPluginsOptions extends ResolveDroraPluginsOptions {}

export interface SetDroraPluginEnabledOptions extends ResolveDroraPluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetDroraPluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface DroraMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface DroraAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface DroraInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface DroraPluginsOverviewData {
  marketplaces: DroraMarketplaceSummaryData[];
  availablePlugins: DroraAvailablePluginData[];
  installedPlugins: DroraInstalledPluginData[];
  restorableBuiltins: DroraAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface DroraMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: DroraMarketplaceSummaryData[];
}

export interface AddDroraMarketplaceOptions extends ResolveDroraPluginsOptions {
...
export interface RemoveDroraMarketplaceOptions extends ResolveDroraPluginsOptions {
  marketplace: string;
}

export interface UpdateDroraMarketplaceOptions extends ResolveDroraPluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallDroraMarketplacePluginOptions extends ResolveDroraPluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallDroraMarketplacePluginOptions extends ResolveDroraPluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`drora plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateDroraMarketplacePluginOptions extends ResolveDroraPluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateDroraPluginPathOptions extends ResolveDroraPluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface DroraPluginUpdateData extends DroraPluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveDroraPluginsOptions {
  pluginId: string;
}

interface ConfigureDroraPluginOptions extends ResolveDroraPluginsOptions {
...
interface ResetDroraPluginConfigOptions extends ResolveDroraPluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateDroraPluginOptions extends ResolveDroraPluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeDroraPluginOptions extends ResolveDroraPluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface DroraPluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== DRORA_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveDroraPlugins(options: ResolveDroraPluginsOptions = {}): PluginLoadOutcome {
...
export function getDroraPluginsOverview(
  ...
  // drora-plugins-official 的内置与 CDN 分片已在 adapter 层合并为唯一 canonical manifest。
  const catalogs: Array<{
    summary: DroraMarketplaceSummaryData;
  ...
  const restorableBuiltins: DroraAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${DRORA_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isDroraCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${DRORA_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
      marketplace: DRORA_OFFICIAL_PLUGIN_MARKETPLACE,
      version: def.version,
      installed: false,
      ...(listing ? { listing } : {}),
    };
...
export function listDroraPlugins(options: ListDroraPluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveDroraPlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${DRORA_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setDroraPluginEnabled(
  options: SetDroraPluginEnabledOptions,
): Promise<SetDroraPluginEnabledResult> {
...
export async function addDroraPluginMarketplace(
  options: AddDroraMarketplaceOptions,
): Promise<DroraMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeDroraPluginMarketplace(
  options: RemoveDroraMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateDroraPluginMarketplace(
  options: UpdateDroraMarketplaceOptions,
): Promise<DroraMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installDroraMarketplacePlugin(
  options: InstallDroraMarketplacePluginOptions,
): Promise<DroraPluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === DRORA_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === DRORA_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallDroraMarketplacePlugin(
  options: UninstallDroraMarketplacePluginOptions,
): Promise<DroraInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const pluginId = resolvePluginIdForMutation(options);

    // 官方 CDN marketplace 与内置插件共享 drora-plugins-official id 空间，且其缓存
...
/**
 * `drora plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateDroraMarketplacePlugin(
  options: UpdateDroraMarketplacePluginOptions,
): Promise<DroraPluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateDroraPluginMarketplace({
    ...options,
  ...
  const installed = await installDroraMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `drora plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateDroraPluginPath(
  options: ValidateDroraPluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const droraCuaPluginId = DRORA_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === droraCuaPluginId &&
    !isDroraCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires DRORA_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureDroraPlugin(options: ConfigureDroraPluginOptions): Promise<void> {
...
export async function resetDroraPluginConfig(
  options: ResetDroraPluginConfigOptions,
...
export async function validateDroraPlugin(
  options: ValidateDroraPluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeDroraPlugin(
  options: DescribeDroraPluginOptions,
...
function resolvePluginContext(options: ResolveDroraPluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallDroraMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveDroraPluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.drora/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".drora", "config.json");
