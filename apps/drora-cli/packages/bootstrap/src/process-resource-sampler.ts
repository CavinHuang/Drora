...
import {
  DRORA_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type DroraProcessResourceSample,
} from "@drora/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const DRORA_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = DRORA_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateDroraProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: DroraProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: DroraProcessResourceSample["platform"];
  arch?: DroraProcessResourceSample["arch"];
...
export interface DroraProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createDroraProcessResourceSampler(
  options: CreateDroraProcessResourceSamplerOptions,
): DroraProcessResourceSampler {
  const platform = options.platform ?? (process.platform as DroraProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as DroraProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: DroraProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, DRORA_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
