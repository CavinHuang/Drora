...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@drora/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@drora/core";
import type { Logger, PluginMetadata } from "@drora/contracts";
import {
  createAgentStateId,
...
interface LoadDroraAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadDroraAgentProfilesResult {
...
export async function loadDroraAgentProfiles(
  input: LoadDroraAgentProfilesInput,
): Promise<LoadDroraAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".drora", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .drora/agents/*.md 是仓库内容，不能通过 frontmatter
