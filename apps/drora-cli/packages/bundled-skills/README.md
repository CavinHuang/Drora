...
Skills that ship with the Drora CLI as a built-in capability, outside the plugin system.
...
- **SEA binaries**: `scripts/sea-bundled-skill-assets.mjs` embeds `skills/**` under the
  `drora-bundled-skills/` asset prefix. On start the CLI extracts them once into
  `~/.drora/cli/bundled-skills/<content-hash>/` and reads from there.
- **Remote hosts**: `scripts/prepare-prebuilds.mjs` stages the directory next to the remote
  `drora.cjs`, like the official plugin packages.

The resolver is `apps/drora-cli/packages/bootstrap/src/app/bundled-skills.ts`. Every file
