...
const cliBundle = resolve(dist, "drora.cjs");
const seaBlobForTarget = (target) => resolve(dist, `drora-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "drora-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/drora.cjs",
        output: `dist/drora-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "drora-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        DRORA_STORAGE_DIR: storageRoot,
      };
      delete env.DRORA_BFS_BINARY;
      delete env.DRORA_RG_BINARY;
      delete env.DRORA_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/drora.cjs. Run `pnpm build` first.");
  }
