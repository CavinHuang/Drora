...
export const DEFAULT_DESTINATION_ROOT = resolve(DEFAULT_MOUNT_ROOT, "drora", "deps");
export const RELEASE_DIRECTORY_PREFIX = "drora-cli-";
...
const usage = `Usage:
  ...
  node packages/cli/scripts/upload-sea-smb.mjs --dest-root /Volumes/shared/drora/deps
...
Options:
  ...
  --dest-root <path>  Mounted SMB release root. Defaults to /Volumes/shared/drora/deps.
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing SEA binaries in ${distDir}: ${missing.join(", ")}. ` +
        "Run `pnpm --filter @drora/cli build:sea` first.",
    );
  }
