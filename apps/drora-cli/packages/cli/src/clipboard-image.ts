...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@drora/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.DRORA_STORAGE_DIR?.trim() || join(homedir(), ".drora");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "drora-clipboard-"));
