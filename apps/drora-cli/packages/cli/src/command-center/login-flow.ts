import type { TuiSelection, TuiSubmitPrompt } from "@drora/tui";
...
export function buildLoginSelection(locale?: string): TuiSelection {
  const copy = getDroraCopy(locale).tui.loginSetup;
  return {
...
export function loginSetupResponse(locale?: string): string {
  return getDroraCopy(locale).tui.loginSetup.response;
}
