...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listDroraCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: drora commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listDroraCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@drora/bootstrap"));
  return (await bootstrap()).inspectDroraCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: DroraCustomCommandInspection,
  options: GlobalOptions,
): string {
