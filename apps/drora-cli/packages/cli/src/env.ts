...
import {
  ...
  normalizeDroraRuntimeEnv,
  sanitizeDroraRuntimeEnv,
  sanitizeDroraRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeDroraRuntimeEnv(env),
    ...buildDroraToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildDroraToolEnvPassthroughEnv(env);
  sanitizeDroraRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[DRORA_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): DroraRuntimeEnv {
  const explicit = normalizeDroraRuntimeEnv(env[DRORA_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.DRORA_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.DRORA_BETA === "1" || env.DRORA_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])drora-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.DRORA_STORAGE_DIR = join(homedir(), ".drora-beta");
}
