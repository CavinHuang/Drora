...
import { createDenyPermissionBroker } from "@drora/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  DroraAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `droraSessionEventTypeSchema`（`shared/src/drora-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `droraSessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createDroraApp` 注入点边界）。
 *
