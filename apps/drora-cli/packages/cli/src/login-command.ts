...
export async function runLoginCommand(
  ...
  try {
    ...
    if (args.length > 1 || (providerId !== "zai" && providerId !== "bigmodel")) {
      throw new Error("Usage: drora login [zai|bigmodel] [--no-browser]");
    }
    ...
    const login = deps.loginDroraCli ?? (await loadBootstrapModule()).loginDroraCli;
...
export async function runLogoutCommand(
  ...
  try {
    ...
    const logout = deps.logoutDroraCli ?? (await loadBootstrapModule()).logoutDroraCli;
