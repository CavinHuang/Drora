...
import type { listDroraPlugins } from "@drora/bootstrap";
import { createInterface } from "node:readline";
import type { GlobalOptions, RunContext } from "@drora/shared-types";
import type { CliEnv } from "./env.js";

export type BootstrapModule = typeof import("@drora/bootstrap");
export type PluginListOutcome = ReturnType<typeof listDroraPlugins>;
 ...
 * 避免 `drora plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addDroraPluginMarketplace",
  getPluginsOverview: "getDroraPluginsOverview",
  installPlugin: "installDroraMarketplacePlugin",
  listPlugins: "listDroraPlugins",
  removeMarketplace: "removeDroraPluginMarketplace",
  setPluginEnabled: "setDroraPluginEnabled",
  uninstallPlugin: "uninstallDroraMarketplacePlugin",
  updateMarketplace: "updateDroraPluginMarketplace",
  updatePlugin: "updateDroraMarketplacePlugin",
  validatePluginPath: "validateDroraPluginPath",
...
/** `drora plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@drora/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by drora. Use: user, project");
  }
