...
import type { SetDroraPluginEnabledResult, DroraPluginInstallData } from "@drora/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: drora plugins <command> [options]
...
Scopes: user (default), project. \`drora plugin\` is an alias of \`drora plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart drora to apply.\n`,
  );
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetDroraPluginEnabledResult; error?: string }> = [];
