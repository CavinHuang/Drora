...
import {
  DRORA_PROCESS_DIAGNOSTIC_PREFIX,
...
function writeProcessErrorDiagnostic(
  ...
  try {
    const detail = formatProcessError(reason).slice(0, DRORA_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS);
    const diagnostic: DroraProcessDiagnostic = {
      version: 1,
      errorId: randomUUID(),
      kind,
    ...
    stderr.write(
      `${DRORA_PROCESS_DIAGNOSTIC_PREFIX}${JSON.stringify(diagnostic)}\n[drora] process error kind=${kind} origin=${origin}\n${detail}\n`,
    );
  } catch {
    // 诊断输出不能再次击穿进程级异常边界。
  }
