...
import { formatJson, type PresentationSurface } from "@drora/core";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createDroraApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createDroraApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createDroraApp ?? bootstrapModule?.createDroraApp;
    if (!createApp) throw new Error("Drora app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareDroraTelemetryEnv ?? bootstrapModule?.prepareDroraTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownDroraTelemetry ?? bootstrapModule?.shutdownDroraTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.DRORA_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `drora -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveDroraCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedDroraSlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listDroraCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadDroraCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: drora hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
