...
import { createNodeLoggerFactory } from "@drora/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@drora/core";
import { color, formatJson, supportsColor } from "@drora/core";
import { getDroraCopy, isUiLocale, type UiLocale } from "@drora/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@drora/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getDroraCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "drora_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("drora doctor", colors)}\n`);
...
const runDroraProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 Drora agent transport closed。
    ...
    const runProtocolAgent =
      deps.runDroraProtocolAgent ?? (await loadBootstrapModule()).runDroraProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("drora").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runDroraProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
