...
type BootstrapModule = typeof import("@drora/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listDroraSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: drora skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listDroraSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@drora/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectDroraSkill;
...
function formatHumanSkillInspection(
  inspection: DroraSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: DroraSkillInspection, cwd: string): string =>
  formatJson({
    cwd,
