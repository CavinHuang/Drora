import type { TuiSessionMetadata } from "@drora/tui";
...
export async function listSkillsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listDroraSkillsForTui = deps.listSkills ?? (await loadBootstrapModule()).listDroraSkills;
  return await listDroraSkillsForTui({
    env,
    logger: deps.logger,
    workingDirectory,
  });
...
export async function listSessionsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listDroraSessionsForTui =
    deps.listSessions ?? (await loadBootstrapModule()).listDroraSessions;
  const sessions = await listDroraSessionsForTui({
