...
// 品牌类型（SessionId）和 createDroraApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createDroraApp ?? bootstrapModule?.createDroraApp;
  if (!createAppFactory) throw new Error("Drora app factory is unavailable.");
  const prepareTelemetry =
    deps.prepareDroraTelemetryEnv ?? bootstrapModule?.prepareDroraTelemetryEnv;
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownDroraTelemetry ?? bootstrapModule?.shutdownDroraTelemetry;
  }
