import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@drora/adapters/config";
import type { SessionEvent } from "@drora/contracts";
import type { DroraAppOptions } from "@drora/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@drora/i18n";
import type { TuiRequestPermission } from "@drora/tui";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createDroraApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<DroraAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createDroraApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createDroraApp"]>>>;
    try {
