...
import { resolveWorkspaceHookTimeoutMs } from "@drora/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "drora-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 Drora camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  return {
    set: {
      ...
      DRORA_PLUGIN_DATA: plugin.dataPath,
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.DRORA_PLUGIN_DATA = plugin.dataPath;
