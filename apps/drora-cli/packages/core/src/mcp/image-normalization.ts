...
} from "@drora/drora-cua/frame-contract";
...
import { preserveOfficialCuaFrameResult } from "@drora/drora-cua/frame-contract";
...
function readBrowserScreenshotContentIndices(result: McpToolCallResult): Set<number> {
  const value = result._meta?.[DRORA_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
