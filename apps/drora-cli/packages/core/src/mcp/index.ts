...
import {
  ...
  DRORA_MCP_ERROR_PRESENTATION_MESSAGE_ONLY,
  DRORA_MCP_ERROR_PRESENTATION_META_KEY,
  type JsonSchema,
  type McpPort,
...
import { DRORA_CUA_OFFICIAL_MCP_NAMESPACE_NAME as DRORA_CUA_OFFICIAL_MCP_SERVER_NAME } from "@drora/shared";
import { OFFICIAL_CUA_FRAME_MODEL_CONTENT_PROTECTION } from "@drora/drora-cua/frame-contract";
...
const DRORA_CUA_CANONICAL_MODEL_PREFIX = "mcp__computer-use__";
const DRORA_CUA_PROVIDER_SPELLING_ALIAS_PREFIX = "mcp__computer_use__";
...
function toRegisteredMcpToolName(
  ...
  if (
    ...
    descriptor.serverName === DRORA_CUA_OFFICIAL_MCP_SERVER_NAME
  ) {
    // adapter 会把官方插件 serverName 命名空间化，descriptor.name 因而是
    // mcp__plugin_drora-cua_computer-use__*；直接沿用它会让 provider 约定的 computer-use
    // 工具永远不存在。可信门成立后仅投影模型可见名称，handler 仍用 descriptor 的原路由。
    return `${DRORA_CUA_CANONICAL_MODEL_PREFIX}${toModelVisibleMcpNamePart(descriptor.toolName)}`;
  }
...
function createMcpToolEntry(
  ...
  const isCuaAppObservation = isDroraCuaGetAppState(descriptor);
...
function officialCuaProviderSpellingAliases(
  ...
  if (
    !officialCuaAuthorityVerified ||
    descriptor.serverName !== DRORA_CUA_OFFICIAL_MCP_SERVER_NAME ||
    !name.startsWith(DRORA_CUA_CANONICAL_MODEL_PREFIX)
  ) {
    return undefined;
  }
  const toolName = name.slice(DRORA_CUA_CANONICAL_MODEL_PREFIX.length);
  return toolName.length > 0
    ? [`${DRORA_CUA_PROVIDER_SPELLING_ALIAS_PREFIX}${toolName}`]
    : undefined;
}
...
function createModelFacingMcpInputSchema(
  ...
  // 原因：title 是 Drora 给用户看的意图摘要，不属于上游 drora-cua 参数。只在模型 contract
...
function isDroraCuaGetAppState(
  ...
  return (
    descriptor.serverName === DRORA_CUA_OFFICIAL_MCP_SERVER_NAME ||
    serverName === "drora-cua" ||
    serverName === "computer-use" ||
    (serverName.includes("drora-cua") && serverName.includes("computer-use"))
  );
}
...
function formatMcpToolResult(output: unknown): ModelMessageContent {
  ...
  const errorPresentation = output._meta?.[DRORA_MCP_ERROR_PRESENTATION_META_KEY];
  return typeof errorPresentation === "string" &&
    errorPresentation === DRORA_MCP_ERROR_PRESENTATION_MESSAGE_ONLY
    ? content
