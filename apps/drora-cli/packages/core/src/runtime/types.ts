...
export interface AgentRuntimeConfig {
  ...
  runtimeFeatures?: {
     ...
     * 由 bootstrap 根据 Drora 官方插件启停推导，不由普通插件 manifest 自声明。
     */
    nodeRepl?: boolean;
    /**
  ...
  /**
   * 动态工作流灰度门：Host 判定后经
   * Drora Protocol 下发，runtime 只消费。**缺席即开启**——TUI、headless `-p` 与
  ...
  agentName?: string; // Default: "drora-agent"
...
export interface ExecuteToolsOptions {
  ...
  subagentModelOverride?: import("@drora/contracts").SubagentRunOptions["modelOverride"];
  model?: Model;
