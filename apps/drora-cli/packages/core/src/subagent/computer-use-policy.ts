...
import {
  DRORA_CUA_OFFICIAL_MCP_NAMESPACE_NAME as DRORA_CUA_OFFICIAL_MCP_SERVER_NAME,
  DRORA_CUA_OFFICIAL_PLUGIN_ID,
} from "@drora/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@drora/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(DRORA_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === DRORA_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(DRORA_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === DRORA_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === DRORA_CUA_OFFICIAL_PLUGIN_ID,
  );
