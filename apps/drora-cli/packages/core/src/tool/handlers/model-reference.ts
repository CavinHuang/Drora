...
import type { ModelCatalogEntry, ModelSelection } from "@drora/contracts";
import {
  DRORA_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@drora/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(DRORA_MODEL_REASONING_SEPARATOR, searchFrom);
