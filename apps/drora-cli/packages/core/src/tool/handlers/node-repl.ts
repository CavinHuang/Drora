...
/**
 * 每个 drora session 一个持久 NodeReplSession，跨多次 js 调用保持内存状态（globalThis）。
...
function isBrowserSurfaceSideEffect(command: import("@drora/contracts").BrowserCommand): boolean {
...
function isAutoScreenshotTriggerCommand(
  command: import("@drora/contracts").BrowserCommand,
): boolean {
  if (
...
function buildJsToolDescription(options: NodeReplToolOptions = {}): string {
  ...
  return (
    ...
    "from its contents. " +
    "`tab.snapshot()` plus ref actions remain only as a drora compatibility fallback. For ordinary navigation, reading, search, and forms, " +
