...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          DRORA_HTTP_PROXY: url,
          DRORA_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.droraCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.DRORA_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.DRORA_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.DRORA_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.DRORA_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.DRORA_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
