...
The official built-in Drora plugin for configuration and self-diagnosis. It is content-only (skills) and enabled by default.

## What it provides

A set of skills that teach both users and agents how to configure Drora's five extension resource types in the Drora client, and how to locate and fix configuration problems for each:

| Skill | Purpose |
|---|---|
| `drora-configuration-guide` | Overview: the locations, scopes, precedence, and merge rules for MCP servers, commands, skills, hooks, and plugins, plus guidance on where to configure each |
