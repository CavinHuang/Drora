...
> Key facts: enable/disable state is stored under `plugins` in `~/.drora/cli/config.json`; the official marketplace is `drora-plugins-official`; and for cloning marketplace repositories behind a proxy, Drora reads the proxy from `DRORA_HTTP_PROXY` (a bare `http_proxy` is not used).
...
2. **Adding a marketplace or installing fails to clone** — an error such as `RPC failed`, `timed out`, or `early EOF` (after retries). The clone process did not inherit the shell proxy. → **Set `DRORA_HTTP_PROXY=http://host:port`** (Drora reads the proxy only from this variable; a bare `http_proxy` is ignored).
...
4. **A built-in plugin still appears after being disabled, or returns after uninstalling** — the suppression state was not applied where it was read. → Confirm the plugin id is in the suppressed-built-ins list in `~/.drora/cli/config.json`; restoring it removes that entry and re-seeds.
...
5. **Installation and network.** Reproduce the clone with the proxy set, confirming `DRORA_HTTP_PROXY`, and look for the retryable-error signatures.
