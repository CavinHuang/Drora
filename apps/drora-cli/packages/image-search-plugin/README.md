# Image Search for Drora

[中文文档](./README_CN.md)

This plugin registers the official Drora image search MCP server, so the agent can look up
...
- A Drora session signed in to an account that can reach `${DRORA_BASE_URL}`; the server uses
  official Drora JWT authentication, so no manual token or API key is needed.
- Network access to the Drora API endpoint.
- Start a new Drora session after installing or updating the plugin.
...
- Search results come from a third-party image index through the Drora service. Check the license of
