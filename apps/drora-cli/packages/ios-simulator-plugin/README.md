...
Drora plugin for model-driven iOS app development on macOS.

The package root is the plugin root:

```bash
drora --plugin-dir /absolute/path/to/ios-simulator-plugin
```

When enabled, Drora loads:
...
Drora bundles this package as the official `ios-simulator@drora-plugins-official`
plugin. Enable the plugin to project its skill, slash command, and MCP server
into the session.
At startup Drora copies the packaged plugin files into
`~/.drora/cli/plugins/cache/drora-plugins-official/ios-simulator/0.1.0/` and
runs the MCP server from that cache directory. SEA builds rewrite the official
plugin's Drora manifest to launch the cached MCP server through Drora's internal
...
Example `~/.drora/cli/config.json` entry:

```json
{
  "plugins": {
    "enabledPlugins": {
      "ios-simulator@drora-plugins-official": true
    },
    "options": {
      "ios-simulator@drora-plugins-official": {
...
The plugin MCP server name is `ios-simulator`. Drora normalizes that name for model-visible MCP tools, so the model sees `mcp__ios_simulator__<tool>`, for example `mcp__ios_simulator__ios_preflight`. The MCP server still implements the raw MCP tool names such as `ios_preflight`; Drora maps between the model-visible name and the server tool name.
...
```bash
pnpm install
pnpm --filter @drora/ios-simulator-plugin typecheck
pnpm --filter @drora/ios-simulator-plugin build
```

The Claude-compatible `.mcp.json` and the Drora manifest both execute
