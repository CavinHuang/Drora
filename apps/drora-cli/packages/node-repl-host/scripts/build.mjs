...
// esbuild 调用一个 define 都没有。于是正式包里 producer 的 `DRORA_CUA_HELPER_BUILD_ID`
// 折叠成空串（见 drora-cua/src/broker/shared/cua-version.ts 的兜底），
// `resolveExpectedCuaHelperBuildId()` 返回 null，helperInstaller 撞上 fail-closed 守卫：
//
//   "Packaged Drora is missing its embedded Computer Use Helper build identity;
...
// 有 5 行，09-14 为 0 行；`~/.drora/computer-use/logs/` 从未创建；把已安装的 Helper 改名后
// 也不会重装。dev 不受影响（ALLOW_UNSIGNED_LOCAL + 非 production runtime 绕过该守卫），
// 所以只在正式包暴露，本地怎么测都测不出来。
//
// 取值与 desktop 侧保持同一来源：CI 注入 DRORA_CUA_HELPER_BUILD_ID env；dev 为空串走兜底
...
export const buildNodeReplHostBundle = async ({
  ...
  await build({
    ...
    bundle: true,
    define: {
      __DRORA_CUA_HELPER_BUILD_ID__: JSON.stringify(cuaHelperBuildId),
    },
  ...
  if (cuaHelperBuildId) {
    ...
    if (!bundled.includes(cuaHelperBuildId)) {
      throw new Error(
        `[node-repl-host] DRORA_CUA_HELPER_BUILD_ID=${cuaHelperBuildId} 未折叠进 ${outfile}：` +
          "__DRORA_CUA_HELPER_BUILD_ID__ define 没有生效，正式包的 Helper 安装会被 fail-closed 拒绝。",
      );
    }
  }
