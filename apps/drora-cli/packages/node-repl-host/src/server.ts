...
import { JsInputJsonSchema } from "@drora/contracts/tools/node-repl";
// 值导入必须走 @drora/core/repl 这条深路径：barrel 会把 core 的整张图拖进 bundle
// （tool handlers → @drora/dynamic-workflow → typescript，实测 21.7MB 且求值即崩
...
} from "@drora/core/repl";
import { createComputerUseRuntime, type ComputerUseRuntime } from "@drora/drora-cua";
import { z } from "zod";
...
const cuaDocumentationRoot = resolve(
  process.env.DRORA_CUA_PLUGIN_ROOT ?? pluginRoot,
  "docs",
);
...
function buildRequestMeta(meta: Record<string, unknown> | undefined): NodeReplRequestMeta {
  const parsed = requestContextSchema.safeParse(meta?.["com.drora/request-context"]);
  // 安全边界：顶层 MCP _meta 是第三方可扩展字段，不能成为 Drora session 路由凭据。
...
export function captureComputerUseRuntimeFromEnvironment(
  ...
  const socketPath = env.DRORA_CUA_PERMISSION_BROKER_SOCKET?.trim();
  if (!socketPath) return undefined;
  return createComputerUseRuntime({
    brokerSocketPath: socketPath,
    refreshMarkerPath: env.DRORA_CUA_PERMISSION_BROKER_REFRESH_MARKER?.trim(),
  });
}
