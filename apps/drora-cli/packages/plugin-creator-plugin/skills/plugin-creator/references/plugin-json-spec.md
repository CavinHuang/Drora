# Drora plugin and marketplace reference
...
For a local marketplace, use `.claude-plugin/marketplace.json` at its root. The root-level `marketplace.json` is also supported by Drora. Sources are resolved against that root, not the `.claude-plugin` directory.
...
Codex's `interface`, availability/authentication `policy`, `.app.json`, implicit `~/.agents/plugins/marketplace.json` discovery and `codex://` handoff links are not this workflow's contract. Use Drora's format, and distinguish source inspection from any schema/runtime validation actually performed.
