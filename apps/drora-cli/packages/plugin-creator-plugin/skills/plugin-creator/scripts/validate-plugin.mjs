...
/** 附加完整性检查，不复制 Drora 的 manifest schema。 */
...
export async function validatePlugin(path, cli = "drora") {
  const errors = await preflightPlugin(path);
  if (errors.length) throw new Error(errors.join("\n"));
  const nodeEntry = /\.[cm]?js$/u.test(cli);
  if (process.platform === "win32" && /\.cmd$/iu.test(cli))
    throw new Error("Use --cli with drora.exe or the CLI JavaScript entrypoint on Windows");
  ...
  if (code !== 0) throw new Error(`Drora plugin validation failed (${code})`);
}
async function main() {
  ...
  if (positionals.length !== 1)
    throw new Error(
      "Usage: node validate-plugin.mjs <plugin-path> [--cli drora-executable-or-js-entry]",
    );
