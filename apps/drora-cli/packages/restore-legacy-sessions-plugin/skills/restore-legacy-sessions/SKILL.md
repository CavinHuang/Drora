...
description: Use when Drora needs to inspect, plan, or execute restoration of old ACP-era Drora sessions from ~/.drora/v2/sessions into the new Drora task/session stores. Trigger when the user wants to choose a previous agent, select a workspace to restore, pick a specific historical conversation, dry-run legacy session migration, or reason about tasks-index.sqlite and ~/.drora/cli/db/db.sqlite consistency.
---
...
Use this skill to guide old ACP-era Drora session restoration. The first job is selection, not mutation: identify the source agent, the workspace, and the exact conversation before proposing any write to the new stores.

## Stores

- Legacy snapshots: `~/.drora/v2/sessions/{workspaceHash}/{legacyTaskId}.json`
- Task index: `~/.drora/v2/tasks-index.sqlite`
- New Drora session DB: `~/.drora/cli/db/db.sqlite`
...
The scan script defaults to the live legacy source `~/.drora/v2/sessions`; pass `--legacy-dir <path>` only when the user explicitly asks to inspect another source directory.

Restored ACP-era conversations are normal Drora Agent history, so persisted provider fields must be `glm`. Treat the snapshot provider as legacy source context only; do not write task-index `migration_source` or `meta_json.migrationSource` for these restores. The `migrationSource` enum is reserved for the existing Claude Code native import path (`claudeCode`).
...
```bash
node scripts/restore-conversation.mjs --snapshot ~/.drora/v2/sessions/<workspaceHash>/<legacyTaskId>.json --dry-run
```

Then apply the restore:

```bash
node scripts/restore-conversation.mjs --snapshot ~/.drora/v2/sessions/<workspaceHash>/<legacyTaskId>.json
```

## Restore States

- `ready`: CLI DB and task-index both already contain the restored task id.
- `needs-cli-db`: task-index exists, but `~/.drora/cli/db/db.sqlite` has no session row; rebuild the real session from the legacy snapshot.
...
- Do not start old ACP runtimes. Restore from snapshots and new Drora stores only.
