#!/usr/bin/env node
/**
 * Restore a single legacy ACP-era conversation snapshot into the new Drora stores.
 *
 * Usage:
 ...
 *   --task-index <path> Task index sqlite path. Default: ~/.drora/v2/tasks-index.sqlite
 *   --cli-db <path>     New Drora session sqlite path. Default: ~/.drora/cli/db/db.sqlite
...
const DRORA_PROVIDER = "glm";
const defaultTaskIndexPath = join(homedir(), ".drora", "v2", "tasks-index.sqlite");
const defaultCliDbPath = join(homedir(), ".drora", "cli", "db", "db.sqlite");
...
function buildRestorePlan(snapshotPath) {
  ...
  const traceId = optionalString(meta.traceId) || `drora-${restoredTaskId}`;
  const createdAt = optionalNumber(meta.createdAt) ?? Date.now();
  const updatedAt = optionalNumber(meta.updatedAt) ?? createdAt;

  // 旧 ACP provider 只代表来源。恢复后的新 Drora 任务必须按 glm 写入，否则任务列表会被过滤掉。
...
function printPlan(plan, dryRun) {
  ...
  console.log(`  RuntimeProvider: ${DRORA_PROVIDER}`);
