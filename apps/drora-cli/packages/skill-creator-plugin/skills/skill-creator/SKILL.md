...
A skill for authoring and iteratively improving local Drora skills.
...
Drora discovers skills in these directories (highest priority first):

- `<project>/.drora/skills/<name>/SKILL.md`
- `<project>/.agents/skills/<name>/SKILL.md`
- `~/.drora/skills/<name>/SKILL.md`
- `~/.agents/skills/<name>/SKILL.md`

**Default to creating new skills under `.agents/skills/`** — it's the standard, cross-tool location for new skills. Note that `.drora/skills` still takes priority during discovery: if the same skill name exists in both, the `.drora/skills` copy wins, so `.drora/skills` is the place to *override* a skill. Pick the `<project>` path for skills that only make sense in this repo; pick the `~/` (user) path for personal skills you want everywhere.
...
Optional (reserved) frontmatter fields are listed in the Drora skill spec; for most skills you only need `name` and `description`.

### Progressive disclosure

Drora loads skills in three layers:
...
Then run them: load the draft skill, hand the model the test prompt, and inspect what happens. Drora does not currently spawn parallel evaluation subagents, so do this one prompt at a time and look at each result with the user.
...
1. Make sure the draft skill is on disk where Drora can discover it (one of the directories listed above).
2. In a fresh Drora turn, give the test prompt to the model. Either let the description trigger the skill, or use `/skill <name> <prompt>` to force-load it.
