...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "drora.agent.turn.duration"),
      agent_step: duration(meter, "drora.agent.step.duration"),
      tool_execution: duration(meter, "drora.tool.execution.duration"),
      command_execution: duration(meter, "drora.command.execution.duration"),
      context_compaction: duration(meter, "drora.context.compaction.duration"),
      detached_operation: duration(meter, "drora.detached.operation.duration"),
      model_call: duration(meter, "drora.model.call.duration"),
      model_attempt: duration(meter, "drora.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("drora.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("drora.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "drora.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "drora.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "drora.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("drora.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "drora.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "drora.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("drora.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("drora.telemetry.abandoned.count", {
