...
import type {
  ...
  ToolTraceStart,
} from "@drora/contracts/telemetry";
import {
  activeWriterContext,
  BaseSpanWriter,
  compactAttributes,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "drora.agent_turn.turn_number": integer(input.turnNumber),
            "drora.agent_turn.input_source": safeEnum(input.inputSource),
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "drora.agent_step.step_id": safeId(input.stepId),
            "drora.agent_step.step_index": integer(input.stepIndex),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "drora.execution.tool_call_id": safeId(input.toolCallId),
            "drora.tool_execution.tool_name": toolName,
            ...toolCompatibilityAttributes({
    ...
    return this.safeCreate(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent?.correlation),
            "drora.context_compaction.trigger": safeEnum(input.trigger),
            "drora.context_compaction.phase": safeEnum(input.phase),
            "drora.context_compaction.model_mode": safeEnum(input.modelMode),
            "drora.context_compaction.outer_attempt": integer(input.outerAttempt),
            "drora.context_compaction.max_attempts": integer(input.maxAttempts),
            "drora.context_compaction.triggering_step_index": integer(input.triggeringStepIndex),
            "drora.context_compaction.policy_context_window_tokens": finiteNonNegative(
              input.policyContextWindowTokens,
            ),
            "drora.context_compaction.threshold_tokens": finiteNonNegative(input.thresholdTokens),
            "drora.context_compaction.token_source": safeEnum(input.tokenSource),
            "drora.context_compaction.recovered_from_logical_call_id": safeId(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "drora.detached_operation.operation": safeEnum(input.operation),
            "drora.detached_operation.execution_kind": safeEnum(input.executionKind),
            "drora.detached_operation.trigger": safeEnum(input.trigger),
            "drora.detached_operation.target_kind": safeEnum(input.targetKind),
            "drora.detached_operation.goal_iteration": integer(input.goalIteration),
            "drora.detached_operation.chunk_index": integer(input.chunkIndex),
            "drora.detached_operation.chunk_count": integer(input.chunkCount),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "drora.execution.logical_call_id": safeId(input.logicalCallId),
            "drora.model_call.operation": safeEnum(input.operation),
            "drora.model_call.streaming": input.streaming,
            "drora.model_call.model_role": safeEnum(input.modelRole),
            "drora.model_call.requested_provider_id": safeString(input.requested.providerId, 128),
            "drora.model_call.requested_model": safeString(input.requested.requestedModel, 128),
            "drora.model_call.reasoning_capability": safeEnum(input.requested.reasoning.capability),
            "drora.model_call.reasoning_requested_state": safeEnum(
              input.requested.reasoning.requestedState,
            ),
            "drora.model_call.reasoning_requested_control": safeEnum(
              input.requested.reasoning.requestedControl,
            ),
            "drora.model_call.reasoning_requested_level": safeString(
              input.requested.reasoning.requestedLevel,
              128,
            ),
            "drora.model_call.reasoning_requested_budget_tokens": integer(
              input.requested.reasoning.requestedBudgetTokens,
            ),
            "drora.model_call.call_cause": safeEnum(input.callCause),
            "drora.model_call.previous_logical_call_id": safeId(input.previousLogicalCallId),
          }),
          context: parentContext,
          correlation: parent?.correlation,
          parent,
    ...
    return this.safeCreate(
      ...
      () => {
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent.correlation),
            "drora.execution.tool_call_id": safeId(parent.toolCallId),
            "drora.command_execution.safe_name": safeString(input.safeName, 128),
            "drora.command_execution.category": safeEnum(input.category),
            "drora.command_execution.command_count": integer(input.commandCount),
            "drora.command_execution.shell_kind": safeEnum(input.shellKind),
            "drora.command_execution.sandboxed": input.sandboxed,
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "drora.execution.tool_call_id": safeId(parent.toolCallId),
            "drora.execution.logical_call_id": parentWriter.logicalCallId,
            "drora.execution.model_operation": parentWriter.operation,
            "drora.execution.model_role": parentWriter.modelRole,
            "drora.model_attempt.request_id": safeId(input.requestId),
            "drora.model_attempt.attempt_number": integer(input.attemptNumber),
            "drora.model_attempt.max_attempts": integer(input.maxAttempts),
            "drora.model_attempt.attempt_cause": safeEnum(input.attemptCause),
            "drora.model_attempt.previous_request_id": safeId(input.previousRequestId),
            "drora.model_attempt.retry_delay_ms": finiteNonNegative(input.retryDelayMs),
            "drora.model_attempt.provider_id": safeString(target.providerId, 128),
            "drora.model_attempt.provider_kind": safeEnum(target.providerKind),
            "drora.model_attempt.provider_origin": safeString(target.providerOrigin),
            "drora.model_attempt.provider_route": safeString(target.providerRoute),
            "drora.model_attempt.requested_model": safeString(target.requestedModel, 128),
            "drora.model_attempt.transport": safeEnum(input.transport),
            "drora.model_attempt.api_operation": safeEnum(input.apiOperation),
            "drora.model_attempt.reasoning_capability": safeEnum(target.reasoning.capability),
            "drora.model_attempt.reasoning_requested_state": safeEnum(
              target.reasoning.requestedState,
            ),
            "drora.model_attempt.reasoning_requested_control": safeEnum(
              target.reasoning.requestedControl,
            ),
            "drora.model_attempt.reasoning_requested_level": safeString(
              target.reasoning.requestedLevel,
              128,
            ),
            "drora.model_attempt.reasoning_requested_budget_tokens": integer(
...
class TurnWriter extends TrackedBaseWriter implements AgentTurnSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() => this.setAttribute("drora.agent_turn.result_type", resultType));
  }
...
class StepWriter extends TrackedBaseWriter implements AgentStepSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() =>
      this.setAttribute("drora.agent_step.terminal_reason", terminalReason),
    );
  }
...
class ToolWriter extends TrackedBaseWriter implements ToolExecutionSpanWriter {
  ...
  setPermissionDecision(decision: "granted" | "denied" | "not_required"): void {
    this.setAttribute("drora.tool_execution.permission_decision", decision);
  ...
  setOutputBytes(bytes: number): void {
    this.setAttribute("drora.tool_execution.output_bytes", finiteNonNegative(bytes));
  }

  setOutputTruncated(truncated: boolean): void {
    this.setAttribute("drora.tool_execution.output_truncated", truncated);
  }
  ...
  finishDenied(reason: "user_denied" | "policy_denied" | "unavailable" | "unknown"): void {
    this.finishDomainOutcomeIfOpen("denied", () =>
      this.setAttribute("drora.tool_execution.permission_denial_reason", reason),
    );
  }
...
class CommandWriter extends TrackedBaseWriter implements CommandExecutionSpanWriter {
  ...
  markFirstOutput(): void {
    ...
    this.setAttribute("drora.command_execution.first_output_ms", elapsed);
  ...
  setExitCode(exitCode: number): void {
    ...
    this.setAttribute("drora.command_execution.exit_code", normalized);
  ...
  setSignal(signal: string): void {
    ...
    this.setAttribute("drora.command_execution.signal", normalized);
    this.setAttributes(commandCompatibilityAttributes({ signal: normalized }));
  }

  setOutputBytes(bytes: number): void {
    this.setAttribute("drora.command_execution.output_bytes", finiteNonNegative(bytes));
  }

  setTimedOut(timedOut: boolean): void {
    this.setAttribute("drora.command_execution.timed_out", timedOut);
  }
...
class CompactionWriter extends TrackedBaseWriter implements ContextCompactionSpanWriter {
  ...
  setInputTokens(tokens: number): void {
    this.setAttribute("drora.context_compaction.input_tokens", finiteNonNegative(tokens));
  }

  setOutputTokens(tokens: number): void {
    this.setAttribute("drora.context_compaction.output_tokens", finiteNonNegative(tokens));
  }
...
class DetachedWriter extends TrackedBaseWriter implements DetachedOperationSpanWriter {
  ...
  setResultType(resultType: "text" | "boolean" | "metadata" | "other"): void {
    this.setAttribute("drora.detached_operation.result_type", resultType);
  }
...
class ModelAttemptWriter extends TrackedBaseWriter implements ModelAttemptSpanWriter {
  ...
  setProviderRequestId(requestId: string): void {
    this.setAttribute("drora.model_attempt.provider_request_id", safeString(requestId, 256));
  }

  setResponseModel(model: ResponseModelTelemetryDescriptor): void {
    const value = safeString(model.model, 128);
    this.setAttribute("drora.model_attempt.response_model", value);
  ...
  setEffectiveReasoningState(state: ModelReasoningState): void {
    this.setAttribute("drora.model_attempt.reasoning_effective_state", safeEnum(state));
  }

  setEffectiveReasoningControl(control: ModelReasoningControlType): void {
    this.setAttribute("drora.model_attempt.reasoning_effective_control", safeEnum(control));
  }

  setEffectiveReasoningLevel(level: string): void {
    this.setAttribute("drora.model_attempt.reasoning_effective_level", safeString(level, 128));
  }

  setEffectiveReasoningBudgetTokens(tokens: number): void {
    this.setAttribute(
      "drora.model_attempt.reasoning_effective_budget_tokens",
  ...
  setFinishReason(reason: ModelFinishReason): void {
    ...
    this.setAttribute("drora.model_attempt.finish_reason", value);
    this.setAttributes(modelResponseCompatibilityAttributes({ finishReason: value }));
  }

  setInputTokens(tokens: number): void {
    this.setUsage("inputTokens", "input", tokens, "drora.model_attempt.input_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ inputTokens: tokens }));
  }

  setOutputTokens(tokens: number): void {
    this.setUsage("outputTokens", "output", tokens, "drora.model_attempt.output_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ outputTokens: tokens }));
  }

  setReasoningTokens(tokens: number): void {
    this.setUsage("reasoningTokens", "reasoning", tokens, "drora.model_attempt.reasoning_tokens");
  }

  setCacheReadTokens(tokens: number): void {
    this.setUsage("cacheReadTokens", "cache_read", tokens, "drora.model_attempt.cache_read_tokens");
  }
  ...
  setCacheWriteTokens(tokens: number): void {
    this.setUsage(
      ...
      "cache_write",
      tokens,
      "drora.model_attempt.cache_write_tokens",
    );
  }

  setStreamOutputCommitted(committed: boolean): void {
    this.setAttribute("drora.model_attempt.stream_output_committed", committed);
  }

  setHttpStatusCode(statusCode: number): void {
    const normalized = integer(statusCode);
    this.setAttribute("drora.model_attempt.http_status_code", normalized);
    this.setAttributes(httpResponseCompatibilityAttributes(statusCode));
  }

  setProviderErrorCode(code: string): void {
    this.setAttribute("drora.model_attempt.provider_error_code", safeString(code, 128));
  }

  setProviderErrorMessage(message: string): void {
    this.setAttribute("drora.model_attempt.provider_error_message", sanitizeErrorMessage(message));
  }

  setRetryAfterMs(delayMs: number): void {
    this.setAttribute("drora.model_attempt.retry_after_ms", finiteNonNegative(delayMs));
  }
  ...
  markFirstProviderEvent(): void {
    ...
    this.setAttribute("drora.model_attempt.time_to_first_provider_event_ms", elapsed);
  ...
  markFirstContent(): void {
    ...
    this.setAttribute("drora.model_attempt.time_to_first_content_ms", elapsed);
  ...
  markFirstText(): void {
    ...
    this.setAttribute("drora.model_attempt.time_to_first_text_ms", elapsed);
