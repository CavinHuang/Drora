...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@drora/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 Drora Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "drora.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "drora.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "drora.execution.turn_id": safeId(correlation.turnId),
    "drora.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "drora.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "drora.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "drora.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "drora.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "drora.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "drora.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
