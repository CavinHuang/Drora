...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@drora/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@drora/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("drora.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("drora.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "drora.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "drora.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("drora.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("drora.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "drora.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("drora.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("drora.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("drora.model.attempt.tokens"),
    cardinalityView("drora.model.attempt.stream_stall.count"),
    cardinalityView("drora.telemetry.creation_drop.count"),
    cardinalityView("drora.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "drora.build.commit_id": resource.buildCommitId,
    "drora.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "drora.product.version": resource.productVersion,
    "drora.runtime.distribution": resource.runtimeDistribution,
    "drora.runtime.surface": resource.runtimeSurface,
    "drora.telemetry.schema_owner": "cli",
    "drora.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
