import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@drora/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getDroraCopy(result.locale ?? input.fallback.locale).tui;
