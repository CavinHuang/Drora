...
if (check) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "drora-bash-registry-"));
  ...
  try {
    ...
    if (!actual.equals(expected)) {
      throw new Error(
        "Generated Bash command registry is stale. Run `pnpm --dir apps/drora-cli registry:generate`.",
      );
    }
  } finally {
