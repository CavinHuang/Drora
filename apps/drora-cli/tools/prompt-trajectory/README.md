# @drora/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting drora-cli prompt assembly.
...
```bash
pnpm --filter @drora/bootstrap^... build
pnpm --filter @drora/bootstrap build

pnpm --filter @drora/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/drora-prompt-trajectory/basic-live

pnpm --filter @drora/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @drora/prompt-trajectory derive -- \
  --out /tmp/drora-prompt-trajectory/basic-live

pnpm --filter @drora/prompt-trajectory model-io -- \
  --input ~/.drora/cli/debug/model-io-<session>.jsonl \
  --out /tmp/drora-prompt-trajectory/model-io-session
```
...
uses the same drora model config resolution as the CLI. The upstream request is
...
`model-io` reads a real Drora `model-io-*.jsonl` file and turns the main
