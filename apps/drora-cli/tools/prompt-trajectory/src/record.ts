...
interface DroraBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createDroraApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface DroraContractsModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadDroraModules();
    ...
    const app = await bootstrap.createDroraApp({
...
async function loadDroraModules(): Promise<{
  bootstrap: DroraBootstrapModule;
  contracts: DroraContractsModule;
  provider: DroraProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<DroraBootstrapModule>,
      import(contractsPath) as Promise<DroraContractsModule>,
      import(providerConfigPath) as Promise<Omit<DroraProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<DroraProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built drora-cli packages. Run `pnpm --filter @drora/bootstrap^... build && pnpm --filter @drora/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
