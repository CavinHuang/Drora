...
export async function logs(input: Logs = {}) {
  ...
  if (input.bundleId) {
    if (!ID.test(input.bundleId)) throw new Error(`Invalid bundle id: ${input.bundleId}`);
    args.push(
      "--predicate",
      `subsystem CONTAINS "${input.bundleId}" OR process CONTAINS "${input.bundleId.split(".").at(-1)}"`,
    );
  }
