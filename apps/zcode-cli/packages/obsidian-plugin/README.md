# @zcode/obsidian-plugin

官方 ZCode 插件：把本机 Obsidian Vault 接入 Agent —— 发现/授权 Vault、安全地浏览与检索
...
```bash
pnpm --dir apps/zcode-cli/packages/obsidian-plugin run build   # tsc + esbuild → dist/mcp/server.js
pnpm --dir apps/zcode-cli/packages/obsidian-plugin run test    # 安全语义单测 + stdio E2E
pnpm --dir apps/zcode-cli/packages/obsidian-plugin run typecheck
```
