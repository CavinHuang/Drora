...
This skill assumes the MCP server is configured in ZCode as `obsidian`. Tools are exposed to the model as `mcp__obsidian__<tool>` (for example `mcp__obsidian__obsidian_read_file`). If the server has a different name, use the corresponding visible `mcp__<server>__...` names from the active tool list.
