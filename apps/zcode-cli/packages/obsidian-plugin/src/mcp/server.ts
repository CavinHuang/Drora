...
function pluginDataDir(): string {
  ...
  return resolve(homedir(), ".zcode", "obsidian-plugin-data");
}
...
server.registerTool(
  "obsidian_configure_vault",
  {
    ...
    inputSchema: {
      ...
      display_name: z.string().optional().describe("Optional display name; defaults to the folder name (managed Vault: 'ZCode Vault')."),
