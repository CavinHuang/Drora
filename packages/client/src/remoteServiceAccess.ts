import { ProxyChannel, type IChannelClient } from "@drora/rpc";
import {
  IFileService,
...
export class RemoteServiceAccess implements IServiceAccessor {
  ...
  readonly droraTaskService: IDroraTaskService;
  readonly windowControllerService: IWindowControllerService;
  readonly droraAgentService: IDroraAgentService;
  readonly droraSessionService: IDroraSessionService;
  ...
  constructor(channelClient: IChannelClient) {
    ...
    this.droraTaskService = ProxyChannel.toService<IDroraTaskService>(
      channelClient.getChannel(IDroraTaskService.channelName),
    );
    ...
    this.droraAgentService = ProxyChannel.toService<IDroraAgentService>(
      channelClient.getChannel(IDroraAgentService.channelName),
    );
    this.droraSessionService = ProxyChannel.toService<IDroraSessionService>(
      channelClient.getChannel(IDroraSessionService.channelName),
    );
