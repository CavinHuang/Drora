...
{
    private const string Protocol = "DRORA_BROWSER_IMPORT_V1";
        ...
        try
        {
            if (HasArgument(args, "--version"))
            {
                ...
                Console.Out.WriteLine(
                    "DRORA_BROWSER_IMPORT_HELPER\t" + HelperVersion + "\t" + GetProcessArchitecture() +
    ...
    private static void RunBrokerCore()
    {
        ...
        try
        {
            ...
            pipeName = "drora-browser-import-broker-" + Guid.NewGuid().ToString("N");
            token = CreateToken();
    ...
    private static void RunTemporarySystemService(
        ...
        int brokerPid)
    {
        string serviceName = "DroraBrowserImport_" + Guid.NewGuid().ToString("N");
        string pipeName = "drora-browser-import-system-" + Guid.NewGuid().ToString("N");
