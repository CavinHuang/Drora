...
const targetArch = (process.env.DRORA_TARGET_ARCH ?? arch()).toLowerCase();
const targetKey = `win32-${targetArch}`;
const outputDir = resolve(desktopRoot, `bundled-tools/${targetKey}/browser-import`);
const outputPath = resolve(outputDir, "drora-browser-import-helper.exe");
...
if (targetArch === "arm64" && !process.env.DRORA_CSC_PATH) {
  throw new Error(
    "Windows arm64 browser import helper requires DRORA_CSC_PATH pointing to an arm64-capable Roslyn csc.exe",
  );
}
if (!cscPath) {
  throw new Error("Windows C# compiler not found; set DRORA_CSC_PATH to a trusted csc.exe");
}
