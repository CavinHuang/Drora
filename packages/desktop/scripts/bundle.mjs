...
export function createElectronRuntimeMirrorEnv(mirror) {
  return {
    DRORA_ELECTRON_RUNTIME_MIRROR: mirror,
...
function resolveDefaultElectronBuilderBinariesMirror(env = process.env) {
  return env.DRORA_DEPS_BASE_URL?.trim() || env.INTRANET_MACHINE_HOST?.trim()
...
环境变量:
  DRORA_TARGET_OS              与 --os 等价
  DRORA_TARGET_ARCH            与 --arch 等价
`);
}
...
function parseArgs(argv) {
  const options = {
    os: process.env.DRORA_TARGET_OS ?? null,
    arch: process.env.DRORA_TARGET_ARCH ?? null,
    skipPrepare: process.env.DRORA_SKIP_PREPARE === "1",
    skipBuild: process.env.DRORA_SKIP_BUILD === "1",
    dryRun: false,
    positionals: [],
  };
