// Agent bundle 的暂存动作：把 apps/drora-cli/packages/cli/dist/drora.cjs 放进
...
// desktopRuntimeEnv.ts 的 resolveBundledDroraAgentBinaryPath() 解析，候选**只有**
...
export const AGENT_BUNDLE_SOURCE_RELATIVE = "apps/drora-cli/packages/cli/dist/drora.cjs";
...
export function resolveAgentBundlePaths({ repoRoot, platformKey }) {
  ...
  return {
    ...
    glmDir,
    stagedBundlePath: resolve(glmDir, "drora.cjs"),
 ...
 * （drora-agent / drora-acp 等）和旧 meta 会被一并打进安装包（CI 干净检出不会有，本地会）。
 */
