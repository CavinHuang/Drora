...
import type { IRemoteBackend } from "@drora/server/remote";
import type { BrowserRecordingArtifact } from "@drora/shared";
...
export async function materializeBrowserRecordingArtifact(input: {
  ...
  const stagingPath = `${targetPath}.drora-recording-${randomUUID()}.tmp`;
  try {
