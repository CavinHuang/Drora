...
import {
  ...
  createDroraAgentConnectionScope,
  type DroraAgentV4ClientMode,
...
import {
  ...
  type DroraAutomationRunOutcome,
  type ModelSelection,
} from "@drora/shared";
import {
...
const remoteMediaRangePreviewEnabled =
  process.env["DRORA_REMOTE_MEDIA_RANGE_PREVIEW_ENABLED"] !== "0";
...
// 这里根据 main 传入的窗口 label 补一层稳定的 drora-* title，方便系统进程列表过滤。
...
function writeHostLog(level: HostLogLevel, ...args: unknown[]): void {
  const prefix = formatLogPrefix("drora-host", process.pid);
  const consoleFn =
...
function createFullFeedbackLogArchiveViaMain(
  ...
  return new Promise((resolve, reject) => {
    ...
    // 收集范围和“导出日志”不一致，缺少 drora-cli 日志、rollout/debug 以及导出链路脱敏。
...
async function resolveOffPeakFilesChanged(params: {
  ...
  try {
    const snapshot = await params.droraTaskService.getTaskSnapshot({
...
async function finalizeOffPeakRun(params: {
  ...
  workspacePath: string;
  workspaceIdentity?: string;
  outcome: DroraAutomationRunOutcome;
  error?: string;
}): Promise<void> {
  ...
  void params.droraTaskService.setTaskUnread({
    taskId: params.taskId,
    workspacePath: params.workspacePath,
    ...(params.workspaceIdentity ? { workspaceIdentity: params.workspaceIdentity } : {}),
    unread: true,
...
function trackOffPeakRunOutcome(params: {
  ...
  const disposable = params.droraTaskService.onDynamicTaskTerminalOutcome(params.taskId)(
    (result) => {
      if (result.inputId !== params.traceId) return;
      disposeOffPeakRunSubscription(key);
      void finalizeOffPeakRun({
        droraTaskService: params.droraTaskService,
...
async function dispatchOffPeakRun(request: OffPeakRunDispatchRequest): Promise<{
  ...
  const droraTaskService = activeServices?.getOptional(IDroraTaskService);
  if (!droraTaskService) {
    throw new Error("Drora task service is not initialized.");
  }
  const runtime = await ensureOffPeakRuntime();
  if (!runtime) {
  ...
  try {
    ...
    if (dispatchKind === "bound-first-run") {
      ...
      const [deletedIds, tasks] = await Promise.all([
        droraTaskService.listDeletedTaskIds(workspaceScope),
        droraTaskService.listTasks(workspaceScope),
      ]);
      assertBoundSessionDispatchable({
        sessionId: taskId,
      ...
      await droraTaskService.setConfigOption({
        taskId,
        traceId,
        configId: "mode",
      ...
      const task = await droraTaskService.createTask({
        ...
        mode: request.permissionMode as DroraTaskMode,
    ...
    await droraTaskService.sendPrompt({
      taskId,
      traceId,
...
async function dispatchCronRun(request: CronRunDispatchRequest): Promise<{
  ...
  const droraTaskService = targetServices.getOptional(IDroraTaskService);
  if (!droraTaskService) {
    throw new Error("Drora task service is not initialized.");
  }
  ...
  try {
    const task = request.targetTaskId
      ...
      : await droraTaskService.createTask({
          workspacePath: request.workspacePath,
          workspaceIdentity: request.workspaceIdentity,
...
function isDroraTaskMeta(value: unknown): value is DroraTaskMeta {
  return (
    typeof value === "object" &&
    value !== null &&
    "taskId" in value &&
...
function createReportingRemoteDroraTaskService<T extends object>(
  service: T,
  options?: {
    ...
    }) => Promise<{ content: string; attachments?: DroraPromptAttachment[] }>;
  },
): T {
  ...
  function subscribeSessionMessageRequests(target: T, meta: DroraTaskMeta): void {
  ...
  async function mirrorRemotePrompt(
    ...
    // droraSessionStore 的整条消费链词表都是 DroraStreamEvent。两个方案的评估结论：
    // a) relay 直接转发 v4 帧、手机端消费 v4 store（正解）：需要重做 relay stream-op
    //    协议 + 手机端 store；
    // b) 帧→DroraStreamEvent 薄映射：等价复刻 adapter mapSessionEvent，
    //    否决。
    // 结论：本镜像保持 legacy 源不动。
    ...
    try {
      ...
      // 远端 drora-server 没有 desktop realtime port；由窗口 Host 内的
  ...
  function finishWorkspaceTask(taskId: string, meta: DroraTaskMeta): void {
    workspaceProxyState.disposeTaskReadySubscription(taskId);
    workspaceTaskTracker.finish(taskId, meta);
  }

  function beginWorkspaceTask(target: T, taskId: string, meta: DroraTaskMeta): boolean {
    ...
    if (typeof onDynamicTaskReady !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote Drora task service does not expose onDynamicTaskReady");
    }
    const subscribe = onDynamicTaskReady.call(target, taskId);
    if (typeof subscribe !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote Drora task ready event is not subscribable");
    }
  ...
  // remote workspace 的 Drora Agent manager 跑在远端 server，desktop main 不能直接看到
  ...
  return new Proxy(service, {
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        let trackedTask: { taskId: string; meta: DroraTaskMeta; started: boolean } | undefined;
...
function warmUpDroraAgent(
  ...
  const droraSessionService = services.getOptional(IDroraSessionService);
  if (!droraSessionService) {
    return;
  }
    ...
    .then((result) => {
      if (!result.available) {
        if (result.reasonCode === "provider_not_ready") {
          logger.info(
            `Drora agent warmup waiting for provider/model (${reason}) workspace=${workspacePath}`,
          );
          return;
        }
        logger.warn(
          `Drora agent warmup unavailable (${reason}) workspace=${workspacePath} reason=${result.reason ?? "unknown"}`,
        );
        return;
      }
      ...
      logger.info(
        `Drora agent warmup ready (${reason}) workspace=${workspacePath} transport=${result.transportKind ?? "unknown"}`,
      );
    })
    .catch((error) => {
      logger.warn(`Drora agent warmup failed (${reason}) workspace=${workspacePath}:`, error);
    });
}
...
const hostResourceUsageResponder = createHostResourceUsageResponder({
  getAgentService: () => activeServices?.getOptional(IDroraAgentService),
...
>({
  ...
  releaseWorkspace: async (services, context) => {
    await services.get(IDroraTaskService).releaseWorkspacePreparation({
  ...
  resolveSource: (scope) => {
    ...
    if (remoteSession?.workspacePath && remoteSession.workspaceIdentity) {
      ...
      return {
        ...
        taskService: services.get(IDroraTaskService),
        agentService: services.getOptional(IDroraAgentService),
    ...
    const taskService = activeServices?.getOptional(IDroraTaskService);
    if (!taskService) {
      return null;
    }
    return {
      ...
      taskService,
      agentService: activeServices?.getOptional(IDroraAgentService),
      sourceAvailability: "online" as const,
    };
  },
});
  ...
  return new Proxy(base, {
    get(target, property, receiver) {
      if (property === "setTaskPinned") {
        return async (params: Parameters<IDroraTaskService["setTaskPinned"]>[0]) => {
      ...
      if (property === "archiveTask" || property === "unarchiveTask") {
        return async (
          params:
            | Parameters<IDroraTaskService["archiveTask"]>[0]
            | Parameters<IDroraTaskService["unarchiveTask"]>[0],
        ) => {
      ...
      if (property === "deleteTask") {
        return async (params: Parameters<IDroraTaskService["deleteTask"]>[0]) => {
          await route(params, { kind: "delete" });
        };
      }
      if (property === "deleteArchivedTasks") {
        return async (params: Parameters<IDroraTaskService["deleteArchivedTasks"]>[0]) => {
      ...
      if (property === "deleteArchivedTask") {
        return async (params: Parameters<IDroraTaskService["deleteArchivedTask"]>[0]) =>
      ...
      if (property === "setTaskUnread") {
        return async (params: Parameters<IDroraTaskService["setTaskUnread"]>[0]) => {
...
function exposeServicesOnMessagePort(
  ...
  services: ServiceCollection,
  deferInit: boolean,
  clientMode: DroraAgentV4ClientMode = "desktop-continuous",
  ...
  const agentService = services.getOptional(IDroraAgentService);
  const connectionScope = agentService
    ? createDroraAgentConnectionScope(agentService, {
  ...
  const taskService = services.getOptional(IDroraTaskService);
  if (taskService) {
    overrides.set(
      IDroraTaskService.channelName,
      createControllerRoutedTaskService(taskService, attachmentScope),
    );
  }
  if (connectionScope) {
    overrides.set(IDroraAgentService.channelName, connectionScope.service);
  }
...
parentPort.on("message", async (e: Electron.MessageEvent) => {
    ...
    void (async () => {
      try {
        const dispatchResult = await dispatchCronRun({
          ...msg,
          mode: msg.mode as DroraTaskMode | undefined,
        });
        parentPort.postMessage({
          type: HostResponseTypes.CronRunResult,
          runId: msg.runId,
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliver) {
    ...
    if (!droraTaskService) {
      parentPort.postMessage({
        ...
        result: {
          error: "Drora task service is not initialized.",
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliveryResult) {
    const droraTaskService = activeServices?.getOptional(IDroraTaskService);
    if (!droraTaskService) {
      logger.warn("session message delivery result received before Drora task service initialized");
      return;
    }
    void droraTaskService.sendSessionMessageDeliveryResult(msg.result).catch((error) => {
  ...
  if (msg.type === HostMessageTypes.InitLocal) {
    ...
    databaseStartup = createHostDatabaseStartup({
      ...
      initializeServices: async () => {
        ...
        const services = await initializeHostApiNetworkTransportOwner({
          ...
          establishOwner: () => {
            const initializedServices = createLocalServices({
              ...
              droraAgentSpawnFallbackCwd: msg.agentSpawnFallbackCwd,
              droraBuiltinProviderConfigFilePath: msg.droraBuiltinProviderConfigFilePath,
        ...
        const droraTaskService = services.getOptional(IDroraTaskService);
        if (droraTaskService) {
          const reportingDroraTaskService = createReportingRemoteDroraTaskService(
            droraTaskService,
            {
              reportRunningPromptCount: false,
            },
          );
          services.register(IDroraTaskService, reportingDroraTaskService);
        }
...
async function setupRemoteConnection(
  ...
  const { createRemoteBackend, connectRemote, pickRemoteRuntimeEnv } =
    await import("@drora/server/remote");
  ...
  const connection = await connectRemote(backend, {
    ...
    // 这里显式透传编译期版本，避免漏导入后生成裸 DRORA_VERSION 引用导致 SSH 初始化直接 ReferenceError。
    appVersion: DRORA_VERSION,
    // 远端 drora-server/agent 是独立进程，不能继承 host 里的测试/生产 endpoint 选择。
