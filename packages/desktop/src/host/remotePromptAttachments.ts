import type { IRemoteBackend, RemoteUploadOptions } from "@drora/server/remote";
import { quotePosixPathArg } from "@drora/server/remote/posixShell.js";
import type { TraceId, DroraPromptAttachment } from "@drora/shared";
import { randomUUID } from "node:crypto";

const REMOTE_PROMPT_ATTACHMENT_ROOT = "~/.drora/tmp/prompt-attachments";
const REMOTE_PROMPT_ATTACHMENT_RELATIVE_ROOT = ".drora/tmp/prompt-attachments";
...
export async function materializeRemotePromptAttachments(
  ...
  const nextAttachments: DroraPromptAttachment[] = [];
...
function getAttachmentLocalPath(attachment: DroraPromptAttachment): string | undefined {
      ...
      return async (...args: unknown[]) => {
        ...
        const prepared = await options.materializePromptAttachments({
          ...
          attachments: Array.isArray((params as { attachments?: unknown }).attachments)
            ? (params as { attachments?: DroraPromptAttachment[] }).attachments
            : undefined,
        });
