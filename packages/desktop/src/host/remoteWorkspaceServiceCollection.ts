...
export function createRemoteWorkspaceServiceCollection(params: {
  ...
  createReportingRemoteDroraTaskService: <T extends object>(service: T) => T;
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    tokenProvider: async () =>
      (await localCredentialService.load(DRORA_JWT_TOKEN_KEY))?.trim() || null,
  });
  const conversationShareService = new ConversationShareService({
    droraAgentService: params.connectionServices.droraAgentService,
  ...
  const reportingRemoteDroraTaskService = params.createReportingRemoteDroraTaskService(
    params.connectionServices.droraTaskService,
  );
  ...
  const remoteDroraTaskService = params.createRemotePromptAttachmentTaskService(
    reportingRemoteDroraTaskService,
  );
  const remoteDroraSessionService = params.createRemotePromptAttachmentSessionService(
    params.connectionServices.droraSessionService,
  );
  ...
  params.connectionServices.droraAgentService.onDynamicSessionRuntimePreferencesRequest()(
    (request) => {
      const startedAt = Date.now();
      const requestContext = {
        event: "drora_protocol.runtime_preferences.host_request_received",
      ...
      void (async () => {
        ...
        await params.connectionServices.droraAgentService.respondSessionRuntimePreferences({
          requestId: request.requestId,
          resolution,
        });
  ...
  // 因此这里为 remote workspace host 补齐本地全局 channel；文件、终端、Drora Agent 仍来自远端，
  ...
  const services = new ServiceCollection()
    ...
    .register(IDroraTaskService, remoteDroraTaskService)
    .register(IDroraAgentService, params.connectionServices.droraAgentService)
    .register(IDroraSessionService, remoteDroraSessionService)
    .register(IConversationShareService, conversationShareService)
    .register(
      IBotsService,
      createBotsService({
        credentialService: localCredentialService,
        droraTaskService: remoteDroraTaskService,
