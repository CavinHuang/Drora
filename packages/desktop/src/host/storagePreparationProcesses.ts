...
import {
  ...
  droraStoragePreparationFrameSchema,
  type DatabaseStartupState,
} from "@drora/shared";
import { resolveDefaultDroraAgentCommand } from "@drora/services/storage-startup";
...
export async function prepareSessionStorage(options: {
  ...
  const command = resolveDefaultDroraAgentCommand({
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      try {
        ...
        const frame = droraStoragePreparationFrameSchema.parse(JSON.parse(line));
        clearTimeout(firstStateTimer);
