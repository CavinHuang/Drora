...
import { matchesTaskListMembershipKind } from "@drora/shared/drora-protocol-v4";
import type {
...
function taskKey(task: Pick<DroraTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: DroraTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@drora/shared/drora-protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: DroraTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  createId: () => string;
  resolveSource: (scope: DroraTaskListWorkspaceScope) => ResolvedWindowHostControllerSource | null;
  onSourceError?: (
    scope: WindowHostControllerSourceScope,
  ...
  const sourceTaskServices = new Map<string, IDroraTaskService>();
  const sourceSnapshotTaskServices = new Map<string, IDroraTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: IDroraTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: IDroraAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@drora/shared").WindowHostAttachmentScope;
