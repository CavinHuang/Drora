...
const ABOUT_APPLICATION_NAME = "Drora Desktop App";
...
> = {
  "zh-CN": {
    ...
    copyright: (year) => `版权所有 © ${year} Drora。`,
  },
  "en-US": {
    aboutTitle: "About Drora",
    versionLabel: "version",
    okButtonLabel: "OK",
    optimizedForAppleSilicon: "Optimized for Apple Silicon.",
    copyright: (year) => `Copyright © ${year} Drora.`,
  },
};
...
export function createAboutSnapshot(options: AboutSnapshotOptions = {}): AboutSnapshot {
  ...
  return {
    appVersion: normalizeValue(options.appVersion ?? buildMetadata?.appVersion ?? DRORA_VERSION),
    buildCommitId: normalizeValue(buildMetadata?.buildCommitId ?? DRORA_COMMIT),
    buildTime: normalizeValue(buildMetadata?.buildTime ?? DRORA_BUILD_TIME),
    environment: normalizeValue(options.environment ?? DRORA_ENV),
