...
import { DRORA_AGENT_LIFECYCLE_LOG_MARKER } from "@drora/shared/process-diagnostic";
import {
...
function hasProductExecutable(
  ...
  // Linux Preview 使用 drora-preview。两者都必须精确匹配，不能放宽成前缀以免混入 helper dump。
...
const armsRumEnv = mapDroraEnvToArmsRumEnv(desktopRuntimeEnv);
...
function startArmsRum(): Promise<void> {
  return armsRum
    .init({
      ...
      endpoint: DRORA_ARMS_RUM_ENDPOINT,
      env: armsRumEnv,
          ...
          (payload?.events ?? []).filter(
            (event) =>
              !(
                ...
                event.message.includes(DRORA_AGENT_LIFECYCLE_LOG_MARKER)
              ),
          ),
    ...
    .then(() => {
      logger.info(`[arms] electron initialized env=${armsRumEnv} version=${DRORA_VERSION}`);
    })
    .catch((error) => {
...
export const armsInitPromise: Promise<void> =
  DRORA_TELEMETRY_ENABLED && DRORA_ARMS_RUM_ENDPOINT ? startArmsRum() : Promise.resolve();
