import type { BrowserCommand, BrowserCommandResult } from "@drora/shared";
import { browserSnapshotElementSchema } from "@drora/shared";
import {
  dispatchClickAt,
  dispatchDrag,
  dispatchDragPath,
...
export async function handleElementInfo(
  ...
  // 视口坐标 (x,y) → document.elementFromPoint → 复用快照元素结构（现分配 ref 存入 __droraRefs）。
  const raw = await view.webContents.executeJavaScript(
