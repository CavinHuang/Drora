...
export class BrowserGuestManager {
  ...
  attachGuest(
    ...
    if (guest.getType() !== "webview") {
      ...
      // CDP/Runtime 输入会直接操作 Drora composer。IAB 只允许真实 <webview> guest fail closed。
      this.log?.(
  ...
  private async installRecordingCursorOverlay(guest: GuestWebContents): Promise<void> {
    await guest.executeJavaScript(`(() => {
      const id = "__drora_browser_recording_cursor";
      ...
      window["__droraBrowserRecordingCursorCleanup"]?.();
      ...
      window["__droraBrowserRecordingCursorCleanup"] = () => {
        ...
        cursor.remove();
        delete window["__droraBrowserRecordingCursorCleanup"];
      };
    })()`);
  }
  ...
  private async removeRecordingCursorOverlay(guest: GuestWebContents): Promise<void> {
    ...
    await guest
      .executeJavaScript('window["__droraBrowserRecordingCursorCleanup"]?.()')
      .catch(() => undefined);
  }
  ...
  private finalizeTabs(
    ...
    // 产品语义：IAB tab 在当前 Drora 进程内默认持久。keep 是状态标记集合，不是清理白名单；
