...
import {
  HELP_TOGGLE_DRORA_STDIO_TAP_MENU_ID,
  HELP_TOGGLE_DEV_TOOLS_MENU_ID,
} from "./desktopCommandHandlers.js";

const HELP_DRORA_ENDPOINT_PRODUCTION_MENU_ID = "help.drora-endpoint.production";
...
export function updateDroraStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_DRORA_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readDroraStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  droraEndpointSelection?: "production" | "test" | "custom";
  ...
  const stdioTapState = readDroraStdioTapDevState();
      ...
      ? [
          {
            ...
            submenu: [
              ...
              ...(DRORA_PRODUCT_FLAVOR === "production"
                ? [
                    {
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_DRORA_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleDroraStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(DesktopCommandIds.ToggleDroraStdioTapDevProxy),
              },
              { type: "separator" as const },
            ]
          : []),
        ...(DRORA_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpDroraEndpoint),
                submenu: [
                  {
                    id: HELP_DRORA_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpDroraEndpointProduction),
                    type: "radio" as const,
                    checked: (options.droraEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetDroraEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpDroraEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetDroraEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpDroraEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetDroraEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        droraEndpointSelection: options.droraEndpointSelection,
