...
import { readDroraStdioTapDevState, setDroraStdioTapDevEnabled } from "@drora/services/node";
...
export const HELP_TOGGLE_DRORA_STDIO_TAP_MENU_ID = "help.toggle-drora-stdio-tap";
const DRORA_ENDPOINT_PROMPT_WIDTH = 460;
const DRORA_ENDPOINT_PROMPT_HEIGHT = 210;
const CODING_PLAN_WEBVIEW_PARTITION = "persist:drora-coding-plan";
...
async function clearAllDataAndRelaunch(options: {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: "确定要清除所有数据吗？",
    detail:
      "将删除 ~/.drora/v2（配置、凭据、日志）和浏览器缓存（localStorage）。操作不可恢复，清除后应用将自动重启。",
  });
  if (response !== 1) {
    return;
  }
  ...
  try {
    ...
    options.logger.info("[clear-all-data] deleted ~/.drora/v2");
  } catch (error) {
    options.logger.error("[clear-all-data] failed to delete ~/.drora/v2:", error);
  }
...
async function promptCustomDroraEndpoint(
  targetWindow: BrowserWindow | null | undefined,
  currentValue: string,
): Promise<string | undefined> {
  return showDroraEndpointPromptWindow({
    currentValue,
...
function buildDroraEndpointPromptHtml(currentValue: string): string {
  const value = escapeHtmlAttribute(currentValue);
  return `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
    <title>Drora Endpoint</title>
    <style>
  ...
  <body>
    <form id="form">
      <label for="endpoint">Drora endpoint origin</label>
    ...
    <script>
      ...
      const submit = (value) => { document.title = "drora-endpoint-submit:" + encodeURIComponent(value); };
      document.getElementById("form").addEventListener("submit", (event) => {
        event.preventDefault();
        submit(input.value);
      });
      document.getElementById("cancel").addEventListener("click", () => {
        document.title = "drora-endpoint-cancel";
      });
      input.focus();
      input.select();
    </script>
...
function showDroraEndpointPromptWindow(options: {
  currentValue: string;
  ...
  return new Promise((resolve) => {
    ...
    const promptWindow = new BrowserWindow({
      width: DRORA_ENDPOINT_PROMPT_WIDTH,
      height: DRORA_ENDPOINT_PROMPT_HEIGHT,
      parent: options.parentWindow,
      modal: Boolean(options.parentWindow),
      resizable: false,
      minimizable: false,
      maximizable: false,
      title: "Drora Endpoint",
      webPreferences: {
    ...
    promptWindow.on("page-title-updated", (event, title) => {
      if (title === "drora-endpoint-cancel") {
        event.preventDefault();
        finish(undefined);
        return;
      }
      if (!title.startsWith("drora-endpoint-submit:")) {
        return;
      }
      event.preventDefault();
      finish(decodeURIComponent(title.slice("drora-endpoint-submit:".length)));
    });
    ...
    void promptWindow.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(
        buildDroraEndpointPromptHtml(options.currentValue),
      )}`,
    );
  });
}

async function setDroraEndpointOverride(options: {
  value: string | undefined;
  settingService: { update(patch: { droraEndpointOrigin?: string | undefined }): Promise<void> };
  onDroraEndpointChanged: () => Promise<void> | void;
  logger: { warn: (...args: unknown[]) => void };
}) {
  if (DRORA_ENV === "production") {
    return;
  }
  const normalized = options.value ? normalizeDroraEndpointOrigin(options.value) : undefined;
  await options.settingService.update({ droraEndpointOrigin: normalized });
  await options.onDroraEndpointChanged();
}
...
function toggleDroraStdioTapDevProxy(options: {
  logger: { info: (...args: unknown[]) => void };
  updateDroraStdioTapDevMenuState: () => void;
}) {
  const current = readDroraStdioTapDevState();
  const next = setDroraStdioTapDevEnabled(!current.enabled);
  options.updateDroraStdioTapDevMenuState();
...
function resolveChangelogUrl(
  ...
  const origin = buildDroraEndpointUrls(endpointOrigin).origin;
...
async function resolveCurrentDroraEndpointOrigin(settingService: {
  get(): Promise<{ droraEndpointOrigin?: string }>;
  ...
  return resolveDroraEndpointOrigin({
    ...
    overrideOrigin: settings.droraEndpointOrigin,
  });
}
...
export async function executeDesktopCommand(options: {
  ...
  settingService: {
    get(): Promise<Pick<AppSettings, "droraEndpointOrigin" | "desktopZoomLevel">>;
    update(
      patch: Partial<Pick<AppSettings, "droraEndpointOrigin" | "desktopZoomLevel">>,
    ): Promise<void>;
  };
  droraEndpointEnvBaseOrigin?: string | null;
  credentialsDir: string;
  ...
  switch (options.command) {
    ...
    case DesktopCommandIds.OpenChangelog:
      await openChangelog(
        ...
        await resolveCurrentDroraEndpointOrigin({
          ...options.settingService,
          envBaseOrigin: options.droraEndpointEnvBaseOrigin,
        }),
      );
      return;
    ...
    case DesktopCommandIds.OpenResourceManager:
      openResourceManager();
      return;
    case DesktopCommandIds.ToggleDroraStdioTapDevProxy:
      toggleDroraStdioTapDevProxy({
        logger: options.logger,
        updateDroraStdioTapDevMenuState: options.updateDroraStdioTapDevMenuState,
      });
      return;
    case DesktopCommandIds.SetDroraEndpointProduction:
      await setDroraEndpointOverride({
        value: DEFAULT_DRORA_ENDPOINT_ORIGIN,
        settingService: options.settingService,
        onDroraEndpointChanged: options.onDroraEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetDroraEndpointTest:
      await setDroraEndpointOverride({
        value: options.droraEndpointEnvBaseOrigin ?? resolveRuntimeDroraEndpointOrigin(),
        settingService: options.settingService,
        onDroraEndpointChanged: options.onDroraEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetDroraEndpointCustom: {
      const current =
        (await options.settingService.get()).droraEndpointOrigin ?? DEFAULT_DRORA_ENDPOINT_ORIGIN;
      const value = await promptCustomDroraEndpoint(targetWindow, current);
      if (!value) {
        return;
      }
      try {
    ...
    case DesktopCommandIds.ResetDroraEndpoint:
      await setDroraEndpointOverride({
        value: undefined,
        settingService: options.settingService,
        onDroraEndpointChanged: options.onDroraEndpointChanged,
