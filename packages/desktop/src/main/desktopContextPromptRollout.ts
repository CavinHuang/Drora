...
export { DRORA_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV };
...
export function createElectronDesktopContextPromptConfigFetcher(options: {
  ...
  return async (signal) => {
    ...
    const url = new URL(`${buildDroraEndpointUrls(endpointOrigin).origin}/api/v1/client/configs`);
    ...
    return await new Promise<unknown>((resolve, reject) => {
      ...
      const sourceHeaders = buildDroraSourceHeadersFromContext({
        appVersion: options.appVersion,
        arch: process.arch,
        deviceMid: options.deviceMid,
        endpointOrigin,
        platform: process.platform,
        releaseChannel: DRORA_ENV,
