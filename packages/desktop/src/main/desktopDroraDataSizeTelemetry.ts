...
import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@drora/shared";

import type { DroraDataSizeScanResult } from "./droraDataSizeScanner.js";
import {
  readDroraDataSizeTelemetryState,
  writeDroraDataSizeTelemetryState,
  type DroraDataSizeTelemetryState,
} from "./droraDataSizeTelemetryState.js";
import { scanDroraDataDirectoryInWorker } from "./droraDataSizeWorkerClient.js";

export type { DroraDataSizeTelemetryState } from "./droraDataSizeTelemetryState.js";

const DRORA_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const DRORA_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const DRORA_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const DRORA_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const DRORA_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const DRORA_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const DRORA_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const DRORA_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const DRORA_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const DRORA_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const DRORA_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const DRORA_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface DroraDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: DroraDataSizeTelemetryTiming = {
  abortedRetryMs: DRORA_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: DRORA_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: DRORA_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: DRORA_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: DRORA_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: DRORA_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: DRORA_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: DRORA_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: DRORA_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: DRORA_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface DroraDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface DroraDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isDroraBusy: () => boolean;
  logger: DroraDataSizeTelemetryLogger;
  readState: () => Promise<DroraDataSizeTelemetryState | null>;
  report: (result: DroraDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<DroraDataSizeScanResult>;
  timing?: Partial<DroraDataSizeTelemetryTiming>;
  writeState: (state: DroraDataSizeTelemetryState) => Promise<void>;
}
...
function buildDroraDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: DroraDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_drora_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      schema_version: 1,
      drora_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / DRORA_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createDroraDataSizeTelemetryScheduler(
  dependencies: DroraDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: DroraDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isDroraBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[drora-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<DroraDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: DroraDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[drora-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("Drora data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("Drora data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[drora-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[drora-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: DroraDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[drora-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createDroraDataSizeTelemetryScheduler> | null = null;

export function registerDesktopDroraDataSizeTelemetry(options: {
  context: Parameters<typeof buildDroraDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isDroraBusy: () => boolean;
  logger: DroraDataSizeTelemetryLogger;
  ...
  stopDesktopDroraDataSizeTelemetry();
  desktopScheduler = createDroraDataSizeTelemetryScheduler({
    ...
    isDroraBusy: options.isDroraBusy,
    logger: options.logger,
    readState: () => readDroraDataSizeTelemetryState(options.stateFile),
    report: (result) => {
      const payload = buildDroraDataSizeArmsPayload({ context: options.context, result });
      armsRum.sendCustom({
        group: payload.group,
        name: payload.name,
    ...
    scan: ({ signal }) =>
      scanDroraDataDirectoryInWorker(
        {
          maxDurationMs: DRORA_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: DRORA_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeDroraDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[drora-data-size] scheduler start failed", error);
  });
}

export function stopDesktopDroraDataSizeTelemetry(): void {
