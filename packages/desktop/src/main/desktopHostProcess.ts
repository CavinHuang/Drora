...
export interface HostInitMessage {
  ...
  /** Main 解析后的 Drora Built-in Provider Config 路径；Host/Services 不感知 Electron 安装布局。 */
...
export function spawnHostProcess(
  ...
  const execArgv = [
    ...(RUNTIME_DRORA_DEBUG ? [`--inspect-brk=${RUNTIME_DRORA_DEBUG}`] : []),
    "--no-warnings",
  ];
  const child = electronUtilityProcess.fork(hostModulePath, [], {
    serviceName: formatDroraHostProcessName(label),
    execArgv,
    env: {
      ...buildHostProcessEnv(dependencies.hostProcessLocalEnv),
      ...buildHostE2ECoverageEnv(),
      DRORA_PROCESS_LABEL: label,
      // macOS-only: the Computer Use Helper launcher runs inside this forked host utilityProcess, whose
      // code-signing identity is a nested Electron helper (NOT dev.drora.app). Publish THIS (main
      // Electron) process's pid — which IS dev.drora.app — so helperLauncher passes it as
    ...
    if (result.data.type === HostResponseTypes.BotRemoteWorkspaceRuntimePortRequest) {
      ...
      if (!handler) {
        child.postMessage({
          ...
          requestId: request.requestId,
          ok: false,
          // Bugfix: 远端 Bot 不能在缺少 runtime bridge 时回落到本地 Drora Agent，
...
export function disposeHostProcess(
  ...
  // 如果 main 仍按 150/300ms 强杀 host，host 会先退出，drora-cli/app-server 子进程就可能被 init 接管成孤儿。
