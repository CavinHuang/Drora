...
import type { ConnectOptions } from "@drora/server/remote";
...
import {
  ...
  DRORA_ENV,
  DRORA_PRODUCT_FLAVOR,
  DRORA_RUNTIME_ENV_KEY,
  DRORA_VERSION,
  buildDroraToolEnvPassthroughEnv,
...
export const desktopRuntimeEnv: DroraRuntimeEnv = isLocalDevelopmentRuntime
  ? "development"
  : "production";
// 身份看编译期 flavor 而不是 DRORA_ENV：DRORA_PREVIEW_IDENTITY=1 的生产后端构建同样是 Preview，
// 需要独立的应用名、Electron 数据目录和 Helper 安装子目录才能与正式版并排运行。
const isPreviewPackagedRuntime = !isLocalDevelopmentRuntime && DRORA_PRODUCT_FLAVOR === "preview";
...
// e2e 运行的是生产构建，默认会和本机正式版 Drora 共用 app name / userData，
// 触发 Electron 单实例锁后只激活已有窗口，Chromedriver 无法接管测试进程。
// 这里允许测试显式隔离运行时身份，正常桌面/远控路径保持原来的默认值。
export const runtimeApplicationName =
  readRuntimeEnvOverride("DRORA_DESKTOP_APPLICATION_NAME") ??
  (isLocalDevelopmentRuntime ? "Drora Dev" : isPreviewPackagedRuntime ? "Drora Preview" : "Drora");
// Electron 的 app.getPath("home") 不一定跟随测试进程里的 HOME 覆盖。
// e2e 默认工作区依赖 home 路径，因此提供显式覆盖，避免测试写到开发者真实 ~/DroraProject。
export const runtimeHomePath = readRuntimeEnvOverride("DRORA_DESKTOP_HOME_DIR");
...
export const shouldUseElectronDefaultUserDataPath = isTruthyRuntimeEnvOverride(
  "DRORA_DESKTOP_USE_ELECTRON_DEFAULT_USER_DATA",
);
export const runtimeUserDataPath =
  readRuntimeEnvOverride("DRORA_DESKTOP_USER_DATA_DIR") ??
...
export const runtimeSessionDataPath =
  readRuntimeEnvOverride("DRORA_DESKTOP_SESSION_DATA_DIR") ??
...
export async function isDockerDaemonAvailable(): Promise<boolean> {
  const { isDockerAvailable } = await import("@drora/server/remote");
  return isDockerAvailable();
}

export async function listAvailableWSLDistros() {
  const { listWSLDistros } = await import("@drora/server/remote");
  return listWSLDistros();
}

export async function listAvailableDockerContainers() {
  const { listDockerContainers } = await import("@drora/server/remote");
...
export function loadHostProcessEnvFromLocalFiles(): Record<string, string> {
  if (isElectronAppPackaged()) {
    ...
    return { DRORA_TELEMETRY_RUNTIME_DISTRIBUTION: "packaged" };
  }
  ...
  return applySelectedDroraEnvLinks(merged);
}
...
function resolveAvailableDevelopmentMockCdnDir(): string | undefined {
  ...
  const releaseDir = join(mockCdnDir, "releases", DRORA_VERSION);
...
function shouldUseRemoteCdnInDevelopment(localEnv: LocalRuntimeEnv = {}): boolean {
  return isTruthyEnvFlag(resolveEnvValue("DRORA_DEV_REMOTE_ASSET_USE_CDN", localEnv));
}
...
function resolveRemoteCdnBaseUrls(
  ...
  const raw = resolveEnvValue("DRORA_REMOTE_ASSET_CDN_BASE_URL", localEnv);
  return resolveOrderedRemoteCdnBaseUrls({
    ...options,
    env: DRORA_ENV,
    overrideBaseUrl: raw,
    version: DRORA_VERSION,
  });
}
...
export function resolveDroraEndpointEnvBaseOrigin(
  localEnv: LocalRuntimeEnv = {},
): string | undefined {
  const buildEnv = readProductEndpointEnv();
  // main 进程临时验证更新服务时不会重新写 .env，命令行传入的 endpoint 必须优先于本地文件。
  return (
    process.env["DRORA_BASE_URL"]?.trim() ||
    process.env["DRORA_ENDPOINT_ORIGIN"]?.trim() ||
    localEnv.DRORA_BASE_URL?.trim() ||
    localEnv.DRORA_ENDPOINT_ORIGIN?.trim() ||
    buildEnv.DRORA_BASE_URL?.trim() ||
    buildEnv.DRORA_ENDPOINT_ORIGIN?.trim() ||
    undefined
  );
}
...
function applySelectedDroraEnvLinks(env: Record<string, string>): Record<string, string> {
  const endpointEnv = {
    ...readProductEndpointEnv(),
    ...env,
    DRORA_ENV,
  ...
  return {
    ...
    ...env,
    DRORA_BASE_URL: env.DRORA_BASE_URL ?? resolveRuntimeDroraEndpointOrigin(endpointEnv),
...
function resolveHostProcessNodeEnv(): DroraRuntimeEnv {
  return desktopRuntimeEnv;
}

function resolveRemoteAssetCacheDir(localEnv: LocalRuntimeEnv = {}): string {
  const overrideCacheDir = resolveEnvValue("DRORA_REMOTE_ASSET_CACHE_DIR", localEnv);
  if (overrideCacheDir) {
...
function resolveBundledDroraAgentBinaryPath(): string | undefined {
  const runtime = DRORA_AGENT_RUNTIME;
...
export function resolveBundledGlmBinaryPath(): string | undefined {
  return resolveBundledDroraAgentBinaryPath();
}
...
function resolveHostProcessBinaryEnv(
  ...
  // Drora Agent 与 app 协议适配强绑定版本，生产包必须优先使用随包携带的固定 runtime。
...
function resolveDynamicWorkflowModeHostEnv(options: {
  ...
  if (!options.isPackaged) {
    ...
    return mode ? { [DRORA_DYNAMIC_WORKFLOW_MODE_ENV]: mode } : {};
  }
  if (options.isPreview) {
    return { [DRORA_DYNAMIC_WORKFLOW_MODE_ENV]: "alwaysOn" };
  }
  return {};
}
...
export function buildHostProcessEnv(hostProcessLocalEnv: Record<string, string>) {
  ...
  const inheritedEnv = applySelectedDroraEnvLinks({
    ...sanitizeDroraRuntimeEnv(rawInheritedEnv),
    ...buildDroraToolEnvPassthroughEnv(rawInheritedEnv),
  });
  ...
  if (packagedDesktop) {
    delete inheritedEnv.DRORA_CUA_HELPER_ALLOW_UNSIGNED_LOCAL;
  }
  const dynamicWorkflowModeHostEnv = resolveDynamicWorkflowModeHostEnv({
    inheritedValue: rawInheritedEnv[DRORA_DYNAMIC_WORKFLOW_MODE_ENV],
  ...
  delete inheritedEnv[DRORA_DYNAMIC_WORKFLOW_MODE_ENV];
  ...
  return {
    ...
    ...agentTelemetryEnv,
    // Drora 运行时不再使用 NODE_ENV；它会被用户 shell、包管理器和测试框架复用。
    // 这里显式下发 DRORA_RUNTIME_ENV，并在继承环境里清掉 NODE_ENV，避免 host/agent/Bash 被污染。
    [DRORA_RUNTIME_ENV_KEY]: resolveHostProcessNodeEnv(),
    // 显式注入编译期产品身份，保证主进程与 host 的身份语义一致；地址独立解析。
    // inheritedEnv 从 .env 通用变量补齐 Drora/ZAI 链接，未覆盖时统一使用线上默认值。
    DRORA_ENV,
    // Preview 与生产版共享任务、配置和凭据，但不同版本的 Helper 不能互相覆盖或触发降级保护。
    // 只隔离 computer-use 下的运行组件，不改写 DRORA_HOME / DRORA_DATA_BASE_DIR 业务数据根。
    ...(isPreviewPackagedRuntime ? { DRORA_CUA_HELPER_INSTALL_VARIANT: "preview" } : {}),
    ...
    [DRORA_APP_VERSION_ENV]: DRORA_VERSION,
    ...(dataBaseDir !== homedir() ? { DRORA_DATA_BASE_DIR: dataBaseDir } : {}),
    ...(windowsAppInstallDir ? { [DRORA_WINDOWS_APP_INSTALL_DIR_ENV]: windowsAppInstallDir } : {}),
