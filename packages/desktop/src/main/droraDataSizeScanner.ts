...
export type DroraDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type DroraDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: DroraDataSizePartialReason }
);

export interface DroraDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface DroraDataSizeScanOptions extends DroraDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("Drora data size scan aborted", "AbortError");
}
 ...
 */
export async function scanDroraDataDirectory(
  options: DroraDataSizeScanOptions,
): Promise<DroraDataSizeScanResult> {
  ...
  let terminalPartialReason: Exclude<DroraDataSizePartialReason, "io_error"> | null = null;
...
export function isDroraDataSizeScanResult(value: unknown): value is DroraDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<DroraDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
