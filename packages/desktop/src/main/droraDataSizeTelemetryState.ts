...
export interface DroraDataSizeTelemetryState {
  lastReportedAt?: number;
  reportReservedAt?: number;
}

export async function readDroraDataSizeTelemetryState(
  stateFile: string,
): Promise<DroraDataSizeTelemetryState | null> {
  try {
    const parsed: unknown = JSON.parse(await readFile(stateFile, "utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      throw new TypeError("Invalid Drora data size telemetry state");
    }
    const record = parsed as Record<string, unknown>;
    const state: DroraDataSizeTelemetryState = {};
    if (record.lastReportedAt !== undefined) {
      if (typeof record.lastReportedAt !== "number" || !Number.isFinite(record.lastReportedAt)) {
        throw new TypeError("Invalid lastReportedAt in Drora data size telemetry state");
      }
    ...
    if (record.reportReservedAt !== undefined) {
      if (
        ...
        throw new TypeError("Invalid reportReservedAt in Drora data size telemetry state");
      }
...
export async function writeDroraDataSizeTelemetryState(
  stateFile: string,
  state: DroraDataSizeTelemetryState,
): Promise<void> {
