...
function getDroraDataDir() {
  return getAppConfigDir();
}

function getDroraCliDir() {
  return join(homedir(), ".drora", "cli");
}

function getDroraCliLogDir() {
  return join(getDroraCliDir(), "log");
}

/**
 * Computer Use Helper 的运行目录。macOS 上 Helper 由 LaunchServices 启动，stderr 被系统丢弃，
 * 所以它把生命周期与后台输入诊断 tee 到 `<socket>.exit.log`（见 drora-cua
 * helperExitLogPathFor）。同目录下还有 `.tokens` broker 凭据，收集时必须按文件名白名单。
 */
function getCuaHelperRunDir() {
  return join(homedir(), ".drora", "computer-use", "run");
}
...
function isExcludedRelativePath(relativePath: string): boolean {
  ...
  // 过去显式收集 ~/.drora/cli/debug 会把这类上下文带进手动导出和反馈完整日志，这里按目录段统一跳过。
  ...
  // ~/.drora/v2/dev 保存 stdio-traffic 等高频协议流，真实机器上会累计到 GB 级。
...
function shouldApplyLogExportRetention(archivePath: string): boolean {
  ...
  if (
    ...
    normalizedArchivePath.startsWith(".drora/cli/log/")
  ) {
    return true;
  }
...
async function createLogArchiveArtifacts(
  ...
  const droraCliLogDir = getDroraCliLogDir();
  // GLM / drora-cli 的运行日志写在 ~/.drora/cli/log，不在应用主数据目录 ~/.drora/v2 下。
  // 如果导出日志只扫描 v2，定位 agent CLI 启动、协议或崩溃问题时会缺少最关键的原生侧日志。
  await collectLogArchiveFilesFromDirectory(
    droraCliLogDir,
    posix.join(".drora", "cli", "log"),
    visitedDirs,
    files,
  );

  const droraCliDir = getDroraCliDir();
  // 排查 agent CLI 问题还需要它的运行配置与模型 IO 轨迹。
  // config.json 是当前生效配置；rollout 是 model-io 调用轨迹，
  // 二者都不在 ~/.drora/cli/log 下，需要额外收集才能完整还原现场。
  await collectLogArchiveFile(
    join(droraCliDir, "config.json"),
    posix.join(".drora", "cli", "config.json"),
    files,
  );
  await collectLogArchiveFilesFromDirectory(
    join(droraCliDir, "rollout"),
    posix.join(".drora", "cli", "rollout"),
    visitedDirs,
    files,
  );
  ...
  // ~/.drora/computer-use/run/<socket>.exit.log，既不在 app data 也不在 ~/.drora/cli 下。
  // 同目录下有 .tokens broker 凭据，因此按文件名白名单只收 *.exit.log，不递归该目录。
  await collectLogArchiveFilesByName(
    getCuaHelperRunDir(),
    posix.join(".drora", "computer-use", "run"),
...
export async function createFeedbackLogArchiveFromExportLogs(
  ...
  return createFeedbackDiagnosticArchive({
    sources: [
      ...
      { directory: getDroraCliLogDir(), archivePrefix: ".drora/cli/log" },
      {
        directory: getCuaHelperRunDir(),
        archivePrefix: ".drora/computer-use/run",
        exitLogsOnly: true,
      },
    ],
...
export async function exportLogs(
  ...
  try {
    ...
    const getSourceDir = dependencies.getDroraDataDir ?? getDroraDataDir;
    ...
    const exportBaseName = `drora-logs-${timestamp}`;
