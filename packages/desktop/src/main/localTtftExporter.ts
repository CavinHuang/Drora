...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@drora/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "drora-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "drora.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "drora.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@drora/local-ttft", "1");
  const duration = meter.createHistogram("drora.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("drora.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("drora.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "drora.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("drora.local_ttft.system.duration", { unit: "ms" });
  const preparationDuration = meter.createHistogram("drora.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("drora.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("drora.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("drora.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("drora.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("drora.local_ttft.records");
  const drops = meter.createCounter("drora.local_ttft.dropped");
