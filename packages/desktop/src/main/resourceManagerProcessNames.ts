...
import { formatDroraRendererProcessName } from "@drora/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatDroraRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatDroraRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatDroraRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
