...
import { DRORA_COMMIT, DRORA_VERSION } from "@drora/shared";
import {
...
const HELPER_PROTOCOL = "DRORA_BROWSER_IMPORT_V1";
const HELPER_VERSION_PROTOCOL = "DRORA_BROWSER_IMPORT_HELPER";
const HELPER_VERSION = "2";
const HELPER_TIMEOUT_MS = 90_000;
const HELPER_MAX_OUTPUT_BYTES = 128 * 1024;
const HELPER_FILENAME = "drora-browser-import-helper.exe";
...
async function verifyAuthenticodePair(
  ...
  const script = [
    "$stream=[IO.File]::Open($droraArg0,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);",
    "try{",
    "$helper=Microsoft.PowerShell.Security\\Get-AuthenticodeSignature -LiteralPath $droraArg0;",
    "$app=Microsoft.PowerShell.Security\\Get-AuthenticodeSignature -LiteralPath $droraArg1;",
...
async function runLockedPackagedHelper(options: {
  ...
  const invocation =
    options.mode === "broker"
      ? "$request=[Console]::In.ReadToEnd();$response=$request | & $droraArg0 --broker --parent-pid $droraArg2;"
      : "$response=& $droraArg0 --version;";
  const script = [
    "$stream=[IO.File]::Open($droraArg0,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);",
    "try{",
    "$sha=[Security.Cryptography.SHA256]::Create();",
    ...
    "finally{$sha.Dispose()};",
    `if($digest -ne $droraArg1){${hashMismatchResponse}};`,
    invocation,
...
async function verifyHelper(helperPath: string, options: ReadAppBoundKeyOptions): Promise<string> {
  ...
  try {
    ...
    const expectedAppVersion = options.expectedAppVersion ?? DRORA_VERSION;
    const expectedBuildCommit = options.expectedBuildCommit ?? DRORA_COMMIT;
    if (
      version.exitCode !== 0 ||
      fields.length !== 5 ||
