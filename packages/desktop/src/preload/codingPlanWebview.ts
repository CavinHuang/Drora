...
// - 在官网页主世界挂 window.droraBridge，暴露三个能力：
...
// - getLang 读 main world 的 window.__droraLang__（由 App executeJavaScript 注入）；
//   onLangChange 在 main world 监听 drora-coding-plan-lang-change CustomEvent
...
const PUBLIC_BRIDGE_KEY = "droraBridge";
const NATIVE_BRIDGE_KEY = "__droraCodingPlanWebviewNativeBridge__";
const LANG_VAR = "__droraLang__";
const LANG_CHANGE_EVENT = "drora-coding-plan-lang-change";
const REPORT_CONTEXT_VAR = "__droraReportContext__";
  ...
  try {
    ...
    if (
      !isTrustedCodingPlanWebviewOrigin(url.origin, {
        e2eStoreBridgeEnabled: process.env.VITE_DRORA_E2E_STORE_BRIDGE === "1",
      })
    ) {
      return false;
    }
    ...
    // 该页支付成功后仍需 droraBridge.notifyPurchaseComplete 通知 App 刷新模型设置。
    ...
    notifyPurchaseComplete(payload: NotifyPurchaseCompletePayload) {
      try {
        ipcRenderer.sendToHost(CodingPlanWebviewChannels.PurchaseComplete, {
          ...
          timestamp: Date.now(),
        } satisfies import("@drora/shared").CodingPlanPurchaseCompletePayload);
      } catch {
  ...
  contextBridge.executeInMainWorld({
    func: (
      ...
      const bridge = {
        ...
        // App locale 变化时用 executeJavaScript 派发 drora-coding-plan-lang-change 事件。
