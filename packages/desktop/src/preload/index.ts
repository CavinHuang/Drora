...
contextBridge.exposeInMainWorld("__DRORA_DEVICE_ID__", parseDeviceIdFromArgs());
...
import {
  InternalChannels,
  PlatformChannels,
  formatDroraRendererProcessName,
...
if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__droraFinalArmsCustomEventsE2E", {
...
function updateRendererProcessTitle(): void {
  process.title = formatDroraRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 drora-* title，便于按窗口角色筛选。
 ...
 */
contextBridge.exposeInMainWorld("drora", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@drora/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  reportLocalTtftBatch: (batch: import("@drora/shared").LocalTtftBatch): void =>
  ...
  importChromeBrowserData: (options?: import("@drora/shared").ChromeBrowserDataImportOptions) =>
  ...
  getDroraStdioTapDevState: (): Promise<DroraStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetDroraStdioTapDevState),
