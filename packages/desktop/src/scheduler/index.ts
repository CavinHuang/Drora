...
function resolveScheduledAt(automation: DroraAutomation, now: number): number {
...
async function handleClaimed(automation: DroraAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: DroraAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: DroraOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: DroraAutomationTrigger =
