...
type PatchedIpcRenderer = IpcRenderer & { __droraArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __droraArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__droraArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__droraArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__droraArmsIpcPatched) {
    return;
  }
  ...
  patched.__droraArmsIpcPatched = true;
}
