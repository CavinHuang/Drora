import { pickProductEndpointEnv } from "@drora/shared/droraEndpoint";
...
function loadEnvFiles(): Record<string, string> {
  ...
  // 真实环境变量优先级最高
  if (process.env.DRORA_ENV) vars.DRORA_ENV = process.env.DRORA_ENV;
  if (process.env.DRORA_BASE_URL) vars.DRORA_BASE_URL = process.env.DRORA_BASE_URL;
  if (process.env.VITE_DRORA_BASE_URL) vars.VITE_DRORA_BASE_URL = process.env.VITE_DRORA_BASE_URL;
...
const { environment: droraEnv } = await loadBuiltinProviderConfig();
// 安装包身份与后端环境分轴：DRORA_PREVIEW_IDENTITY=1 让生产后端的构建仍以 Drora Preview 身份打包运行。
const droraProductFlavor = resolveDesktopProductFlavor({ ...process.env, DRORA_ENV: droraEnv });
console.log(`[tsup] DRORA_ENV=${droraEnv} DRORA_PRODUCT_FLAVOR=${droraProductFlavor}`);

export function resolveDesktopTsupBundleSecurityOptions(
  runtimeEnv: Record<string, string | undefined> = process.env,
) {
  const isProduction = runtimeEnv.NODE_ENV === "production";
  const isE2ECoverageBuild = runtimeEnv.DRORA_E2E_COVERAGE === "1";
  return {
...
function createSharedDefines() {
  return {
    __DRORA_VERSION__: JSON.stringify(buildMetadata.appVersion),
    __DRORA_COMMIT__: JSON.stringify(buildMetadata.buildCommitId),
    __DRORA_BUILD_TIME__: JSON.stringify(buildMetadata.buildTime),
    __DRORA_ENV__: JSON.stringify(droraEnv),
    __DRORA_ENDPOINT_ENV__: JSON.stringify(pickProductEndpointEnv(env)),
    __DRORA_PRODUCT_FLAVOR__: JSON.stringify(droraProductFlavor),
    // Computer Use Helper build identity — helperInstaller 读它决定下载哪个 Helper bundle。
    // 缺失时 installer 抛 "Packaged Drora is missing its embedded Computer Use Helper build identity"。
    // CI 构建时通过 DRORA_CUA_HELPER_BUILD_ID env 注入；dev 为空串走兜底（dev helper 不走下载）。
    __DRORA_CUA_HELPER_BUILD_ID__: JSON.stringify(
      process.env.DRORA_CUA_HELPER_BUILD_ID?.trim() ?? "",
    ),
    // 客户端只有一个 CDN 配置，与发布端 OSS 目标列表分离。
    __DRORA_CDN_BASE_URL__: JSON.stringify(env.DRORA_CDN_BASE_URL?.trim() || ""),
  };
}
...
export default defineConfig([
  {
    ...
    entry: {
      ...
      "main/droraDataSizeWorker": "src/main/droraDataSizeWorker.ts",
    ...
    noExternal: [
      ...
      // electron-builder 又会排除 node_modules/@drora，导致安装包启动即 ERR_MODULE_NOT_FOUND。
  ...
  },
  {
    ...
    platform: "node",
    target: "node22",
    // 与 host 同构：常驻 cron scheduler 进程复用 @drora/services（tasks-index + cron），
