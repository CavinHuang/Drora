...
import { resolveDroraEndpointOrigin, pickProductEndpointEnv } from "@drora/shared/droraEndpoint";
...
function resolveDroraEnv(value: string | undefined): "test" | "production" {
...
function createE2EUIRendererCoveragePlugin(repoRoot: string): Plugin {
  ...
  return {
    name: "drora:e2e-ui-source-coverage",
    enforce: "pre",
...
export default defineConfig(({ mode }) => {
  // `.env*` 只提供链接常量；当前产品环境由启动脚本或 CI 注入 DRORA_ENV。
  const env = { ...loadEnv(mode, "../..", ""), ...process.env };
  const repoRoot = resolve(__dirname, "../..");
  const droraEnv = resolveDroraEnv(env.DRORA_ENV);
  // 安装包身份与后端环境分轴；renderer 用它决定是否展示更新入口。
  const droraProductFlavor = resolveDesktopProductFlavor({
    ...process.env,
    ...env,
    DRORA_ENV: droraEnv,
  });
  const e2eCoverageEnabled =
    env.DRORA_E2E_COVERAGE === "1" || process.env.DRORA_E2E_COVERAGE === "1";
  const e2eStoreBridgeEnabled =
    env.VITE_DRORA_E2E_STORE_BRIDGE === "1" || process.env.VITE_DRORA_E2E_STORE_BRIDGE === "1";
  const droraEndpointOrigin = resolveDroraEndpointOrigin({
    env: droraEnv,
    envBaseOrigin: env.DRORA_BASE_URL ?? env.DRORA_ENDPOINT_ORIGIN,
  });
  ...
  return {
    ...
    define: {
      ...
      __DRORA_LOCAL_DEVELOPMENT_RUNTIME__: JSON.stringify(mode !== "production"),
      "import.meta.env.VITE_DRORA_BASE_URL": JSON.stringify(droraEndpointOrigin),
