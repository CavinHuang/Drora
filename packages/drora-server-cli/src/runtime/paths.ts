...
function getDefaultServerDataRoot(): string {
  const configured = process.env.DRORA_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".drora", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\drora-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".drora") {
