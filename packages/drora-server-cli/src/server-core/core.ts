...
import { IDroraAgentService } from "@drora/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitDroraBuiltinProviderConfigFilePath =
    process.env[DRORA_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const droraBuiltinProviderConfigFilePath = explicitDroraBuiltinProviderConfigFilePath
    ? explicitDroraBuiltinProviderConfigFilePath
    : typeof __DRORA_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeDroraBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __DRORA_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!droraBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 Drora Built-in Provider Config，且未设置 ${DRORA_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IDroraAgentService));
