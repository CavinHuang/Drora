...
function exposeWebSocket(
  ...
  services.exposeOnChannelServer(
    server,
    scope ? new Map([[IDroraAgentService.channelName, scope.service]]) : new Map(),
  );
  socket.onClose(() => {
    void scope?.dispose();
    rawServer.dispose();
...
export async function createCoreHttpServer(
  ...
  const info: ServerRemoteInfo = {
    serverId: options.serverId ?? hostname() ?? "drora-server",
  ...
  app.use("/ws/host", async (context, next) => {
    const capability = context.req.header(DRORA_RPC_HOST_CAPABILITY_HEADER);
