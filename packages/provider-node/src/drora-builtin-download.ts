import { z } from "zod";
import { decodeDroraBuiltinRelease, type DroraBuiltinRelease } from "./drora-builtin-release.js";
...
export interface DroraBuiltinDownloadOptions {
...
export async function downloadDroraBuiltinRelease(
  options: DroraBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeDroraBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`Drora Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
