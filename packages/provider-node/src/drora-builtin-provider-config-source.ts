...
import { atomicWritePrivateTextFile, withFileLock } from "@drora/shared/node";
import {
  decodeDroraBuiltinRelease,
  encodeDroraBuiltinRelease,
  serializeDroraBuiltinRelease,
  type DroraBuiltinRelease,
} from "./drora-builtin-release.js";

export interface NodeDroraBuiltinProviderConfigSourceOptions {
  readonly bundledFilePath: string;
  readonly activeFilePath?: string;
  readonly watch?: boolean;
}

export type ApplyDroraBuiltinReleaseResult = "updated" | "unchanged" | "stale";

/** Bundled、Active/LKG 与 Remote 共用同一 Release，并最终发布为现有 Config Snapshot。 */
export class NodeDroraBuiltinProviderConfigSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #bundledFilePath: string;
  readonly #activeFilePath: string;
  ...
  constructor(options: NodeDroraBuiltinProviderConfigSourceOptions) {
    const bundledFilePath = options.bundledFilePath.trim();
    if (!bundledFilePath) throw new Error("Drora Built-in bundledFilePath 不能为空");
  ...
  async read(): Promise<ProviderConfigLayerSnapshot> {
    ...
    let release: DroraBuiltinRelease;
    try {
  ...
  async applyRemoteRelease(release: DroraBuiltinRelease): Promise<ApplyDroraBuiltinReleaseResult> {
    this.#assertNotDisposed();
    await this.#ensureWatcher();
    const result = await withFileLock(this.#activeFilePath, async () => {
      ...
      if (release.revision === current.revision) {
        if (serializeDroraBuiltinRelease(release) === serializeDroraBuiltinRelease(current)) {
          return "unchanged" as const;
        }
        throw new Error(`Drora Built-in 相同 revision ${release.revision} 对应不同内容`);
      }
  ...
  async #readAndMaterializeLocked(): Promise<DroraBuiltinRelease> {
  ...
  async #writeActiveLocked(release: DroraBuiltinRelease): Promise<void> {
    this.#assertNotDisposed();
    await atomicWritePrivateTextFile(
      this.#activeFilePath,
      JSON.stringify(encodeDroraBuiltinRelease(release), null, 2),
    );
  }
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("NodeDroraBuiltinProviderConfigSource 已 dispose");
  }
}

export function createNodeDroraBuiltinProviderConfigSource(
  options: NodeDroraBuiltinProviderConfigSourceOptions,
): NodeDroraBuiltinProviderConfigSource {
  return new NodeDroraBuiltinProviderConfigSource(options);
}

interface ReleaseCandidate {
  readonly release?: DroraBuiltinRelease;
  readonly error?: unknown;
}

async function readReleaseCandidate(filePath: string): Promise<ReleaseCandidate | null> {
  try {
    return { release: decodeDroraBuiltinRelease(JSON.parse(await readFile(filePath, "utf8"))) };
  } catch (error) {
...
function selectReleaseCandidate(
  ...
  if (
    ...
    serializeDroraBuiltinRelease(bundled.release) !== serializeDroraBuiltinRelease(active.release)
  ) {
  ...
  const valid = [bundled?.release, active?.release].filter(
    (candidate): candidate is DroraBuiltinRelease => candidate !== undefined,
  );
  if (valid.length === 0) {
    throw new AggregateError(
      [bundled?.error, active?.error].filter((error) => error !== undefined),
      "Bundled 与 Active Drora Built-in Release 均不可用",
    );
  }
...
function snapshotFromRelease(
  release: DroraBuiltinRelease,
  sourceKey: string,
): ProviderConfigLayerSnapshot {
  return Object.freeze({
    revision: `drora-builtin:${release.revision}:${sourceKey}`,
...
function signatureOf(release: DroraBuiltinRelease): string {
  return `${release.revision}:${serializeDroraBuiltinRelease(release)}`;
}
