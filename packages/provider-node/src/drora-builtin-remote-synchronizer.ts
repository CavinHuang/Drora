...
import type { NodeDroraBuiltinProviderConfigSource } from "./drora-builtin-provider-config-source.js";
import type { DroraBuiltinRelease } from "./drora-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type DroraBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface DroraBuiltinRemoteSynchronizerOptions {
  ...
  readonly fetchRelease: (
    endpointKey: string,
    signal: AbortSignal,
  ) => Promise<DroraBuiltinRelease | null>;
  readonly onRefreshResult?: (event: DroraBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface DroraBuiltinRefreshEvent {
  readonly result: DroraBuiltinRefreshResult;
...
export class DroraBuiltinRemoteSynchronizer {
  readonly #options: DroraBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<DroraBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: DroraBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<DroraBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<DroraBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("Drora Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: DroraBuiltinRefreshEvent): void {
