  ...
  async #runRefreshLoop(): Promise<AccountProviderConfigSnapshot> {
    ...
    while (this.#pendingReasons.size > 0) {
      ...
      try {
        ...
        const configuredProviders = config.droraBuiltinProviders;
        const { providers, states } = await this.#resolve({
          configRevision: config.droraBuiltinRevision,
          configuredProviders,
        ...
        if (
          ...
          currentConfig.droraBuiltinRevision !== config.droraBuiltinRevision
        ) {
          this.#pendingReasons.add("superseded-resolution");
          continue;
        }
        const basedOnDroraBuiltinRevision = config.droraBuiltinRevision;
