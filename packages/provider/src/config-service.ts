...
import type { ModelSelection } from "@drora/shared/model-selection";
...
export interface ProviderConfigServiceDependencies {
  readonly droraBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    this.#droraBuiltinSource = dependencies.droraBuiltinSource;
    this.#personalRepository = dependencies.personalRepository;
    this.#sourceDisposers = [
      this.#droraBuiltinSource.onDidChange((reason) => this.#emit(`droraBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [droraBuiltin, personal] = await Promise.all([
      this.#droraBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([droraBuiltin.revision, personal.revision]),
      droraBuiltinRevision: droraBuiltin.revision,
      personalRevision: personal.revision,
      droraBuiltinProviders: droraBuiltin.providers,
      droraBuiltinProviderTemplates: droraBuiltin.providerTemplates ?? ProviderTemplateMap.empty(),
      personalProviders: personal.providers,
      droraBuiltinModelRules: droraBuiltin.models,
  ...
  async savePersonalProviderOverlay(
    ...
    const droraBuiltin = await this.#droraBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = droraBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = droraBuiltin.providers.overlay(current.providers);
      ...
      if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
        ...
        throw new Error(
          `固定 Account Provider 的 Access 只能由 Drora Built-in Config 声明: ${providerId}`,
        );
      }
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(droraBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = droraBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? droraBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    await this.#updatePersonal((current) => {
      const occupied = new Set([...droraBuiltin.providers.keys(), ...current.providers.keys()]);
      const providerId = nextPersonalProviderId(occupied, templateId);
      createdProviderId = providerId;
      const effectiveProviders = resolvePersonalProviderBaselines(droraBuiltin, current.providers);
  ...
  async reorderPersonalProviders(
    ...
    return this.#updatePersonal((current) => ({
      ...
      models: current.models,
      providerOrder: normalizeProviderOrder(droraBuiltin.providers, current.providers, providerIds),
    }));
  }
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        droraBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(droraBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(droraBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(droraBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(droraBuiltin, current.providers, normalizedProviderId);
  ...
  async setPersonalModelEnabled(
    ...
    const builtin = await this.#droraBuiltinSource.read();
    return this.#updatePersonal((current) => {
...
function normalizeProviderOrder(
  _droraBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  droraBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    droraBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(droraBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? droraBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return droraBuiltin.providers.overlay(personal);
}
