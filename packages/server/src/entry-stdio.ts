import { disposeServiceResourcesAndWait, getAppConfigDir } from "@drora/services/node";
import {
  DRORA_VERSION,
...
const log = (...args: unknown[]) =>
  console.error(formatLogPrefix("drora-server:stdio", process.pid), ...args);
...
if (process.argv.includes("--version")) {
  process.stdout.write(DRORA_VERSION + "\n");
  process.exit(0);
}
...
async function main() {
  ...
  // 远端主机没有 Desktop main，没人写 telemetry-state.json，services 发往 Drora endpoint
