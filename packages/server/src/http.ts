...
const log = (...args: unknown[]) =>
  console.log(formatLogPrefix("drora-server:http", process.pid), ...args);
...
function resolveServerId(options: HttpServerOptions): string {
  return (
    options.serverId?.trim() || readTrimmedEnv("DRORA_SERVER_ID") || hostname() || "drora-server"
  );
}
...
function resolveServerWorkspaces(options: HttpServerOptions): ServerRemoteWorkspaceInfo[] {
  ...
  const workspacePath = readTrimmedEnv("DRORA_SERVER_WORKSPACE") || process.cwd();
  return [
    {
      path: workspacePath,
...
function createServerInfo(options: HttpServerOptions): ServerRemoteInfo {
  return {
    ...
    ...(options.name?.trim() || readTrimmedEnv("DRORA_SERVER_NAME")
      ? { name: options.name?.trim() || readTrimmedEnv("DRORA_SERVER_NAME") }
      : {}),
    version: DRORA_VERSION,
    protocolVersion: SERVER_REMOTE_PROTOCOL_VERSION,
    authRequired: options.authRequired ?? Boolean(readTrimmedEnv("DRORA_SERVER_TOKEN")),
...
const droraLiteTokenCookieName = "drora_lite_token";
...
function hasValidLiteToken(c: Context, token: string): boolean {
  ...
  if (url.searchParams.get("token") === token) {
    c.header(
      "Set-Cookie",
      `${droraLiteTokenCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`,
    );
    return true;
  }
  return parseCookieHeader(c.req.header("cookie")).get(droraLiteTokenCookieName) === token;
}
...
export function createHttpServer(
  ...
  app.use("/ws/host", async (c, next) => {
    const capability = c.req.header(DRORA_RPC_HOST_CAPABILITY_HEADER);
  ...
  const handleBotCallback = async (c: Context) => {
    ...
    const webhookSecret = c.req.header("x-drora-bot-secret");
