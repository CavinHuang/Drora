import { SocketProtocol, ChannelClient } from "@drora/rpc";
...
/**
 * Connect to a remote drora server via an IRemoteBackend.
 *
 * Steps:
...
async function connectRemoteUnchecked(
  ...
  stream.stderr.on("data", (chunk: Buffer) => {
    // 远端 drora-server 的服务日志走 stderr，直接写 host stderr 时可能被结构化日志中继吞掉。
...
function buildRemoteServerCommand(
  ...
  const envParts = [
    ...
    'DRORA_SERVER_RUNTIME_ROOT="$HOME/.drora/server"',
  ];
  ...
  if (appVersion) {
    ...
    envParts.push(`${DRORA_APP_VERSION_ENV}=${quotePosixShellArg(appVersion)}`);
  }
  if (remoteRuntimeNetwork?.authoritative) {
    envParts.push(`${DRORA_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY}='1'`);
    if (remoteRuntimeNetwork.httpProxy !== undefined) {
      envParts.push(
        `${DRORA_REMOTE_HTTP_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.httpProxy)}`,
      );
    }
    if (remoteRuntimeNetwork.noProxy !== undefined) {
      envParts.push(
        `${DRORA_REMOTE_NO_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.noProxy)}`,
      );
    }
  }
  return `${envParts.join(" ")} ~/.drora/server/node ~/.drora/server/drora-server.cjs`;
}
