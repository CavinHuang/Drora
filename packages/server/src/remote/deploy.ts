...
import { deployDroraAgentRuntime } from "./droraAgentDeploy.js";
import {
  deployNodePtyPrebuilds,
  deployNodeRuntime,
  createRemoteComponentVersionResolver,
  logDeployRequired,
} from "@drora/server/remote/remoteAssetDeployDecision.js";
import {
  REMOTE_BASE,
  fileExists,
  formatOptionalValue,
...
import { quotePosixPathArg } from "@drora/server/remote/posixShell.js";
import { checkServerBundleRequiredMarkers } from "@drora/server/remote/serverBundleDeployCheck.js";
import { deployRuntimeTools } from "@drora/server/remote/runtimeToolDeploy.js";
import { REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS } from "@drora/server/remote/droraAgentOfficialPluginAssets.js";
import {
  ensureRemoteReleaseDirFromCdn,
  selectRemoteAssetManifestComponents,
  type RemoteAssetManifestRef,
} from "@drora/server/remote/remoteAssetCache.js";
import {
...
} from "@drora/server/remote/remoteAssetInstaller.js";
import {
...
import { detectRemoteAssetTools } from "@drora/server/remote/remoteAssetPreflight.js";
import { assertSupportedRemoteEnvironment } from "@drora/server/remote/remotePlatformSupport.js";
import { acquireRemoteDeployLock } from "@drora/server/remote/remoteDeployLock.js";
import type { RemoteAssetNetworkPort } from "@drora/server/remote/remoteAssetNetwork.js";
...
/**
 * Deploy the drora server to the remote machine.
...
export async function deployServer(
  ...
  const deployWithDecision = async (
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/drora-server.cjs 可启动，不代表随包工具仍存在。
    ...
    await installer.installFile({
      componentId: SERVER_BUNDLE_COMPONENT_ID,
      sourceRelativePath: "server/drora-server.cjs",
      remotePath: `${REMOTE_BASE}/drora-server.cjs`,
    ...
    // 部署 Drora Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployDroraAgentRuntime(
      backend,
      env,
      {
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    const serverPath = `${REMOTE_BASE}/drora-server.cjs`;
    ...
    log("remote version:", JSON.stringify(version), "local:", DRORA_VERSION);
    if (version !== DRORA_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${DRORA_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", DRORA_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/drora-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "glm":
        requiredPaths.add(`glm/${platformArch}/drora.cjs`);
