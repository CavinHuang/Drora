...
import { deployDevelopmentDroraAgentRuntime } from "@drora/server/remote/droraAgentDevDeploy.js";
import {
...
} from "@drora/server/remote/remoteAssetLiveIdentity.js";

const REMOTE_AGENT_RUNTIME_BASE = `${REMOTE_BASE}/agents`;

export interface DeployDroraAgentRuntimeOptions extends RemoteAssetDeployOptions {
  platformArch: string;
...
function isSelectedDroraAgentComponent(
  componentId: string,
...
async function shouldSkipDroraAgentDeploy(params: {
  ...
  // wrapper 在、但 drora.cjs 缺失（被清理 / 旧原生二进制部署残留）时也要重新部署。
  ...
  params.loggers.log(
    `[drora-agent-deploy] ${DRORA_AGENT_PROVIDER}: 制品 SHA ${params.expectedArtifactSha256} 已部署，跳过`,
  );
  return true;
}
...
/**
 * 部署 Drora Agent runtime 到远程机器。
 *
 * 生产态只用 manifest SHA 判断制品是否变化；语义版本不参与跳过决策。
 */
export async function deployDroraAgentRuntime(
  backend: IRemoteBackend,
  env: RemoteEnvironment,
  options: DeployDroraAgentRuntimeOptions,
  loggers: DeployLoggers,
): Promise<void> {
  const provider = DRORA_AGENT_PROVIDER;
  const runtime = DRORA_AGENT_RUNTIME;
  const componentId = provider;
  if (!isSelectedDroraAgentComponent(componentId, options.selectedResourcePackageIds)) {
    loggers.log(`[drora-agent-deploy] ${provider}: 未选择资源包 ${componentId}，跳过检查和部署`);
    return;
  }

  // binaryName 指 wrapper 可执行文件名（如 drora-agent / drora-agent.exe）——
  // 一个调用远端 node 执行 drora.cjs 的壳脚本。
  const binaryName = runtime.resolveEntrySegments(env.platform).at(-1);
  if (!binaryName) {
    loggers.logWarn(`[drora-agent-deploy] ${provider}: 无法解析 agent 入口名称，跳过部署`);
    return;
  }
  ...
  if (
    await deployDevelopmentDroraAgentRuntime(
      backend,
      {
  ...
  try {
    ...
    loggers.logWarn(
      `[drora-agent-deploy] ${provider}: 读取 manifest SHA 失败，将重新部署: ${String(error)}`,
    );
  }

  if (
    await shouldSkipDroraAgentDeploy({
      backend,
      remoteBinaryPath,
      remoteBundlePath,
      runtimeResourceDir: runtime.bundledResourceDir,
  ...
  loggers.log(`[drora-agent-deploy] ${provider}: 开始部署 v${runtime.version}...`);
  ...
  if (permissionRepairSucceeded) {
    ...
    await installOfficialPluginPackages();
    // 2) packages 替换成功后再安装编译产物 drora.cjs（跨平台同一份，glm 组件里就是它）。
    await installBundle();
  }
  // 3) 写入 wrapper（即 resolver 期望的 drora-agent），用远端已部署的 node 执行 drora.cjs。
  ...
  loggers.log(`[drora-agent-deploy] ${provider}: 部署完成 v${runtime.version}`);
}
