import { DRORA_AGENT_PROVIDER } from "@drora/shared";
import type { IRemoteBackend } from "@drora/server/remote/backend.js";
import { type DeployLoggers, waitForClose } from "@drora/server/remote/deployShared.js";
...
export async function repairLegacyRemoteOfficialPluginDirectoryPermissions(params: {
  ...
  params.loggers.logWarn(
    `[drora-agent-deploy] ${DRORA_AGENT_PROVIDER}: 检查并修复旧 builtin plugin 目录权限 ${params.remoteOfficialPluginDir}`,
  );
  ...
  try {
    ...
    params.loggers.logWarn(
      `[drora-agent-deploy] ${DRORA_AGENT_PROVIDER}: 修复旧 builtin plugin 目录权限失败，将继续尝试替换 packages: ${
