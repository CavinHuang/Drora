...
import { REMOTE_BASE } from "@drora/server/remote/deployShared.js";
...
function buildRemoteDeployLockScript(lockDir: string, ownerToken: string): string {
  const acquiredMarker = `drora-deploy-lock-acquired:${ownerToken}`;
  const releaseMarker = `drora-deploy-lock-release:${ownerToken}`;
  return [
    "set -eu",
