...
import { type IDroraTaskService as IDroraTaskServiceShape } from "../session/droraTaskService.js";
...
export function createBotRemoteWorkspaceService(params: {
  ...
  return {
    ...
    async getDroraTaskService(target: {
      workspacePath: string;
      workspaceIdentity: string;
    }): Promise<IDroraTaskServiceShape | null> {
      return (await getRuntimeServices(target))?.droraTaskService ?? null;
    },
    ...
    async syncAppRuntimePreferences(preferences: DroraAgentAppRuntimePreferences): Promise<void> {
      ...
      await Promise.all(
        Array.from(runtimeServicesByWorkspaceKey.values()).map((services) =>
          services.droraAgentService.syncAppRuntimePreferences(preferences),
        ),
      );
    },
    dispose(): void {
  ...
  async function getRuntimeServices(target: {
    ...
    // task wrapper 命令走 IDroraTaskService，session 主状态走 Drora session facade。
    ...
    while (true) {
      ...
      const preferences: DroraAgentAppRuntimePreferences = cachedPreferences
        ? cachedPreferences
      ...
      await services.droraAgentService.syncAppRuntimePreferences(preferences);
