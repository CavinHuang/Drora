/* eslint-disable max-lines -- Bots 服务仍复用原 RPC 文件名，先把鉴权、命令路由、Drora Agent 桥接收口集中在同一服务内。 */
...
import {
  ...
  buildDroraAssistantPresentation,
  ...
  type DroraElicitationRequest,
  type DroraElicitationQuestion,
...
import type { IDroraTaskService } from "../session/droraTaskService.js";
...
import type { DroraAgentAppRuntimePreferences } from "../drora-agent/droraAgent.js";
...
function formatBotModelSelectionValue(selection: ModelSelection | undefined): string | undefined {
  ...
  return selection.providerId === DRORA_AGENT_PROVIDER
    ? selection.modelId
...
function parseBotModelOptionValue(value: string): ModelSelection | undefined {
  ...
  return value.trim() ? { providerId: DRORA_AGENT_PROVIDER, modelId: value.trim() } : undefined;
}
...
interface BotRemoteWorkspaceService {
  ...
  getDroraTaskService?(target: BotRemoteWorkspaceTarget): Promise<IDroraTaskService | null>;
...
interface PreparedBotMessageContent {
  content: string;
  droraAttachments: DroraPromptAttachment[];
}
...
function resolveAutomationBotDeliveryTarget(
  actor: BotActor,
): DroraAutomationBotDeliveryTarget | undefined {
...
function sortBotPermissionOptions(
  options: readonly DroraPermissionOption[],
): DroraPermissionOption[] {
...
function formatBotPermissionOptionLabel(option: DroraPermissionOption, locale?: Locale): string {
...
function formatBotPermissionOptionDescription(
  option: DroraPermissionOption,
  request: Pick<DroraPermissionRequest, "title" | "description" | "kind" | "raw">,
  locale?: Locale,
): string | undefined {
  const displayKind = getBotPermissionOptionDisplayKind(option);
...
function isBotPermissionRejectOption(option: DroraPermissionOption): boolean {
  const displayKind = getBotPermissionOptionDisplayKind(option);
...
const DEFAULT_BOT_DRORA_PROVIDER: DroraProvider = DRORA_AGENT_PROVIDER;
...
export function createBotsService(
  ...
  async function prepareBotMessageContent(
    ...
    const droraAttachments: DroraPromptAttachment[] = [];
    ...
    for (const rawAttachment of rawAttachments) {
      ...
      if (cached.kind === "image" || cached.kind === "audio") {
        droraAttachments.push({
          kind: cached.kind,
          filename: cached.filename,
          mimeType: cached.mimeType,
          dataBase64,
          // Bugfix：Bot 已把附件缓存到本地，DroraPromptAttachment 也必须携带该路径。
          // 只在 prompt 文本里描述路径会让下游附件策略无法选择本地文件读取。
          localPath: cached.localPath,
        });
        // Bugfix: bot 附件已经被 gateway 下载并缓存到本地。只把图片作为 Drora Agent image block 传入时，
  ...
  async function resolveDroraTaskServiceForContext(
    context: Pick<BotContextState, "workspacePath" | "workspaceIdentity">,
  ): Promise<IDroraTaskService> {
    if (!context.workspaceIdentity) {
      return deps.droraTaskService;
    }
    const remoteDroraTaskService = await deps.remoteWorkspaceService?.getDroraTaskService?.({
      workspacePath: context.workspacePath,
      workspaceIdentity: context.workspaceIdentity,
    });
    if (remoteDroraTaskService) {
      return remoteDroraTaskService;
    }
    // Bugfix: 远端 workspace 的 bot 请求不能缺 runtime 时静默走本地 droraTaskService。
  ...
  async function listActiveTaskConfigOptions(
    ...
    const droraTaskService = await resolveDroraTaskServiceForContext(context);
    return droraTaskService.getTaskConfigOptions({ taskId });
  }

  function findSelectConfigOption(
    options: readonly DroraConfigOption[],
    configId: "model" | "mode" | "thoughtLevel",
  ): (DroraConfigOption & { type: "select" }) | undefined {
    const category = configId === "thoughtLevel" ? "thought_level" : configId;
    return options.find(
      (item): item is DroraConfigOption & { type: "select" } =>
  ...
  function listConfigSelectOptions(
    options: readonly DroraConfigOption[],
    configId: "model" | "mode" | "thoughtLevel",
    context: { locale?: Locale; provider?: DroraProvider } = {},
  ): BotModelOption[] {
  ...
  function getModeDisplayLabel(
    locale: Locale | undefined,
    provider: DroraProvider | undefined,
    ...
    const labels: Partial<Record<DroraProvider, Record<string, string>>> = {
      glm: {
  ...
  async function listModelProviderOptionsForActiveTask(
    task: Pick<DroraTaskMeta, "model" | "workspacePath" | "workspaceIdentity">,
    _activeProvider: DroraProvider,
  ): Promise<BotModelProviderOption[]> {
  ...
  async function listProviderConfigOptionsForActiveTask(
    task: Pick<DroraTaskMeta, "workspacePath" | "workspaceIdentity">,
    activeProvider: DroraProvider,
  ): Promise<DroraConfigOption[]> {
  ...
  function normalizeBotDraftOptions(draftOptions: BotDraftOptions): BotDraftOptions {
    ...
    // 如果直接复用，/new 后首条消息会重新创建第三方 runtime，绕过 Drora Agent 单一事实源。
    return {
      ...draftOptions,
      provider: normalizeAgentProviderToDroraAgent(draftOptions.provider),
    };
  }
  ...
  async function buildInitializedDraftOptions(
    ...
    const requestedProvider = normalizeAgentProviderToDroraAgent(
      provider ?? DEFAULT_BOT_DRORA_PROVIDER,
    );
    if (context.workspaceIdentity && !(await isRemoteWorkspaceConnected(context))) {
      // Bugfix: 远端断连时初始化草稿也不能偷偷申请远端 Drora Agent runtime。
  ...
  async function buildActiveTaskDraftOptions(context: BotContextState): Promise<BotDraftOptions> {
    ...
    const resolvedProvider = normalizeAgentProviderToDroraAgent(activeTask.provider);
  ...
  async function applyDraftConfigOptions(
    ...
    if (modeOption?.id && forcedDraftMode) {
      const droraTaskService = await resolveDroraTaskServiceForContext(context);
      await droraTaskService.setMode({
        taskId,
        mode: forcedDraftMode as DroraTaskMode,
      });
    } else if (modeOption?.id) {
      // provider 不支持 yolo（非 Drora Agent）：保持其自身默认模式，避免首条消息回调失败。
      botsLogger.debug(
        traceId,
  ...
  function updateLiveStatusProgress(event: DroraStreamEvent): void {
  ...
  async function readTaskMeta(
    ...
    const droraTaskService = await resolveDroraTaskServiceForContext(context);
    const tasks = await droraTaskService.listTasks({
      workspacePath: context.workspacePath,
      workspaceIdentity: context.workspaceIdentity,
    });
    ...
    const entries = (
      await Promise.all(
        workspaces.map(async (workspace) => {
          const droraTaskService = await resolveDroraTaskServiceForContext(workspace);
          const tasks = await droraTaskService
            .listTasks({
              workspacePath: workspace.workspacePath,
              workspaceIdentity: workspace.workspaceIdentity,
            })
  ...
  async function readContextActiveTaskMeta(
    ...
    return (
      (
        await (
          await resolveDroraTaskServiceForContext(context)
        )
          .getTaskSnapshot({
  ...
  async function processProviderCallback(
    ...
    const preparedPayload = callbackBot
      ? ((await adapter.prepareCallbackPayload?.(callbackBot, payload).catch((error: unknown) => ({
          droraCallbackPrepareError: error instanceof Error ? error.message : String(error),
        }))) ?? payload)
      : payload;
    if (
      isRecord(preparedPayload) &&
      typeof preparedPayload.droraCallbackPrepareError === "string"
    ) {
      return {
        ok: false,
        replies: [],
        responseBody: { error: preparedPayload.droraCallbackPrepareError },
        status: 401,
      };
    }
    ...
    const parsePayload =
      isFeishuBotProvider(provider) && isRecord(preparedPayload)
        ? { droraProvider: provider, ...preparedPayload }
        : preparedPayload;
    ...
    for (const inbound of parsedInboundMessages) {
      ...
      if (bot) {
        ...
        const handledByFeishuSynchronousCardAction =
          ...
          isRecord(preparedPayload) &&
          preparedPayload.droraFeishuSynchronousCardAction === true &&
          Boolean(outbound[0]);
  ...
  function normalizeBotElicitationQuestions(
    event: Extract<DroraStreamEvent, { type: "elicitation_request" }>,
    locale: Locale | undefined,
  ): DroraElicitationQuestion[] {
  ...
  function mergeElicitationFormValues(
    question: DroraElicitationQuestion,
  ...
  async function submitPendingElicitation(
    ...
    const droraTaskService = await resolveDroraTaskServiceForContext(auth.context);
    const submitted = await droraTaskService.respondElicitation({
      taskId: pending.taskId,
      workspacePath: auth.context.workspacePath,
  ...
  async function handlePendingElicitationValue(
    ...
    if (question.multiSelect) {
      ...
      await writeContext({ ...auth.context, pendingElicitation: nextPending });
      // Bugfix: 多选题在 Bot 里 toggle 后不会触发 Drora Agent response，必须主动同步草稿给 UI。
      await broadcastPendingElicitationProgress(auth.context, nextPending);
      return createElicitationReply(actor, nextPending, auth.locale);
    }
    return advancePendingElicitation(auth, actor, pending, {
  ...
  async function handleElicitationRequest(
    ...
    // Bugfix: Bot 原先只消费 permission_request，没有把 Drora Agent 的
  ...
  async function watchTaskStream(
    ...
    let assistantParts: DroraAssistantMessagePart[] = [];
    ...
    const handleStreamEvent = async (
      event: DroraStreamEvent | TaskStreamMirrorableEvent,
      ...
      if (event.type === "permission_request") {
        ...
        // Bugfix: UI 会把 Drora Agent 原始权限选项规整成“允许/始终允许/拒绝”的固定顺序和文案；
      ...
      if (event.type === "task_complete" || event.type === "task_error") {
        ...
        // Bugfix: Drora Agent 终态事件可能先于 task index/meta 落盘广播到 Bots。
        ...
        const mode = getMode();
        const locale = await readMessageLocale();
        const completedSnapshot = await droraTaskService
          .getTaskSnapshot({
            taskId: event.taskId,
            workspacePath: context.workspacePath,
            workspaceIdentity: context.workspaceIdentity,
    ...
    const enqueueStreamEvent = (
      ...
      // Bugfix: Drora Agent 事件分发不保证等待 async listener。微信这类离散消息如果并发发送，
    ...
    const dynamicTaskEvent = (
      droraTaskService as Partial<Pick<IDroraTaskService, "onDynamicTaskEvent">>
    ).onDynamicTaskEvent;
    ...
    const streamDisposable = dynamicTaskEvent
      ...
      : droraTaskService.onDynamicStreamEvent(context.activeTaskId)(enqueueStreamEvent);
  ...
  function sendPromptInBackground(
    ...
    void resolveDroraTaskServiceForContext(context)
      .then((droraTaskService) =>
        droraTaskService.sendPrompt({
          taskId,
          traceId,
          content,
  ...
  async function handleMessage(message: BotInboundMessage): Promise<BotOutboundMessage[]> {
    ...
    if (auth.context.mode === "task" && auth.context.activeTaskId) {
      const taskService = await resolveDroraTaskServiceForContext(auth.context);
    ...
    if (auth.context.mode === "draft" || !auth.context.activeTaskId) {
      ...
      const droraTaskService = await resolveDroraTaskServiceForContext(auth.context);
      const task = await droraTaskService.createTask({
        workspacePath: auth.context.workspacePath,
        workspaceIdentity: auth.context.workspaceIdentity,
      ...
      const taskTitle = deriveTaskTitle(preparedMessage.content, preparedMessage.droraAttachments);
      ...
      await broadcastTaskListChange(context, task.taskId, "prompt_sent", {
        ...
        prompt: {
          ...
          attachments:
            preparedMessage.droraAttachments.length > 0
              ? preparedMessage.droraAttachments
              : undefined,
    ...
    const droraTaskService = await resolveDroraTaskServiceForContext(auth.context);
    await droraTaskService.resumeTask({
      taskId: auth.context.activeTaskId,
      workspacePath: auth.context.workspacePath,
      workspaceIdentity: auth.context.workspaceIdentity,
    });
    ...
    const originalSelection = await droraTaskService.getTaskModelSelection({
      taskId: auth.context.activeTaskId,
    });
  ...
  async function resolveTaskSelectionEntry(
    ...
    const droraTaskService = await resolveDroraTaskServiceForContext(context);
    const snapshot = await droraTaskService
      .getTaskSnapshot({
        taskId: value.trim(),
        workspacePath: context.workspacePath,
        workspaceIdentity: context.workspaceIdentity,
  ...
  async function isContextActiveTaskRunning(context: BotContextState): Promise<boolean> {
    ...
    const droraTaskService = await resolveDroraTaskServiceForContext(context);
    const activeTaskSnapshot = await droraTaskService
      .getTaskSnapshot({
        taskId: context.activeTaskId,
        workspacePath: context.workspacePath,
        workspaceIdentity: context.workspaceIdentity,
    ...
    if (
      ...
      // Drora Agent 历史任务的 status 为空也可能只是旧数据，不代表 UI 仍在运行；只有本进程确实发起
      ...
      return enqueueInboundProcessing(message.actor, async () => {
        ...
        switch (command.type) {
          ...
          case "model.list": {
            ...
            const activeProvider = normalizeAgentProviderToDroraAgent(active.task.provider);
            if (!activeProvider) {
              return [createOutbound(message.actor, msg(auth.locale, "modelProviderMissing"))];
            }
            const providers = await listModelProviderOptionsForActiveTask(
          ...
          case "model.set": {
            ...
            const droraTaskService = await resolveDroraTaskServiceForContext(active.task);
            const configOptions = await droraTaskService.setModel({
            ...
            const optionSource =
              commandName === "mode" && active.task.provider
                ? await listProviderConfigOptionsForActiveTask(
                    active.task,
                    normalizeAgentProviderToDroraAgent(active.task.provider),
                  )
                : active.configOptions;
            const currentValue =
              commandName === "mode"
            ...
            const currentLabel =
              commandName === "mode"
                ? readConfigSelectLabelForValue(optionSource, commandName, currentValue, {
                    locale: auth.locale,
                    provider: normalizeAgentProviderToDroraAgent(active.task.provider),
                  })
          ...
          case "thoughtLevel.set": {
            ...
            const traceId = generateTraceId(active.taskId);
            const droraTaskService = await resolveDroraTaskServiceForContext(auth.context);
            const configOptions = await droraTaskService.setConfigOption({
              taskId: active.taskId,
              traceId,
          ...
          case "permission.respond": {
            ...
            const droraTaskService = await resolveDroraTaskServiceForContext(auth.context);
            const submitted = await droraTaskService.respondPermission({
              taskId: auth.context.activeTaskId,
