...
function parseWebhookElicitationResponse(
  ...
  if (payload.type !== "drora.bot.elicitation_response") {
    return undefined;
  }
  const requestId =
...
export function createWebhookBotProvider(deps: WebhookProviderDeps): BotProviderAdapter {
  return {
      ...
      if (bot.webhookUrl) {
        ...
        const response = await postWebhookWithRetry(bot.webhookUrl, {
          ...
          headers: {
            ...
            [bot.webhookAuthHeaderName || "x-drora-bot-secret"]: secret ?? "",
          },
          body: JSON.stringify({
            type: "drora.bot.test",
            botId: bot.id,
    ...
    async send(bot, message) {
      ...
      if (secret) {
        headers[bot.webhookAuthHeaderName || "x-drora-bot-secret"] = secret;
      }
      const response = await postWebhookWithRetry(bot.webhookUrl, {
        method: "POST",
        headers,
        body: JSON.stringify({
          type: "drora.bot.message",
          botId: bot.id,
          provider: "webhook",
          ...
          ...(message.elicitation ? { type: "drora.bot.elicitation_request" } : {}),
          sentAt: Date.now(),
        }),
      });
      if (!response.ok) {
