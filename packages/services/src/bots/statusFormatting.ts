...
export function taskStatus(task: DroraTaskMeta): string {
...
function readRunningTaskStartedAt(snapshot: DroraSessionFile | null, task: DroraTaskMeta): number | null {
...
export function formatStatusTaskLine(task: DroraTaskMeta, label = "Task"): string {
  return `${label}: ${task.title} (${task.taskId})`;
}

export function readTaskWorkedDurationMs(snapshot: DroraSessionFile | null, task: DroraTaskMeta): number | null {
...
export function readLatestAssistantTurnChangeSummary(snapshot: DroraSessionFile | null): DroraTaskMeta["changeSummary"] | null {
...
function formatStatusToolProgress(tool: DroraPersistedToolCall | undefined): string | null {
  if (!tool) {
    return null;
  }
...
export function formatStatusStreamToolProgress(event: Extract<DroraStreamEvent, { type: "tool_call" | "tool_call_update" }>): string | null {
...
export function readLatestTaskProgress(snapshot: DroraSessionFile | null): string | null {
  const messages = snapshot?.messages ?? [];
