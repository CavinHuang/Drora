...
const DRORA_CLIENT_CONFIG_API_PREFIX = "/api/v1/client/configs";
...
interface DroraClientConfigEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: {
...
export class BigModelCodingPlanSubscriptionProvider {
  ...
  private clientConfigSnapshot: DroraClientConfigEnvelope | null = null;
  private clientConfigSnapshotExpiresAt = 0;
  private clientConfigRequest: Promise<DroraClientConfigEnvelope> | null = null;
  ...
  async getOffPeakClientConfig(options?: { forceRefresh?: boolean }): Promise<OffPeakClientConfig> {
    if (process.env["DRORA_OFFPEAK_MOCK"] === "1") {
   ...
   *   1. 本地覆盖（DRORA_DYNAMIC_WORKFLOW_MODE）在任何网络动作之前裁决，命中即返回——
  ...
  async getDynamicWorkflowClientConfig(options?: {
    ...
    if (normalizeDynamicWorkflowMode(process.env[DRORA_DYNAMIC_WORKFLOW_MODE_ENV])) {
  ...
  async getModelContextBudgetStrategy(): Promise<DroraModelContextBudgetStrategy> {
  ...
  async preview(request: CodingPlanPreviewRequest): Promise<CodingPlanPreviewResponse> {
    return this.post<CodingPlanPreviewResponse>(
      ...
      resolvePreviewPath(request.providerId),
      {
        ...
        // Coding Plan 试算接口默认按 Maas 渠道处理，不显式标记会丢失 drora 来源归因。
        salesChannel: request.salesChannel ?? "drora",
      },
    );
  }
  ...
  private async getClientConfigs(): Promise<DroraClientConfigEnvelope> {
    ...
    // client/configs 和其他 Drora 平台接口必须共享运行时 endpoint；
    // E2E/测试环境会通过 DRORA_BASE_URL 指向本地 mock，硬编码线上域名会让套餐状态不可控。
    ...
    this.clientConfigRequest = readCodingPlanApiJson<DroraClientConfigEnvelope>(
      this.apiClient,
      url,
      {
        method: "GET",
  ...
  private async loadBigModelAccessToken(): Promise<string> {
    ...
    const droraJwtToken = (await this.credentialService.load(DRORA_JWT_TOKEN_KEY))?.trim();
    if (droraJwtToken && token === droraJwtToken) {
      // 旧版 BigModel OAuth callback 曾把 drora JWT 同时写进
      // oauth:bigmodel:access_token，付费套餐预览会拿它去打 bigmodel.cn 并报令牌过期。
      // 这里在服务边界拦截旧污染状态，避免继续向 BigModel 业务接口发送错误凭据。
      log.warn(undefined, "BigModel access token is stale drora JWT; login required");
      throw new Error("bigmodel_oauth_required");
    }
    return token;
  }
  ...
  protected async ensureSubscribedTeamPlanProjectApiKeys(
    ...
    for (const teamContext of teamContexts) {
      try {
        ...
        // 多团队套餐每个项目都需要独立 drora-team-api-key。
...
function resolveCodingPlanClientConfigUrl(env: NodeJS.ProcessEnv): URL {
  return new URL(buildRuntimeDroraApiUrl(env, DRORA_CLIENT_CONFIG_API_PREFIX));
}
...
function unwrapClientConfigProducts(
  ...
  if (!products || typeof products !== "object") {
    throw new Error("Drora client config missing Coding Plan products");
  }
...
function unwrapClientConfigTeamProducts(
  ...
  if (!products || typeof products !== "object") {
    throw new Error("Drora client config missing Coding Plan team products");
  }
  for (const providerProducts of Object.values(products)) {
    if (
      ...
      throw new Error("Drora client config has invalid Coding Plan team products");
    }
  }
...
function unwrapClientConfigStartPlanPreview(
  ...
  if (
    ...
    throw new Error("Drora client config invalid Start Plan preview");
  }
  return {
 ...
 * 来自 Drora Built-in Provider / Model Config。
 * mock 模式（DRORA_OFFPEAK_MOCK=1）只替代产品曝光与套餐状态；模型候选仍来自 Registry。
 */
export function resolveOffPeakClientConfig(
  ...
  if (env["DRORA_OFFPEAK_MOCK"] === "1") {
    return {
      enabled: hasModels,
      modelSelectionView,
      // DRORA_OFFPEAK_MOCK_NO_PLAN=1 演示「非 coding plan 锁定」态；缺省视为已订阅。
      codingPlanActive: env["DRORA_OFFPEAK_MOCK_NO_PLAN"] !== "1",
    };
  }
