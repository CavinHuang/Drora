...
import type { ConversationShareArtifactDescriptor } from "@drora/shared";
import type { ArtifactRow, ConversationRow } from "@drora/shared/drora-protocol-v4";
...
function assertSafeString(value: string): void {
  if (/^drora-artifact:\/\//iu.test(value)) {
...
function isPublicArtifactRef(value: string): boolean {
  return /^drora-artifact:\/\/share\/[A-Za-z0-9._~-]+$/u.test(value);
}
  ...
  switch (row.kind) {
    ...
    case "artifact": {
      ...
      return {
        ...
        sha256: row.sha256,
        ref: `drora-artifact://share/${artifactVersionId}`,
        state: row.state,
      };
    }
    case "timelineMarker":
  ...
  for (const [index, row] of input.rows.entries()) {
    ...
    if (row.kind === "turnHeader") {
      ...
      if (
        ...
        row.ref !== `drora-artifact://share/${row.artifactVersionId}`
      ) {
        throwProjectionError(
          "invalid_conversation",
