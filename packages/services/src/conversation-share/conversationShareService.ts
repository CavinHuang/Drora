...
import type { IDroraSessionService } from "#src/drora-session/droraSession.js";
...
interface ConversationShareServiceOptions {
  droraAgentService: ConversationShareAgentService;
  ...
  droraSessionService?: Pick<IDroraSessionService, "createSession" | "listSessions">;
...
function hasUnsupportedArtifactReference(value: unknown): boolean {
  if (typeof value === "string") return /^drora-artifact:\/\//iu.test(value);
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 drora-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readDroraAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === DRORA_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === DRORA_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === DRORA_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === DRORA_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === DRORA_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === DRORA_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readDroraAttachmentFaultCode(error);
  return (
    faultCode === DRORA_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === DRORA_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly droraAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly droraSessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.droraAgentService = options.droraAgentService;
    ...
    this.droraSessionService = options.droraSessionService;
    ...
    // 配置的 Drora origin）导入后回链仍指向生产站，点分割线打开的是另一个环境的分享。
    // 改用与 API base 同一个环境解析器（buildRuntimeDroraApiUrl 也走它），保证同环境。
    // 优先级不变：显式 option > DRORA_CONVERSATION_SHARE_WEB_URL > 按环境推导。
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.DRORA_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeDroraEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".drora-share-imports.json");
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.droraAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".drora-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".drora-share-import.json");
    ...
    try {
      ...
      if (
        ...
        const sessions = await this.droraSessionService.listSessions({ workspacePath, limit: 100 });
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.drora-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.droraSessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .drora-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".drora-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.droraAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.droraAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      droraAgentService: agentService,
  ...
  private async cleanupAbandonedImports(): Promise<void> {
    ...
    for (const entry of imports) {
      ...
      if (typeof marker.sessionId !== "string" || !marker.sessionId.startsWith("share-import-"))
        continue;
      try {
        const sessions = await this.droraSessionService.listSessions({
