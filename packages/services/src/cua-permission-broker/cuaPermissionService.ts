...
// to @drora/drora-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @drora/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @drora/services root
...
// at compile time; Vite never resolves @drora/drora-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@drora/drora-cua/broker/ports";
