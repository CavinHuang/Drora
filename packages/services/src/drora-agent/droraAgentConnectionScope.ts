...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@drora/rpc";
import {
...
export type DroraAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface DroraAgentV4ConnectionContext {
  connectionId: string;
  clientMode: DroraAgentV4ClientMode;
...
const TRUSTED_CONNECTION_FIELD = "__droraTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__droraTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: DroraAgentV4ConnectionContext;
};

interface TrustedDroraAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedDroraAgentV4UnsubscribeRoute;
};

export function readTrustedDroraAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedDroraAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedDroraAgentV4Connection(
  value: unknown,
): DroraAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: DroraAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedDroraAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: DroraAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: DroraAgentWorkspaceTarget): DroraAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: DroraAgentWorkspaceTarget;
  connection: DroraAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: DroraAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: DroraAgentV4ConnectionContext): HelloMessage {
...
export interface DroraAgentConnectionScope {
  readonly service: IDroraAgentService;
...
export function createDroraAgentConnectionScope(
  base: IDroraAgentService,
  context: DroraAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): DroraAgentV4ConnectionContext => {
    const downstream = readTrustedDroraAgentV4Connection(params);
  ...
  const ownershipKey = (kind: SubscriptionKind, target: DroraAgentWorkspaceTarget, topic: string) =>
    `${kind}\0${workspaceKey(target)}\0${topic}`;
  const routedEventKey = (kind: SubscriptionKind, target: DroraAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: DroraAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies DroraAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: DroraAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedDroraAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedDroraAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: DroraAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: DroraAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: DroraAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IDroraAgentService> = {
