import { resolveDroraAgentSpawnCwd } from "#src/drora-agent/droraAgentSpawnCwd.js";
import type { DroraAgentStorageStartupSnapshot } from "#src/drora-agent/droraAgent.js";
/* eslint-disable max-lines -- droraAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseDroraProcessDiagnostic,
  DRORA_AGENT_LIFECYCLE_LOG_MARKER,
  DRORA_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  DRORA_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  DRORA_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@drora/shared/process-diagnostic";
import {
  DRORA_AGENT_RUNTIME,
  DRORA_AGENT_PROVIDER,
  DRORA_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveDroraRuntimeEnv,
  sanitizeDroraRuntimeEnv,
} from "@drora/shared";
import {
  findDroraAgentRuntimeBinary,
  findDroraAgentRuntimeNodeBundle,
...
import { DroraProtocolClient } from "./droraProtocolClient.js";
import { DroraStdioTransport } from "./droraStdioTransport.js";
import { readDroraStdioTapDevState } from "./droraStdioTapDevConfig.js";
import type { DroraAgentPresentationSurface } from "./droraAgentPresentationSurface.js";
...
export interface DroraAgentCommand {
...
export interface DroraAgentCommandResolverContext {
  presentationSurface?: DroraAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type DroraAgentCommandResolver = (
  context: DroraAgentCommandResolverContext,
) => Promise<DroraAgentCommand | null> | DroraAgentCommand | null;

export interface DroraAgentProcessManagerOptions {
  commandResolver?: DroraAgentCommandResolver;
  presentationSurface?: DroraAgentPresentationSurface;
...
interface ManagedDroraAgentProcess {
  ...
  runtimeIdentity: DroraAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.DRORA_E2E_ARTIFACT_DIR?.trim();
  if (env.DRORA_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "drora-e2e-coverage-preload.cjs");
 ...
 */
interface DroraAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: DroraAgentRuntimeIdentity;
}

export interface DroraAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: DroraAgentRuntimeIdentity;
...
interface DroraAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface DroraAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("drora-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("DRORA_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildDroraAgentSpawnPreflight(
  command: DroraAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<DroraAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveDroraAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceDroraAgentCommand(
  context: DroraAgentCommandResolverContext,
): DroraAgentCommand | null {
  const distEntrypoint = findUpward("apps/drora-cli/packages/cli/dist/drora.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.DRORA_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "drora.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 drora-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/drora-cli/packages/cli/src/main.ts");
...
function resolveDeployedDroraAgentBinaryCommand(
  context: DroraAgentCommandResolverContext,
): DroraAgentCommand | null {
  // 旧 resolver 只识别 DRORA_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 drora-server.cjs 单文件部署到 ~/.drora/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 drora-agent 已经部署到 ~/.drora/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "Drora agent server command is not configured"。
  // 这里复用 findDroraAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.drora/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findDroraAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: DRORA_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeDroraAgentCommand(
  context: DroraAgentCommandResolverContext,
): DroraAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 drora-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 drora.cjs，
  ...
  const bundlePath = findDroraAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...DRORA_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultDroraAgentCommand(
  context: DroraAgentCommandResolverContext,
): DroraAgentCommand | null {
  const command = process.env.DRORA_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.DRORA_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.DRORA_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 drora.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceDroraAgentCommand(context) ??
    resolveElectronRuntimeDroraAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedDroraAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: DroraAgentCommand | null,
  presentationSurface: DroraAgentCommandResolverContext["presentationSurface"],
): DroraAgentCommand | null {
...
function wrapDroraAgentCommandWithStdioTapDevProxy(
  command: DroraAgentCommand,
  workspaceKey: string,
): DroraAgentCommand {
  const tapState = readDroraStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/drora-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("Drora stdio tap proxy enabled but script not found");
    return command;
  }
...
export class DroraAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedDroraAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedDroraAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<DroraProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: DroraAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): DroraAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: DroraAgentCommandResolver;
  private readonly presentationSurface: DroraAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: DroraAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: DroraAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<DroraAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<DroraAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 DroraAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: DroraAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultDroraAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("Drora agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedDroraAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedDroraAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("Drora agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedDroraAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedDroraAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedDroraAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("Drora agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("Drora agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessForShutdown(managed: ManagedDroraAgentProcess): Promise<void> {
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`Drora agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<DroraProtocolClient> {
    if (this.disposed) {
      throw new Error("Drora agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("Drora agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("Drora agent process start wait completed", {
        workspaceKey,
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "Drora agent server command is not configured. Set DRORA_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("Drora agent process start was cancelled.");
    }
    const effectiveCommand = wrapDroraAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("Drora agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("Drora agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildDroraAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 DRORA_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 Drora 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveDroraRuntimeEnv(process.env);
    log("Drora agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeDroraRuntimeEnv(process.env),
        [DRORA_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new DroraStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseDroraProcessDiagnostic(line);
    ...
    const client = new DroraProtocolClient(transport, {
    ...
    const runtimeIdentity: DroraAgentRuntimeIdentity = {
    ...
    const managed: ManagedDroraAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("Drora agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `Drora agent process error${this.processLifecycleReporter?.onError ? ` ${DRORA_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("Drora agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `Drora agent process exited unexpectedly${this.processLifecycleReporter ? ` ${DRORA_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "Drora agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("Drora agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("Drora agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<DroraAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("Drora agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "DRORA_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("Drora agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("Drora agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，drora-cli 会残留为孤儿进程。
