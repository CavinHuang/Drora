import type {
  ...
  ModelSelection,
  DroraSessionMode,
  DroraSessionSettingsState,
  DroraSessionStateSnapshot,
  DroraTaskModeInfo,
} from "@drora/shared";
import {
  formatModelPickerValue as formatSharedModelSelection,
  getDroraAgentAvailableModes as getSharedDroraAgentAvailableModes,
  normalizeAvailableDroraMode as normalizeSharedAvailableDroraMode,
...
function resolveTaskMetaModelSelectionFromSnapshot(
  snapshot: Pick<DroraSessionStateSnapshot, "messages" | "settings">,
): ModelSelection | undefined {
...
export function normalizeAvailableDroraMode(mode: DroraSessionMode): string {
  return normalizeSharedAvailableDroraMode(mode);
}

export function getDroraAgentAvailableModes(): DroraTaskModeInfo[] {
  return getSharedDroraAgentAvailableModes();
}

export function settingsToConfigOptions(settings: DroraSessionSettingsState): DroraConfigOption[] {
  // service 侧曾经维护了一份三项 mode 白名单，edit 模式上线后没有同步，
  // 切换模式时 mode_update 会把 UI 菜单覆盖成 build/plan/yolo。这里统一复用 shared 的事实源。
  return droraSessionSettingsToDroraConfigOptions(settings);
}
