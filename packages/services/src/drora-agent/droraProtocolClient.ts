import { DroraStorageStartupGate } from "#src/drora-agent/droraStorageStartupGate.js";
...
import type { V4Method } from "@drora/shared/drora-protocol-v4";
import type { z } from "zod";
import type { DroraProtocolTransport } from "./droraProtocolTransport.js";

/** 客户端可发的方法名：旧 droraProtocolMethods + v4/*（并存，收敛为 v4）。 */
type DroraProtocolClientMethod = DroraProtocolMethod | V4Method;

interface DroraProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface DroraProtocolRequestTimeoutEvent {
  method: DroraProtocolClientMethod;
  requestId: DroraProtocolRequestId;
  timeoutMs: number;
}
...
interface DroraProtocolClientRequestOptions {
...
const DEFAULT_DRORA_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class DroraProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "DroraProtocolClientError";
  }
}

export class DroraProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: DroraProtocolClientMethod,
    readonly requestId: DroraProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`Drora Protocol request timed out: ${method}`);
    this.name = "DroraProtocolRequestTimeoutError";
  }
}

export class DroraProtocolClient implements IDisposable {
  readonly storageStartup: DroraStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<DroraProtocolNotification>();
  private readonly requestEmitter = new Emitter<DroraProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<DroraProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "Drora Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: DroraProtocolTransport,
    options?: DroraProtocolClientOptions,
  ) {
    this.storageStartup = new DroraStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_DRORA_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`Drora agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new DroraProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: DroraProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: DroraProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: DroraProtocolRequestId; result: unknown }).id,
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as DroraProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as DroraProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as DroraProtocolNotification);
    }
  }

  private resolveResponse(id: DroraProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`Drora Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: DroraProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("Drora Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("Drora Protocol client is disposed");
    }
  }
}
