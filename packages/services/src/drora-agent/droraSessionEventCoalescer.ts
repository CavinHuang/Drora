import type { DroraSessionEvent } from "@drora/shared";
import type { DroraAgentServiceEvent } from "#src/drora-agent/droraAgent.js";
...
type SessionServiceEvent = Extract<DroraAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: DroraAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: DroraSessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: DroraSessionEvent,
  next: DroraSessionEvent,
): DroraSessionEvent {
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: DroraAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
