...
import { droraProtocolMessageSchema } from "@drora/shared";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import type {
  DroraProtocolTransport,
  DroraProtocolTransportClosedEvent,
} from "./droraProtocolTransport.js";
import {
...
interface DroraStdioTransportOptions {
...
const processTreeLogger = createServiceLogger("drora-agent-process-tree");

export class DroraStdioTransport implements DroraProtocolTransport {
  readonly kind = "stdio" as const;

  private readonly messageEmitter = new Emitter<DroraProtocolMessage>();
  private readonly closeEmitter = new Emitter<DroraProtocolTransportClosedEvent>();
  ...
  constructor(
    ...
    private readonly options?: DroraStdioTransportOptions,
  ) {
    this.stderrCollector = new AgentStderrCollector(child.stderr, options?.onStderrLine);

    // Drora Protocol stdio 帧边界只认 LF。Node readline 会把 U+2028/U+2029
  ...
  async send(message: DroraProtocolMessage): Promise<void> {
    if (this.disposed || this.closed || this.child.killed || !this.child.stdin.writable) {
      throw new Error("Drora agent stdio transport is closed");
    }
  ...
  private async disposeAndWaitOnce(): Promise<void> {
    ...
    if (!this.hasChildExited()) {
      ...
      const configuredEofWaitMs =
        process.env.DRORA_E2E_COVERAGE === "1"
  ...
  private handleStdoutFrame(frame: string): void {
    ...
    try {
      const parsed = droraProtocolMessageSchema.parse(JSON.parse(line));
  ...
  private handleStreamError(stream: "stdin" | "stdout", error: Error): void {
    ...
    // drora-server 会因未处理的 EPIPE 直接崩溃，UI 只能看到远端连接断开而不是协议请求失败。
    // 这里把 stream error 视为 transport 已关闭，阻止后续继续向失效 agent 写入。
    this.fireClose({ reason: `${stream}_error: ${error.message}` });
  }

  private fireClose(event: DroraProtocolTransportClosedEvent): void {
    if (this.closed) {
      return;
    }
    this.closed = true;
