...
import { repairSubagentTaskIndex } from "#src/drora-agent/repairSubagentTaskIndex.js";
import {
  deriveDroraTaskStatusFromSessionSnapshot,
  generateTraceId,
  getDroraUserVisibleMessages,
  isDroraGoalContinuationReminderText,
  isDroraModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@drora/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { DroraWorkspaceEventSubscriptionParams } from "#src/session/droraTaskListTypes.js";
import type {
  IDroraAgentService,
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./droraConfigOptions.js";

const logger = createServiceLogger("drora-task-index-syncer");
...
export interface DroraTaskIndexTerminalEvent {
...
export interface DroraTaskIndexReadyEvent {
  target: DroraAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface DroraTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: DroraAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，droraSessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: DroraWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: DroraWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: DroraAgentSessionTarget, model: string): Promise<DroraTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: DroraTaskMeta | undefined,
    reason: DroraWorkspaceTaskListChanged["reason"],
    options?: Pick<DroraWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): DroraWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): DroraTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
function buildBaselineMetaFromSummary(
  target: DroraAgentWorkspaceTarget,
...
export function createDroraTaskIndexSyncer(
  options: CreateDroraTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<DroraWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<DroraTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<DroraTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === DRORA_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<DroraWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<DroraWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: DroraAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: DroraAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 droraSessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: DroraAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 droraSessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 droraSessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 droraSessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useDroraConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 DroraConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useDroraConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: DroraAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: DroraSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isDroraGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isDroraModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: DroraSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getDroraUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<DroraMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromDroraGoal(
  goal: NonNullable<DroraSessionStateSnapshot["projection"]["target"]>,
): DroraTaskGoal {
  return {
