/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 Drora session 的协议适配。 */
...
import {
  coalesceConsecutiveDroraAssistants,
  createSessionTraceId,
  decodeCustomModelValue,
  deriveDroraTaskStatusFromSessionSnapshot,
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  generateTraceId,
  getDroraGoalActiveIterationCount,
  getDroraGoalIterationByAssistantMessageId,
  getDroraUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeDroraApiRetryStatus,
  attachDroraBackgroundTaskNotificationToRaw,
  collectDroraBackgroundTaskNotificationsByToolUseId,
  mergeDroraBackgroundTaskControlItems,
  parseDroraBackgroundTaskControlItems,
  parseDroraBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveDroraVisibleSessionTitle,
  textFromDroraMessageParts,
  DRORA_AGENT_PROVIDER,
  droraBackgroundTaskNotificationToolUpdateStatus,
  appendDroraStreamingToolInputDelta,
  buildDroraStreamingToolInputPreview,
  createDroraToolProjectionMemory,
  finalizeDroraToolProjectionInput,
  forgetDroraToolProjectionMetadata,
  isDroraModelRetryRecoveryProgressPayload,
  markDroraStreamingToolInputPreviewMaterialized,
  resolveDroraToolProjectionMetadata,
  droraApiRetryFromModelNetworkStatusPayload,
  droraApiRetryFromStreamRecoveryPayload,
  droraTaskNetworkDebugStatusFromPayload,
  shouldMaterializeDroraStreamingToolInputPreview,
  type DroraApiRetryStatus,
  type DroraAssistantMessageFeedback,
  type DroraBackgroundTaskNotificationInfo,
  type DroraBackgroundTaskControlItem,
  type DroraBackgroundTurnAttribution,
  type DroraAutomationBotDeliveryTarget,
  type DroraCancelTaskCommandResult,
  type DroraConfigOption,
  type DroraEnqueueTaskCommandResult,
  type DroraError,
  type DroraGoalVerificationTimelineMeta,
  type DroraImportSessionsResult,
  type DroraImportableSessionCandidate,
  type DroraUsage,
  type DroraPersistedMessage,
  type DroraPersistedMessagePart,
  type DroraPersistedToolCall,
  type DroraPromptAttachment,
  type DroraProvider,
  type DroraSessionFile,
  type DroraTaskGoal,
  type DroraTaskGoalStats,
  type DroraTaskMode,
  type DroraPlanStep,
  type DroraSlashCommand,
  type DroraStreamEvent,
  type DroraTaskCreateResult,
  type DroraTaskMeta,
  type DroraTaskClientMode,
  type DroraTaskRuntimeCommand,
  type DroraTaskSnapshot,
  type DroraTaskSnapshotBody,
  type DroraTaskSnapshotRefContent,
  type DroraTaskSnapshotToolCallsSlice,
  type DroraTaskTokenUsageResult,
  type DroraTodoGroup,
  type DroraTurnSteerCommandKind,
  type DroraTurnSteerSource,
  type DroraWorkspaceEvent,
  type DroraWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  droraContextUsageBreakdownSchema,
  droraSessionSettingsStateSchema,
  type DroraDeliveryKind,
  type DroraMessagePart,
  type DroraMessageWithParts,
  type ModelSelection,
  type DroraPermissionOption,
  type DroraPermissionRequestParams,
  type DroraPermissionRequest,
  type DroraSessionEvent,
  type DroraSessionMode,
  type DroraSessionSettingsState,
  type DroraSessionStateSnapshot,
  type DroraStateUpdatedNotification,
  type DroraContextCompactionTimelineMeta,
  type DroraTimelineMeta,
  type DroraTimelineStatus,
  type DroraTimelineTrigger,
  type DroraToolProjectionMemory,
...
import type {
  DroraTaskListQuery,
  DroraTaskListResult,
  DroraWorkspaceEventSubscriptionParams,
  IDroraTaskService,
  DroraArchivedTaskDeletionResult,
  DroraTaskReadyOutcome,
  DroraTaskTerminalOutcome,
} from "../session/droraTaskService.js";
...
import type {
  DroraTaskIndexReadyEvent,
  DroraTaskIndexSyncer,
  DroraTaskIndexTerminalEvent,
} from "./droraTaskIndexSyncer.js";
import { readModelTrajectory } from "./modelTrajectory.js";
import { errorAttributionSchema, type CommandPayloadMap } from "@drora/shared/drora-protocol-v4";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./droraV4HostCommand.js";
...
import {
  ...
  getDroraAgentAvailableModes,
  normalizeAvailableDroraMode,
  settingsToConfigOptions,
} from "./droraConfigOptions.js";
...
interface CreateDroraTaskServiceAdapterOptions {
  ...
  taskIndexSyncer: DroraTaskIndexSyncer;
...
type WorkspaceEventInput = string | DroraWorkspaceEventSubscriptionParams;
type DroraSendPromptRuntimeCommand = Extract<DroraTaskRuntimeCommand, { type: "send_prompt" }>;
type DroraTerminalStreamEvent =
  | Extract<DroraStreamEvent, { type: "task_complete" }>
  | Extract<DroraStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: DroraProvider = DRORA_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: DroraSlashCommand[] = [];
const logger = createServiceLogger("drora-task-service");
...
function formatDroraAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveDroraAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.DRORA_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(homedir(), ".drora", "cli", "log");
  return join(logDir, `drora-${formatDroraAgentLogDate(now)}.jsonl`);
}

export function createDroraTaskServiceAdapter(
  options: CreateDroraTaskServiceAdapterOptions,
): IDroraTaskService {
  const errorEmitter = new Emitter<DroraError>();
  const taskEmitters = new Map<string, Emitter<DroraStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<DroraStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, DroraTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, DroraApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, DroraBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, DroraToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`Drora task service adapter does not support IDroraTaskService.${name} yet.`),
      {
        code: "DRORA_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: DroraBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: DroraAgentMcpServer[] | undefined,
  ): Promise<DroraAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    // Drora task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 Drora session 事件回投 Drora Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "Drora task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.droraAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.droraAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "Drora task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "Drora task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: DroraTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: DroraSendPromptRuntimeCommand,
  ): DroraSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: DroraSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: DroraSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "Drora task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: DroraTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is DroraSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "Drora task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "Drora task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "Drora task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<DroraStreamEvent, { type: "tool_call_update" }>["status"],
  ): DroraPersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: DroraStreamEvent): void {
    ...
    const nextTool: DroraPersistedToolCall =
  ...
  function persistedToolId(tool: DroraPersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: DroraPersistedToolCall | undefined,
    liveTool: DroraPersistedToolCall,
  ): DroraPersistedToolCall {
    if (!snapshotTool) {
      return liveTool;
    }
    return {
  ...
  function findToolLocation(
    messages: readonly DroraPersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly DroraPersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly DroraPersistedMessagePart[] | undefined,
    toolIndex: number,
  ): DroraPersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: DroraPersistedMessage,
    toolIndex: number,
    tool: DroraPersistedToolCall,
  ): DroraPersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    ...
    if (changed) {
      logger.debug(undefined, "Drora snapshot 合并 live tool projection", {
        event: "drora_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`Drora session target is not loaded: ${taskId}`), {
        code: "DRORA_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): DroraToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createDroraToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<DroraWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<DroraStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<DroraStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<DroraStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: DroraStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: DroraTaskMeta | undefined,
    reason: DroraWorkspaceTaskListChanged["reason"],
  ) {
  ...
  function emitWorkspaceConfig(
    ...
    settings: DroraSessionSettingsState,
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.droraAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.droraAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 DroraSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.droraAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.droraAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: DroraSessionStateSnapshot): DroraTaskMeta {
    const target = {
    ...
    return applyOverlayToMeta(
      {
        ...
        status: deriveDroraTaskStatusFromSessionSnapshot(snapshot),
        ...
        target: snapshot.projection.target
          ? fromDroraGoal(snapshot.projection.target)
  ...
  function rememberIndexedTaskMeta(meta: DroraTaskMeta): DroraTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 Drora session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: DroraTaskMeta): Promise<DroraTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 Drora session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: DroraStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: DroraStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToDrora(
    ...
    const goalIterationByAssistantMessageId = getDroraGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectDroraBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveDroraAssistants(
              getDroraUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: DroraPermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<DroraStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseDroraBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: DroraAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 Drora snapshot 到 task index 失败", error);
      });
      const snapshotEvent: DroraStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: DroraTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: DroraTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeDroraAgentServiceAndWait(): Promise<void> {
    const agentService = options.droraAgentService as IDroraAgentService & {
  ...
  const service: IDroraTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.droraAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.droraAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<DroraTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: DroraSessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.droraAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.droraAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.droraAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        if (params.v4Create === true) {
          ...
          const ack = assertV4CommandAckOk(
            "createSession",
            await options.droraAgentService.sendConversationCommandV4({
              ...target,
              envelope: createHostCommandEnvelope({
                type: "createSession",
                sessionId: null,
                payload: {
                  ...
                  config: {
                    ...
                    ...(model?.options?.reasoningLevel
                      ...
                      : {}),
                    ...(params.mode ? { mode: toDroraMode(params.mode) } : {}),
                  },
                  ...(mcpServers ? { mcpServers } : {}),
                },
              }),
          ...
          snapshot = await options.droraAgentService.createSession({
            ...target,
            sessionTraceId: createSessionTraceId(),
            mode: toDroraMode(params.mode),
            model: requestedSelection,
            thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<DroraEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: DroraTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<DroraEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<DroraCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "Drora task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "Drora task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "Drora task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "Drora task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "Drora task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.droraAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[drora-task-service] goalSession start", {
      ...
      const result = await options.droraAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[drora-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[drora-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.droraAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<DroraTaskMeta> {
    ...
    async listTasks(params): Promise<DroraTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<DroraTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: DroraTaskListQuery): Promise<DroraTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<DroraTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<DroraTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<DroraTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<DroraTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const droraSnapshot = limitTaskSnapshotMessages(
        snapshotToDrora(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(droraSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[drora-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(droraSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...droraSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[drora-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[drora-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[drora-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<DroraTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<DroraTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<DroraTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<DroraTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<DroraConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.droraAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<DroraSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as DroraAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<DroraImportableSessionCandidate[]> {
    ...
    async importClaudeSessions(params: {
      ...
      workspaceIdentity?: string;
      sessionIds: string[];
    }): Promise<DroraImportSessionsResult> {
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.droraAgentService.createSession({
          ...
          // 导入后的任务必须是真实 Drora session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toDroraMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<DroraConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.droraAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toDroraMode(params.value as DroraTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<DroraConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.droraAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<DroraConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveDroraAgentCurrentLogFilePath();
      // 返回 Drora Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // Drora Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[DroraTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<DroraTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 Drora Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.droraAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.drora-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<DroraArchivedTaskDeletionResult> {
      const result: DroraArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<DroraTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[DroraTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[DroraTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[DroraTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[DroraTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[DroraTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<DroraTaskMeta> {
    ...
    async setTaskUnread(params): Promise<DroraTaskMeta> {
    ...
    async archiveTask(params): Promise<DroraTaskMeta> {
    ...
    async unarchiveTask(params): Promise<DroraTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<DroraTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<DroraStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<DroraTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<DroraTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<DroraStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 DroraStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.droraAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toDroraDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<DroraWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.droraAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 drora-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeDroraAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: DroraTaskMeta, overlay: TaskOverlay): DroraTaskMeta {
  return {
    ...meta,
...
function deriveTitleFromSnapshot(snapshot: DroraSessionStateSnapshot): string {
  return resolveDroraVisibleSessionTitle({
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<DroraTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: DroraTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 drora-cli。
...
function toDroraMode(mode: DroraTaskMode | undefined): DroraSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function fromDroraMode(mode: DroraSessionMode): DroraTaskMode {
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `drora-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<DroraPersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<DroraGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `drora-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `drora-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: DroraPersistedMessage[],
  timelineMessages: DroraPersistedMessage[],
): DroraPersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: DroraSessionStateSnapshot): number {
  ...
  return getDroraGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toDroraDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<DroraStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseDroraBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = droraBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachDroraBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: DroraMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, DroraBackgroundTaskNotificationInfo>,
): DroraPersistedMessage {
  const tools: DroraPersistedToolCall[] = [];
  const parts: DroraPersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: DroraTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly DroraMessagePart[],
): DroraPromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<DroraMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is DroraPromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<DroraMessagePart, { type: "file" }>,
): DroraPromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<DroraMessagePart, { type: "text" }>,
): DroraTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<DroraMessagePart, { type: "compaction" }>,
): DroraTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<DroraMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, DroraBackgroundTaskNotificationInfo>,
): DroraPersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // Drora Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachDroraBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? droraBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly DroraMessagePart[]): string {
  return textFromDroraMessageParts(parts);
}

function reasoningFromParts(parts: readonly DroraMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<DroraMessagePart, { type: "reasoning" }> => part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function fromDroraGoal(goal: unknown): DroraTaskGoal {
  ...
  return {
    ...
    status: isDroraTaskGoalStatus(status) ? status : "active",
...
function isDroraTaskGoalStatus(status: string | undefined): status is DroraTaskGoal["status"] {
  return (
    status === "active" ||
    status === "paused" ||
...
function sessionTodosToPlanSteps(
  todos: DroraSessionStateSnapshot["todos"],
): DroraPlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: DroraSessionStateSnapshot["goalStats"],
): DroraTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: DroraSessionStateSnapshot["todoGroups"],
): DroraTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = droraSessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableDroraMode(parsedSettings.data.mode.current),
      availableModes: getDroraAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: DroraToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, DroraBackgroundTaskControlItem[]>,
  ...
  // Drora Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 Drora prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: DroraStreamEvent = {
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "Drora compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "Drora session failed",
...
): Extract<DroraStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isDroraModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendDroraStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeDroraStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildDroraStreamingToolInputPreview(state.rawInput);
    markDroraStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildDroraStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeDroraToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "Drora streaming tool input projected", {
    ...details,
    event: "drora.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<DroraStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<DroraMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): DroraContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<DroraStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveDroraToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeDroraToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: DroraStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // Drora Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 Drora Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetDroraToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // Drora Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: DroraPermissionRequestParams,
): DroraPermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: DroraSessionStateSnapshot["projection"]["pendingPermissions"][number],
): DroraPermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: DroraUserInputRequestParams,
): DroraStreamEvent {
  const questions =
...
): Extract<DroraStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: DroraSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<DroraStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: DroraSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<DroraStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, DroraBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: DroraBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): DroraBackgroundTaskControlItem[] | null {
  const parsedJobs = parseDroraBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeDroraBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): DroraPermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as DroraPermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<DroraStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = droraTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromDroraGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromDroraGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseDroraBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<DroraStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): DroraApiRetryStatus | null | undefined {
...
function recordAgentModelNetworkTelemetry(event: DroraSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `drora_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<DroraStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: DroraSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: DroraSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // Drora Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = droraContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): DroraUsage | undefined {
...
function timelineStatusValue(value: unknown): DroraTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): DroraTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): DroraTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): DroraTurnSteerCommandKind | undefined {
