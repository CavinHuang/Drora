...
class DroraV4CommandRejectedError extends Error {
  readonly code = "DRORA_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "DroraV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new DroraV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new DroraV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
