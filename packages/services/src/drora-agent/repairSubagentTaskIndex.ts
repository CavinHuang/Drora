import { resolveWorkspaceKey, DRORA_AGENT_PROVIDER } from "@drora/shared";
import type { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { IDroraAgentService, DroraAgentWorkspaceTarget } from "#src/drora-agent/droraAgent.js";
...
export async function repairSubagentTaskIndex(params: {
  ...
  agentService: Pick<IDroraAgentService, "listSessions">;
  taskIndexRepo: Pick<TaskIndexRepo, "listTaskMetas" | "updateTaskState">;
  isCurrent: () => boolean;
  onRemoved: () => void;
}): Promise<void> {
  const { target, taskIndexRepo, agentService, isCurrent } = params;
  const rows = await taskIndexRepo.listTaskMetas({ ...target, provider: DRORA_AGENT_PROVIDER });
  const ids = rows
