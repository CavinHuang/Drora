/* eslint-disable max-lines -- droraSessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { IDroraAgentService } from "#src/drora-agent/droraAgent.js";
import type { DroraTaskIndexSyncer } from "#src/drora-agent/droraTaskIndexSyncer.js";
...
import type {
  ...
  DroraSessionEventsParams,
  DroraSessionListParams,
  DroraSessionMessagesParams,
  DroraSessionReadParams,
  DroraSessionReadWorkspacePresentationParams,
  DroraSessionResumeParams,
  DroraSessionSetModeParams,
  DroraSessionSetModelParams,
  DroraSessionSetThoughtLevelParams,
  DroraTaskTarget,
  DroraSessionWorkspaceTarget,
} from "#src/drora-session/droraSession.js";
import { formatModelPickerValue } from "#src/drora-agent/droraConfigOptions.js";
import { createDroraSessionApiRetryRuntimeTracker } from "#src/drora-session/droraSessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/drora-session/importedClaudeSessionRepair.js";
import { createDroraDeferredDraftRegistry } from "#src/drora-session/droraSessionDraftRegistry.js";
...
interface CreateDroraSessionServiceOptions {
...
export function createDroraSessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateDroraSessionServiceOptions): IDroraSessionService {
  const { withApiRetryRuntime } = createDroraSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createDroraDeferredDraftRegistry();

  function notifySyncer(
    target: DroraTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: DroraSessionStateSnapshot) {
  ...
  // droraSessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[drora-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: DroraSessionStateSnapshot,
    params: DroraSessionResumeParams | DroraSessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends DroraSessionCreateParams | DroraSessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: DroraSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: DroraSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: DroraSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: DroraSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[drora-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[drora-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 Drora task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[drora-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: DroraSessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[drora-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[drora-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[drora-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: DroraSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: DroraSessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[drora-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: DroraSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: DroraSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: DroraTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[drora-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: DroraTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: DroraTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[drora-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: DroraSessionSetModelParams) {
    ...
    async setThoughtLevel(params: DroraSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: DroraSessionSetModeParams) {
