...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@drora/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/drora/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "Drora Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@drora.local",
    GIT_COMMITTER_NAME: "Drora Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@drora.local",
  };
}
