...
} from "@drora/shared/workspace-hook-discovery";
import { parseWorkspaceHookTrustStoreContent } from "@drora/shared/workspace-hook-trust-store-file";
...
const DRORA_CONFIG_FILE = "config.json";
...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "drora") {
    return workspacePath ? join(baseDir, ".drora") : join(baseDir, ".drora", "cli");
  }
...
function getConfigPath(source: SettingsDirectorySource, workspacePath?: string): string {
  return join(
    ...
    source === "drora" ? DRORA_CONFIG_FILE : SETTINGS_FILE,
  );
}
...
async function readUserDroraSource(): Promise<WorkspaceHookSourceInput | undefined> {
  const path = getConfigPath("drora");
...
async function readPersistentWorkspaceHookTrustDigests(
  ...
  const userConfig = (await readJsonFile<Record<string, unknown>>(getConfigPath("drora"))) ?? {};
...
async function writeDroraHooksConfig(
  workspacePath: string | undefined,
  hooks: Hook[],
): Promise<void> {
  const configPath = getConfigPath("drora", workspacePath);
  const existingConfig = (await readJsonFile<DroraConfigFile>(configPath)) ?? {};
  ...
  await atomicWriteWorkspaceHookConfig(configPath, {
    ...
    hooks: {
      ...
      events: toDroraHooksEvents(hooks),
    },
  });
}
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".drora", "config.json");
  const userHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      (!hook.location || (hook.location.source === "drora" && hook.location.scope === "user")),
  );
  const projectHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      hook.location?.source === "drora" &&
      hook.location.scope === "project" &&
      (!hook.configuredState ||
        resolve(hook.configuredState.sourcePath) === currentProjectConfigPath),
  );
  await writeDroraHooksConfig(undefined, userHooks);
  await writeDroraHooksConfig(params.workspacePath, projectHooks);
}
