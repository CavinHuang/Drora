...
const DRORA_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  source: "drora",
  directorySource: "drora",
  userConfigDirSegments: [".drora", "cli"],
  workspaceConfigDirSegments: [".drora"],
...
export function createMcpSyncService(
  ...
  return {
    ...
    async checkRemoteUserMcpWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(dirname(getUserDroraMcpConfigPath()));
    },
...
function getUserDroraMcpConfigPath(): string {
  return buildUserConfigPath(DRORA_MCP_DESCRIPTOR);
}
...
async function collectEffectiveUserMcpRecords(): Promise<UserMcpRecord[]> {
  const droraRecords = await readUserMcpRecordsFromFile(DRORA_MCP_DESCRIPTOR);
  if (droraRecords.length > 0) {
    return sortMcpRecords(droraRecords);
  }
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  if (payload.action === "set-enabled") {
    ...
    const location =
      payload.location ??
      buildDirectoryMcpLocation(DRORA_MCP_DESCRIPTOR, scope, payload.projectPath);
  ...
  await writeDroraServersToFile(scope, nextServers, payload.projectPath);
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  const droraServers = await readDirectoryServersFromFile(
    DRORA_MCP_DESCRIPTOR,
    scope,
    workspacePath,
  );
  if (droraServers.length > 0) {
    return droraServers;
  }
...
async function writeDroraServersToFile(
  scope: Exclude<McpScope, "common">,
  servers: Record<string, Record<string, unknown>>,
  workspacePath?: string,
): Promise<void> {
  const filePath = buildDirectoryConfigPath(DRORA_MCP_DESCRIPTOR, scope, workspacePath);
  const current = (await readJsonObject(filePath)) ?? {};
  const next = writeServerMapToJson(current, DRORA_MCP_DESCRIPTOR.configKeyName, servers);
  await writeTextAtomic(filePath, `${JSON.stringify(next, null, 2)}\n`);
}

async function readUserCliConfig(): Promise<Record<string, unknown>> {
  return (await readJsonObject(getUserDroraMcpConfigPath())) ?? {};
}

async function writeUserCliConfig(config: Record<string, unknown>): Promise<void> {
  await writeTextAtomic(getUserDroraMcpConfigPath(), `${JSON.stringify(config, null, 2)}\n`);
}
...
async function importMcpServers(params: {
  ...
  const targetPath = getUserDroraMcpConfigPath();
  const current = (await readJsonObject(targetPath)) ?? {};
  const targetServers = readServerMapFromJson(current, DRORA_MCP_DESCRIPTOR.configKeyName);
  const existingByName = await collectEffectiveUserMcpRecordByName();
  ...
  if (changed) {
    await writeTextAtomic(
      targetPath,
      `${JSON.stringify(writeServerMapToJson(current, DRORA_MCP_DESCRIPTOR.configKeyName, targetServers), null, 2)}\n`,
    );
  }
