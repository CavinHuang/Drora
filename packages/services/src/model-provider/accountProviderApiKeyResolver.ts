import { BIGMODEL_PROVIDER_ID, resolveBigModelApiOrigin, ZAI_PROVIDER_ID } from "@drora/shared";
...
export class AccountProviderApiKeyResolver {
  ...
  private async resolveBizApiKey(
    ...
    let apiKeyEntry = apiKeys.find((item) => item.name === DRORA_API_KEY_NAME) ?? null;

    if (!apiKeyEntry) {
      apiKeyEntry = await this.fetchRemoteData<RemoteApiKeySummary>(listUrl, {
        method: "POST",
        headers: createBizAuthHeaders(authorization),
        body: JSON.stringify({ name: DRORA_API_KEY_NAME }),
      });
    }
