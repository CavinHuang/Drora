...
interface TeamPlanRequestKeyDependencies {
  ...
  readonly access: Extract<DroraAccountAccess, { planKind: "team-coding-plan" }>;
}
...
export async function resolveAccountTeamPlanRuntimeApiKey(
  ...
  const droraJwtToken = (await params.credentialService?.load(DRORA_JWT_TOKEN_KEY))?.trim() ?? "";
  if (family === "bigmodel" && droraJwtToken && token === droraJwtToken) {
    // BigModel /api/biz 只接受登录 access token，不能使用旧版本误存的 Drora JWT。
    log.warn(undefined, "Team Plan runtime key projection skipped: stale drora JWT token", {
      family,
      projectId,
    });
    return null;
