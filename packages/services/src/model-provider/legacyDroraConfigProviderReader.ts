/* oxlint-disable eslint(max-lines) -- 已发布旧 Drora config.json 的多版 Provider 结构读取集中在同一边界。 */
...
function normalizeBigModelCodingPlanAnthropicBaseUrlForEnv(
  ...
  if (resolveRuntimeDroraEnv(env) === "production") {
...
function getDroraConfigFilePath(): string {
...
interface DroraConfigFile {
  $schema?: string;
  provider?: Record<string, DroraOpenCodeProviderConfig>;
  [key: string]: unknown;
}

interface DroraOpenCodeProviderConfig {
  api?: string;
  name?: string;
  env?: string[];
  id?: string;
  ...
  createdAt?: number;
  updatedAt?: number;
  models?: Record<string, DroraOpenCodeModelConfig>;
  drora?: DroraProviderConfigExtension;
  [key: string]: unknown;
}

interface DroraOpenCodeModelConfig {
  id?: string;
  name?: string;
  family?: string;
  release_date?: string;
  attachment?: boolean;
  reasoning?: boolean | DroraReasoningConfigExtension;
  temperature?: boolean;
  tool_call?: boolean;
  interleaved?: unknown;
  cost?: JsonObject;
  ...
  modified?: boolean;
  deleted?: boolean;
  drora?: DroraModelConfigExtension;
  [key: string]: unknown;
}

interface DroraReasoningConfigExtension {
  enabled?: boolean;
  variants?: string[];
...
interface DroraProviderConfigExtension {
  enabled?: boolean;
  systemDisabledReason?: ModelProviderSystemDisabledReason;
  endpoints?: ModelProviderEndpoints;
  apiFormat?: ModelProviderApiFormat;
...
interface DroraModelConfigExtension {
  kinds?: ModelProviderKind[];
  defaultKind?: ModelProviderKind;
  modelIdByKind?: Partial<Record<ModelProviderKind, string>>;
  disabledReason?: string;
...
const droraModelProviderModalitySchema = z.enum(["text", "image", "video", "audio", "pdf"]);
const droraProviderModelMappingsSchema = z.record(z.string(), z.unknown());

const droraReasoningConfigExtensionSchema = z
  .object({
...
const droraModelConfigExtensionSchema = z
  .object({
...
const droraOpenCodeModelConfigSchema: z.ZodType<DroraOpenCodeModelConfig> = z
  .object({
    id: z.string().optional(),
    name: z.string().optional(),
    reasoning: z.union([z.boolean(), droraReasoningConfigExtensionSchema]).optional(),
    limit: z
      .object({
    ...
    modalities: z
      .object({
        input: z.array(droraModelProviderModalitySchema).optional(),
        output: z.array(droraModelProviderModalitySchema).optional(),
      })
      .optional(),
    ...
    modified: z.boolean().optional(),
    deleted: z.boolean().optional(),
    drora: droraModelConfigExtensionSchema.optional(),
  })
  .passthrough();

const droraProviderConfigExtensionSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    defaultKind: modelProviderKindSchema.optional(),
    providerMappings: droraProviderModelMappingsSchema.optional(),
...
const droraOpenCodeProviderConfigSchema: z.ZodType<DroraOpenCodeProviderConfig> = z
  .object({
    ...
    createdAt: z.number().optional(),
    updatedAt: z.number().optional(),
    models: z.record(z.string(), droraOpenCodeModelConfigSchema).optional(),
    drora: droraProviderConfigExtensionSchema.optional(),
  })
  .passthrough();

const droraConfigFileSchema = z
  .object({
    $schema: z.string().optional(),
    provider: z.record(z.string(), droraOpenCodeProviderConfigSchema).optional(),
  })
  .passthrough();
...
function inferModelProviderKindFromOpenCodeProvider(
  provider: DroraOpenCodeProviderConfig,
): ModelProviderKind {
  if (provider.kind) {
...
function hasOpenCodeProviderRuntimeFields(provider: DroraOpenCodeProviderConfig): boolean {
  // config.json 的权威运行态字段是 kind/options.baseURL。
  // 旧 endpoints/apiFormat/defaultKind/drora 可能是历史迁移残留，若继续优先读取会覆盖新配置。
  return Boolean(
    provider.kind ||
...
function resolveOpenCodeProviderEndpoints(
  ...
  return provider.endpoints ?? provider.drora?.endpoints ?? {};
}

function stripDroraPrefixedProviderMappings(
  ...
  const entries = Object.entries(providerMappings ?? {}).filter(
    ([key]) => !key.toLowerCase().startsWith("drora"),
  );
  return Object.fromEntries(entries);
}

function openCodeReasoningToModelReasoning(
  model: DroraOpenCodeModelConfig,
  ...
  if (!options.preferOpenCodeFields && model.drora?.reasoning) {
    return stripModelProviderReasoningPatches(model.drora.reasoning);
  }
...
function openCodeModelToModelProviderModel(
  ...
  const drora = options.preferOpenCodeFields ? undefined : model.drora;
  const droraExtensions = model.drora;
  const contextWindow =
  ...
  return createModelProviderModelConfig({
    ...
    priority: readFiniteNumber(model.priority) ?? readFiniteNumber(droraExtensions?.priority),
    disabledReason: model.disabledReason ?? drora?.disabledReason,
    supportsTools: model.supportsTools ?? drora?.supportsTools,
    supportsStructuredOutput: model.supportsStructuredOutput ?? drora?.supportsStructuredOutput,
    modified: model.modified ?? droraExtensions?.modified,
    deleted: model.deleted ?? droraExtensions?.deleted,
  });
}
...
function openCodeProviderToModelProviderConfig(
  ...
  for (const deletedModelId of normalizeDeletedModelIds(drora?.deletedModels)) {
  ...
  const apiKeyRequired =
    provider.apiKeyRequired ?? drora?.apiKeyRequired ?? readBoolean(options.apiKeyRequired);
  const providerMappings = preferOpenCodeFields
    ? undefined
    : stripDroraPrefixedProviderMappings(provider.providerMappings ?? drora?.providerMappings);
  return normalizeProviderForStore({
    ...
    ...(provider.enabled !== undefined
      ...
      : drora?.enabled !== undefined
        ? { enabled: drora.enabled }
        : {}),
    ...((provider.systemDisabledReason ?? drora?.systemDisabledReason)
      ? {
          systemDisabledReason: provider.systemDisabledReason ?? drora?.systemDisabledReason,
        }
      : {}),
    endpoints,
    apiFormat: preferOpenCodeFields
      ? resolveModelProviderKindApiFormat(defaultKind)
      : (provider.apiFormat ?? drora?.apiFormat ?? resolveModelProviderKindApiFormat(defaultKind)),
    ...((provider.source ?? drora?.source)
      ? { source: provider.source ?? drora?.source }
      : { source: "custom" }),
    ...((provider.catalogSourceId ?? drora?.catalogSourceId)
      ? { catalogSourceId: provider.catalogSourceId ?? drora?.catalogSourceId }
      : {}),
    ...((provider.catalogProviderId ?? drora?.catalogProviderId)
      ? {
          catalogProviderId: provider.catalogProviderId ?? drora?.catalogProviderId,
        }
      : {}),
    ...((provider.modelsDevProviderId ?? drora?.modelsDevProviderId)
      ? {
          modelsDevProviderId: provider.modelsDevProviderId ?? drora?.modelsDevProviderId,
        }
      : {}),
    ...(apiKeyRequired !== undefined ? { apiKeyRequired } : {}),
    ...((provider.headers ?? drora?.headers ?? readStringRecord(options.headers))
      ? {
          headers: provider.headers ?? drora?.headers ?? readStringRecord(options.headers),
        }
      : {}),
    ...((provider.logoUrl ?? drora?.logoUrl)
      ? { logoUrl: provider.logoUrl ?? drora?.logoUrl }
      : {}),
    apiKey,
    ...((provider.apiKeyUrl ?? drora?.apiKeyUrl)
      ? { apiKeyUrl: provider.apiKeyUrl ?? drora?.apiKeyUrl }
      : {}),
    defaultKind,
    models,
    ...(providerMappings ? { providerMappings } : {}),
    createdAt:
      (!preferOpenCodeFields ? (provider.createdAt ?? drora?.createdAt) : undefined) ?? now,
    updatedAt:
      (!preferOpenCodeFields ? (provider.updatedAt ?? drora?.updatedAt) : undefined) ?? now,
  });
}

async function readRawDroraConfigFile(): Promise<DroraConfigFile | null> {
  const filePath = getDroraConfigFilePath();
  let raw: string;
  try {
  ...
  const parsed = droraConfigFileSchema.safeParse(json);
  if (!parsed.success) {
...
async function readDroraConfigProviders(): Promise<ModelProviderConfig[] | null> {
  const config = await readRawDroraConfigFile();
  if (!config?.provider) {
    return null;
  }
...
function normalizeProviderForStore(provider: ModelProviderConfig): ModelProviderConfig {
  ...
  return {
    ...
    providerMappings: stripDroraPrefixedProviderMappings(
...
function applyProviderStoreMigrations(providers: ModelProviderConfig[]): ModelProviderConfig[] {
  ...
  // reasoning mapping 由 Drora Built-in/Personal ModelConfigRules 在 Registry 中解析；
...
export async function readLegacyDroraConfigProviders(): Promise<ModelProviderConfig[]> {
  const configProviders = await readDroraConfigProviders();
