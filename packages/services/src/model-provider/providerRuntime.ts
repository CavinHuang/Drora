...
export class ProviderRuntime {
  ...
  constructor(dependencies: ProviderRuntimeDependencies) {
    ...
    this.#disposeBuiltinRecovery = this.#configRuntime.onDidCheckDroraBuiltin(async () => {
      const [config, account] = await Promise.all([
        this.configService.read(),
        accountSource.read(),
      ]);
      if (!this.#disposed && config.droraBuiltinRevision !== account.basedOnDroraBuiltinRevision) {
  ...
  return {
    ...
    refreshSources: async (reason) => {
      const sourceResults = await Promise.allSettled([
        configRuntime.refreshDroraBuiltin({ force: true }),
