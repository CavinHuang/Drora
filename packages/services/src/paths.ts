...
import { DATA_BASE_DIR_FORBIDDEN_WINDOWS_INSTALL_DIR_ERROR_CODE } from "@drora/shared";

let _dataBaseDir: string | null = null;
export const DRORA_WINDOWS_APP_INSTALL_DIR_ENV = "DRORA_WINDOWS_APP_INSTALL_DIR";
const envDataBaseDir = process.env.DRORA_DATA_BASE_DIR?.trim() || null;
...
/** Get the current base directory. Priority: setDataBaseDir() > env DRORA_DATA_BASE_DIR > homedir(). */
...
/** {dataBaseDir}/.drora */
export function getDroraDataRootDir(): string {
  return join(getDataBaseDir(), ".drora");
}

/** 非项目对话共享的真实工作目录；默认 ~/.drora/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getDroraDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.drora/v2 */
export function getAppConfigDir(): string {
  return join(getDroraDataRootDir(), "v2");
}
...
function collectWindowsForbiddenAppInstallDirs(
  ...
  const candidates = [
    ...
    readEnvValue(env, DRORA_WINDOWS_APP_INSTALL_DIR_ENV),
    programFiles ? win32.join(programFiles, "Drora") : null,
    programFilesX86 ? win32.join(programFilesX86, "Drora") : null,
    programW6432 ? win32.join(programW6432, "Drora") : null,
    localAppData ? win32.join(localAppData, "Programs", "Drora") : null,
  ];
...
export function getExportLogStageDir(): string {
  return join(getDroraDataRootDir(), "export-log-stage");
}

export function getExportLogDir(): string {
  return join(getDroraDataRootDir(), "export-log");
}

export function getFeedbackRootDir(): string {
  return join(getDroraDataRootDir(), "feedback");
}
...
export function getGitCheckpointIndexRootDir(): string {
  return join(getDroraDataRootDir(), "git-checkpoint-index");
}

/** ~/.drora/v2/tasks-index.sqlite */
...
/** 与 Drora session 持久化一致：使用 workspaceKey 的 SHA-256 前 12 位 */
...
/** ~/.drora/v2/sessions/{workspaceHash} */
...
/** ~/.drora/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.drora/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .drora/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".drora", "v2");
  const newDir = join(newBaseDir, ".drora", "v2");
