...
export interface HostResourceUsageAgent {
  ...
  children: readonly DroraProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: DroraProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatDroraAgentProcessName(owner.agent.provider, owner.agent.workspacePath)
