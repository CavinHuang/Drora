...
import {
  ...
  normalizeDroraSourceHeaderValue,
  DRORA_ENV,
...
export { DRORA_SOURCE_HEADERS };

interface DroraSourceHeaderOptions {
  appVersion?: string;
  arch?: string;
...
function normalizePrintableHeaderValue(value: string | undefined): string | undefined {
  return normalizeDroraSourceHeaderValue(value);
}
...
export function buildDroraSourceHeaders(
  options: DroraSourceHeaderOptions = {},
): Record<string, string> {
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  const appVersion = normalizePrintableHeaderValue(options.appVersion ?? DRORA_VERSION);
  const releaseChannel = normalizePrintableHeaderValue(options.releaseChannel ?? DRORA_ENV);
  ...
  return buildDroraSourceHeadersFromContext({
    appVersion,
    arch,
    clientLanguage,
    clientTimezone,
