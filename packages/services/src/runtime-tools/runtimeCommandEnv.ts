...
import { buildDroraToolEnvPassthroughEnv, sanitizeDroraRuntimeEnvInPlace } from "@drora/shared";
...
function buildLoginShellEnvPatch(
  ...
  return {
    ...patch,
    ...buildDroraToolEnvPassthroughEnv(snapshot),
  };
}
...
export function buildRuntimeProcessEnvPatch(
  ...
  const toolEnvPassthroughPatch = buildDroraToolEnvPassthroughEnv({
    ...normalizedBaseEnv,
    ...loginShellEnvPatch,
  });
  ...
  const envPatch: Record<string, string> = {
    ...
    // encoding is set. Drora/Bash tool output is consumed as UTF-8, so force Python
...
export function initializeRuntimeProcessEnv(
  ...
  // 网络变量会先封存为 DRORA_TOOL_ENV_PASSTHROUGH_JSON，只有 Bash/tool 子进程边界才恢复原名。
  sanitizeDroraRuntimeEnvInPlace(process.env);
