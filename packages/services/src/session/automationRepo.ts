...
import {
  ...
  resolveWorkspaceKey,
  droraAutomationBotDeliveryTargetSchema,
  modelSelectionSchema,
  droraTaskModeSchema,
  type DroraAutomation,
  type DroraAutomationBotDeliveryTarget,
  type DroraAutomationCreateParams,
  type DroraAutomationDispatchStatus,
  type DroraAutomationLifecycleStatus,
  type ModelSelection,
  type DroraAutomationRun,
  type DroraAutomationRunDispatchStatus,
  type DroraAutomationRunOutcome,
  type DroraAutomationTrigger,
  type DroraAutomationUpdateParams,
} from "@drora/shared";
...
function rowToAutomation(row: AutomationRow): DroraAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as DroraAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as DroraAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as DroraAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): DroraAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: DroraAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): DroraAutomation["mode"] | undefined {
  const parsed = droraTaskModeSchema.safeParse(mode);
  return parsed.success ? parsed.data : undefined;
}

function assertValidAutomationMode(mode: unknown): void {
  if (mode === undefined || mode === null) return;
  if (!droraTaskModeSchema.safeParse(mode).success) {
    // 读取兼容历史脏数据不代表允许继续写脏数据；Repo 是绕过 RPC 时的最终持久化边界。
    throw new Error(`Invalid automation mode: ${String(mode)}`);
  }
}

function rowToRun(row: AutomationRunRow): DroraAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as DroraAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as DroraAutomationRunDispatchStatus,
    outcome: (row.outcome as DroraAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  // 互相覆盖全局值，导致 repo 与裸 SQL 操作在并发窗口内写进真实库 ~/.drora/v2（历史脏数据
  ...
  async create(
    params: DroraAutomationCreateParams,
    options: { nextRunAt: number | null; lifecycleStatus?: DroraAutomationLifecycleStatus },
  ): Promise<DroraAutomation> {
  ...
  async getBotDeliveryTarget(
    ...
    try {
      const parsed = droraAutomationBotDeliveryTargetSchema.safeParse(JSON.parse(raw));
      return parsed.success ? parsed.data : undefined;
    } catch {
  ...
  async get(automationId: string, workspaceKey?: string): Promise<DroraAutomation | null> {
    await this.ensureReady();
    const row = this.getRow(automationId, workspaceKey);
  ...
  async update(
    automationId: string,
    params: DroraAutomationUpdateParams,
    options?: {
      nextRunAt?: number | null;
      lifecycleStatus?: DroraAutomationLifecycleStatus;
      resetRetry?: boolean;
    },
    workspaceKey?: string,
  ): Promise<DroraAutomation | null> {
    assertValidAutomationMode(params.mode);
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<DroraAutomation[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
  ...
  async ensureRunClaimed(params: {
    ...
    workspaceKey: string;
    scheduledAt: number | null;
    trigger: DroraAutomationTrigger;
  }): Promise<void> {
    await this.ensureReady();
    const now = Date.now();
    this.getDatabase()
  ...
  async markRunDispatch(params: {
    runId: string;
    dispatchStatus: DroraAutomationRunDispatchStatus;
  ...
  async listRuns(automationId: string, workspaceKey?: string): Promise<DroraAutomationRun[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
  ...
  async getRun(runId: string): Promise<DroraAutomationRun | null> {
    await this.ensureReady();
    const row = this.getDatabase()
