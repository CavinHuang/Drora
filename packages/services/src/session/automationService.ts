import type {
  DroraAutomation,
  DroraAutomationCreateParams,
  DroraAutomationRun,
  DroraAutomationScheduleRule,
  DroraAutomationUpdateParams,
} from "@drora/shared";
...
function assertValidAutomationScheduleRule(rule: DroraAutomationScheduleRule): void {
...
export class AutomationService {
  ...
  async create(params: DroraAutomationCreateParams): Promise<DroraAutomation> {
  ...
  async update(
    ...
    // 同时顺手修复历史上已经存在的同类脏数据。
    const normalizedParams: DroraAutomationUpdateParams = hasIntervalCarrier
    ...
    const options: {
      ...
      lifecycleStatus?: DroraAutomation["lifecycleStatus"];
      resetRetry?: boolean;
    } = {};
    ...
    const effectiveDirectScheduleRule = hasDirectScheduleRule
      ? directScheduleRule
      : carrierScheduleRule;
    const normalizedWithSchedule: DroraAutomationUpdateParams =
  ...
  async runNow(
    automationId: string,
    scope?: AutomationWorkspaceScope,
  ): Promise<{ automation: DroraAutomation; run: DroraAutomationRun } | null> {
    const workspaceKey = resolveScopeKey(scope);
    const existing = await this.repo.get(automationId, workspaceKey);
    if (!existing) return null;
