...
import type { DroraImportableSessionCandidate } from "@drora/shared";
...
class ClaudeNativeSessionImportRepo {
  private getNativeProjectsRoots(): string[] {
    ...
    // 关键业务逻辑：扫描 Claude Code 原生历史目录 ~/.claude/projects，不是 drora 自己的数据目录。
    // 当 DRORA_DATA_BASE_DIR 把 .drora 放到别处时，原生 .claude 往往仍在真实用户 HOME 下。
  ...
  async scanImportableSessions(params: {
    ...
    const candidates: DroraImportableSessionCandidate[] = [];
  ...
  async findImportableSession(params: {
    workspacePath?: string;
    sessionId: string;
  }): Promise<DroraImportableSessionCandidate | null> {
    const workspaceKey = params.workspacePath
      ? normalizePathForComparison(params.workspacePath)
      : null;
  ...
  async copySessionFileToWorkspace(params: {
    ...
    // 导入副本沿用历史目录布局 ~/.drora/v2/agent-config/claude/{workspaceHash}/projects；
