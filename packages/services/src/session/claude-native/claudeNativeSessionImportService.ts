...
import type { DroraImportSessionsResult, DroraTaskMeta } from "@drora/shared";
...
export async function importClaudeNativeSessions(params: {
  ...
  sessionIds: string[];
  createImportedSession?: (source: ClaudeNativeImportedSessionSource) => Promise<DroraTaskMeta>;
  onTaskImported: (meta: DroraTaskMeta) => void;
}): Promise<DroraImportSessionsResult> {
  const normalizedSessionIds = [
    ...new Set(params.sessionIds.map((item) => item.trim()).filter(Boolean)),
  ];
  const result: DroraImportSessionsResult = {
    imported: [],
    skipped: [],
    failed: [],
  };
