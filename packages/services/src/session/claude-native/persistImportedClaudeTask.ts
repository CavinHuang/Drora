...
export function buildSearchableTextFromMessages(messages: DroraSessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.drora/v2/sessions/{hash}/{taskId}.json。现在真实 Drora session 承担续聊，legacy snapshot
