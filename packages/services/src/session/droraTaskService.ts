/* eslint-disable max-lines -- Drora task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@drora/rpc";
import { ServiceChannels } from "@drora/shared";
import type { CommandPayloadMap } from "@drora/shared/drora-protocol-v4";
import { createServiceDescriptor } from "#src/descriptors.js";
import type {
  DroraImportSessionsResult,
  DroraImportableSessionCandidate,
  DroraSessionCompactResult,
  DroraSessionGoalAction,
  DroraSessionGoalResult,
  DroraTaskCreateResult,
  DroraTaskMeta,
  DroraStreamEvent,
  DroraTaskMode,
  DroraConfigOption,
  DroraError,
  DroraAssistantMessageFeedback,
  DroraPromptAttachment,
  DroraProvider,
  DroraWorkspaceEvent,
  TraceId,
  DroraSessionFile,
  DroraAgentMcpServer,
  DroraTaskSnapshot,
  DroraTaskSnapshotBody,
  DroraTaskSnapshotToolCallsSlice,
  DroraTaskSnapshotRefContent,
  DroraEnqueueTaskCommandResult,
  DroraCancelTaskCommandResult,
  DroraTaskClientMode,
  DroraTaskTokenUsageResult,
...
export interface DroraTaskSnapshotWithEtagResult {
  snapshot: DroraTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface DroraArchivedTaskDeletionResult {
...
export type DroraModelTrajectoryContentPart =
...
export interface DroraModelTrajectoryMessage {
  role: string;
  parts: DroraModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface DroraModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type DroraModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface DroraModelTrajectoryCallSource {
  kind: DroraModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface DroraModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: DroraModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: DroraModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: DroraModelTrajectoryContentPart[];
    usage?: DroraModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface DroraModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 Drora Agent 落盘 model-io）。 */
  available: boolean;
  records: DroraModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface DroraTaskTerminalOutcome {
  taskId: string;
...
export interface DroraTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IDroraTaskService — Drora task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * Drora Agent server 维护，新功能应优先走 IDroraSessionService。
 */
export interface IDroraTaskService {
  // ---- 生命周期 ----
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<DroraEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: DroraTaskClientMode;
  }): Promise<DroraCancelTaskCommandResult>;
  ...
  }): Promise<DroraSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<DroraSessionGoalResult>;
   ...
   */
  listTaskList(params: DroraTaskListQuery): Promise<DroraTaskListResult>;
  ...
  applyGroupedTaskViewOrder(params: DroraGroupedTaskViewOrderInput): Promise<DroraGroupedTaskView>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<DroraTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<DroraTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<DroraTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<DroraTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<DroraTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskConfigOptions(params: { taskId: string }): Promise<DroraConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: DroraAssistantMessageFeedback | null;
  }): Promise<DroraSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: DroraTaskMode }): Promise<void>;
  ...
  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.drora/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 Drora Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<DroraModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<DroraTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 Drora Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  onDynamicWorkspaceEvent(
    workspace: string | DroraWorkspaceEventSubscriptionParams,
  ): Event<DroraWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<DroraError>;
}

export const IDroraTaskService = createServiceDescriptor<IDroraTaskService>(
  ServiceChannels.DroraTask,
);
