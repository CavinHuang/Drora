...
export interface IOffPeakTaskService {
  ...
  createTask(params: DroraOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<DroraOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<DroraOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<DroraOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<DroraOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<DroraOffPeakTask | null>;
  list(): Promise<DroraOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<DroraOffPeakTask | null>;
}
