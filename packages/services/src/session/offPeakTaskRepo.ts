...
/* off-peak 任务仓库：off_peak_tasks 的 sqlite schema、状态机守卫写入与调度认领。
   ...
   禁止往 automations 表或 DroraAutomation 类型上加字段。 */
...
function rowToTask(row: OffPeakTaskRow): DroraOffPeakTask {
  ...
  return {
    ...
    prompt: row.prompt,
    permissionMode: row.permission_mode as DroraTaskMode,
    ...
    status: row.status as DroraOffPeakTaskStatus,
...
function readOffPeakModelSelection(row: OffPeakTaskRow): DroraOffPeakTask["modelSelection"] | null {
...
function serializeOffPeakModelSelection(
  selection: NonNullable<DroraOffPeakTask["modelSelection"]>,
): string {
...
export class OffPeakTaskRepo {
   ...
   */
  async create(
    params: DroraOffPeakTaskCreateParams,
    options?: {
      /** 测试注入用；缺省 Date.now()。 */
      now?: number;
  ...
  async invalidateModelSelection(
    offPeakTaskId: string,
    modelSelection: NonNullable<DroraOffPeakTask["modelSelection"]>,
    options?: { now?: number },
  ...
  async updateEditableFields(
    ...
    params: {
      ...
      modelSelection?: DroraOffPeakTask["modelSelection"] | null;
    },
    options?: { now?: number },
  ): Promise<DroraOffPeakTask | null> {
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<DroraOffPeakTask[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: DroraOffPeakTask[] = [];
  ...
  async listNonTerminal(): Promise<DroraOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
  ...
  async listUnsettledTerminal(): Promise<DroraOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
