...
import {
  ...
  type DroraOffPeakTaskCreateParams,
} from "@drora/shared";
...
import type { ModelSelection, ModelSelectionValidation } from "@drora/provider";
...
function isValidCreateParams(params: DroraOffPeakTaskCreateParams): boolean {
  return (
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: DroraOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: DroraOffPeakTaskCreateParams = {
      ...params,
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<DroraOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<DroraOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<DroraOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<DroraOffPeakTask | null> {
  ...
  async list(): Promise<DroraOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<DroraOffPeakTask | null> {
  ...
  ): Promise<DroraOffPeakTask[]> {
    const repaired: DroraOffPeakTask[] = [];
  ...
  private async settleOne(task: DroraOffPeakTask): Promise<void> {
