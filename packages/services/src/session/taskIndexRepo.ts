...
import type {
  ...
  DroraTaskListResult,
  DroraTaskListItem,
} from "#src/session/droraTaskListTypes.js";
import type {
  DroraGroupedTaskRef,
  DroraGroupedTaskView,
  DroraGroupedTaskViewNode,
  DroraGroupedTaskViewOrderInput,
  DroraGroupedTaskViewQuery,
  DroraGroupedTaskViewStructure,
  DroraGroupedTaskViewStructureMember,
  DroraGroupedTaskViewStructureTopOrder,
  DroraGroupedTaskViewTopLevelNodeRef,
  DroraTaskGroup,
  DroraTaskGroupColor,
} from "#src/session/droraTaskListTypes.js";
...
function appendDroraAgentIndexedProviderFilter(
  where: string[],
...
interface TaskIndexStatePatch {
  ...
  unreadAt?: number;
  model?: string;
  status?: DroraTaskMeta["status"];
  lastError?: DroraTaskMeta["lastError"];
  target?: DroraTaskMeta["target"];
  updatedAt?: number;
}
...
const GROUPED_WORKSPACE_BOOTSTRAP_ONCE_KEY = "__drora_internal_grouped_workspace_bootstrap_once__";
const DEFAULT_TASK_GROUP_COLOR: DroraTaskGroupColor = "gray";
...
function isTerminalTaskStatus(status: DroraTaskMeta["status"]): boolean {
  return status === "completed" || status === "error";
}

function shouldPreserveNewerTerminalStatus(
  existingMeta: DroraTaskMeta | null,
  incomingMeta: DroraTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): DroraTaskMeta {
  const workspaceIdentity = resolveTaskIndexRowWorkspaceIdentity(row);
  try {
    const parsed = droraTaskMetaSchema.safeParse(JSON.parse(row.meta_json));
    if (parsed.success) {
      return {
        ...(parsed.data as DroraTaskMeta),
  ...
  return {
    ...
    mode: row.mode as DroraTaskMeta["mode"],
    model: row.model ?? undefined,
    provider: row.provider === DRORA_AGENT_PROVIDER ? DRORA_AGENT_PROVIDER : undefined,
    migrationSource: (row.migration_source as DroraTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as DroraTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: DroraTaskMeta): string {
...
function normalizeWorkspaceBootstrapScopes(
  scopes: Array<{
    workspacePath: string;
    workspaceIdentity?: string;
    workspacePurpose?: import("@drora/shared").WorkspacePurpose;
  }>,
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): DroraTaskListItem {
...
function isTaskGroupColor(value: string): value is DroraTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): DroraTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): DroraTaskGroupColor {
...
function groupedTopNodeOrderRef(node: DroraGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareGroupedNodes(
  left: DroraGroupedTaskViewNode,
  right: DroraGroupedTaskViewNode,
): number {
...
function compareCronGroupTasks(left: DroraTaskListItem, right: DroraTaskListItem): number {
...
function compareGroupTasks(
  left: DroraTaskListItem,
  right: DroraTaskListItem,
...
export class TaskIndexRepo {
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendDroraAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): DroraTaskMeta {
  ...
  async syncTaskMetaAtGroupedTop(params: {
    ...
    titleOverridden?: boolean;
    searchableText?: string;
  }): Promise<{ meta: DroraTaskMeta; initializedGroupedOrder: boolean }> {
      ...
      try {
        ...
        const meta: DroraTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 Drora session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 Drora 任务。
   ...
   */
  private ensureCronGroupMembership(meta: DroraTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<DroraTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: DroraTaskMeta): Promise<DroraTaskMeta> {
    await this.ensureReady();
  ...
  }): Promise<{ meta: DroraTaskMeta; cleared: boolean }> {
    await this.ensureReady();
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<DroraTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async queryTaskList(
    params: DroraTaskListQuery & { provider?: DroraProvider },
  ): Promise<DroraTaskListResult> {
    await this.ensureReady();
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<DroraTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: DroraGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: DroraGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    params: DroraGroupedTaskViewQuery & { provider?: DroraProvider },
  ): Promise<DroraGroupedTaskView> {
    await this.ensureReady();
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 Drora Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendDroraAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
  ...
  async queryGroupedTaskViewStructure(params: {
  ...
  }): Promise<DroraGroupedTaskViewStructure> {
    await this.ensureReady();
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: DroraGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
    ...
    const orderRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupViewNodeOrderRow[];
    const topLevelOrders: DroraGroupedTaskViewStructureTopOrder[] = [];
  ...
  async applyGroupedTaskViewOrder(
    params: DroraGroupedTaskViewOrderInput & { provider?: DroraProvider },
  ): Promise<DroraGroupedTaskView> {
    await this.ensureReady();
    ...
    const validateTaskRef = (task: DroraGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 Drora Agent 视图只接受当前 glm task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: DroraGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: DroraGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: DroraGroupedTaskRef[] = [];
