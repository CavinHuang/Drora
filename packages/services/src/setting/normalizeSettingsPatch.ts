...
export function normalizeSettingsPatch(patch: Partial<AppSettings>): Partial<AppSettings> {
  ...
  if (
    "droraEndpointOrigin" in normalizedPatch &&
    typeof normalizedPatch.droraEndpointOrigin === "string"
  ) {
    // 非生产 endpoint override 需要支持 Reset 清空；RPC/JSON 对 undefined 不稳定时，用空串也能回到默认生产域。
    const trimmedDroraEndpointOrigin = normalizedPatch.droraEndpointOrigin.trim();
    normalizedPatch.droraEndpointOrigin =
      trimmedDroraEndpointOrigin.length > 0 ? trimmedDroraEndpointOrigin : undefined;
  }
