...
export function createSkillSyncService(options?: { maxArchiveBytes?: number }): ISkillSyncService {
  ...
  return {
    ...
    async listRemoteUserSkillStatuses(params): Promise<SkillSyncRemoteStatusResult> {
      const root = getUserDroraSkillRoot();
    ...
    async checkRemoteUserSkillWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(getUserDroraSkillRoot());
    },
...
async function collectUserSkillCandidates(): Promise<SkillSyncCandidate[]> {
  ...
  const userDroraSkillRoot = getUserDroraSkillRoot();
  const rawDroraCandidates = await collectUserSkillCandidatesInRoot(userDroraSkillRoot);
  const droraDirectoryNames = await collectCoveredSkillDirectoryNamesInRoot(userDroraSkillRoot);
  const droraSkillNameKeys = new Set(
    rawDroraCandidates.map((candidate) => normalizeSkillNameKey(candidate.name)),
  );
  const droraCandidates = await dedupeCandidatesByCanonicalSkillPath(
    rawDroraCandidates,
    seenSkillRealpaths,
  );
  const agentsCandidates = await dedupeCandidatesByCanonicalSkillPath(
    (await collectUserSkillCandidatesInRoot(getUserAgentsSkillRoot())).filter(
      (candidate) =>
        !droraDirectoryNames.has(candidate.directoryName) &&
        !droraSkillNameKeys.has(normalizeSkillNameKey(candidate.name)),
    ),
    seenSkillRealpaths,
  );

  return [...droraCandidates, ...agentsCandidates].sort((left, right) =>
...
async function collectUserSkillDirectoryPathByName(): Promise<Map<string, string>> {
  const result = await collectSkillDirectoryPathByName(getUserDroraSkillRoot());
...
async function importArchive(
  ...
  const targetRoot = getUserDroraSkillRoot();
  ...
  try {
    ...
    // 远端 SkillsService 会同时读取用户级 .drora/skills 和 .agents/skills。
    // 同名 skill 已在兼容目录存在时也必须跳过，避免同步后在 .drora 下生成重复来源。
    const existingSkillPathByName = await collectUserSkillDirectoryPathByName();
