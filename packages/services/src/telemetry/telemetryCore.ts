...
import {
  ...
  buildDroraSourceHeadersFromContext,
...
function sessionCreateEventId(userId: string, sessionId: string): string {
  const bytes = createHash("sha256")
    .update(JSON.stringify(["drora:session_create:v1", userId, sessionId]))
    .digest()
    .subarray(0, 16);
...
function resolveTelemetryStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".drora", "v2", "telemetry-state.json");
  }
...
function resolveTelemetryLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".drora", "v2", "telemetry-state.lock");
  }
...
export function createTelemetryCore(dependencies: TelemetryCoreDependencies = {}) {
  ...
  const appVersion = dependencies.appVersion ?? DRORA_VERSION;
  ...
  async function sendReport(
    ...
    if (!DRORA_TELEMETRY_ENABLED || !DRORA_TELEMETRY_REPORT_ENDPOINT) {
      return;
    }
    ...
    const endpoint = String(
      rewriteDroraEndpointUrl(
        DRORA_TELEMETRY_REPORT_ENDPOINT,
        (await dependencies.resolveDroraEndpointOrigin?.()) ?? DRORA_TELEMETRY_REPORT_ENDPOINT,
      ),
    );

    const headers = buildDroraSourceHeadersFromContext({
      appVersion,
      platform,
      arch: dependencies.arch ?? process.arch,
      osVersion,
      releaseChannel: dependencies.releaseChannel ?? DRORA_ENV,
