...
import type { ProviderFamilyDomain } from "@drora/shared";
...
export class BigModelUsageQuotaProvider {
  ...
  ): Promise<DroraAccountAccess | undefined> {
  ...
  private async getStartPlanSnapshot(
    providerId: string,
    accountAccess: DroraAccountAccess | undefined,
  ...
  async getCodingPlanResetStatus(
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeDroraApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/status`),
      {
        method: "GET",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  async useCodingPlanReset(request: CodingPlanResetUseRequest): Promise<CodingPlanResetUseResult> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeDroraApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/use`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
        headers: {
  ...
  async requestCodingPlanResetOpportunity(
    ...
    try {
      payload = await readCodingPlanResetApiJson(
        this.apiClient,
        buildRuntimeDroraApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/opportunity`),
        {
          method: "POST",
          timeoutMs: REQUEST_TIMEOUT_MS,
          headers: {
  ...
  async markCodingPlanResetHistoryRead(request: CodingPlanResetScopeRequest): Promise<void> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeDroraApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/history/read`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  private async resolveCodingPlanResetAuthorization(
    ...
    const droraJwt = (await this.credentialService?.load(DRORA_JWT_TOKEN_KEY))?.trim() ?? "";
    if (!droraJwt) {
      throw new Error("coding_plan_reset_drora_jwt_required");
    }
    ...
    return {
      droraAuthorization: /^Bearer\s/i.test(droraJwt) ? droraJwt : `Bearer ${droraJwt}`,
  ...
  private async resolveAuthorization(
    ...
    if (request.allowEnvApiKey !== false && request.requirePreferredProvider !== true) {
      const envApiKey =
        readEnv(this.env, "DRORA_BIGMODEL_USAGE_API_KEY") ??
...
function createCodingPlanResetHeaders(
  ...
  const headers: Record<string, string> = {
    Authorization: authorization.droraAuthorization,
...
function resolveTeamPlanContext(accountAccess: DroraAccountAccess): TeamPlanContext | null {
...
function resolveQuotaUrlFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return readEnv(env, "DRORA_BIGMODEL_USAGE_QUOTA_URL") ?? readEnv(env, "BIGMODEL_USAGE_QUOTA_URL");
}
