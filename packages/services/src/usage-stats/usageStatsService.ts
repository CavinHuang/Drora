...
import { isCodingPlanModelProviderId } from "@drora/shared";
...
import type { OfficialMcpCredentialSource } from "./providers/droraMcpQuotaProvider.js";
...
interface UsageStatsServiceDependencies {
  ...
  /** App Usage 经 Drora Protocol 读取 agent 数据库真实统计。 */
  droraAgentService: Pick<IDroraAgentService, "getAppUsageStats">;
  /**
...
export function createUsageStatsService(
  ...
  return {
    async getAppUsageSnapshot(request: AppUsageRequest): Promise<AppUsageSnapshot> {
      ...
      // 经 Drora Protocol usage/stats 取回。不再读本地 session JSON 估算。
      return dependencies.droraAgentService.getAppUsageStats({
        range: request.range,
