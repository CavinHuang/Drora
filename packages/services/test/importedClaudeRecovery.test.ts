...
for (const clientMode of ["desktop-continuous", "web-remote-replayable"] as const) {
  test(`previously imported Claude history becomes a real session for ${clientMode}`, async () => {
    const dir = await mkdtemp(join(tmpdir(), "drora-import-recovery-"));
    setDataBaseDir(dir);
    const meta = {
    ...
    type CreateInput = Parameters<Options["droraAgentService"]["createSession"]>[0];
    const created: CreateInput[] = [];
    let session: DroraSessionStateSnapshot | undefined;
    ...
    const service = createDroraTaskServiceAdapter({
      ...
      droraAgentService: {
        ...
        async createSession(input: CreateInput) {
          created.push(input);
          session = droraSessionStateSnapshotSchema.parse({
