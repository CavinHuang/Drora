...
import { createDroraTaskServiceAdapter } from "../src/drora-agent/droraTaskServiceAdapter.js";
import {
...
test("current Project Memory catalog and files remain readable", async () => {
  const dir = await mkdtemp(join(tmpdir(), "drora-current-memory-"));
  setDataBaseDir(dir);
  try {
...
test("opening the task index leaves retired ACP IDs and user rows untouched", async () => {
  const dir = await mkdtemp(join(tmpdir(), "drora-acp-index-"));
...
test("missing sessions report the owner error even when a valid ACP snapshot exists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "drora-acp-snapshot-"));
  setDataBaseDir(dir);
  ...
  type Options = Parameters<typeof createDroraTaskServiceAdapter>[0];
  const service = createDroraTaskServiceAdapter({
    droraAgentService: {
      async resumeSession() {
        throw ownerError;
      },
      disposeAll() {},
    } as unknown as Options["droraAgentService"],
    taskIndexRepo: {
      async getTaskMeta() {
        return meta;
      },
...
test("current session recovery preserves Desktop and replayable projections", async () => {
  const snapshot = droraSessionStateSnapshotSchema.parse({
    protocol: { name: DRORA_PROTOCOL_NAME, version: DRORA_PROTOCOL_VERSION },
    session: {
...
test("current Provider configuration starts without an old config migration callback", async () => {
  const dir = await mkdtemp(join(tmpdir(), "drora-provider-current-"));
  const runtime = createProviderConfigRuntime({
    droraBuiltinFilePath: fileURLToPath(
      new URL("../../../config/provider/drora-builtin.json", import.meta.url),
    ),
  ...
  try {
    ...
    assert.ok(config.droraBuiltinRevision);
