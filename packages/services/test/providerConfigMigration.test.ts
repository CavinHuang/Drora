...
import { readLegacyDroraConfigProviders } from "../src/model-provider/legacyDroraConfigProviderReader.js";
...
async function setup() {
  const dir = await mkdtemp(join(tmpdir(), "drora-provider-migration-"));
  setDataBaseDir(dir);
  ...
  const runtime = createProviderConfigRuntime({
    ...
    readLegacyProviders: async () => {
      reads += 1;
      return readLegacyDroraConfigProviders();
    },
...
test("startup migrates published Drora config into personal config without changing the source", async () => {
