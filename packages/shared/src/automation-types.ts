import type { DroraTaskMode } from "./drora-task-types-core.js";
import type { DroraAutomationBotDeliveryTarget } from "./bots.js";
...
/** 整条 automation 的生命周期。 */
export type DroraAutomationLifecycleStatus = "active" | "completed" | "failed" | "paused";

/** 单条 automation 当前一轮的派发信息态（供 UI/诊断）。 */
export type DroraAutomationDispatchStatus =
  | "idle"
  | "claimed"
  | "dispatched"
  | "failed_to_dispatch";
...
export type DroraAutomationLocationKind = "local" | "remote";

/** 触发来源：定时 or 立即运行。 */
export type DroraAutomationTrigger = "schedule" | "manual";

/** 自定义重复规则；cronExpr 保留为兼容展示，调度以本字段为权威。 */
export interface DroraAutomationScheduleRule {
 ...
 */
export type DroraAutomationIntervalUnit = DroraAutomationScheduleRule["unit"];

/** 单次 run 的派发结果（scheduler 权威，驱动重试）。skipped=错过触发窗口。 */
export type DroraAutomationRunDispatchStatus =
  | "claimed"
  | "dispatched"
  | "failed_to_dispatch"
  | "skipped";
...
export type DroraAutomationRunOutcome = "running" | "succeeded" | "failed" | "stopped";

/** 一条定时任务定义 + 调度状态。 */
export interface DroraAutomation {
  automationId: string;
  title: string;
  /** 5 段 cron，本地时区。 */
  cronExpr: string;
  ...
  targetTaskId?: string;
  locationKind: DroraAutomationLocationKind;
  ...
  nextRunAt?: number;
  lastRunAt?: number;
  dispatchStatus: DroraAutomationDispatchStatus;
...
export interface DroraAutomationRunIdParts {
  automationId: string;
...
/**
 * runId 契约（见 DroraAutomationRun.runId）的唯一解析入口；desktop host/scheduler 与
 * CLI telemetry 不各自按字符串猜格式。不符合两种声明格式的输入一律返回 null。
 */
export function parseAutomationRunId(runId: string): DroraAutomationRunIdParts | null {
...
export interface DroraAutomationRun {
...
export interface DroraAutomationCreateParams {
  title: string;
  cronExpr: string;
   ...
   */
  intervalUnit?: DroraAutomationIntervalUnit;
  /** 1–200 的整数间隔，必须与 intervalUnit 配对。 */
  interval?: number;
}

/** 编辑 automation 的可变字段。 */
export interface DroraAutomationUpdateParams {
  title?: string;
  cronExpr?: string;
  prompt?: string;
