export type DroraBackgroundTaskControlStatus =
  | "pending"
  | "running"
  | "completed"
  | "failed"
...
export interface DroraBackgroundTaskControlItem {
  jobId: string;
  toolCallId?: string;
  command: string;
  taskKind: "agent" | "bash";
  cancellable?: boolean;
  title?: string;
  status: DroraBackgroundTaskControlStatus;
  startedAt?: number;
  elapsedMs?: number;
  pid?: number;
  stdoutTail?: string;
...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<DroraBackgroundTaskControlStatus>([
  "running",
]);
...
type BackgroundTaskControlKind = DroraBackgroundTaskControlItem["taskKind"];
 ...
 */
export function resolveDroraBackgroundTaskControlKind(
  value: unknown,
): BackgroundTaskControlKind | undefined {
...
function normalizeStatus(value: string | undefined): DroraBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // Drora Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseDroraBackgroundTaskControlItems(
  value: unknown,
): DroraBackgroundTaskControlItem[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const jobsById = new Map<string, DroraBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveDroraBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
    ...
    const job: DroraBackgroundTaskControlItem = {
      jobId,
...
export function isActiveDroraBackgroundTaskControlItem(
  job: DroraBackgroundTaskControlItem,
): boolean {
  return ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES.has(job.status);
}

export function getDroraBackgroundTaskControlItemElapsedMs(
  job: DroraBackgroundTaskControlItem,
  now = Date.now(),
): number {
...
export function collectVisibleDroraBackgroundTaskControlItems(
  jobs: readonly DroraBackgroundTaskControlItem[],
  now = Date.now(),
  thresholdMs = 30_000,
): Array<DroraBackgroundTaskControlItem & { elapsedMs: number }> {
  return jobs
    .filter(isActiveDroraBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getDroraBackgroundTaskControlItemElapsedMs(job, now),
    }))
