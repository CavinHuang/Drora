...
import { DRORA_AGENT_PROVIDER, DRORA_AGENT_PROVIDER_LABEL } from "./drora-agent-policy.js";
import type {
  DroraConfigOption,
...
import {
  ...
  type DroraInteractionRequestOrigin,
  type DroraPermissionResponse,
 ...
 */
export const droraAutomationBotDeliveryTargetSchema = z
  .object({
...
export type DroraAutomationBotDeliveryTarget = z.infer<
  typeof droraAutomationBotDeliveryTargetSchema
>;
...
export interface BotTaskSummary {
  taskId: string;
  title: string;
  status: DroraTaskRuntimeStatus | "persisted-completed" | "persisted-error" | "unknown";
  workspacePath: string;
  workspaceIdentity?: string;
  provider?: DroraProvider;
  model?: string;
...
export interface BotTaskBroadcastPayload {
  ...
  permissionRequest?: DroraPermissionRequest;
  elicitationRequest?: DroraElicitationRequest;
  requestId?: string;
  error?: string;
}
...
export const botCurrentOptionsSchema = z
  .object({
    ...
    // 兼容旧 bot-config.json；CLI provider 现在统一由 Drora Protocol 侧配置决定。
    cli: z.literal(DRORA_AGENT_PROVIDER).optional(),
  })
  .strict();

export const botDraftOptionsSchema = z
  .object({
    provider: z.literal(DRORA_AGENT_PROVIDER),
    modelSelection: modelSelectionSchema.optional(),
    mode: z.string().min(1).optional(),
  })
  .strict();
...
export const BOT_DRORA_PROVIDER_OPTIONS: Array<{
  id: DroraProvider;
  label: string;
}> = [{ id: DRORA_AGENT_PROVIDER, label: DRORA_AGENT_PROVIDER_LABEL }];
