export type DroraAgentBinaryKind = "native-binary";

export interface DroraAgentRuntimeDescriptor {
  binaryKind: DroraAgentBinaryKind;
  binaryEnvVar: string;
  ...
  /**
   * 桌面端把 agent 的 JS bundle（drora.cjs）打进 resources/glm，由 app 内置的 Electron Node runtime
...
export const DRORA_AGENT_RUNTIME: DroraAgentRuntimeDescriptor = {
  binaryKind: "native-binary",
  binaryEnvVar: "GLM_BINARY_PATH",
  bundledResourceDir: "glm",
  version: "0.13.3",
  spawnArgs: ["app-server", "--stdio"],
  nativeConfigDir: ".drora/cli",
  nativeConfigFileName: "config.json",
  missingBinaryMessage:
    "[Drora Agent] glm binary 未找到，请设置 GLM_BINARY_PATH 或先准备 GLM 运行时资源",
  resolveEntrySegments: (platform) => [resolvePlatformBinaryName("drora-agent", platform)],
  nodeBundleEntryFile: "drora.cjs",
  resolveNodeBundleSegments() {
    return [this.nodeBundleEntryFile];
  },
};

export function getDroraAgentRuntime(): DroraAgentRuntimeDescriptor {
  return DRORA_AGENT_RUNTIME;
}
