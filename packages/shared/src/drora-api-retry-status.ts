import type { DroraApiRetryStatus } from "./drora-task-types-core.js";

export function normalizeDroraApiRetryStatus(
  value: unknown,
...
export function droraApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): DroraApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeDroraApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = droraApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isDroraModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function droraApiRetryFromStreamRecoveryPayload(
  value: unknown,
): DroraApiRetryStatus | undefined {
  const record = asRecord(value);
