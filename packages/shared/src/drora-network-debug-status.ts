import type {
  ...
  TraceId,
  DroraStreamEvent,
  DroraTaskNetworkDebugStatusType,
} from "./drora-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<DroraTaskNetworkDebugStatusType>([
...
export function droraTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<DroraStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): DroraTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as DroraTaskNetworkDebugStatusType)
    : undefined;
}
