// ── 旧协议兼容面（过渡期）──────────────────────────────
// 剩余 1 个导出：deriveDroraTaskStatusFromSessionSnapshot。
// 消费者：droraTaskServiceAdapter/droraTaskIndexSyncer/droraSessionProjection（旧投影栈）。
import type { DroraSessionStateSnapshot } from "./drora-protocol/index.js";
import { getDroraUserVisibleMessages } from "./drora-session-visible-content.js";
import type { DroraTaskMeta } from "./drora-task-types-core.js";
type DroraTaskStatus = DroraTaskMeta["status"];

function statusFromDroraSession(
  status: DroraSessionStateSnapshot["session"]["status"],
): DroraTaskStatus {
...
function hasBlockingActiveSnapshotRuntime(snapshot: DroraSessionStateSnapshot): boolean {
...
function hasActiveSnapshotRuntime(snapshot: DroraSessionStateSnapshot): boolean {
...
function hasCompletedVisibleAssistantTurn(snapshot: DroraSessionStateSnapshot): boolean {
  const visibleMessages = getDroraUserVisibleMessages(snapshot.messages, {
...
export function deriveDroraTaskStatusFromSessionSnapshot(
  ...
  const status = statusFromDroraSession(snapshot.session.status);
