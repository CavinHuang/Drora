...
// 消费者：droraTaskServiceAdapter/droraSessionProjection/droraTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromDroraMessageParts 已迁
// drora-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { DroraSessionGoal } from "./drora-protocol/index.js";
...
export function isDroraGoalContinuationReminderText(text: string): boolean {
...
export function isDroraGoalContinuationReminderMessage(message: DroraMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isDroraGoalContinuationReminderText(textFromDroraMessageParts(message.parts)))
  );
}

export function isDroraGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isDroraGoalModelOnlyReminderMessage(message: DroraMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isDroraGoalContinuationReminderMessage(message) ||
      isDroraGoalStateReminderText(textFromDroraMessageParts(message.parts)))
  );
}

export function isDroraModelOnlySyntheticUserMessage(message: DroraMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isDroraCompactSummaryMessage(message: DroraMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getDroraUserVisibleMessages(
  messages: readonly DroraMessageWithParts[],
  _options: { target?: DroraSessionGoal | null } = {},
): DroraMessageWithParts[] {
  const visibleMessages: DroraMessageWithParts[] = [];
  for (const message of messages) {
    if (isDroraModelOnlySyntheticUserMessage(message) || isDroraCompactSummaryMessage(message)) {
...
export function getDroraGoalIterationByAssistantMessageId(
  messages: readonly DroraMessageWithParts[],
  options: { maxGoalIteration?: number; target?: DroraSessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareDroraMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isDroraGoalContinuationReminderMessage(message)) {
...
export interface DroraGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getDroraGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly DroraGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareDroraMessagesByCreatedTime(
  left: DroraMessageWithParts,
  right: DroraMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: DroraMessageWithParts, target: DroraSessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveDroraVisibleSessionTitle(input: {
  title?: string;
  messages: readonly DroraMessageWithParts[];
  target?: DroraSessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isDroraGoalContinuationReminderText(normalizedTitle) &&
    !isDroraGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isDroraGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromDroraMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
