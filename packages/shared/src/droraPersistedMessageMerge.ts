import type { DroraPersistedMessage, DroraPersistedMessagePart } from "./drora-task-types-core.js";

// Drora runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveDroraAssistants(
  messages: readonly DroraPersistedMessage[],
): DroraPersistedMessage[] {
  const merged: DroraPersistedMessage[] = [];
...
function mergeAssistantPair(
  first: DroraPersistedMessage,
  next: DroraPersistedMessage,
): DroraPersistedMessage {
  ...
  const shiftedNextParts: DroraPersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
