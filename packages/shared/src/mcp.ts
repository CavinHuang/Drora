/**
 * MCP (Model Context Protocol) types for Drora
 * Based on the original Tauri implementation
 */

import type { SettingsDirectoryLocation } from "./settings-source.js";
import type { McpServerFailureKind } from "./drora-protocol/index.js";

// CUA official plugin 身份常量（port 自 feat；UI 设置面板 + bootstrap 复用以避免字面量漂移）。
export const DRORA_CUA_OFFICIAL_PLUGIN_ID = "computer-use@drora-plugins-official";
// CUA server 身份串（port 自 feat mcp.ts）：server key = 模型可见工具前缀段（刻意不带 drora-）；
// namespace name = official plugin 运行时命名空间 plugin:<pluginId>:<serverKey>。
export const DRORA_CUA_OFFICIAL_MCP_NAMESPACE_NAME = "plugin:computer-use:computer-use";
// 插件身份 env key：resolver（adapters/src/plugins/mcp.ts）权威写入 loaded.id，manifest/user env 不可覆盖。
// bootstrap + cli/plugin-host-command.ts 复用此常量识别 official drora-cua plugin server，避免字面量漂移。
export const DRORA_PLUGIN_ID_ENV_KEY = "DRORA_PLUGIN_ID";

export type McpSource = "mcp" | "droraagentmcp";
...
export interface McpConfig {
  ...
  droraagentmcp: CliMcpConfig;
}

export interface DroraMcpServer {
  id: string;
  name: string;
  config: McpServerConfig;
  enabled: boolean;
...
export type DroraAgentMcpServer =
  | {
      name: string;
      command: string;
      args: string[];
...
// drora-cua MCP server 识别的单一事实源。desktop 产品 broker resolver（@drora/services 的
// mcpBrokerInjection）与 CLI bootstrap（apps/drora-cli 的 mcp-config）两条注入入口必须用
// 完全一致的判定；否则同一 MCP 配置在不同入口行为不同，可能漏注入 product broker，让
// Python/uvx 自己持有 macOS TCC 权限（违反 fail-closed 边界）。改这里即同时改两条链路。
function droraCuaArgLeaf(value: string): string {
  // 先去掉结尾的路径分隔符再取叶子：`.../drora-cua/` 直接 split 会得到空串叶子 → 漏判 → fail-open。
  return (
    value
      .replace(/[\\/]+$/, "")
      .split(/[\\/]/)
...
// 单个候选串是否为 drora-cua 的包规格。PyPI 视 `_`/`-` 等价，故先把下划线归一成短横（drora_cua →
// drora-cua）；覆盖 uv/npm 的 `@version`、pip 的 `==version`、extras `[...]`、git 的 `.git`/`.git@`，
// 以及 `python -m drora_cua.server` 这种点号子模块（`drora-cua.<submodule>`）。fail-closed 边界宁可
// 过判也不漏判；仍不会误判 `drora-cua-proxy`（短横续接，不以 `.`/`@`/`[`/`==` 边界续接）。
function matchesDroraCuaSpec(candidate: string): boolean {
  const c = candidate.replace(/_/g, "-");
  return (
    c === "drora-cua" ||
    c.startsWith("drora-cua[") ||
    c.startsWith("drora-cua@") ||
    c.startsWith("drora-cua==") ||
    // `.` 分支同时覆盖 `drora-cua.git` / `drora-cua.git@v1` 与 `drora-cua.server` 等 python 子模块。
    c.startsWith("drora-cua.")
  );
}

/**
 * MCP server 的 command 是否指向 drora-cua。用与 args 相同的包规格判定（并比对路径叶子），
 * 覆盖 `command: "drora-cua"`、`/opt/bin/drora-cua`，以及把包规格直接当 command 的写法
 * （`drora-cua@1.2.3` 等）。对 fail-closed 边界宁可过判也不漏判。
 */
export function isDroraCuaMcpCommand(command: string): boolean {
  return matchesDroraCuaSpec(command) || matchesDroraCuaSpec(droraCuaArgLeaf(command));
}

/**
 * 单个 arg 是否为 drora-cua 的包规格。覆盖 `drora-cua`、`drora-cua[macos]`、`drora-cua@1.2.3`、
 * `drora-cua==1.2.3`、`drora_cua`，以及 git / 本地路径形态（`.../drora-cua`、`drora-cua.git`、
 * `git+https://.../drora-cua.git@v1`）。同时比对原始值与路径叶子，覆盖 `--from <path>`、`--from <git-url>`。
 */
export function isDroraCuaMcpPackageArg(value: string): boolean {
  return matchesDroraCuaSpec(value) || matchesDroraCuaSpec(droraCuaArgLeaf(value));
}

export function convertToDroraAgentMcpServer(
  name: string,
  config: McpServerConfig,
): DroraAgentMcpServer | null {
  ...
  if (isStdio && config.command) {
    ...
    if (isWin32) {
      ...
      if ((lowerCmd === "cmd" || lowerCmd === "cmd.exe") && args[0] === "/c" && unwrappedCommand) {
        ...
        // 先显式取值并判空，既满足类型收窄，也避免把空命令传给 Drora Agent。
