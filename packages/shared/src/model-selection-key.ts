...
export function buildNativeSupplierKey(droraProvider: DroraProvider): string {
  return `${NATIVE_SUPPLIER_KEY_PREFIX}${droraProvider}`;
}
...
export function resolveSupplierKeyFromModelDisplayValue(
  ...
  return buildNativeSupplierKey(droraProvider);
}
