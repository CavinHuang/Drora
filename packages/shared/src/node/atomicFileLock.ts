import { DRORA_FILE_LOCK_TIMEOUT_ERROR_CODE } from "../errors.js";
...
function createFileLockTimeoutError(
  ...
  const error = new Error(
    `Timed out after ${waitedMs}ms waiting for the Drora file lock: ${lockFile}`,
  ) as NodeJS.ErrnoException & { cause?: unknown };
  error.code = DRORA_FILE_LOCK_TIMEOUT_ERROR_CODE;
  error.path = filePath;
  error.syscall = "mkdir";
  error.cause = cause;
  return error;
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    try {
      ...
      if (
        ...
        throw Object.assign(new Error("Drora file lock ownership changed during acquire"), {
          code: "EEXIST",
        });
      }
      return async () => {
