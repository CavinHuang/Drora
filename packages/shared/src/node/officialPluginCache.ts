...
import { DRORA_OFFICIAL_PLUGIN_MARKETPLACE_ID } from "../plugin-marketplaces.js";
...
/**
 * 扫描 `<plugins storage>/cache/drora-plugins-official/<name>/<version>/`。
...
export async function scanOfficialPluginCacheRoots(
  ...
  const cacheRoot = join(pluginStorageRoot, "cache", DRORA_OFFICIAL_PLUGIN_MARKETPLACE_ID);
  let pluginEntries;
  try {
