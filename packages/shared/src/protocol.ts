...
/** Drora 运行中继续输入时的交互行为 */
export type DroraInteractionBehavior = "queue" | "guide";
...
export interface ResourceUsageProcess {
  pid: number;
  /** 进程显示名，如 drora-main / drora-agent-drora-demo / node_repl */
  name: string;
...
export interface AppSettings {
   ...
   * 只影响内置浏览器出口，不影响 Drora 自身对后端与模型 API 的请求。默认关闭，重启后生效。
   */
  ...
  /** Drora 运行中继续输入时，是排队到下一轮，还是引导到下一次工具调用后运行 */
  ...
  /** 数据目录的根路径（替代 homedir），默认为 os.homedir()；.drora/v2 后缀不变 */
  dataBaseDir?: string;
