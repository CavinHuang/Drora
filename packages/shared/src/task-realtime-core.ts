...
import { z } from "zod";
import type { DroraTaskMigrationSource, DroraTaskMode } from "./drora-task-types-core.js";
import { droraAgentProviderSchema } from "./drora-agent-policy.js";
import { droraPermissionResponseSchema } from "./drora-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./drora-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./drora-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const droraTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly DroraTaskMode[];
const droraTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly DroraTaskMigrationSource[];
const droraTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 DroraTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(droraTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: droraAgentProviderSchema.optional(),
  migrationSource: z.enum(droraTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: droraTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const droraPromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const droraTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const droraTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  droraTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: droraTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: droraTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
