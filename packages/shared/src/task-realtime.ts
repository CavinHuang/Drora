...
// drora-task-types-core.ts。本文件与旧 realtime 总线组同生命周期。
import type {
  DroraTaskClientMode,
...
import type { DroraPermissionResponse } from "./drora-protocol-legacy-types.js";
import type { WorkspaceHookReviewDecision } from "./drora-protocol-v4/workspace-hook-review.js";
...
export type TaskOwnerCommandRequest =
  ...
  | {
      ...
      taskId: string;
      runId: string;
      taskCommand: Extract<DroraTaskRuntimeCommand, { type: "send_prompt" }>;
    }
  | {
      commandRequestId: string;
      ...
      runId: string;
      commandId: string;
      clientMode: Extract<DroraTaskClientMode, "web-remote-replayable">;
    }
  | {
      commandRequestId: string;
...
export type TaskOwnerCommandResult =
  | { commandRequestId: string; success: true; taskCommand?: DroraTaskRuntimeCommand }
  | {
      commandRequestId: string;
      success: false;
      error: string;
