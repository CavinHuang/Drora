import type { DroraPlanStep } from "./drora-task-types-core.js";
...
function normalizePlanStatus(value: unknown): DroraPlanStep["status"] | null {
...
function parsePlanStep(value: unknown, index: number): DroraPlanStep | null {
...
function extractPlanStepsFromValue(value: unknown): DroraPlanStep[] | null {
  ...
  const steps = collection
    ...
    .filter((step): step is DroraPlanStep => step !== null);
