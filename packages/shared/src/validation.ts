...
import { droraProviderSchema } from "./providers.js";
...
import { PROTOCOL_V4_LIMITS } from "./drora-protocol-v4/core.js";
import { errorAttributionSchema } from "./drora-protocol-v4/snapshot.js";
import { sessionWorkflowActivitySchema } from "./drora-protocol-v4/sessions-index-workflow-activity.js";
import {
...
export { droraTaskModeSchema } from "./drora-task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("drora-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  droraBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 DroraTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { droraProviderSchema };

export const droraTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: droraProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = droraProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `droraProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: droraMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: droraToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: droraMcpTelemetryEventSchema,
  })
  .strict();
...
export const droraTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const droraPromptImageAttachmentSchema = z.object({
...
export const droraPromptVideoAttachmentSchema = z.object({
...
export const droraPromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const droraPromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const droraPromptAttachmentSchema = z.discriminatedUnion("kind", [
  droraPromptImageAttachmentSchema,
  droraPromptVideoAttachmentSchema,
  droraPromptPdfAttachmentSchema,
  droraPromptFileAttachmentSchema,
]);

export const droraPersistedToolCallSchema = z.object({
...
const droraPersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const droraPersistedMessageSchema = z.object({
  ...
  attachments: z.array(droraPromptAttachmentSchema).optional(),
  tools: z.array(droraPersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(droraPersistedMessagePartSchema).optional(),
...
export const droraTaskGoalStatusSchema = z.enum(["active", "paused", "budget_limited", "complete"]);

export const droraTaskTargetChangedActionSchema = z.enum([
  "set",
  "status_updated",
  "cleared",
  "usage_accounted",
...
export const droraTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const droraTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: droraTaskGoalStatusSchema,
...
export const droraTaskGoalChangedPatchSchema = z.object({
  action: droraTaskTargetChangedActionSchema,
  source: droraTaskTargetChangedSourceSchema,
  target: droraTaskGoalSchema.nullable(),
  previousTarget: droraTaskGoalSchema.nullable().optional(),
});

export const droraTaskTargetStatusSchema = droraTaskGoalStatusSchema;
export const droraTaskTargetSchema = droraTaskGoalSchema;
export const droraTaskTargetChangedPatchSchema = droraTaskGoalChangedPatchSchema;

export const droraTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: droraTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: droraAgentProviderSchema.optional(),
  migrationSource: droraTaskMigrationSourceSchema.optional(),
  ...
  status: droraTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: droraTaskGoalSchema.nullable().optional(),
});

export const droraTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const droraPinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(droraTaskIndexEntrySchema),
});

const droraPersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const droraPersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(droraPersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const droraPersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const droraSessionFileSchema = z.object({
  meta: droraTaskMetaSchema,
  messages: z.array(droraPersistedMessageSchema),
  fileChanges: z.array(droraPersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(droraPersistedTurnCheckpointSchema).optional(),
});
