...
import { normalizeDroraEndpointOrigin } from "./droraEndpoint.js";
import {
...
const droraInteractionBehaviorSchema = z.enum(["queue", "guide"]);
...
const droraEndpointOriginSchema = z.preprocess((value) => {
  ...
  try {
    return normalizeDroraEndpointOrigin(trimmed);
  } catch {
    return undefined;
  }
}, z.string().optional());

function sanitizeDroraEndpointOrigin(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const raw = value as Record<string, unknown>;
  if (!("droraEndpointOrigin" in raw)) {
    return value;
  }
  const parsed = droraEndpointOriginSchema.safeParse(raw.droraEndpointOrigin);
  if (parsed.success && typeof parsed.data === "string") {
    return { ...raw, droraEndpointOrigin: parsed.data };
  }
  const { droraEndpointOrigin: _droraEndpointOrigin, ...next } = raw;
...
const appSettingsObjectSchema = z.object({
  ...
  droraInteractionBehavior: droraInteractionBehaviorSchema.default("queue"),
        ...
        migrateMessageStreamShowReasoningDefault(
          migrateCloseToTrayOnWindowsDefault(
            migrateLegacyLocalePreference(
              sanitizeDroraEndpointOrigin(migrateLegacyWorkspaceSession(value)),
            ),
          ),
        ),
      ),
...
export const appSettingsPatchSchema = z.object({
  ...
  droraInteractionBehavior: droraInteractionBehaviorSchema.optional(),
