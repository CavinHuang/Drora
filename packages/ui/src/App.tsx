...
import type { GitChangeSourceId, WorkspacePurpose } from "@drora/shared";
import { useDroraStore } from "@/store/StoreProvider.js";
import { getVisibleTaskMetas, useDroraSessionStore } from "@/store/droraSessionStore.js";
...
import { DRORA_PRODUCT_DOCS_URL } from "@/lib/productDocs.js";
import appLogoUrl from "@/assets/drora-mark.svg";
...
import { useWorkspaceShellDroraState } from "@/app-shell/useWorkspaceShellDroraState.js";
...
export function App({
  ...
  const { intl, locale, setLocale } = useDroraIntl();
  ...
  const workspaceReadOnlyReason = workspaceReadOnly
    ...
    : undefined;
  const { workspaceShellDroraState, reloadSessionDisabled } = useWorkspaceShellDroraState(
    workspaceAbsPath,
    workspaceIdentity,
  );
  const activeTaskId = workspaceShellDroraState.activeTaskId;
  ...
  const draftSessionId = useDroraSessionStore(
  ...
  const draftFocusVersion = workspaceShellDroraState.draftFocusVersion;
  const {
    isTerminalOpen,
    setIsTerminalOpen,
    sidePaneState,
  ...
  const notificationEnabled = useDroraStore((s) => s.notificationEnabled);
  ...
  const theme = useDroraStore((s) => s.theme);
  const setTheme = useDroraStore((s) => s.setTheme);
  const {
    isMacFullscreen,
  ...
  const {
    ...
    workspaceRemoteSessionId,
    workspaceIdentity,
    selectedProvider: workspaceShellDroraState.selectedProvider,
    intl,
  });
  ...
  const handleOpenProductDocs = useCallback(() => {
    platform.openExternal(DRORA_PRODUCT_DOCS_URL);
  }, [platform]);
  ...
  const handleStartDraftInWorkspace = useCallback(
    (
      ...
      createSource?: import("@drora/shared").SessionCreateSource,
    ) => {
      const store = useDroraSessionStore.getState();
      ...
      const targetSelectedProvider = resolveWorkspaceSwitchDraftProvider({
        currentSelectedProvider: workspaceShellDroraState.selectedProvider,
        targetWorkspacePath,
    ...
    [
      ...
      workspaceAbsPath,
      workspaceIdentity,
      workspaceShellDroraState.selectedProvider,
    ],
  );
  ...
  const testActions = useMemo<TestActions>(
    () => ({
      ...
      getPluginsOverview: (params) => services.droraAgentService.getPluginsOverview(params),
      addPluginMarketplace: (params) => services.droraAgentService.addPluginMarketplace(params),
      updatePluginMarketplace: (params) =>
        services.droraAgentService.updatePluginMarketplace(params),
      installPlugin: (params) => services.droraAgentService.installPlugin(params),
      listPlugins: (params) => services.droraAgentService.listPlugins(params),
      getPluginReferenceCatalog: (params) =>
        services.droraAgentService.getPluginReferenceCatalog(params),
    }),
    [locale, services.droraAgentService, setLocale, theme, setTheme, testMessages],
  );
  ...
  const handleSelectAdjacentConversation = useCallback(
    (direction: "previous" | "next") => {
      runVisibleWorkspaceCommand(() => {
        const sessionState = useDroraSessionStore.getState();
  ...
  return (
    <>
      ...
      <WorkspaceShellLayout
        ...
        workspaceShellDroraState={workspaceShellDroraState}
        theme={theme}
