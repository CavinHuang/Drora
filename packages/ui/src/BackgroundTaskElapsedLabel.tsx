...
function createElapsedBaseline(job: DroraBackgroundTaskControlItem) {
  const mountedAt = Date.now();
  return {
    elapsedMs: getDroraBackgroundTaskControlItemElapsedMs(job, mountedAt),
...
function elapsedMsForClock(input: {
  ...
  const elapsedFromJob = getDroraBackgroundTaskControlItemElapsedMs(input.job, input.now);
