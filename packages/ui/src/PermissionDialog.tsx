...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@drora/shared/drora-protocol-v4";
...
function readPermissionRuleScopes(option: DroraPermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: DroraPermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: DroraPermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<DroraProvider, Record<string, string>>
> = {
  glm: {
    // GLM/Drora Agent 通过 Drora Agent 发来的项目级记忆授权文案是英文原文。
...
function getPermissionDisplayReason(request: DroraPermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: DroraPermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: DroraPermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  // 当前 Drora Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // Drora Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。
