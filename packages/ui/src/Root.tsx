...
import { StoreProvider, useDroraStore } from "@/store/StoreProvider.js";
...
import { consumeDroraJwtInvalidRestartMarker } from "@/root/droraJwtInvalidRestartMarker.js";
...
function RootInner({
  ...
  const user = useDroraStore((state) => state.user);
  const isRestoringOAuthSession = useDroraStore((state) => state.isRestoringOAuthSession);
  const setUser = useDroraStore((state) => state.setUser);
  const setIsRestoringOAuthSession = useDroraStore((state) => state.setIsRestoringOAuthSession);
  const setOAuthError = useDroraStore((state) => state.setOAuthError);
  const oauthPollingActive = useDroraStore((state) => state.oauthPollingActive);
  const setOAuthPollingActive = useDroraStore((state) => state.setOAuthPollingActive);
  const markOAuthSuccess = useDroraStore((state) => state.markOAuthSuccess);
  const {
  ...
  const [welcomeScreenOpenReason, setWelcomeScreenOpenReason] =
    useState<WelcomeScreenOpenReason | null>(() =>
      consumeDroraJwtInvalidRestartMarker() ? "session-expired" : null,
    );
  const [providerFamilyDomainMigrationComplete, setProviderFamilyDomainMigrationComplete] =
    useState(false);
  const loginEntryRequest = useDroraStore((state) => state.loginEntryRequest);
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel === APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL) {
        ...
        void services.droraAgentService.syncAppRuntimePreferences(parsed.data).catch((error) => {
  ...
  }, [refreshAppSettings, services.broadcastService, services.droraAgentService]);

  useEffect(() => {
    if (!appSettings) {
      return;
    }
    void services.droraAgentService
  ...
  const totalUnreadTaskCount = useDroraSessionStore((state) =>
  ...
  // 这里把 provider 启动检查纳入 workspace 恢复门禁，避免未连接账号前启动 Drora session。
  ...
  if (isStartupRenderBlocked) {
    ...
    return (
      <RootShell>
        ...
        {/* HTML 启动壳已经展示 Drora SVG，但 React 接管 root 后旧壳会被整棵替换。
