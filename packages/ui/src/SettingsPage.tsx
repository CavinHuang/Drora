...
export function SettingsPage({
  ...
  >([]);
  const interfaceMode = useDroraStore((state) => state.interfaceMode);
  const setInterfaceMode = useDroraStore((state) => state.setInterfaceMode);
  const theme = useDroraStore((state) => state.theme);
  const setTheme = useDroraStore((state) => state.setTheme);
  const codePreviewSettings = useDroraStore((state) => state.codePreviewSettings);
  const setCodePreviewSettings = useDroraStore((state) => state.setCodePreviewSettings);
  const uiFontSizePx = useDroraStore((state) => state.uiFontSizePx);
  const setUiFontSizePx = useDroraStore((state) => state.setUiFontSizePx);
  const notificationEnabled = useDroraStore((state) => state.notificationEnabled);
  const setNotificationEnabled = useDroraStore((state) => state.setNotificationEnabled);
  const notificationSoundEnabled = useDroraStore((state) => state.notificationSoundEnabled);
  const setNotificationSoundEnabled = useDroraStore((state) => state.setNotificationSoundEnabled);
  ...
  const setNewUserOnboardingOpen = useDroraStore((state) => state.setNewUserOnboardingOpen);
  ...
  const [droraInteractionBehavior, setDroraInteractionBehavior] =
    useState<DroraInteractionBehavior>("queue");
  ...
  useEffect(() => {
    services.settingService
      ...
      .then((settings: AppSettings) => {
        ...
        setDroraInteractionBehavior(settings.droraInteractionBehavior ?? "queue");
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    setDroraInteractionBehavior(sharedSettings.droraInteractionBehavior ?? "queue");
  ...
  const handleDroraInteractionBehaviorChange = useCallback(
    async (behavior: DroraInteractionBehavior) => {
      await runSettingsActionAsync({
        featureId: "settings.conversation",
        action: "change_interaction_behavior",
        trigger: "select",
        operation: () => updateSharedSettings({ droraInteractionBehavior: behavior }),
        completed: { resultSource: "shared_settings", valueAfter: behavior },
      });
      setDroraInteractionBehavior(behavior);
    },
    [updateSharedSettings],
  );
                      ...
                      <div className="space-y-8">
                        {activeSection === "general" ? (
                          <GeneralSectionContent
                            ...
                            droraInteractionBehavior={droraInteractionBehavior}
                            ...
                            onDroraInteractionBehaviorChange={handleDroraInteractionBehaviorChange}
