...
import type { DroraTaskPendingInteraction } from "@drora/shared";
import { cn } from "@/components/lib/utils.js";

interface TaskInteractionBadgeProps {
  interaction?: DroraTaskPendingInteraction;
...
function isAskUserQuestionInteraction(
  interaction: DroraTaskPendingInteraction | undefined,
): boolean {
...
export function TaskInteractionBadge({
  ...
  const style =
    progress !== undefined
      ? ({
          "--drora-interaction-progress": String(progress),
          "--drora-interaction-remaining-ms": `${remainingMs}ms`,
        } as CSSProperties)
      : undefined;
  ...
  const badgeContent = (
    <>
      {progress !== undefined ? (
        <span
          aria-hidden="true"
          className="drora-task-interaction-countdown-fill absolute inset-0 origin-left bg-interaction-ask-fill transition-opacity group-hover/interaction-badge:opacity-0 group-focus-visible/interaction-badge:opacity-0 motion-reduce:transition-none"
        />
      ) : null}
      {canSnooze ? (
