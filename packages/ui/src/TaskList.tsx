...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@drora/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<DroraTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<DroraTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<DroraTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<DroraTaskMeta | null>;
  ...
  const startDraft = useDroraSessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: DroraTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareDroraTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {
