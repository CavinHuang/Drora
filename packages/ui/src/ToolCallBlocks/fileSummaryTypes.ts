...
import type { WorkflowRunState } from "@drora/shared/drora-protocol-v4";
...
export function readRawToolCallInput(raw: unknown): unknown {
  ...
  // Drora protocol 的 permission/request payload 按 schema 把工具参数放在 input，
...
export interface WorkflowRunCardSummary {
   ...
   *
   * 两个数都出自 `workflowRunStepCounts`（@drora/shared）：`nodes` 表内 + `usage.nodesUnlisted`
