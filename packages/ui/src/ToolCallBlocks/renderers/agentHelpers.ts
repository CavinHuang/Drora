import type { useDroraIntl } from "@/i18n/IntlProvider.js";
import type { ToolCallBlockRenderContext } from "@/ToolCallBlocks/shared.js";
import { isSubagentColor } from "@/lib/subagentColors.js";

type AgentIntl = ReturnType<typeof useDroraIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "drora", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "drora", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "drora", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "drora", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const drora = meta && isPlainRecord(meta.drora) ? meta.drora : null;
  const droraBackgroundAgent =
    drora && isPlainRecord(drora.backgroundAgent) ? drora.backgroundAgent : null;
  const taskNotification =
    drora && isPlainRecord(drora.taskNotification) ? drora.taskNotification : null;
  const backgroundAgent = droraBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "drora", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "drora", "taskNotification", "summary"]);
