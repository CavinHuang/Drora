...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@drora/shared";
import type { DroraPlanStep } from "@drora/shared";
...
const todoTextClasses: Record<DroraPlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): DroraPlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: DroraPlanStep["status"] }) {
  if (status === "completed") {
