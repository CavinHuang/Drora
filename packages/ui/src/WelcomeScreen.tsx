...
import { DroraAboutLogo } from "@/components/ui/DroraAboutLogo.js";
...
function LoginPanel({ active, onComplete }: LoginPanelProps) {
  ...
  const user = useDroraStore((s) => s.user);
  const oauthError = useDroraStore((s) => s.oauthError);
  const setOAuthError = useDroraStore((s) => s.setOAuthError);
  const oauthSuccessSeq = useDroraStore((s) => s.oauthSuccessSeq);
  const lastOAuthSuccessProvider = useDroraStore((s) => s.lastOAuthSuccessProvider);
  const loginEntryRequest = useDroraStore((s) => s.loginEntryRequest);
  const clearLoginEntryRequest = useDroraStore((s) => s.clearLoginEntryRequest);
  const markLoginEntryAttemptStatus = useDroraStore((s) => s.markLoginEntryAttemptStatus);
...
function LoginPanelLogo() {
  return (
    ...
    <div
      ...
      role="img"
    >
      <DroraAboutLogo className="h-auto w-10" />
    </div>
  );
}
