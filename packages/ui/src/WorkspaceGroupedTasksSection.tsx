...
import type { DroraGroupedTaskView, DroraTaskGroupColor } from "@drora/services";
import { OFF_PEAK_DEFAULT_GROUP_ID, type DroraTaskMeta } from "@drora/shared";
...
function getGroupedTaskViewSignature(view: DroraGroupedTaskView): string {
  return view.nodes
    .map((node) =>
      node.type === "task"
        ? `t:${taskKey(node.task)}`
...
export function WorkspaceGroupedTasksSection({
  ...
  const groupedDraftTask = useDroraSessionStore(
    (state) =>
      selectWorkspaceDroraState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .groupedDraftTask,
  );
  const groupedDraftFocusVersion = useDroraSessionStore(
    (state) =>
      selectWorkspaceDroraState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .draftFocusVersion,
  );
  const clearGroupedDraftTask = useDroraSessionStore((state) => state.clearGroupedDraftTask);
  const {
  ...
  const dragOriginViewRef = useRef<DroraGroupedTaskView | null>(null);
  const dragPreviewViewRef = useRef<DroraGroupedTaskView | null>(null);
  ...
  const handleStartRenameTask = useCallback((task: DroraTaskMeta) => {
    setRenamingTaskKey(taskKey(task));
    setRenameDraft(task.title ?? "");
  }, []);

  const handleMoveTaskToGroup = useCallback(
    (task: DroraTaskMeta, groupId: string | null) => {
  ...
  const handleSubmitRenameTask = useCallback(async () => {
    ...
    try {
      const meta = await workspaceServices.services.droraTaskService.renameTask({
        taskId: task.taskId,
  ...
  const handleMarkTaskAsUnread = useCallback(
    (task: DroraTaskMeta) => {
      ...
      void workspaceServices.services.droraTaskService
        .setTaskUnread({
          taskId: task.taskId,
          workspacePath: task.workspacePath,
          unread: true,
  ...
  const handleUpdateGroupColor = useCallback(
    (groupId: string, color: DroraTaskGroupColor) => {
