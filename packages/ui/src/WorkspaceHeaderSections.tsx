...
export function WorkspaceHeaderTitleSection({
  ...
  const buildHeaderTaskSnapshot = (
    fallbackTask: DroraTaskMeta,
    overrides?: Partial<DroraTaskMeta>,
  ): DroraTaskMeta => {
    if (activeTaskMeta) {
      return {
        ...activeTaskMeta,
  ...
  const handleSubmitRenameTask = async () => {
    ...
    try {
      const renamedTask = await services.droraTaskService.renameTask({
  ...
  const handleArchiveTask = async () => {
    ...
    void services.droraTaskService
      ...
      .then((meta) => {
        // Header 更多菜单不能只依赖 droraTaskMetaMerge：归档后只有旧列表状态被更新。
