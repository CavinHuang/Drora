...
import { prepareWorkspaceWithDroraSessionService } from "@/hooks/useWorkspacePrepare.js";
...
export function useWorkspaceSessionReload({
  ...
  const handleReloadSession = useCallback(
    async (options?: { resumeTaskId?: string | null; provider?: DroraProvider | null }) => {
      ...
      const latestWorkspaceState = droraSessionStore.getWorkspaceState(
        workspaceAbsPath,
        workspaceIdentity,
      );
      const actionScope = resolveWorkspaceModelConfigSyncScope(latestWorkspaceState);
      const provider: DroraProvider = options?.provider ?? actionScope.provider;
      const resumeTaskId =
      ...
      droraSessionStore.setWorkspaceInitState(
        workspaceAbsPath,
        "initializing",
        null,
        workspaceIdentity,
      ...
      if (shouldPrepareWorkspace) {
        droraSessionStore.setConfigOptionsStatus(workspaceAbsPath, "loading", workspaceIdentity);
        // 草稿态点击 reload 后若不清空旧错误，输入区会继续显示上一轮失败提示，
        // 用户会误判本次重建仍失败。这里在新一轮重建开始时先清空草稿错误。
        droraSessionStore.setDraftError(workspaceAbsPath, null, workspaceIdentity);
        droraSessionStore.setTaskState(workspaceAbsPath, "idle", null, workspaceIdentity);
      }

      try {
        await services.droraTaskService.restartWorkspaceProcess({
        ...
        if (shouldPrepareWorkspace) {
          const prepareResult = await prepareWorkspaceWithDroraSessionService({
            workspacePath: workspaceAbsPath,
            workspaceIdentity,
            provider,
            droraSessionService: services.droraSessionService,
          });

          const latestAfterPrepare = droraSessionStore.getWorkspaceState(
            workspaceAbsPath,
            workspaceIdentity,
          );
          if (latestAfterPrepare.selectedProvider === provider) {
            const latestAfterResolve = droraSessionStore.getWorkspaceState(
              workspaceAbsPath,
              workspaceIdentity,
            );
            if (latestAfterResolve.selectedProvider === provider) {
              droraSessionStore.setConfigOptions(
                workspaceAbsPath,
                prepareResult.configOptions ?? [],
                workspaceIdentity,
              );
              droraSessionStore.setConfigOptionsStatus(
                workspaceAbsPath,
                "ready",
                workspaceIdentity,
              );
              droraSessionStore.setSlashCommands(
                workspaceAbsPath,
        ...
        droraSessionStore.setWorkspaceInitAttempts(workspaceAbsPath, 0, workspaceIdentity);
        droraSessionStore.setWorkspaceInitState(workspaceAbsPath, "ready", null, workspaceIdentity);
        ...
        if (shouldPrepareWorkspace) {
          droraSessionStore.setConfigOptionsStatus(workspaceAbsPath, "error", workspaceIdentity);
          // 草稿态下 reload 前会先清空旧错误；如果失败后不回填 draftError，
          // 聊天区只剩 toast，用户看不到可重试的详细报错。这里统一回填标准化错误到输入区。
          droraSessionStore.setDraftError(workspaceAbsPath, reloadDraftError, workspaceIdentity);
        }
