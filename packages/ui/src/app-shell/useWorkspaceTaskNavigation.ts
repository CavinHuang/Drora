...
export function useWorkspaceTaskNavigation({
  ...
  const setActiveTaskId = useDroraSessionStore((s) => s.setActiveTaskId);
  const taskNavHistory = useDroraSessionStore((s) => s.taskNavHistory);
  const taskNavPushAutomations = useDroraSessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useDroraSessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useDroraSessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useDroraSessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useDroraSessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useDroraSessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.droraTaskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    const currentWorkspaceState = useDroraSessionStore
      .getState()
    ...
    while (entry) {
      ...
      const navWorkspaceState = useDroraSessionStore
        .getState()
  ...
  const currentWorkspaceState = useDroraSessionStore.getState().getWorkspaceState(workspaceAbsPath);
