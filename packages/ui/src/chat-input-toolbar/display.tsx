...
import { DRORA_MODE_OPTION_DESCRIPTION_IDS, DRORA_MODE_OPTION_LABEL_IDS } from "./display-help.js";
...
function isConfigSelectValueInOptions(option: DroraConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: DroraConfigOption): string | undefined {
...
function getConfigSelectItemTestId(
  option: DroraConfigOption,
  entry: DroraConfigSelectValue,
): string | undefined {
  if (option.category === "thought_level") {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 Drora Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<DroraConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: DroraProvider | undefined,
  entry: Pick<DroraConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return DRORA_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return DRORA_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
