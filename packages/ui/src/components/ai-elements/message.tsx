 ...
 * Modified by Drora: local integration, formatting and adaptations.
...
import type { EditorInfo, FileStat, OpenInEditorOptions } from "@drora/shared";
...
import { createDroraFileCitationRemarkPlugin } from "@/lib/droraFileCitationRemarkPlugin.js";
import { windowsFileLinkEscapeRemarkPlugin } from "@/lib/windowsFileLinkEscapeRemarkPlugin.js";
import { projectDroraFileCitations } from "@/lib/droraFileCitation.js";
import { rewriteMarkdownArtifactImageSources } from "@drora/shared";
...
export type MessageResponseProps = {
  ...
  /** 仅 Assistant 正文开启：把完整 drora-file-citation 投影为现有文件链接。 */
  renderDroraFileCitations?: boolean;
};
...
export function buildMessageStreamdownRenderKey(params: {
  ...
  return [
    ...
    params.renderDroraFileCitations ? "citations" : "plain",
...
export const messageResponsePropsAreEqual = (
  ...
  nextProps.renderDroraFileCitations === prevProps.renderDroraFileCitations &&
...
export const MessageResponse = memo(
  ({
    ...
    onOpenFileLink,
    onOpenExternalUrl,
    renderDroraFileCitations = false,
    workspacePath,
    workspaceHomePath,
    workspaceIdentity,
    ...
    const projectedCitationMarkdown = useMemo(
      () =>
        renderDroraFileCitations
          ? projectDroraFileCitations(rawMarkdown, { streaming: renderStreaming }).visibleText
          : rawMarkdown,
      [rawMarkdown, renderStreaming, renderDroraFileCitations],
    );
    ...
    const messageRemarkPlugins = useMemo<PluggableList>(
      () => [
        ...
        ...(renderDroraFileCitations && workspacePath
          ? [createDroraFileCitationRemarkPlugin(workspacePath, workspaceHomePath)]
          : []),
      ],
      [renderDroraFileCitations, workspaceHomePath, workspacePath],
    );
    ...
    const boundaryResetKey = useMemo(
      () =>
        renderStreaming
          ...
          : `${streamdownMode}:${renderDroraFileCitations ? "citations" : "plain"}:${hashMarkdownCacheKey(targetMarkdown)}`,
      [renderStreaming, renderDroraFileCitations, streamdownMode, targetMarkdown],
    );
