/* eslint-disable max-lines -- Proma LiveMarkdownEditor 整文件保真移植：ink-mde 宿主生命周期、受控值、查找状态层与 Live Preview 装饰共用同一组 CodeMirror 扩展闭包，拆分会打断状态共享；沿用 BotsDialog/VaultView 的整文件豁免先例。 */
...
} from "./LiveMarkdownPreview.js";
import {
  shouldRebuildMarkdownHeadingDecorations,
  shouldRebuildMarkdownSyntaxDecorations,
} from "./live-markdown-lifecycle.js";
export type { ChangeLiveMarkdownProperties } from "./LiveMarkdownPreview.js";
export type { LiveMarkdownPropertyEntry } from "./live-markdown-frontmatter.js";
import type { LiveMarkdownPropertyEntry } from "./live-markdown-frontmatter.js";

export type { LiveMarkdownFindOptions } from "./LiveMarkdownPreview.js";
...
export interface LiveMarkdownEditorHandle {
  ...
  subscribeToFindUpdates: (
    listener: (update: { matchCount: number; activeIndex: number }) => void,
  ) => () => void;
...
function buildLiveMarkdownFindDecorations(
  matches: readonly LiveMarkdownFindMatch[],
  activeIndex: number,
): DecorationSet {
  ...
  matches.forEach((match, index) => {
    builder.add(
      ...
      Decoration.mark({
        class:
          index === activeIndex
            ? "live-markdown-find-match live-markdown-find-match-active"
            : "live-markdown-find-match",
      }),
    );
  });
...
const liveMarkdownFindMatchesField = StateField.define<LiveMarkdownFindState>({
  ...
  update: (value, transaction) => {
    ...
    if (effect) {
      return {
        decorations: buildLiveMarkdownFindDecorations(
...
function markdownSyntaxDecorations(state: EditorState, focused: boolean): DecorationSet {
  ...
  syntaxTree(state).iterate({
    enter: ({ type, from, to }) => {
      ...
      const markerEnd =
        type.name === "HeaderMark" && state.doc.sliceString(to, to + 1) === " " ? to + 1 : to;
 ...
 */
export const LiveMarkdownEditor = React.forwardRef<
...
>(function LiveMarkdownEditor(
  ...
  const findUpdateListenersRef = React.useRef(
    new Set<(update: { matchCount: number; activeIndex: number }) => void>(),
  );
  ...
  const onChangePropertiesProxy = React.useCallback(
  ...
  const applyFindMatches = React.useCallback(
    (
      query: string,
      options: LiveMarkdownFindOptions,
      activeIndex: number,
      ...
      const nextActiveIndex =
        matches.length === 0 ? -1 : Math.max(0, Math.min(activeIndex, matches.length - 1));
      ...
      const findEffect = liveMarkdownFindMatchesEffect.of({
        matches,
        activeIndex: nextActiveIndex,
      });
      view.dispatch({
        effects:
          scrollIntoView && nextActiveIndex >= 0
            ? [
                findEffect,
                EditorView.scrollIntoView(matches[nextActiveIndex]!.from, {
                  y: "center",
                  yMargin: 24,
                }),
              ]
      ...
      findUpdateListenersRef.current.forEach((listener) =>
        listener({ matchCount: matches.length, activeIndex: nextActiveIndex }),
      );
  ...
  React.useImperativeHandle(
    ...
    () => ({
      ...
      scrollToPosition: (position) => {
        ...
        view.dispatch({
          effects: EditorView.scrollIntoView(safePosition, { y: "start", yMargin: 8 }),
        });
      },
      setFindMatches: (query, options, activeIndex) =>
        applyFindMatches(query, options, activeIndex, true),
      ...
      clearFindMatches: () => {
        ...
        view.dispatch({
          effects: liveMarkdownFindMatchesEffect.of({ matches: [], activeIndex: -1 }),
        });
        findUpdateListenersRef.current.forEach((listener) =>
          listener({ matchCount: 0, activeIndex: -1 }),
        );
      },
          ...
          ViewPlugin.define((view) => {
            ...
            return {
              update: (update) => {
                ...
                const isPointerSelection = update.transactions.some((transaction) =>
  ...
  return (
    <div
      ref={hostRef}
      className={cn("live-markdown-editor vault-ink-mde h-full min-h-0", className)}
    />
  );
});
