/* eslint-disable max-lines -- Proma LiveMarkdownTableEditor 整文件保真移植：可编辑 GFM 表格的单元格焦点/草稿/右键结构操作共用一套提交闭包，拆分会打断 blur 与 CodeMirror dispatch 的时序契约；沿用 BotsDialog/VaultView 的整文件豁免先例。 */
...
import {
  liveMarkdownTableCellDraft,
  renderLiveMarkdownTableInline,
} from "./live-markdown-table-inline.js";
import {
...
import type {
  LiveMarkdownFindController,
  LiveMarkdownFindOptions,
  LiveMarkdownFindState,
...
export function doesLiveMarkdownTableCellMatch(
  value: string,
  query: string,
...
export function findLiveMarkdownTableCellSourceRange(
  source: string | undefined,
  sourceRange: { from: number; to: number } | undefined,
  ...
  const lineOffset =
    sourceRange.from +
    lines.slice(0, lineIndex).reduce((offset, current) => offset + current.length + 1, 0);
...
export function LiveMarkdownTableEditor({
  ...
  const [findState, setFindState] = useState<LiveMarkdownFindState>(
    () => findController?.getState() ?? EMPTY_FIND_STATE,
  );
  const [draft, setDraft] = useState(() =>
    autoFocusCell ? liveMarkdownTableCellDraft(cellValue(table, autoFocusCell)) : "",
  );
  ...
  const dispatchTableCommit = (nextTable: LiveMarkdownTable, focusCell?: LiveMarkdownTableCell) => {
    ...
    try {
      ...
      queueMicrotask(() => {
        isDispatchingTableChangeRef.current = false;
      });
    }
  };
  ...
  const activate = (cell: LiveMarkdownTableCell, element: HTMLButtonElement) => {
    ...
    if (activeCell) {
      ...
      if (shouldCommitLiveMarkdownTableCell(original, draft)) {
        dispatchTableCommit(
          updateLiveMarkdownTableCell(table, activeCell.row, activeCell.column, draft),
          cell,
        );
        return;
      }
  ...
  const nextCell = (from: LiveMarkdownTableCell, backwards: boolean): LiveMarkdownTableCell =>
    nextLiveMarkdownTableCell(table, from, backwards);
    ...
    const cellContent = (
      <Cell className={className}>
        {isActive ? (
          ...
          <span
            className="live-markdown-table-value"
            dangerouslySetInnerHTML={{ __html: renderLiveMarkdownTableInline(value) }}
          />
        ) : (
          <button
            type="button"
        ...
        <ContextMenuContent className="z-[9999] min-w-48 p-0.5">
          ...
          <ContextMenuItem
            onSelect={() =>
              applyTableChange(
                (currentTable) =>
                  insertLiveMarkdownTableRow(currentTable, header ? 0 : bodyRowIndex + 1),
  ...
  return (
    <div className="vault-markdown-table live-markdown-table-editor">
      {!readOnly && (
        <div className="live-markdown-table-editor-hint" aria-hidden="true">
          点击编辑、右键操作更多
        </div>
      )}
      <table aria-label="Markdown 表格">
        <thead>
          <tr>{table.header.map((_, column) => renderCell(0, column, true))}</tr>
        </thead>
        {table.rows.length > 0 && (
          <tbody>
            {table.rows.map((_, row) => (
              <tr key={row}>
                {table.header.map((_, column) => renderCell(row + 1, column, false))}
              </tr>
            ))}
          </tbody>
        )}
