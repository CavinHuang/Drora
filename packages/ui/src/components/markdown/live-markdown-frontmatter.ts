 ...
 */
export function getLeadingFrontmatterRange(
  lines: readonly string[],
): LiveMarkdownFrontmatterRange | null {
...
/**
 * Parse the conservative subset of YAML frontmatter that Drora can safely edit
 * without reserializing unrelated YAML syntax. Nested values remain source text.
 */
export function parseLeadingFrontmatter(
  lines: readonly string[],
): LiveMarkdownPropertyEntry[] | null {
...
export function getLeadingFrontmatter(
  lines: readonly string[],
): { endLine: number; entries: LiveMarkdownPropertyEntry[] } | null {
 ...
 */
export function serializeFlatLeadingFrontmatter(
  documentValue: string,
  entries: readonly LiveMarkdownPropertyEntry[],
): string {
  const newline = documentValue.includes("\r\n") ? "\r\n" : "\n";
  const lines = documentValue.replace(/\r\n?/g, "\n").split("\n");
  const frontmatter = getLeadingFrontmatter(lines);
  if (!frontmatter || !entries.every((entry) => isSafeFlatEntry(entry.key, entry.value)))
