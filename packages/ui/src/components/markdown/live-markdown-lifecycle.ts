...
export function shouldRebuildMarkdownHeadingDecorations(
  change: MarkdownHeadingDecorationChange,
): boolean {
  return change.documentChanged || change.syntaxTreeChanged;
}

/** Live Preview 标记同时受文档、选区、焦点与后台语法树影响。 */
export function shouldRebuildMarkdownSyntaxDecorations(
  change: MarkdownSyntaxDecorationChange,
): boolean {
  return (
