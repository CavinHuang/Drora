...
export function findInlineLiveMarkdownPreviews(line: string): InlineLiveMarkdownPreview[] {
  ...
  const mathPatterns = [
    {
      pattern: /(^|[^\\])\$([^$\n]+)\$/g,
      contentIndex: 2,
      markerOffset: (match: RegExpExecArray) => match[1]!.length,
    },
  ...
  return previews
    ...
    .filter(
      (preview, index, sorted) =>
        !sorted
          .slice(0, index)
          .some((previous) => previous.from <= preview.from && previous.to >= preview.to),
    );
}
