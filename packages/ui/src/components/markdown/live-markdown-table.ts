...
export function parseLiveMarkdownTable(source: string): LiveMarkdownTable | null {
  ...
  if (lines.length < 2 || !lines[0]?.includes("|") || !isLiveMarkdownTableSeparator(lines[1] ?? ""))
    return null;
...
export function serializeLiveMarkdownTable(table: LiveMarkdownTable): string {
  ...
  const separator = Array.from({ length: width }, (_, index) =>
    serializeSeparator(table.alignments[index] ?? null),
  );
...
export function isLikelyLiveMarkdownLatex(value: string): boolean {
  ...
  if (
    /\\(?:frac|dfrac|tfrac|sqrt|sum|prod|int|oint|lim|sin|cos|tan|log|ln|cdot|times|div|pm|mp|leq|geq|neq|approx|equiv|infty|partial|nabla|alpha|beta|gamma|delta|theta|lambda|mu|pi|sigma|omega|to|Rightarrow|Leftarrow|Leftrightarrow|cup|cap|setminus|emptyset|subset|subseteq|supset|supseteq|in|notin|forall|exists|land|lor|not|bar|hat|vec|dot|ddot|left|right|quad|qquad)\b/.test(
      trimmed,
    )
  )
    return true;
...
export function insertLiveMarkdownTableRow(
  table: LiveMarkdownTable,
  bodyInsertIndex: number,
): LiveMarkdownTable {
  if (
    !Number.isInteger(bodyInsertIndex) ||
    bodyInsertIndex < 0 ||
    bodyInsertIndex > table.rows.length
  )
    return table;
...
export function deleteLiveMarkdownTableRow(
  table: LiveMarkdownTable,
  bodyRowIndex: number,
): LiveMarkdownTable {
  if (!Number.isInteger(bodyRowIndex) || bodyRowIndex < 0 || bodyRowIndex >= table.rows.length)
    return table;
  return {
    ...table,
    rows: table.rows.filter((_, index) => index !== bodyRowIndex).map((row) => [...row]),
  };
}

/** 在指定列索引前插入一列，并将所有行同步保持为矩形。 */
export function insertLiveMarkdownTableColumn(
  table: LiveMarkdownTable,
  columnIndex: number,
): LiveMarkdownTable {
  if (!Number.isInteger(columnIndex) || columnIndex < 0 || columnIndex > table.header.length)
    return table;
  const header = [...table.header];
...
export function deleteLiveMarkdownTableColumn(
  table: LiveMarkdownTable,
  columnIndex: number,
): LiveMarkdownTable {
  if (
    ...
    columnIndex < 0 ||
    columnIndex >= table.header.length ||
