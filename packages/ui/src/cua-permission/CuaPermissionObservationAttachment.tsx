...
import type { CuaAccessibilitySettingsResult, CuaPermissionKind } from "@drora/shared";
import { requiredCuaPermissionsForRequestAccessStatus } from "@drora/shared/drora-protocol-v4";
import {
  isCuaPermissionStatusAvailable,
  type CuaPermissionRestartOptions,
  type CuaPermissionStatusResult,
  type DroraAgentCuaPermissionObservation,
} from "@drora/services";
...
function cuaPermissionObservationKey(observation: DroraAgentCuaPermissionObservation): string {
...
export function CuaPermissionObservationAttachment() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      typeof services.droraAgentService.onDynamicCuaPermissionObservation !== "function"
    ) {
      return;
    }
    ...
    let disposed = false;
    const subscription = services.droraAgentService.onDynamicCuaPermissionObservation()(
      (observation) => {
        const missing = requiredCuaPermissionsForRequestAccessStatus(observation.permissionStatus);
