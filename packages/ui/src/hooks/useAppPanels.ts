...
import { createUuid } from "@drora/shared";
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@drora/shared";
...
export function useAppPanels(options: {
  ...
  const { droraAgentService, droraSessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await droraAgentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,
