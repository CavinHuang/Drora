...
import type { DroraImportSessionsResult, DroraImportableSessionCandidate } from "@drora/shared";
import { logger } from "@/logger.js";
import { useDroraTaskService } from "@/hooks/useDroraTaskService.js";
...
export function useClaudeSessionMigration(params: {
  ...
  const droraTaskService = useDroraTaskService(
  ...
  const [candidates, setCandidates] = useState<DroraImportableSessionCandidate[]>([]);
  const [selectedSessionIds, setSelectedSessionIds] = useState<string[]>([]);
  const [scanError, setScanError] = useState<string | null>(null);
  const [importError, setImportError] = useState<string | null>(null);
  const [lastImportResult, setLastImportResult] = useState<DroraImportSessionsResult | null>(null);
  ...
  const scan = useCallback(async () => {
    ...
    try {
      ...
      const nextCandidates = await droraTaskService.scanImportableClaudeSessions({
  ...
  const importSessions = useCallback(
    async (sessionIds: string[]) => {
      ...
      try {
        ...
        const result = await droraTaskService.importClaudeSessions({
          workspacePath: effectiveWorkspacePath,
          ...(effectiveWorkspaceIdentity ? { workspaceIdentity: effectiveWorkspaceIdentity } : {}),
          sessionIds,
        });
