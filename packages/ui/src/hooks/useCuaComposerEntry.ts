...
import { isRemoteWorkspaceIdentity, DRORA_CUA_OFFICIAL_PLUGIN_ID } from "@drora/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/droraSessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 droraSessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 drora-cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useDroraSessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === DRORA_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === DRORA_CUA_OFFICIAL_PLUGIN_ID,
