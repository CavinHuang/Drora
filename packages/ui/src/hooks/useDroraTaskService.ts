...
import type { IDroraTaskService } from "@drora/services";
import type { DroraTaskSnapshot } from "@drora/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<IDroraTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<DroraTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IDroraTaskService["getTaskSnapshotWithEtag"]>[0];

const droraTaskServiceProxyCache = new WeakMap<IDroraTaskService, IDroraTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IDroraTaskService,
  Map<string, { etag: string; snapshot: DroraTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "drora-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: DroraTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: DroraTaskSnapshot }>();
...
function writePersistedSnapshotEntry(key: string, etag: string, snapshot: DroraTaskSnapshot): void {
  ensurePersistedSnapshotCacheLoaded();
...
function createDroraTaskServiceProxy(service: IDroraTaskService): IDroraTaskService {
...
/** 获取 Drora task wrapper 服务实例 */
export function useDroraTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): IDroraTaskService {
  // Drora task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.droraTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = droraTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createDroraTaskServiceProxy(rawService);
  droraTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
