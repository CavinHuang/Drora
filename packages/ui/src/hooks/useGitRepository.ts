...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, DroraPersistedFileSnapshot>,
  file: DroraTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: DroraPersistedFileChange | null;
  summary: DroraTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, DroraPersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 Drora Agent 已持久化的单轮文件快照，
