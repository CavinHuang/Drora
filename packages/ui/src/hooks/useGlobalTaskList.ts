...
function buildWorkspaceScopes(workspaceTabs: WorkspaceTabState[]): DroraTaskListWorkspaceScope[] {
  const scopes = new Map<string, DroraTaskListWorkspaceScope>();
...
export function useGlobalTaskList(params: {
  ...
  const taskListVersionSignature = useDroraSessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs
        .map((tab) => {
          const workspace = selectWorkspaceDroraState(
            state,
            tab.workspacePath,
