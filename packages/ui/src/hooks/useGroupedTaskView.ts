...
function collectViewWorkspaceScopes(
  ...
  const addTask = (task: DroraTaskMeta) => {
...
function mergeGroupedTaskViewWithOptimistic(params: {
  ...
  const optimisticTaskByKey = new Map<string, DroraTaskMeta>();
  const placementTaskByKey = new Map<string, DroraTaskMeta>();
  ...
  const groupMissingTasksByGroupId = new Map<string, DroraTaskMeta[]>();
  const topMissingTasks: DroraTaskMeta[] = [];
...
function collectGroupedViewTaskKeys(view: DroraGroupedTaskView): Set<string> {
...
function groupedNodeIdentityKey(node: DroraGroupedTaskView["nodes"][number]): string {
  if (node.type === "group") {
...
function areGroupedNodesEquivalent(
  previous: DroraGroupedTaskView["nodes"][number],
  next: DroraGroupedTaskView["nodes"][number],
): boolean {
...
function stabilizeGroupedView(
  previous: DroraGroupedTaskView,
  next: DroraGroupedTaskView,
): DroraGroupedTaskView {
...
function nodeToTopLevelRef(
  node: DroraGroupedTaskView["nodes"][number],
): DroraGroupedTaskViewTopLevelNodeRef {
  if (node.type === "group") {
...
function viewToOrderInput(params: { view: DroraGroupedTaskView }): DroraGroupedTaskViewOrderInput {
  return {
...
const groupedViewCacheBySignature = new Map<string, DroraGroupedTaskView>();

function readCachedGroupedView(signature: string): DroraGroupedTaskView | undefined {
  return groupedViewCacheBySignature.get(signature);
}

function writeCachedGroupedView(signature: string, view: DroraGroupedTaskView): void {
...
export function useGroupedTaskView(params: { workspaceTabs: WorkspaceTabState[] }) {
  ...
  const [view, setView] = useState<DroraGroupedTaskView>(
  ...
  const taskListVersionSignature = useDroraSessionStore((state) =>
    JSON.stringify(
  ...
  const clearPromotedGroupedDraftTask = useDroraSessionStore(
  ...
  const displayedViewRef = useRef<DroraGroupedTaskView>({ nodes: [] });
  ...
  const [remoteDataLoader] = useState(
    () =>
      new GroupedRemoteDataSingleFlight<{
        structure: DroraGroupedTaskViewStructure;
    ...
    try {
      ...
      const remoteData = await remoteDataLoader.load(remoteDataKey, async () => {
        const [structureResult, membershipResult] = await Promise.all([
          services.droraTaskService.listGroupedTaskViewStructure({
            workspaceScopes: scopes,
          }),
          fetchTaskListMembershipSets({
            service: services.droraTaskService,
  ...
  useEffect(() => {
    ...
    const promotedGroupTasks: DroraTaskMeta[] = [];
    const settledRootTasks: DroraTaskMeta[] = [];
    ...
    void services.droraTaskService
  ...
  useEffect(() => {
    const disposables = scopes.map((scope) =>
      services.droraTaskService.onDynamicWorkspaceEvent(scope)((event) => {
  ...
  }, [invalidateRemoteData, scopes, services.droraTaskService]);
  ...
  const createGroup = useCallback(async (): Promise<DroraTaskGroup> => {
    setSaving(true);
    try {
      const group = await services.droraTaskService.createTaskGroup();
  ...
  }, [invalidateRemoteData, refresh, services.droraTaskService]);
  ...
  const renameGroup = useCallback(
    async (groupId: string, title: string) => {
      ...
      const optimisticView: DroraGroupedTaskView = {
      ...
      setSaving(true);
      try {
        const renamedGroup = await services.droraTaskService.renameTaskGroup({
          groupId,
          title: nextTitle,
    ...
    [invalidateRemoteData, scopes, services.droraTaskService, view],
  );

  const updateGroupColor = useCallback(
    async (groupId: string, color: DroraTaskGroupColor) => {
      const previousView = view;
      const groupNode = view.nodes.find(
        (node) => node.type === "group" && node.group.id === groupId,
      );
      ...
      setSaving(true);
      try {
        const updatedGroup = await services.droraTaskService.updateTaskGroupColor({
          groupId,
          color,
          workspaceScopes: collectViewWorkspaceScopes(optimisticView),
        });
  ...
  const applyOrder = useCallback(
    async (nextView: DroraGroupedTaskView) => {
      const previousView = view;
      setView(nextView);
      setSaving(true);
      try {
        ...
        await services.droraTaskService.applyGroupedTaskViewOrder(
          viewToOrderInput({
            view: nextView,
          }),
        );
    ...
    [invalidateRemoteData, refresh, services.droraTaskService, view],
  );
  ...
  const ungroupGroup = useCallback(
    async (groupId: string) => {
      ...
      const nextView: DroraGroupedTaskView = {
      ...
      try {
        ...
        await services.droraTaskService.deleteTaskGroup({
          groupId,
    ...
    [applyOrder, invalidateRemoteData, refresh, scopes, services.droraTaskService, view],
  );
