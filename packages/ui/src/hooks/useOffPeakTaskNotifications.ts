...
import type { DroraOffPeakTaskStatus } from "@drora/shared";
import type { IOffPeakTaskService } from "@drora/services";
...
function notifiableStatus(status: DroraOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, DroraOffPeakTaskStatus>());
