...
interface PluginReferenceCatalogState {
  entries: DroraPluginReferenceCatalogEntry[];
...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<DroraPluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<DroraPluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<DroraPluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    const subscription = services.droraAgentService.onAgentRuntimeRestarted((event) => {
  ...
  useEffect(() => {
    ...
    let request: Promise<DroraPluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<DroraPluginsReferenceCatalogResult>> | undefined;
