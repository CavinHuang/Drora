...
import type { DroraSessionEndedSubagent, DroraSessionSubagentsResult } from "@drora/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: DroraSessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly DroraSessionEndedSubagent[],
  incoming: readonly DroraSessionEndedSubagent[],
): DroraSessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const droraAgentService = services.droraAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<DroraSessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof droraAgentService?.listSessionSubagents !== "function") {
      ...
      return droraAgentService.listSessionSubagents({
