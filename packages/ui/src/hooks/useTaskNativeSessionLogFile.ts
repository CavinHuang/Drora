...
function supportsTaskNativeSessionLogFile(_provider: DroraProvider | null | undefined): boolean {
 ...
 *
 * 路径规则统一通过 droraTaskService 解析，避免 UI 层猜 provider 自己的目录结构。
 */
export function useTaskNativeSessionLogFile(
  workspacePath: string,
  taskId: string | null,
  providerHint?: DroraProvider | null,
  workspaceIdentity?: string,
  options: { enabled?: boolean } = {},
) {
  const droraTaskService = useDroraTaskService(workspacePath, undefined, workspaceIdentity);
  ...
  }, [enabled, droraTaskService, providerHint, taskId, workspaceIdentity, workspacePath]);
