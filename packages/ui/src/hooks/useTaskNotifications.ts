...
import type { IPlatformService, TaskNotificationPayload } from "@drora/shared";
import type { ConversationSnapshot, SessionSummary } from "@drora/shared/drora-protocol-v4";
...
export function useWorkspaceTerminalTaskNotifications({
  ...
  useEffect(() => {
    ...
    const store = acquireSessionsIndex(registryScope, droraAgentService);
    const syncState = () => {
      setIndexState({
        signature,
