...
function areResolvedTaskMetasEqual(left: DroraTaskMeta | null, right: DroraTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: DroraTaskMeta | null) {
  const stableTaskMetaRef = useRef<DroraTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useDroraSessionStore((state) =>
    selectWorkspaceDroraState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 droraTaskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task
