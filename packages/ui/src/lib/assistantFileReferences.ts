...
import { extractConversationPreviewFileReferences } from "@drora/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@drora/shared";
import {
  extractDroraFileCitationDirectives,
  resolveDroraFileCitationPreviewKind,
} from "@/lib/droraFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractDroraFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 drora-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
