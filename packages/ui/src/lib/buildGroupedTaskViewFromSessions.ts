...
// sessions-index 只补实时 activity/detail，最终拼出旧 DroraGroupedTaskView 同形结果。
...
function taskKeyOf(task: DroraTaskMeta): string {
...
function taskOrderMapKey(task: DroraTaskMeta): string {
...
function nodeMapKey(node: DroraGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, DroraGroupedTaskViewStructureMember>,
): DroraTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: DroraTaskListItem; addedAt: number; key: string }> = [];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, DroraTaskListItem>();
  ...
  const membersByGroupId = new Map<string, DroraGroupedTaskViewStructureMember[]>();
  ...
  const nodes: DroraGroupedTaskViewNode[] = structure.groups.map((group) => {
