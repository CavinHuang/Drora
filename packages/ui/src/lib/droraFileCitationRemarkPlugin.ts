...
import { extractDroraFileCitations } from "@/lib/droraFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractDroraFileCitations(value);
...
export function createDroraFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function droraFileCitationRemarkPlugin() {
    return (tree: unknown) => {
