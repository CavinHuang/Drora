/* oxlint-disable eslint(max-lines) -- Drora session 到当前聊天 projection 的迁移桥需要同时保持 snapshot 和 event 映射一致。 */
import {
...
const DRORA_AGENT_MODE_OPTIONS = [
  {
    id: "build",
...
] as const satisfies readonly DroraTaskModeInfo[];
const DRORA_AGENT_MODE_ID_SET = new Set<string>(DRORA_AGENT_MODE_OPTIONS.map((mode) => mode.id));
...
export function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    ...
    // Drora Protocol 必须收到严格的 providerId/modelId 结构。
    return {
...
export function droraSessionSettingsToConfigOptions(
  settings: DroraSessionSettingsState,
): DroraConfigOption[] {
  const configOptions: DroraConfigOption[] = [
    {
      id: MODEL_CONFIG_ID,
      name: "Model",
      category: "model",
    ...
    },
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableDroraMode(settings.mode.current),
      options: getDroraAgentModeSelectOptions(),
    },
  ];
...
export function droraWorkspacePresentationToConfigOptions(
  ...
  return [
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableDroraMode(mode),
      options: getDroraAgentModeSelectOptions(),
    },
  ];
}
...
function resolveSettingsThoughtLevelCurrentValue(
  thoughtLevel: DroraSessionSettingsState["thoughtLevel"],
): string | undefined {
  ...
  const defaultThoughtLevel =
    thoughtLevel.defaultLevel && thoughtLevelValues.has(thoughtLevel.defaultLevel)
      ...
      : undefined;
  // Drora Protocol 的 defaultLevel 是模型事实，current 为空时表示用户尚未显式修改。
...
export function droraSessionSnapshotToTaskMeta(snapshot: DroraSessionStateSnapshot): DroraTaskMeta {
  return {
...
function normalizeAvailableDroraMode(mode: DroraSessionMode): string {
  return DRORA_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

function getDroraAgentModeSelectOptions(): NonNullable<DroraConfigOption["options"]> {
  return DRORA_AGENT_MODE_OPTIONS.map((mode) => ({
    value: mode.id,
    name: mode.name,
