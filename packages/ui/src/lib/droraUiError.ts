import type { DroraError, TraceId } from "@drora/shared";
import { errorAttributionSchema, type ErrorAttribution } from "@drora/shared/drora-protocol-v4";

export interface DroraUiError extends DroraError {
...
interface NormalizeDroraUiErrorOptions {
  fallbackCode?: string;
...
const GENERIC_DRORA_UI_ERROR_MESSAGES = new Set([
  "Internal error",
  "Turn execution failed",
  "Compact failed",
  "Rewind failed",
  "Drora session failed",
]);
...
function collectMessageCandidatesFromRecord(record: Record<string, unknown>): string[] {
  ...
  const messagePaths: Array<readonly string[]> = [
    ...
    ["data", "message"],
    ["data", "detail"],
    // Drora Agent 常把可读原因放在 data.details（复数）里；之前只识别 detail，
    ...
    ["data", "error", "detail"],
    ["data", "error", "details"],
    // drora-cli 会把模型/网络错误摘要放在 data.drora.error 下。
    // 之前 UI 只读 data.error，导致已经结构化好的 provider 根因仍被 “Internal error” 盖住。
    ["data", "drora", "error", "message"],
    ["data", "drora", "error", "detail"],
    ["data", "drora", "error", "details"],
  ];
...
export function normalizeDroraUiError(
  error: unknown,
  options: NormalizeDroraUiErrorOptions = {},
): DroraUiError {
  const candidates = collectMessageCandidates(error);
  // drora-cli 已经把 provider/network 根因放进 detail 或 data.drora.error，
  // 外层仍可能保留 "Internal error" 这类包装文案。主提示优先选非泛化候选，避免根因被盖住。
  const primaryMessage =
    candidates.find((candidate) => !GENERIC_DRORA_UI_ERROR_MESSAGES.has(candidate)) ??
    candidates[0] ??
    options.fallbackMessage ??
    "Internal error";
  const detailMessage = candidates.find(
    (candidate) => candidate !== primaryMessage && !GENERIC_DRORA_UI_ERROR_MESSAGES.has(candidate),
  );
  ...
  const underlyingErrorMessage = readFirstStringFromPaths(error, [
    ...
    ["data", "drora", "error", "underlyingErrorMessage"],
  ]);
  const underlyingErrorDetail = readFirstStringFromPaths(error, [
    ...
    ["data", "drora", "error", "underlyingErrorDetail"],
  ]);
