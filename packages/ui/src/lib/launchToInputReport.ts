import type { LaunchMarks } from "@drora/shared";
...
export function readRendererLaunchTimings(): {
  ...
  const w = window as Window & {
    ...
    __DRORA_REACT_COMMIT_AT__?: number;
  };
  const rendererStart = w.__DRORA_RENDERER_START__;
  const reactCommit = w.__DRORA_REACT_COMMIT_AT__;
  if (typeof rendererStart !== "number" || typeof reactCommit !== "number") {
    return null;
  }
  return { marks: w.__DRORA_LAUNCH_MARKS__ ?? null, rendererStart, reactCommit };
}
