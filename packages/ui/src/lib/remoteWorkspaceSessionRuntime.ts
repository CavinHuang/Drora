import type { DroraTaskRuntimeStatus } from "@drora/shared";
import { getWorkspaceDisplayedTaskState } from "@/store/droraSessionStore.js";
import type { DroraSessionStoreState, WorkspaceDroraUIState } from "@/store/droraSessionStore.js";
...
interface MarkRemoteWorkspaceRunningTasksFailedParams {
  ...
  getWorkspaceState: DroraSessionStoreState["getWorkspaceState"];
  setTaskRuntimeState: DroraSessionStoreState["setTaskRuntimeState"];
  reason: string;
}

function isRunningRuntimeStatus(status: DroraTaskRuntimeStatus): boolean {
...
function collectRemoteWorkspaceRunningTaskIds(workspaceState: WorkspaceDroraUIState): string[] {
