...
function resolveSkillSourceType(skillPath: string): SkillSourceType {
  ...
  if (normalized.includes("/.drora/skills/")) {
    return "glm";
  }
  if (normalized.includes("/.drora/cli/plugins/cache/")) {
    return "glm";
  }
  return "unknown";
}
...
function isDroraSkill(skill: { id?: string; path: string; scope?: string }): boolean {
  return (
    // plugin skill 的真实路径在 CLI plugin cache 下，不在 `.drora/skills`。
...
export function filterSkillsForProvider<T extends { path: string; id?: string; scope?: string }>(
  skills: T[],
  _legacyProvider: DroraProvider,
): T[] {
  return skills.filter(isDroraSkill);
}
