import type {
  ...
  DroraTaskChangedFileSummary,
  DroraTaskMeta,
} from "@drora/shared";
import { computeLineChangeStat } from "@drora/shared";
...
export function getTaskChangeSummary(
  task: Pick<DroraTaskMeta, "changeSummary"> | null | undefined,
): DroraTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: DroraTaskChangeSummary,
...
export function buildTaskChangeSummary(
  fileChanges: readonly DroraPersistedFileChange[] | undefined,
): DroraTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: DroraTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: DroraPersistedFileChange | null | undefined,
): DroraTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: DroraTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly DroraPersistedFileChange[] | undefined,
): Map<number, DroraTaskChangeSummary> {
  const result = new Map<number, DroraTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }
