// re-home 产物（为删除 droraChatMessages 铺路，模式同 drora-task-types-core）。
// 本文件承载旧消息转换层（droraChatMessages/droraChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（droraChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  DroraAssistantMessagePart,
  DroraAssistantMessageFeedback,
  DroraPromptAttachment,
  DroraTimelineMeta,
  DroraTaskSnapshotBodyRef,
  DroraTaskSnapshotToolFieldRef,
  DroraTaskSnapshotToolSlice,
} from "@drora/shared";
...
export interface TaskChatToolCall {
  ...
  /** Drora 固定工具名；kind 仍保留给旧 Drora Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = DroraAssistantMessagePart;
