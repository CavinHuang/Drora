...
> &
  Partial<Pick<IDroraTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<DroraTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: DroraTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: DroraTaskMeta[][]): DroraTaskMeta[] {
  const itemByEntityKey = new Map<string, DroraTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, DroraTaskMeta>();
