import type { DroraWorkspaceTaskListChanged } from "@drora/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<DroraWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<DroraWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<DroraWorkspaceTaskListChanged, "reason">,
): boolean {
