import type { DroraConfigOption, DroraTaskMeta } from "@drora/shared";
import { getDroraAgentModeSelectOptions } from "@drora/shared";
...
function resolveRecoveredTaskModelValue(
  taskMeta: Pick<DroraTaskMeta, "provider" | "model">,
): string | null {
...
function ensureModelOptionValue(option: DroraConfigOption, modelValue: string): DroraConfigOption {
...
function createRecoveredModelOption(modelValue: string): DroraConfigOption {
  return {
    category: "model",
    currentValue: modelValue,
    id: "model",
...
function createRecoveredModeOption(modeValue: string): DroraConfigOption {
  return {
    category: "mode",
...
function createRecoveredThoughtLevelOption(thoughtLevel: string): DroraConfigOption {
  return {
...
export function resolveTaskRestorePreloadConfigOptions({
  ...
  taskMeta: Pick<DroraTaskMeta, "provider" | "model"> &
    Partial<Pick<DroraTaskMeta, "mode" | "thoughtLevel">>;
  cachedTaskConfigOptions?: readonly DroraConfigOption[];
}): DroraConfigOption[] {
