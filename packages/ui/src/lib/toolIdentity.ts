import {
  getDroraToolFamilyForName,
  isTodoPlanToolName,
  isDroraFileContentWriteToolName,
  normalizeDroraToolName,
  type DroraKnownToolName,
  type DroraToolFamily,
} from "@drora/shared";
...
export interface ToolCallIdentity {
  toolName: DroraKnownToolName | string | null;
...
function identityFromKnownToolName(
  ...
  const family = getDroraToolFamilyForName(toolName);
  if (!family) {
    return null;
  }
  return {
...
function readRawToolNameCandidates(raw: unknown) {
  return {
    ...
    drora: readNestedString(raw, ["_meta", "drora", "toolName"]),
...
function isLegacyAgentTool(
  ...
  const rawDrora = normalizeLegacyToken(rawNames.drora);
...
export function resolveToolCallIdentity(toolCall: ToolIdentityLike): ToolCallIdentity {
  ...
  for (const candidate of [
    ...
    { value: rawNames.drora, source: "raw-drora-meta" as const },
  ]) {
  ...
  if (isLegacyAgentTool(toolCall, rawNames)) {
    return identityFromLegacyFamily(
      rawNames.direct ?? rawNames.drora ?? "Agent",
      "agent",
      "legacy-payload",
    );
  }

  // `Task` 现在是现役 Claude 兼容工具名，但历史 Drora Agent 投影会用
  ...
  if (
    // 当前 Drora Agent 传给 app 的 plan mode 退出工具是 ExitPlanMode，
...
export function isFileContentWriteToolCall(
  ...
  if (isDroraFileContentWriteToolName(identity.toolName)) {
    return true;
  }
