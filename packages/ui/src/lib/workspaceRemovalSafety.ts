import type { FileEntry, DroraTaskMeta, DroraTaskRuntimeStatus } from "@drora/shared";
import type { IFileService } from "@drora/services";
import type { WorkspaceDroraUIState } from "@/store/droraSessionStoreTypes.js";
...
export function hasRunningWorkspaceChat(params: {
  workspaceState: Pick<WorkspaceDroraUIState, "draftRuntime" | "taskRuntimeByTaskId">;
  taskItems: Pick<DroraTaskMeta, "taskId">[];
}): boolean {
...
function isWorkspaceRemovalBlockingRuntimeStatus(status: DroraTaskRuntimeStatus): boolean {
