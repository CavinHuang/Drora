import { formatLogPrefix } from "@drora/shared";
  ...
  return (
    (
      globalThis as typeof globalThis & {
        __DRORA_RENDERER_DISABLE_LOGGING__?: boolean;
      }
    ).__DRORA_RENDERER_DISABLE_LOGGING__ === true
  );
}
...
function log(level: LogLevel, ...args: unknown[]) {
  ...
  // ui 包单独 typecheck 时拿不到 desktop renderer 注入的 window.drora 声明，
  ...
  if (level !== "debug" && typeof window !== "undefined") {
    ...
    (window as DesktopLogBridgeWindow).drora?.log?.(level, args);
  }
}
...
export function logMemoryDiagnostics(line: string): void {
  ...
  if (typeof window !== "undefined") {
    const bridge = (window as DesktopLogBridgeWindow).drora?.log;
    if (bridge) {
      bridge("info", [line]);
      return;
    }
