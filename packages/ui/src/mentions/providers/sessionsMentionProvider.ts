// composer parity：`#` 会话候选的数据源从旧 droraSessionStore/taskQueryCache/remote*
...
import type { DroraProvider, DroraTaskMeta } from "@drora/shared";
...
function getSessionLabel(task: DroraTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: DroraTaskMeta): string {
  const raw = task.workspacePath.trim() || task.workspaceIdentity?.trim() || "";
  return raw.split(/[\\/]/).filter(Boolean).at(-1) ?? raw;
}

function mapTaskToMentionItem(task: DroraTaskMeta, provider: DroraProvider): SessionMentionItem {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, DroraTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.droraAgentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.droraAgentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.droraAgentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.droraAgentService,
        },
      ];
    }
