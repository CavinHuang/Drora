...
import type { OnboardingRecordEntry } from "@drora/shared";
...
export function OccupationOnboarding({
  ...
  const requested = useDroraStore((state) => state.newUserOnboardingOpen);
  const setRequested = useDroraStore((state) => state.setNewUserOnboardingOpen);
  // 登录态变化（useRootOAuthEffects 登录成功后 setUser）时按 userId 重新判定是否触发引导。
  const userId = useDroraStore((state) => state.user?.id) ?? null;
  ...
  const savedInterfaceMode = useDroraStore((state) => state.interfaceMode);
  ...
  const requestOnboardingDialog = useDroraStore((state) => state.requestOnboardingDialog);
