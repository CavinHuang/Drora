...
import type { DroraConfigOption } from "@drora/shared";
import {
  buildTaskContextUsageFromUsageUpdate,
  recordTaskContextUsageUpdate,
} from "@/lib/droraTaskUsageFallback.js";
import { normalizeDroraUiError } from "@/lib/droraUiError.js";
...
export function syncBotTaskConfigOptionsToStore(params: {
  droraSessionStore: Pick<
    ReturnType<typeof useDroraSessionStore.getState>,
  ...
  params.droraSessionStore.setTaskConfigOptions(
...
export function shouldMirrorBotTaskStreamToStore(params: {
  ...
  // Bugfix: 远端 Bot task 的 stream 来自 bot runtime host，不一定会被当前 ChatView 的 Drora Agent stream 订阅收到。
...
export function useBotBroadcastEffects(
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      ...
      if (stream) {
        const droraSessionStore = useDroraSessionStore.getState();
        const workspaceState = droraSessionStore.getWorkspaceState(
        ...
        switch (event.type) {
          ...
          case "tool_call":
            droraSessionStore.setTaskRuntimeState(
              stream.workspacePath,
              stream.taskId,
              "streaming",
              undefined,
          ...
          case "permission_request":
            droraSessionStore.setTaskPermissionRequest(stream.workspacePath, stream.taskId, event, stream.workspaceIdentity);
            droraSessionStore.setTaskRuntimeState(
              stream.workspacePath,
              stream.taskId,
              "streaming",
          ...
          case "task_complete":
            ...
            droraSessionStore.setTaskPermissionRequest(stream.workspacePath, stream.taskId, null, stream.workspaceIdentity);
            droraSessionStore.setTaskError(stream.workspacePath, stream.taskId, null, stream.workspaceIdentity);
            break;
          case "task_error": {
            const normalizedError = normalizeDroraUiError(
              {
                message: event.error,
                detail: event.detail,
                code: event.code,
            ...
            droraSessionStore.setTaskPermissionRequest(stream.workspacePath, stream.taskId, null, stream.workspaceIdentity);
            droraSessionStore.setTaskError(stream.workspacePath, stream.taskId, normalizedError, stream.workspaceIdentity);
            break;
          }
          case "task_warning":
            droraSessionStore.setTaskError(
              stream.workspacePath,
              stream.taskId,
              normalizeDroraUiError(
                {
          ...
          case "usage_update":
            {
              ...
              droraSessionStore.setTaskContextWindow(
                stream.workspacePath,
                stream.taskId,
                event.size,
                stream.workspaceIdentity,
              );
              droraSessionStore.setTaskUsage(
                stream.workspacePath,
                stream.taskId,
                nextUsage,
                stream.workspaceIdentity,
          ...
          case "session_info_update":
            if (event.apiRetry !== undefined) {
              droraSessionStore.setTaskApiRetryStatus(
                stream.workspacePath,
                stream.taskId,
                event.apiRetry ?? null,
                stream.workspaceIdentity,
      ...
      if (provider && shouldSyncVisibleTaskConfig) {
        // Bugfix: /model、/mode 可以从第三方 Bot 修改当前 task 的真实 Drora Agent 状态。
        // 这些操作不经过 ChatInputToolbar，本地 store 以前不会同步 provider/configOptions，
        // 导致 Bot 回复已切换但 UI 下拉仍显示旧状态。
        droraSessionStore.bindRuntimeProvider(
          refresh.workspacePath,
          provider,
          refresh.workspaceIdentity,
        );
      ...
      // 会自然出现该消息；本地拼装面（droraChatMessages）随旧 ChatView 退役。
      if (refresh.event === "permission_request" && refresh.permissionRequest) {
        droraSessionStore.setTaskPermissionRequest(
          refresh.workspacePath,
          refresh.taskId,
        ...
        droraSessionStore.removeTaskPermissionRequest(
          refresh.workspacePath,
          refresh.taskId,
          refresh.requestId,
          refresh.workspaceIdentity,
        ...
        // 当前 UI 窗口不一定有同一条 Drora Agent stream 订阅，必须把新的 elicitation_request 显式写回 store。
        droraSessionStore.setTaskElicitationRequest(
          refresh.workspacePath,
          refresh.taskId,
        ...
        // Bugfix: Bot 代用户提交 AskUserQuestion 时，当前 UI 窗口不一定能收到 Drora Agent stream 的
        // elicitation_response。通过 bots:task 明确同步 requestId 出队，避免问答弹窗一直挂着。
        droraSessionStore.removeTaskElicitationRequest(
          refresh.workspacePath,
          refresh.taskId,
          refresh.requestId,
          refresh.workspaceIdentity,
      ...
      if (shouldRefreshBotTaskList(refresh.event, Boolean(refresh.task))) {
        droraSessionStore.bumpTaskListVersion(refresh.workspacePath, refresh.workspaceIdentity);
      }
    });
