...
import {
  ...
  type DroraAutomationScheduleRule,
} from "@drora/shared";
import {
...
interface AutomationEditViewProps {
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: DroraAutomation) => Promise<void> | void;
...
function resolveRunStatus(run: DroraAutomationRun): RunStatusKind {
...
function normalizeAutomationScheduleRule(
  rule: DroraAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: DRORA_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<DroraAutomationScheduleRule | undefined>(
    () =>
