...
import { RENDERER_DRORA_ENDPOINT_URLS } from "@/lib/rendererDroraEndpoint.js";
...
interface CodingPlanEmbeddedWebviewDialogProps {
   ...
   * 官网页购买成功后的回调。
   *
   * 官网页通过 preload 注入的 window.droraBridge.notifyPurchaseComplete({ provider })
   * 发送 drora:coding-plan-purchase-complete 频道消息，本组件在 webview 的
...
export function CodingPlanEmbeddedWebviewDialog({
  ...
  const theme = useDroraStoreWithDefault((state) => state.theme, "zai-dark");
  const userId = useDroraStoreWithDefault((state) => state.user?.id ?? null, null);
  ...
  const computedWebviewUrl = useMemo(() => {
    ...
    const origin = resolveCodingPlanEmbeddedOrigin({
      endpointOrigin: RENDERER_DRORA_ENDPOINT_URLS.origin,
      e2eStoreBridgeEnabled: env.VITE_DRORA_E2E_STORE_BRIDGE === "1",
  ...
  const handleWebviewRef = useCallback(
    (element: ElectronWebviewTag | null) => {
      ...
      // ipc-message: 官网页通过 preload 的 window.droraBridge.notifyPurchaseComplete
      // 发回购买完成信号（drora:coding-plan-purchase-complete）。
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          ...
          <webview
            ...
            partition="persist:drora-coding-plan"
            src={webviewUrl}
            className={cn(
