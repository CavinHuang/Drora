...
//  - 顶部一个总开关：开/关 drora-cua 插件（连带其 MCP server 与 skill 一起启用/禁用）。
...
import type { CuaOsSupport, CuaPermissionKind, RemoteTarget } from "@drora/shared";
import {
  DesktopCommandIds,
  isRemoteWorkspaceIdentity,
  DRORA_CUA_OFFICIAL_PLUGIN_ID,
} from "@drora/shared";
import { isCuaPermissionStatusAvailable, type CuaPermissionRestartOptions } from "@drora/services";
...
export function ComputerUseSection({
  ...
  // 总开关 = drora-cua 插件启用态（读自插件管理 store；切换即同步启用/禁用插件及其 MCP + skill）。
  ...
  const cuaPlugin = plugins.find((plugin) => plugin.id === DRORA_CUA_OFFICIAL_PLUGIN_ID);
  const cuaEnabled = cuaPlugin?.enabled ?? false;
  const cuaToggling = togglingPluginId === DRORA_CUA_OFFICIAL_PLUGIN_ID;
  ...
  useEffect(() => {
    ...
    // 复用 Plugins 分区同一条初始化路径，确保 store 已加载 drora-cua 的 enabled 态。
  ...
  // 重启 Helper 后验证仍持续 stale → 显示"重启 Drora"兜底按钮。accessibility 变 granted 时自愈清除。
  ...
  // 兜底:重启 Helper 后仍持续 stale 时,用户可一键重启 Drora(复用 OAuth 登出同款 RelaunchApp)。
  // 新 Drora 进程会干净地重新拉起 Helper,绕过当前进程里可能卡住的重启机制(孤儿/socket/状态污染)。
  ...
  const onTogglePlugin = useCallback(
    async (next: boolean) => {
      ...
      // 切换 drora-cua 插件 = 同步其 MCP server + skill 一起启用/禁用。
      const completed = await runAfterSuccessfulPluginEnabledChange({
        submit: () => setPluginEnabled(DRORA_CUA_OFFICIAL_PLUGIN_ID, next, pluginManagementService),
        isCurrent: () =>
          mountedRef.current &&
  ...
  // 自愈:accessibility 在后续任一次查询里变成 granted 时,清除"重启 Drora"兜底(说明问题已解决)。
  useEffect(() => {
  ...
  // 保留重新授权入口与“重启 Drora”兜底，避免把不可由单次 Helper 重启修复的状态误导成已解决。
  ...
  return (
    <div className="space-y-4">
      {/* 总开关：开/关 drora-cua 插件（同步其 MCP + skill） */}
      <SettingsGroupCard>
        <SettingsRow
