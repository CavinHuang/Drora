...
import type { Hook, HookConfig, DroraInstalledPluginSummary, DroraPluginInfo } from "@drora/shared";
...
import { invalidateDeferredDraftSessionForRuntimeChange } from "@/lib/droraDraftSkillInvalidation.js";
...
function isEditableHook(hook: Hook): boolean {
  ...
  return hook.editable ?? (!hook.location || hook.location.source === "drora");
}

// workspace-hook-trust：editable=false 且 source=drora 的行是「上游/祖先 drora.json
// 里的只读工作区 Hook」。它们不是外部格式兼容导入源，塞进 Legacy 会让 Import 按钮
// 必然失败（importHook 拒绝 source=drora），也违反「只读但可逐条 Trust」的约定。
// 这类行应留在 Installed 分组，由信任状态门控 Switch，走行内 Trust 流程。
function isReadOnlyDroraHook(hook: Hook): boolean {
  return hook.editable === false && (hook.location?.source ?? "drora") === "drora";
}

function isInCompatibilitySection(hook: Hook): boolean {
  return !isEditableHook(hook) && !isReadOnlyDroraHook(hook);
}
...
function buildPluginHookRows(
  plugins: readonly Pick<DroraPluginInfo, "enabled" | "hookDetails" | "id" | "name">[],
  installedPlugins: readonly Pick<DroraInstalledPluginSummary, "id" | "scope">[],
...
export function HooksSection({ workspacePath, workspaceIdentity }: HooksSectionProps) {
  ...
  const editableHooks = useMemo(
    () =>
      hooksState.hooks.filter(
        (hook) =>
          (isEditableHook(hook) || isReadOnlyDroraHook(hook)) &&
  ...
  const invalidateDraft = useCallback(
    ...
    [targetWorkspaceIdentity, targetWorkspacePath, droraSessionService],
  );
