 ...
 * Supports the unified Drora Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToDroraAgentMcpServer,
...
import { isDroraAgentMcpStatusModeUnsupportedError, type IMcpSyncService } from "@drora/services";
...
const DEFAULT_MCP_SOURCE: ServerScope = "droraagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 droraAgentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isDroraAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "droraagentmcp" || !server.enabled) {
      continue;
    }
    ...
    result.push(droraAgentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: DroraAgentMcpServer[]): {
  mode: "status";
  mcpServers: DroraAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: DroraMcpServer[]): string {
  return servers
    .filter((server) => server.source === "droraagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: DroraPluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, DroraAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<DroraMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForDrora("drora");
  ...
  async function handleSave(form: FormState, prev?: DroraMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: DroraMcpServer) {
  ...
  function handleEdit(server: DroraMcpServer) {
  ...
  function handleOpenAuthorization(server: DroraMcpServer) {
