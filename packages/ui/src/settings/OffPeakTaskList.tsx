...
interface OffPeakTaskListProps {
  ...
  onOpen: (task: DroraOffPeakTask) => void;
  onPause: (task: DroraOffPeakTask) => void;
  onContinue: (task: DroraOffPeakTask) => void;
  onCancel: (task: DroraOffPeakTask) => void;
  onDelete: (task: DroraOffPeakTask) => void;
  onOpenSession: (task: DroraOffPeakTask) => void;
}

// 按状态分组会让任务在运行和终态切换时跳位，破坏用户对已有卡片位置的预期；
// 列表只按不可变的创建时间倒序，状态变化不再影响顺序。
function sortOffPeakTasksByCreatedAt(tasks: readonly DroraOffPeakTask[]): DroraOffPeakTask[] {
