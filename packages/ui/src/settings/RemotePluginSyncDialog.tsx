...
import type {
  ...
  RemoteTarget,
  DroraPluginOptionValue,
  DroraPluginInfo,
  DroraPluginMarketplaceSummary,
  DroraPluginUserConfigOption,
  DroraPluginsOverviewResult,
} from "@drora/shared";
import type { IPluginSyncService, IDroraAgentService } from "@drora/services";
...
interface RemotePluginSyncPluginOptions {
  configuredOptions: Record<string, DroraPluginOptionValue>;
  userConfig: Record<string, DroraPluginUserConfigOption>;
}
...
interface RemotePluginSyncDialogProps {
  ...
  localDroraAgentService?: RemotePluginSyncAgentService | null;
  remoteDroraAgentService?: RemotePluginSyncAgentService | null;
...
function buildMarketplaceRemotePluginSyncCandidates(
  overview: DroraPluginsOverviewResult,
...
function buildMarketplaceRemotePluginStatuses(
  candidates: readonly RemotePluginSyncCandidate[],
  remoteOverview: DroraPluginsOverviewResult,
...
function componentTypesFromPluginInfo(plugin: DroraPluginInfo | undefined): string[] {
  return plugin?.components?.map((component) => component.kind) ?? [];
}

function buildRemotePluginSyncPluginOptions(
  plugin: DroraPluginInfo | undefined,
...
function serializeMarketplaceSourceInput(
  marketplace: DroraPluginMarketplaceSummary,
): string | undefined {
...
async function loadRemotePluginSyncCandidates(params: {
  ...
  if (!params.localDroraAgentService || !params.remoteDroraAgentService) {
  ...
  const [localOverview, localPluginsResult, remoteOverview] = await Promise.all([
    params.localDroraAgentService.getPluginsOverview({
      workspacePath: params.localWorkspacePath,
    }),
    params.localDroraAgentService.listPlugins({
      workspacePath: params.localWorkspacePath,
    }),
    params.remoteDroraAgentService.getPluginsOverview({
...
function isPathPluginOption(option: DroraPluginUserConfigOption | undefined): boolean {
  return option?.type === "file" || option?.type === "directory";
}

function isPortablePluginOptionValue(
  value: DroraPluginOptionValue,
  localOption: DroraPluginUserConfigOption | undefined,
  remoteOption: DroraPluginUserConfigOption | undefined,
): boolean {
...
function buildPortablePluginOptionsSyncPlan(
  ...
  remotePlugin: DroraPluginInfo,
): { options: Record<string, DroraPluginOptionValue>; skippedCount: number } {
  const options: Record<string, DroraPluginOptionValue> = {};
  let skippedCount = 0;
...
function arePluginOptionsEqual(
  left: Record<string, DroraPluginOptionValue>,
  right: Record<string, DroraPluginOptionValue>,
): boolean {
...
async function syncPortablePluginOptions(
  ...
  const remoteDroraAgentService = params.remoteDroraAgentService;
  if (!remoteDroraAgentService) {
    emitRemotePluginSyncProgress(
      params,
      row,
      "syncing",
  ...
  const remotePlugins = await awaitRemotePluginSyncStep(
    ...
    () => remoteDroraAgentService.listPlugins(workspace),
  );
  ...
  await awaitRemotePluginSyncStep(
    ...
    () =>
      remoteDroraAgentService.configurePlugin({
        ...workspace,
...
async function syncMarketplaceRemotePlugin(
  ...
  try {
    throwIfRemotePluginSyncStopped(params, row);
    if (!params.remoteDroraAgentService || !marketplacePlugin) {
    ...
    const installResult = await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        remoteDroraAgentService.installPlugin({
          ...workspace,
    ...
    await awaitRemotePluginSyncStep(
      ...
      () =>
        remoteDroraAgentService.setPluginEnabled({
          ...workspace,
          enabled: candidate.enabled,
          pluginId: candidate.pluginId,
          scope: REMOTE_PLUGIN_SYNC_CONFIG_SCOPE,
...
async function prepareRemoteMarketplaceSource(
  params: {
    ...
    onItemProgress?: (event: RemotePluginSyncProgressEvent) => void;
    remotePluginSyncService: IPluginSyncService;
    remoteDroraAgentService: RemotePluginSyncAgentService;
    selectedMarketplacePluginNames: ReadonlyMap<string, string[]>;
    signal?: AbortSignal;
    stopControl?: RemotePluginSyncStopControl;
  },
  ...
  if (marketplaceSourceInput) {
    await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        params.remoteDroraAgentService.addPluginMarketplace({
          ...workspace,
          operationId,
      ...
      const result = await syncSelectedRemotePlugins({
        ...
        stopControl: {
          cancelOperation: async (operationId) => {
            if (!remoteDroraAgentService?.cancelPluginOperation) {
              return;
            }
            await remoteDroraAgentService.cancelPluginOperation({ operationId });
          },
