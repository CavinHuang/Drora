...
import type { CodingPlanWebviewLocale } from "@drora/shared";
...
const CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY = "drora:coding-plan:report-context";
...
export function resolveCodingPlanEmbeddedOrigin({
  ...
  if (
    ...
    return normalizeDroraEndpointOrigin(normalizedOverride);
  }
  const normalizedEndpointOrigin = normalizeDroraEndpointOrigin(endpointOrigin);
...
export function buildCodingPlanEmbeddedWebviewUrl({
  ...
  const url = new URL("/coding-plan", normalizeDroraEndpointOrigin(origin));
...
export function createCodingPlanAuthInjectionScript({
  ...
  // App 当前 locale，写入 window.__droraLang__ 供 droraBridge.getLang() 读取，
  ...
  const values: Record<string, string | null> =
    provider === "zai"
      ? {
          ...
          drorajwttoken: credentials.droraJwtToken?.trim() || null,
  ...
  return `(() => {
  ${storageUpdates}
  const droraTheme = ${JSON.stringify(theme)};
  document.documentElement.classList.toggle("dark", droraTheme === "zai-dark");
  document.documentElement.classList.toggle("theme-zai-light", droraTheme === "zai-light");
  document.documentElement.classList.toggle("theme-zai-dark", droraTheme === "zai-dark");
  localStorage.setItem("drora-theme", droraTheme);
  localStorage.setItem("drora:coding-plan:embedded", "app");
  // 写入当前 App locale，供官网 droraBridge.getLang() 读取。
  // 注意：这是注入 webview 执行的原始 JS，不能用 TS 语法（如 as any）。
  window.__droraLang__ = ${JSON.stringify(resolvedLocale)};
  const droraReportContext = ${JSON.stringify(normalizedReportContext)};
  window.__droraReportContext__ = droraReportContext;
  localStorage.setItem(${JSON.stringify(CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY)}, JSON.stringify(droraReportContext));
  window.dispatchEvent(new CustomEvent("drora-coding-plan-auth-ready", {
    detail: { ...${JSON.stringify({ provider, locale: resolvedLocale })}, reportContext: droraReportContext },
  }));
})()`;
}
...
export function createCodingPlanCredentialClearScript(): string {
  ...
  delete window.__droraReportContext__;
})()`;
}

export function createCodingPlanScrollbarHideScript(): string {
  return `(() => {
  const styleId = "drora-coding-plan-hide-scrollbar";
 ...
 * 重写 window.__droraLang__ 并派发 drora-coding-plan-lang-change 事件，
 * 官网侧（droraBridge.onLangChange 或 window 监听）据此无感切换语言。
 */
export function createCodingPlanLangInjectionScript(locale: CodingPlanWebviewLocale): string {
  ...
  window.__droraLang__ = ${JSON.stringify(resolvedLocale)};
  window.dispatchEvent(new CustomEvent("drora-coding-plan-lang-change", {
...
export function getCodingPlanCredentialKeys(provider: CodingPlanWebsiteProvider): string[] {
  ...
  return provider === "zai"
    ...
    : [`oauth:${BIGMODEL_PROVIDER_ID}:access_token`, "drorajwttoken"];
}
