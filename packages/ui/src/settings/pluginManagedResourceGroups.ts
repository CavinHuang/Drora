import type {
  ...
  DroraPluginInfo,
  DroraPluginComponentKind,
  DroraPluginsDescribeResult,
} from "@drora/shared";
import { isPluginCommand, isUserCommand, DRORA_COMMAND_AGENT_SOURCE } from "@drora/shared";
...
const COMPONENT_KIND_ORDER: DroraPluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: DroraPluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<DroraPluginComponentKind, PluginComponentDisplayGroup>();
...
export interface PluginMcpServerItem {
  active: boolean;
  authorization?: DroraMcpServerStatusSnapshot["authorization"];
  error?: string;
  failureKind?: DroraMcpServerStatusSnapshot["failureKind"];
  id: string;
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === DRORA_COMMAND_AGENT_SOURCE &&
...
export function filterLocalMcpServers(servers: DroraMcpServer[], query: string): DroraMcpServer[] {
  return servers.filter((server) => {
    if (server.source !== "droraagentmcp") {
      return false;
    }
...
function toPluginMcpServerDisplayName(plugin: DroraPluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: DroraPluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: DroraMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
