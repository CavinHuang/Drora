import type { DroraCommand, DroraSlashCommand } from "@drora/shared";
...
function commandToSlashCommand(command: DroraCommand): DroraSlashCommand | null {
  if (!command.enabled) {
    return null;
  }
...
export function mergeSlashCommandsAfterCommandRefresh(
  currentSlashCommands: readonly DroraSlashCommand[],
  refreshedCommands: readonly DroraCommand[],
): DroraSlashCommand[] {
  ...
  const customCommands: DroraSlashCommand[] = [];
