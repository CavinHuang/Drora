...
export function resolveUniquePluginListingByName(
  plugins: readonly Pick<DroraAvailablePluginSummary, "name" | "listing">[],
  name: string,
): DroraPluginStoreListing | undefined {
  const normalizedName = name.trim().toLocaleLowerCase();
  let match: DroraPluginStoreListing | undefined;
  let count = 0;
...
export interface StorePluginItem {
  ...
  orphaned: boolean;
  listing?: DroraPluginStoreListing;
  summary?: DroraAvailablePluginSummary;
  /** 运行时信息（仅已发现的已安装插件有）：启用态、组件、manifest 回退字段。 */
  info?: DroraPluginInfo;
  installedMeta?: DroraInstalledPluginSummary;
}

export type PluginUpdateStatus = NonNullable<DroraInstalledPluginSummary["updateStatus"]>;
...
const OFFICIAL_MARKETPLACE_ORDER: readonly string[] = [DRORA_OFFICIAL_PLUGIN_MARKETPLACE_ID];
...
export function sortMarketplaceSources(
  marketplaces: readonly DroraPluginMarketplaceSummary[],
  locale: string,
): DroraPluginMarketplaceSummary[] {
...
export function buildStoreItems(input: {
  marketplaces: DroraPluginMarketplaceSummary[];
  marketplaceAvailabilityKnown: boolean;
  availablePlugins: DroraAvailablePluginSummary[];
  installedPlugins: DroraInstalledPluginSummary[];
  plugins: DroraPluginInfo[];
  restorableBuiltins: DroraAvailablePluginSummary[];
}): StorePluginItem[] {
