...
function SavedWorkflowLastRunBadge({
  ...
  run: DroraSavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: DroraSavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: DroraSavedWorkflowEntry) => void;
  onRun: (entry: DroraSavedWorkflowEntry) => void;
  onRevise: (entry: DroraSavedWorkflowEntry) => void;
  onCopyPath: (entry: DroraSavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: DroraSavedWorkflowEntry) => void;
  onDelete: (entry: DroraSavedWorkflowEntry) => void;
}
