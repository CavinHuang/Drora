...
import {
  ...
  type DroraSavedWorkflowEntry,
  type DroraSavedWorkflowMeta,
  type DroraSavedWorkflowRun,
  type DroraWorkflowsGetResult,
} from "@drora/shared";
import type { IDroraAgentService, DroraAgentSavedWorkflowTarget } from "@drora/services";
...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<DroraWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: DroraSavedWorkflowMeta): MetaDraft {
  return {
...
interface SavedWorkflowDetailViewProps {
  ...
  target: DroraAgentSavedWorkflowTarget;
  agentService: IDroraAgentService;
  name: string;
  /** 面包屑「自动化 › <项目名> › <工作流名>」里的项目一级。 */
  projectLabel: string;
  entry: DroraSavedWorkflowEntry | undefined;
  runs: readonly DroraSavedWorkflowRun[];
  now: number;
  busy: boolean;
  canOpenRun: boolean;
  onBack: () => void;
  ...
  onMove?: () => void;
  onDelete: () => void;
  onOpenRun: (run: DroraSavedWorkflowRun) => void;
  /** 产物 chip → `workflow-artifact` tab；缺席即 chips 只读。 */
  onOpenArtifact?: (run: DroraSavedWorkflowRun, artifactId: string) => void;
  onMetaSaved: () => void;
  /** 传入即在运行历史每行渲染项目列（全局工作流跨项目历史）。 */
  resolveRunProject?: (run: DroraSavedWorkflowRun) => SavedWorkflowRunProject | null;
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: DroraSavedWorkflowMeta = {
      description,
