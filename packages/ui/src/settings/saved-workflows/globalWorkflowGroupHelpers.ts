...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type DroraSavedWorkflowRun } from "@drora/shared";
import type { DroraAgentSavedWorkflowTarget } from "@drora/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: DroraAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: DroraSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
