...
import type { DroraSavedWorkflowRun, DroraSavedWorkflowRunStatus } from "@drora/shared";
...
): Map<string, DroraSavedWorkflowRun> {
  const byName = new Map<string, DroraSavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: DroraSavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: DroraSavedWorkflowRun, now: number): number {
