...
const StoreContext = createContext<DroraStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<DroraStore | null>(null);
  if (!storeRef.current) {
 ...
 */
export function useDroraStore<T>(selector: (state: DroraState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useDroraStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useDroraStore（store 耦合剥离配套）。
 *
 ...
 */
export function useDroraStoreWithDefault<T>(
  selector: (state: DroraState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
