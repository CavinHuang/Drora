/**
 * Drora Session Store 选择器与内部辅助函数
 *
 * 从 droraSessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { DroraTaskRuntimeStatus, DroraTaskMeta } from "@drora/shared";
...
import {
  ...
  type DroraSessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceDroraUIState,
} from "./droraSessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceDroraUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceDroraUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: DroraSessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceDroraUIState) => WorkspaceDroraUIState,
  workspaceIdentity?: string,
  ...
  if (nextWorkspaceState === current) {
    // 单 Drora Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceDroraUIState,
  taskId: string,
): TaskRuntimeState {
...
export function getTaskUiState(workspaceState: WorkspaceDroraUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceDroraUIState
    | Partial<Pick<WorkspaceDroraUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
): DroraTaskMeta | null {
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, DroraTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<DroraTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceDroraState(
  state: DroraSessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceDroraUIState, "workspaceInit">,
): WorkspaceInitState {
