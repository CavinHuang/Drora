...
import { getDefaultWorkspaceState } from "@/store/droraSessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly DroraTaskMeta[]): DroraTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: DroraPermissionRequest["options"],
  right: DroraPermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: DroraPermissionRequest,
  right: DroraPermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useDroraChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToDroraAgent(params.provider),
