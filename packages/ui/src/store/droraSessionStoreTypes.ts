/* oxlint-disable eslint(max-lines) -- Drora Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * Drora Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 droraSessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
  ...
  type DroraWorkspaceInitStatus,
  type DroraTaskRuntimeStatus,
  type DroraProvider,
  type DroraTaskMeta,
  type DroraContextCacheUsage,
  ...
  type DroraSessionActiveTurnKind,
  type DroraContextUsageBreakdownItem,
  type InputId,
  type SessionCreateSource,
} from "@drora/shared";
import type { DroraUiError } from "@/lib/droraUiError.js";
import type {
...
export interface WorkspaceInitState {
  status: DroraWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: DroraTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 Drora Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: DroraBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: DroraPermissionRequest | null;
  pendingPermissionRequests: DroraPermissionRequest[];
  elicitationRequest: DroraElicitationRequest | null;
  pendingElicitationRequests: DroraElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: DroraUiError | null;
}
...
export interface WorkspaceDroraUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * Drora Agent 的 workspace 初始化状态。
   *
   * 单 Drora Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  taskConfigOptionsByTaskId: Record<string, DroraConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, DroraTaskMeta>;
  ...
  /** 当前 workspace 选中的 Drora Agent provider */
  ...
  /** Drora Agent configOptions（模型、模式、思考级别等） */
  configOptions: DroraConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: DroraTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface DroraSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceDroraUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceDroraUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: DroraPermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: DroraElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  /** 任务列表变更时调用（创建/删除任务），驱动 droraTaskMetaMerge 重新拉取列表 */
...
const FALLBACK_PROVIDER: DroraProvider = DRORA_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceDroraUIState {
  // 单 Drora Agent 迁移后默认 provider 必须收敛到 glm。
