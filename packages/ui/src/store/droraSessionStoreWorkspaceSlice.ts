import type { SessionCreateSource } from "@drora/shared";
...
} from "@/store/droraSessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/droraSessionStoreSelectors.js";

type SetFn = (
  partial:
    | DroraSessionStoreState
    | Partial<DroraSessionStoreState>
    | ((state: DroraSessionStoreState) => DroraSessionStoreState | Partial<DroraSessionStoreState>),
) => void;
...
function normalizeThoughtLevelConfigOption(option: DroraConfigOption): DroraConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: DroraConfigOption[]): DroraConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly DroraConfigOption[]): DroraConfigOption[] {
...
function isModeConfigOption(option: DroraConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<DroraConfigOption["options"]>;
}): DroraConfigOption {
...
function replaceModeConfigOption(
  currentOptions: DroraConfigOption[] | null,
  nextModeOption: DroraConfigOption | null,
): DroraConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceDroraUIState,
  updater: (options: DroraConfigOption[] | null) => DroraConfigOption[] | null,
): Pick<WorkspaceDroraUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceDroraUIState,
  taskId: string,
): { configOptions: DroraConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: DroraTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as DroraTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToDroraAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToDroraAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToDroraAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
