...
import type {
  ...
  DroraMcpListMode,
  DroraMcpServerStatusSnapshot,
  DroraMcpServer,
} from "@drora/shared";
import { convertToDroraAgentMcpServer } from "@drora/shared";
...
import {
  importLegacyCommonServersToDroraAgent,
  migrateStoredCommonMcpToDroraAgent,
...
interface McpStoreState {
  ...
  servers: DroraMcpServer[];
  statusSnapshots: Record<string, DroraMcpServerStatusSnapshot>;
  ...
  addDroraAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  updateDroraAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  deleteDroraAgentMcpServer: (name: string, projectPath?: string) => void;
  ...
  beginServerStatusListRefresh: (mode?: DroraMcpListMode) => number;
  ...
  mergeServerStatusSnapshots: (
    statuses: Record<string, DroraMcpServerStatusSnapshot>,
    requestEpoch: number,
    mode?: "connect" | "status",
  ) => void;
  getServer: (id: string) => DroraMcpServer | undefined;
  setCurrentProjectPath: (
    path: string,
    workspaceIdentity?: string,
    directoryService?: McpDirectoryService | null,
  ) => void;
  getEnabledMcpServersForDrora: (provider: string) => DroraAgentMcpServer[];
...
export const useMcpStore = create<McpStoreState>((set, get) => {
  ...
  const statusListEpochs: Record<DroraMcpListMode, number> = {
    connect: 0,
    status: 0,
  };
  ...
  function resolveMcpDirectoryService(
    ...
    if (!effectiveWorkspaceIdentity?.trim()) {
      ...
      // 到用户级 Drora Agent MCP 的迁移；目录服务只用于远端 workspace 覆盖路由。
      return null;
    }
  ...
  return {
    ...
    loadConfig: () => {
      ...
      // MCP 启停状态已经迁移到 ~/.drora/cli/config.json，不能再读取旧 localStorage，
      // 否则旧的本地开关会覆盖新的 Drora Agent 配置来源。
      ...
      loadMcpPromise = (async () => {
        try {
          ...
          if (!activeDirectoryService) {
            servers = await migrateStoredCommonMcpToDroraAgent(
              mcpPlatformService,
              servers,
              latestWorkspacePath,
            );
    ...
    addMcpServer: (name, config) => get().addDroraAgentMcpServer(name, config),
    updateMcpServer: (name, config) => get().updateDroraAgentMcpServer(name, config),
    deleteMcpServer: (name) => get().deleteDroraAgentMcpServer(name),
    addScopedMcpServer: async (source, name, config, projectPath) => {
      invalidateStatusListRequests();
      const targetSource: CliMcpSource = source === "mcp" ? "droraagentmcp" : source;
    ...
    addDroraAgentMcpServer: (name, config, projectPath) =>
      get().addScopedMcpServer("droraagentmcp", name, config, projectPath),
    updateDroraAgentMcpServer: (name, config, projectPath) =>
      get().updateScopedMcpServer("droraagentmcp", name, config, projectPath),
    deleteDroraAgentMcpServer: (name, projectPath) =>
      get().deleteScopedMcpServer("droraagentmcp", name, projectPath),
    ...
    markServerStatusListRefreshFailed: (error, requestEpoch, mode = "connect") => {
      ...
      set((state) => ({
        ...
        servers: state.servers.map((server) => {
          const enabled = state.enabledStates[server.id] ?? server.enabled;
          if (server.source !== "droraagentmcp" || !enabled || server.status !== "connecting") {
            return server;
          }
          return {
            ...server,
    ...
    getEnabledMcpServersForDrora: (_provider) => {
      const { servers, enabledStates, currentProjectPath } = get();
      const candidates = new Map<string, DroraMcpServer>();

      for (const server of servers) {
        const isEnabled = enabledStates[server.id] ?? server.enabled;
        if (!isEnabled) continue;
        if (server.source !== "droraagentmcp") continue;
      ...
      const result: DroraAgentMcpServer[] = [];
      for (const server of candidates.values()) {
        const droraAgentServer = convertToDroraAgentMcpServer(server.name, server.config);
        if (droraAgentServer) result.push(droraAgentServer);
      }
      return result;
    },
    ...
    mergePreloadedMcpServers: (preloaded) => {
      ...
      const targetSource: CliMcpSource = "droraagentmcp";
    ...
    migrateLegacyCommonMcp: async () => {
      ...
      const migration = await importLegacyCommonServersToDroraAgent(
        mcpPlatformService,
        result.servers ?? {},
