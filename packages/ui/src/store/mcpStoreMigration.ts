import type { McpServerConfig, NativeMcpServerRecord } from "@drora/shared";
...
function isDroraAgentUserServer(server: NativeMcpServerRecord): boolean {
  return (
    server.source === "droraagentmcp" &&
    server.scope === "user" &&
    (!server.location || server.location.source === "drora")
  );
}

export async function importLegacyCommonServersToDroraAgent(
  ...
  const existingNames = new Set(
    nativeServers.filter(isDroraAgentUserServer).map((server) => server.name),
  );
  let importedCount = 0;
  let skippedCount = 0;
...
export async function migrateStoredCommonMcpToDroraAgent(
  platform: McpPlatformService | null,
  nativeServers: NativeMcpServerRecord[],
  ...
  const migration = await importLegacyCommonServersToDroraAgent(
    ...
    legacyServers,
    nativeServers,
    "localStorage:drora-mcp-config",
  );
  if (migration.completed) {
    // 只有确认写入 drora agent 目录后才清理旧数据，避免 Web 端没有 desktop bridge 时丢配置。
    clearLegacyCommonMcpServers();
  }
  ...
  logger.info(
    `[mcpStore] migrated ${migration.importedCount} legacy common MCP servers to drora agent config`,
  );
