...
import type {
  ...
  DroraPluginDiagnostic,
  DroraPluginInfo,
  DroraPluginMarketplaceSummary,
  DroraPluginScope,
  DroraPluginsDescribeResult,
} from "@drora/shared";
import type { IPluginManagementService } from "@drora/services";
...
export interface PluginDescribeEntry {
  ...
  data?: DroraPluginsDescribeResult;
  error?: string;
}

// 设置页「插件管理」的数据源: 经 IPluginManagementService 薄服务由 drora-cli 提供 (list + enable/disable)。
// UI 不再直触 IDroraAgentService，plugins/* 旧协议词的消费收拢到服务实现一处。
// 与已 retired 的 marketplace pluginStore 无关, 故单独建一个精简 store。
export interface PluginManagementState {
  workspacePath: string | null;
  workspaceIdentity: string | null;
  configScope: DroraPluginScope | null;
  ...
  diagnostics: DroraPluginDiagnostic[];
  loading: boolean;
  error: string | null;
  /**
...
function throwForErrorDiagnostic(diagnostics: DroraPluginDiagnostic[]): void {
