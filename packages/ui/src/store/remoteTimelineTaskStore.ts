...
import type { DroraTaskListSortBy, IDroraTaskService } from "@drora/services";
...
interface RemoteTimelineTaskState {
  itemsByWorkspaceKey: Record<string, DroraTaskMeta[]>;
  ...
  refreshWorkspace: (params: {
    ...
    droraTaskService: Pick<IDroraTaskService, "listTaskList">;
    sortBy: DroraTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: DroraTaskMeta, sortBy?: DroraTaskListSortBy) => void;
...
function sortTimelineTasks(
  tasks: DroraTaskMeta[],
  sortBy: DroraTaskListSortBy = "updated",
): DroraTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, droraTaskService, sortBy, limit }) {
    ...
    try {
      const result = await droraTaskService.listTaskList({
        kind: "timeline",
        workspaceScopes: [
          {
            workspacePath,
...
export async function refreshRemoteTimelineTasksForSession({
  ...
  await useRemoteTimelineTaskStore.getState().refreshWorkspace({
    workspacePath,
    workspaceIdentity,
    droraTaskService: session.services.droraTaskService,
    sortBy,
    limit,
  });
}
