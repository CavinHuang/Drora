...
import {
  ...
  type DroraSavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: DroraSavedWorkflowEntry[];
  invalid: DroraSavedWorkflowInvalidEntry[];
  runs: DroraSavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: DroraAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: DroraAgentSavedWorkflowTarget,
  agentService: IDroraAgentService,
): Promise<{ list: DroraWorkflowsListResult; runs: DroraSavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as DroraSavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: DroraAgentSavedWorkflowTarget | null,
