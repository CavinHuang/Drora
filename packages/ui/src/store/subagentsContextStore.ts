...
export function getSubagentsContextKey(
  ...
  return `${workspaceIdentity?.trim() || workspacePath}::${normalizeAgentProviderToDroraAgent(provider)}`;
}
...
async function loadContext(
  ...
  const provider = normalizeAgentProviderToDroraAgent(params.provider);
