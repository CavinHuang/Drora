...
import type { ISubagentsService } from "@drora/services";
...
interface SubagentsStoreState {
  ...
  loadedProvider: DroraProvider | null;
  agents: AgentSummary[];
  ...
  initialize: (
    ...
    providerOrSubagentsService: DroraProvider | ISubagentsService,
...
export const useSubagentsStore = create<SubagentsStoreState>((set, get) => ({
  ...
  async initialize(
    ...
    const provider = normalizeAgentProviderToDroraAgent(
      hasProvider ? providerOrSubagentsService : DRORA_AGENT_PROVIDER,
    );
  ...
  async refresh(subagentsService: ISubagentsService, workspaceIdentity?: string) {
    ...
    const provider = normalizeAgentProviderToDroraAgent(get().provider);
  ...
  async createAgent(
    ...
    try {
      const result = await subagentsService.createAgent({
        config,
        provider: normalizeAgentProviderToDroraAgent(provider),
      });
      await get().refresh(subagentsService, workspaceIdentityFromState);
      return result.agent;
    } catch (error) {
...
export async function refreshLoadedSubagentsStoreForWorkspace(params: {
  ...
  const contextKey = getSubagentsContextKey(workspacePath, DRORA_AGENT_PROVIDER, workspaceIdentity);
