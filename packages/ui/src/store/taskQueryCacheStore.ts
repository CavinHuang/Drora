...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: DroraTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: DroraTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: DroraTaskMeta;
...
type CachedTaskListItem = DroraTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: DroraTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, DroraTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareDroraTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
...
export function upsertTaskQueryCacheTaskMeta(task: DroraTaskMeta): void {
  useTaskQueryCacheStore.getState().upsertTaskMeta(task);
}

export function updateTaskQueryCacheTaskMetaPreservingMembership(task: DroraTaskMeta): void {
