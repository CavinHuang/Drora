  ...
  const revalidateAndPrependPlugin = useCallback(
    async (current: DraftSuggestedPluginFlow, requestVersion: number) => {
      await invalidateDeferredDraftSessionForSkillChange({
        droraSessionService: resolution.services.droraSessionService,
        workspacePath,
        workspaceIdentity,
    ...
    [
      ...
      resolution.services.droraSessionService,
      targetParams,
      workspaceIdentity,
      workspacePath,
    ],
      ...
      if (
        ...
        (kind === "install" &&
          (!flow.result?.pluginName ||
            flow.result.marketplace !== DRORA_OFFICIAL_PLUGIN_MARKETPLACE_ID ||
