...
import { localizeConversationShareUrl } from "@drora/shared";
import type {
...
import { parseModelPickerValue } from "@/lib/droraSessionProjection.js";
...
export function SessionPane({
  ...
  const { conversationShareService, modelSelectionService, droraSessionService, droraTaskService } =
    useServices();
  ...
  // send_result 的落定信号：用户消息真正画进对话历史。drora 没有乐观渲染，
  ...
  const [sendSubmissionError, setSendSubmissionError] = useState<DroraUiError | null>(null);
  ...
  const workspaceConfigOptions = useDroraSessionStore(
  ...
  useEffect(() => {
    ...
    useDroraSessionStore.getState().invalidateDraftRuntime(workspacePath, workspaceIdentity);
  ...
  const promoteGroupedDraftTask = useDroraSessionStore((state) => state.promoteGroupedDraftTask);
  ...
  const codePreviewSettings = useDroraStoreWithDefault(
  ...
  const dispatchCommand = useCallback(
    async (
      ...
      const groupedDraftTask =
        type === "createSession"
          ? useDroraSessionStore.getState().getWorkspaceState(workspacePath, workspaceIdentity)
              .groupedDraftTask
          : null;
    ...
    async (reference?: ConversationSelectionReference, forceNew = false) => {
      ...
      try {
        ...
        if (targetChildSessionId) {
          try {
            await droraSessionService.readSession({
              workspacePath,
              ...(workspaceIdentity ? { workspaceIdentity } : {}),
  ...
  const dispatchSendText = useCallback(
    (text: string, options?: ConversationComposerSendOptions) => {
      const createSource = useDroraSessionStore
        .getState()
    ...
    async (
      ...
      try {
        ...
        setSendSubmissionError({
          code: runtimeModelUnavailable ? "DRORA_RUNTIME_MODEL_UNAVAILABLE" : "SEND_FAILED",
  ...
  const handleRecoverCustomModelSelection = useCallback(
    async (value: string, sourceModel: ModelSelectionSource | null) => {
      ...
      try {
        await droraTaskService.restartWorkspaceProcess({
          workspacePath,
          workspaceIdentity,
          ...
          >
            <ConversationTimeline
              ...
              headerSlot={
                ...
                // rows 为空，但只读块必须留下来显示「需要更新 Drora」，不能整块消失。
                importedShare &&
