...
import type { DroraElicitationRequest, DroraPermissionOption, DroraProvider } from "@drora/shared";
...
} from "@/store/droraSessionStore.js";
import type { ElicitationFormDraft } from "@/store/droraSessionStoreTypes.js";
...
function resolveV4ElicitationRequest(
  projected: DroraElicitationRequest | null,
  botProgress: DroraElicitationRequest | null,
): DroraElicitationRequest | null {
...
function buildV4ElicitationProgressKey(request: DroraElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  const notificationEnabled = useDroraStoreWithDefault((state) => state.notificationEnabled, true);
  const botElicitationProgress = useDroraSessionStore(
    (state) =>
      getTaskUiState(getWorkspaceState(state, workspacePath, workspaceIdentity), sessionId)
        .elicitationRequest,
  );
  const localElicitationDraft = useDroraSessionStore((state) => {
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useDroraSessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: DroraPermissionOption, feedback?: string) => {
