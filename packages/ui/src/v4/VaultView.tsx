/* eslint-disable max-lines -- Vault 面板按 Proma VaultView 整面板保真移植：树/查看编辑/授权引导共用同一套组件内状态，沿用单文件边界（BotsDialog/GitPane 先例），后续按功能边界单独拆分。 */
/**
 * VaultView —— Proma Obsidian Vault 面板的 Drora 移植版（重写上一轮 textarea 起步版）。
 *
 * 对照 Proma `components/vault/VaultView.tsx` 逐行适配的三处差异：
 * 1. jotai atomFamily → module 级 Map + useSyncExternalStore 的轻量 family store
 *    （per-key 隔离、刷新令牌全局失效、选中文件 per-session 语义不变）；
 * 2. window.electronAPI IPC → IObsidianVaultService RPC（packages/services/src/obsidian-vault，
 *    签名以本仓服务实现为准）；
 * 3. side-chat 联动 → 本仓 selectionSideChat 机制
 *    （dispatchConversationSelectionAdd 注入当前任务输入；
 *    requestSelectionSideChatOpen 打开右侧问答，opener 由 SessionPane 注册）。
 * 其余编辑器、滚动记忆、文档控制器、树模型等逻辑保持 Proma 原实现。
 */
...
import type {
  ObsidianVaultCandidate as VaultCandidate,
  ObsidianVaultFocus as VaultFocus,
  ObsidianVaultReadResult as VaultReadResult,
  ObsidianVaultSummary as VaultSummary,
  ObsidianVaultTreeEntry as VaultTreeEntry,
} from "@drora/services";
import {
...
import { resolveVaultWikiLink } from "./vault/vault-wikilinks.js";
import { VaultLiveMarkdownEditor } from "./vault/VaultLiveMarkdownEditor.js";
import { VaultNoteTitle } from "./vault/VaultNoteTitle.js";
import { focusVaultBody } from "./vault/vault-title-focus.js";
import { commitVaultTitle } from "./vault/vault-title-commit.js";
import { useVaultScrollMemory } from "./vault/useVaultScrollMemory.js";
import { getVaultScrollKey } from "./vault/vault-scroll-memory.js";
import type {
  LiveMarkdownEditorHandle,
  LiveMarkdownTextSelection,
} from "@/components/markdown/LiveMarkdownEditor.js";
import { VaultContentErrorBoundary } from "./vault/VaultContentErrorBoundary.js";
import { getVaultEditorKey, shouldRemountVaultEditor } from "./vault/vault-editor-lifecycle.js";
import { getVaultDocumentController } from "./vault/vault-document-controller.js";
import {
  buildVaultTree,
  getInitialVaultExpandedFolders,
  getVaultFolderAncestors,
  hasSameVaultTreeEntries,
  type VaultFileEntry,
  type VaultFolderNode,
} from "./vault/vault-tree-model.js";
import {
...
interface VaultStoreCell<T> {
  value: T;
  listeners: Set<() => void>;
}

/**
 * jotai atomFamily 的最小等价物：module 级 Map 持有 per-key 单元，
 * 组件经 useSyncExternalStore 订阅。语义对齐：per-key 隔离、跨实例共享、
 * 刷新令牌全局失效、选中文件 per-session。
 */
function createVaultStoreFamily<T>(initial: T) {
  const cells = new Map<string, VaultStoreCell<T>>();
  const getCell = (key: string): VaultStoreCell<T> => {
    let cell = cells.get(key);
    if (!cell) {
      cell = { value: initial, listeners: new Set() };
      cells.set(key, cell);
    }
    return cell;
  };
  return function useVaultStoreValue(key: string): [T, (next: T | ((previous: T) => T)) => void] {
    ...
    const set = React.useCallback(
      (next: T | ((previous: T) => T)) => {
        const resolved =
          typeof next === "function" ? (next as (previous: T) => T)(cell.value) : next;
        if (resolved === cell.value) return;
        cell.value = resolved;
...
function getVaultSessionScope(sessionId?: string): string {
  return sessionId ?? "__standalone_vault__";
}

const useSelectedVaultFileFamily = createVaultStoreFamily<string | null>(null);
const useFocusedVaultFolderFamily = createVaultStoreFamily<string | null>(null);
const useVaultReadResultFamily = createVaultStoreFamily<VaultReadResult | null>(null);
const useVaultRefreshToken = createVaultStoreFamily<number>(0);
const VAULT_REFRESH_TOKEN_KEY = "__global__";

// ---- toast 适配：Proma 的 sonner 风格调用点原样保留，落到本仓 toast 组件 ----
...
const toast = {
  ...
  info: (message: string): void => {
    droraToast(message, { variant: "info" });
  },
...
// ---- 选区动作浮层（对照 Proma components/selection/SelectionActionPopover，仅保留 Vault 使用面） ----
...
function SelectionActionPopover({
  ...
  const horizontalTransform = alignRight
    ? "-translate-x-full"
    : alignLeft
      ? "translate-x-0"
      : "-translate-x-1/2";
...
// ---- 确认弹窗（对照 Proma components/ui/confirm-dialog，落在本仓 AlertDialog 上） ----
...
function ConfirmDialog({
  ...
  return (
    <AlertDialog
      open={open}
      onOpenChange={(next) => {
        if (!next && !loading) onOpenChange(next);
      }}
    >
      <AlertDialogContent>
        <AlertDialogHeader>
...
function getVaultCandidateDisplayName(candidate: VaultCandidate): string {
  return candidate.isDroraManaged ? DRORA_MANAGED_VAULT_DISPLAY_NAME : candidate.displayName;
}
...
function isVaultFileNotFoundError(error: unknown): boolean {
  // Drora 服务面（vault-fs）对缺失笔记抛出的错误文案含「不存在」。
...
function VaultMarkdownEditor({
  ...
  vaultId: string;
  /** 当前 Agent 会话与工作区（接入本仓引用注入与右侧问答）；无会话时选区动作降级提示。 */
  ...
  const sessionId = selectionTarget?.sessionId ?? undefined;
  ...
  const { onEditorReady: restoreEditorScroll, takeOver: takeOverScrollRestore } =
    useVaultScrollMemory({
  ...
  React.useEffect(() => {
    if (
      !editorReady ||
      !bodyFocusRequest ||
      bodyFocusRequest.vaultId !== vaultId ||
      bodyFocusRequest.relativePath !== readResult.relativePath
    )
      return;
  ...
  // 右侧问答入口由已挂载的主 SessionPane 注册；Vault 面板只按 workspace + 父会话路由，
  // 可用性由 requestSelectionSideChatOpen 的返回值兜底。
  const sideChatKey =
    selectionTarget?.sessionId && selectionTarget.workspaceKey
      ? buildSelectionSideChatKey(selectionTarget.workspaceKey, selectionTarget.sessionId)
      : null;

  const clearSelection = React.useCallback(() => setSelection(null), []);
  const handleTextSelectionChange = React.useCallback(
    (nextSelection: LiveMarkdownTextSelection | null) => {
      if (!nextSelection) {
        clearSelection();
        return;
      }
  ...
  const createSelectionReference = React.useCallback(() => {
    if (!selection || !readResult) return null;
    return createConversationSelectionReference({
      contentType: "markdown",
      sourceKey: `vault:${readResult.relativePath}`,
      sourceTitle: displayDocumentTitle(
        readResult.relativePath.split("/").pop() ?? readResult.relativePath,
      ),
  ...
  const handleEditorPageWheel = (event: React.WheelEvent<HTMLDivElement>): void => {
    ...
    const scroller = editorPageRef.current?.querySelector<HTMLElement>(
      ".vault-ink-mde .cm-scroller",
    );
    if (!scroller) return;
  ...
  const flushPendingSave = React.useCallback(
    (): Promise<boolean> => saveLatest(true),
    [saveLatest],
  );
          ...
          {saveConflict && (
            <div className="flex shrink-0 items-center gap-1.5 text-xs text-destructive">
              ...
              <button
                type="button"
                onClick={() => {
                  documentController.discardLocalDraft();
                  onReload();
                }}
                className="rounded px-1.5 py-1 hover:bg-destructive/10"
              >
  ...
  return (
    <section className="flex min-w-0 flex-1 flex-col bg-muted/25">
      <VaultContentErrorBoundary
        resetKey={getVaultEditorKey(readResult.relativePath, reopenVersion)}
      >
        <VaultMarkdownEditor
...
export function VaultView({
  ...
  /** 预留：嵌入 Agent 右侧工作区时的紧凑模式。 */
  embedded?: boolean;
  /** 当前 Agent 会话；提供后接入焦点上报与选区引用注入。 */
  sessionId?: string;
  ...
  const [bodyFocusRequest, setBodyFocusRequest] = React.useState<VaultBodyFocusRequest | null>(
    null,
  );
  ...
  const [selectedFile, setSelectedFile] = useSelectedVaultFileFamily(vaultSessionScope);
  const [focusedFolder, setFocusedFolder] = useFocusedVaultFolderFamily(vaultSessionScope);
  const [readResult, setReadResult] = useVaultReadResultFamily(vaultSessionScope);
  const [refreshToken, setRefreshToken] = useVaultRefreshToken(VAULT_REFRESH_TOKEN_KEY);
  ...
  const [vaultTreeAction, setVaultTreeAction] = React.useState<{
    type: "expand" | "collapse";
    version: number;
  }>({ type: "collapse", version: 0 });
  ...
  const flushCurrentEditor = React.useCallback(
    async (): Promise<boolean> => (editorFlushRef.current ? editorFlushRef.current() : true),
    [],
  );
  ...
  React.useLayoutEffect(() => {
    ...
    const target = vaultSidebarCollapsed
      ? vaultSidebarExpandButtonRef
      : vaultSidebarCollapseButtonRef;
  ...
  const handleVaultSidebarResizeStart = React.useCallback(
    (event: React.MouseEvent<HTMLDivElement>): void => {
      ...
      const maxWidth = Math.max(
        ...
        Math.min(VAULT_SIDEBAR_MAX_WIDTH, window.innerWidth - 320),
      );

      const onMouseMove = (moveEvent: MouseEvent): void => {
        const nextWidth = Math.min(
          maxWidth,
          Math.max(VAULT_SIDEBAR_MIN_WIDTH, startWidth + moveEvent.clientX - startX),
        );
      ...
      const cleanup = (): void => {
        ...
        if (vaultSidebarDragCleanupRef.current === cleanup)
          vaultSidebarDragCleanupRef.current = null;
      };
  ...
  const updateAgentFocus = React.useCallback(
    (focus: Omit<VaultFocus, "sequence"> | null): void => {
      if (!sessionId || !vaultService) return;
      const next = focus ? { ...focus, sequence: ++focusSequenceRef.current } : null;
      // 焦点是展示性上下文；目标被外部删除等服务端校验失败时静默放弃。
      void vaultService.setUserContext({ sessionId, focus: next }).catch(() => undefined);
    },
    [sessionId, vaultService],
  );

  React.useEffect(() => {
    if (!sessionId || !vaultService) return;
    return () => {
      void vaultService.setUserContext({ sessionId, focus: null }).catch(() => undefined);
    };
  }, [sessionId, vaultService]);
  ...
  const refresh = React.useCallback(
    async ({ showLoading = false }: { showLoading?: boolean } = {}): Promise<void> => {
      ...
      try {
        const nextConfig = await vaultService.getSummary();
        setConfig(nextConfig);
        const nextEntries = nextConfig ? await vaultService.listFiles() : [];
        setEntries((current) =>
        ...
        if (!nextConfig) {
          ...
          if (
            !nextEntries.some(
              (entry) => entry.kind === "file" && entry.relativePath === relativePath,
            )
          ) {
            selectFile(null);
            setReadResult(null);
          ...
          try {
            const result = await vaultService.readFile({ relativePath });
    ...
    [selectFile, setReadResult, vaultService],
  );
  ...
  // RPC. Poll only the currently open note, never the whole Vault tree, so this
  ...
  React.useEffect(() => {
    ...
    const checkCurrentFile = async (): Promise<void> => {
      if (checking || cancelled || !vaultService || selectedFileRef.current !== relativePath)
        return;
      checking = true;
      try {
        const next = await vaultService.readFile({ relativePath });
        if (!cancelled && selectedFileRef.current === relativePath && next.sha256 !== sha256)
          setReadResult(next);
      } catch {
  ...
  }, [readResult?.relativePath, readResult?.sha256, setReadResult, vaultService]);

  const refreshVaultCandidates = React.useCallback(async (): Promise<void> => {
    if (!vaultService) return;
    setCandidatesLoading(true);
    try {
      setCandidates(await vaultService.listCandidates());
    } catch {
      setCandidates([]);
    } finally {
  ...
  const openFile = React.useCallback(
    async (
      relativePath: string,
      {
        ...
        forceReopen = false,
      }: { discardLocalDraft?: boolean; forceReopen?: boolean } = {},
    ): Promise<void> => {
      if (!vaultService) return;
      if (!discardLocalDraft && !(await flushCurrentEditor())) return;
      const remountEditor = shouldRemountVaultEditor(
    ...
    [flushCurrentEditor, refresh, selectFile, setReadResult, vaultService],
  );

  const openWikiLink = React.useCallback(
    (target: string): void => {
      if (!readResult) return;
      const path = resolveVaultWikiLink(
        target,
        readResult.relativePath,
  ...
  const selectVaultManually = async (): Promise<void> => {
    ...
    // Proma 的 selectVault 在主进程弹目录选择框；Drora 的服务面只接受显式 rootPath，
    // 目录选择由 IPlatformService.selectDirectory 承担。
    const rootPath = await platform.selectDirectory();
    if (!rootPath) return;
    const selected = await vaultService.configureVault({
      rootPath,
      options: { allowAgentWrites: false },
    });
    setConfig(selected);
    selectFile(null);
    setReadResult(null);
  ...
  const createDroraVault = async (): Promise<void> => {
    ...
    if (!(await flushCurrentEditor())) return;
    try {
      const selected = await vaultService.selectManagedVault({});
      setConfig(selected);
      selectFile(null);
      setReadResult(null);
      setVaultSwitcherOpen(false);
  ...
  const connectDiscoveredVault = async (candidate: VaultCandidate): Promise<void> => {
    ...
    if (!(await flushCurrentEditor())) return;
    try {
      const selected = candidate.isDroraManaged
        ? await vaultService.selectManagedVault({})
        : await vaultService.authorizeCandidate({
            path: candidate.path,
            options: { allowAgentWrites: false },
          });
      setConfig(selected);
  ...
  const createNote = async (): Promise<void> => {
    if (!config || !vaultService) return;
    try {
      const result = await vaultService.createUntitledFile();
      if (!result.ok) return;
      setRefreshToken((value) => value + 1);
  ...
  const createNoteInFolder = async (folderPath: string): Promise<void> => {
    if (!config || !vaultService) return;
    try {
      const result = await vaultService.createUntitledFileInFolder({ folderPath });
      if (!result.ok) return;
      setRefreshToken((value) => value + 1);
      await openFile(result.relativePath);
    } catch (error) {
  ...
  const createFolder = async (): Promise<void> => {
    if (newFolderParentPath === null || !vaultService) return;
    const name = newFolderName.trim();
    if (!name) return;
    setCreatingFolder(true);
    try {
      const relativePath = newFolderParentPath ? `${newFolderParentPath}/${name}` : name;
      await vaultService.createFolder({ relativePath });
  ...
  const save = React.useCallback(
    async (
      ...
      try {
        ...
        const nextEntries = await vaultService.listFiles();
        setEntries((current) =>
          hasSameVaultTreeEntries(current, nextEntries) ? current : nextEntries,
        );
    ...
    [setReadResult, vaultService],
  );

  const rename: VaultRename = async (name, flush, shouldFocusBody) => {
    if (!readResult || !config || !vaultService) return false;
    ...
    try {
      const result = await commitVaultTitle({
        name,
        title: displayDocumentTitle(relativePath.split("/").pop() ?? relativePath),
        relativePath,
        flush,
        isVaultCurrent,
        isNoteCurrent: () => selectedFileRef.current === relativePath,
        read: (path) => vaultService.readFile({ relativePath: path }),
        rename: (input) => vaultService.renameFile(input),
      });
      ...
      if (result.isNoteCurrent && selectedFileRef.current === relativePath) {
        ...
        if (shouldFocusBody?.())
          setBodyFocusRequest({
            vaultId,
            relativePath: result.renamed?.relativePath ?? relativePath,
          });
      }
      if (result.renamed) {
        setRefreshToken((value) => value + 1);
  ...
  const deleteNote = async (): Promise<void> => {
    if (!deleteTarget || deleting || !vaultService) return;
    ...
    try {
      ...
      const current = deletingCurrentFile
        ? await vaultService.readFile({ relativePath: deleteTarget.relativePath })
        : null;
      await vaultService.deleteFile({
      ...
      if (deletingCurrentFile) {
        ...
        if (sessionId) await vaultService.setUserContext({ sessionId, focus: null });
      }
  ...
  return (
    <>
      <main
        className={cn(
          "flex h-full min-h-0 flex-col bg-muted/25",
                  ...
                  <PopoverContent side="top" align="start" className="w-72 p-1.5">
                      ...
                      {candidatesLoading ? (
                        ...
                        candidates.map((candidate) => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              void connectDiscoveredVault(candidate);
                            }}
                          ...
                          >
                            ...
                            <span className="min-w-0 flex-1 truncate">
                              {getVaultCandidateDisplayName(candidate)}
                            </span>
                    ...
                    <div className="mt-1 border-t border-border/60 pt-1">
                      ...
                      >
                        ...
                        创建 {DRORA_MANAGED_VAULT_DISPLAY_NAME}
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                          void selectVaultManually();
                        }}
                        className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-left text-sm text-foreground transition-colors hover:bg-muted"
                      >
        ...
        <DialogContent>
          ...
          <DialogFooter>
            <Button
              ...
              onClick={() => setNewFolderParentPath(null)}
            >
              取消
            </Button>
            <Button
