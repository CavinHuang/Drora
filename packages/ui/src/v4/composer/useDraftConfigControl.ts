...
import { DRORA_AGENT_PROVIDER, resolveExecutionState } from "@drora/shared";
...
import type { SessionConfigState } from "@drora/shared/drora-protocol-v4";
import type { IModelSelectionService } from "@drora/services";
...
import { submissionModeSchema } from "@drora/shared/drora-protocol-v4";
...
function shouldHydrateWorkspaceCatalog(params: {
  ...
  slashCommands: readonly DroraSlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const droraSessionService = useDroraSessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithDroraSessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useDroraSessionStore.getState();
