// sessions-index 的 SessionSummary → 侧栏实时 detail DroraTaskMeta 映射。
...
import type { TraceId, DroraProvider, DroraTaskMeta } from "@drora/shared";
...
function phaseToStatus(phase: SessionSummary["phase"]): DroraTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
/**
 * SessionSummary → 实时 detail DroraTaskMeta。sessions-index 不携带的字段
