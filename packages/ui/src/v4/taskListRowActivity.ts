...
const TASK_LIST_ROW_ACTIVITY_FIELD = "__droraSessionActivity" as const;
...
export type TaskListMetaWithActivity = DroraTaskMeta & {
  [TASK_LIST_ROW_ACTIVITY_FIELD]: TaskListRowActivity;
};

export function attachTaskListRowActivity<T extends DroraTaskMeta>(
  task: T,
...
export function getTaskListRowActivity(task: DroraTaskMeta): TaskListRowActivity | null {
...
function isTaskListRowRunning(task: DroraTaskMeta): boolean {
 ...
 */
export function isTaskListRowActive(task: DroraTaskMeta): boolean {
...
export function mergeTaskListMembershipFields(
  activityTask: DroraTaskMeta,
  membershipTask: DroraTaskMeta,
): DroraTaskMeta {
