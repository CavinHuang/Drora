...
import { resolveWorkspaceTelemetryDetail, type IPlatformService } from "@drora/shared";
import { createConversationTelemetryService, type IServiceAccessor } from "@drora/services";
...
function acquireSupervisor(
  ...
  const key = attachmentScopeKey(scope, services.droraAgentService);
...
function ensureSupervisorSubscription(
  ...
  const telemetryService = createConversationTelemetryService(services.droraAgentService);
...
export function ConversationTelemetryWorkspaceAttachment({
  ...
  const lease = useMemo(() => {
    const agentService = services.droraAgentService as object | null | undefined;
