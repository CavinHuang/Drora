...
// 复用 treemappingActivity 的既有解析规则，不复活 droraChatMessages。
//
...
import type { ConversationRow, ToolCallRow } from "@drora/shared/drora-protocol-v4";
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(droraAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [droraAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
