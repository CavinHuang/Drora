import type { ObsidianVaultReadResult as VaultReadResult } from "@drora/services";
...
export class VaultDocumentController {
  ...
  observeRemote(next: VaultReadResult): VaultDocumentRemoteDisposition {
    ...
    const alreadyConflicted =
      this.snapshot.conflict && this.snapshot.remoteConflict?.sha256 === next.sha256;
  ...
  private disposeIfUnobservedAndClean(): void {
    if (
      ...
      this.snapshot.draft === this.snapshot.base.content
    ) {
...
export function getVaultDocumentController(
  initial: VaultReadResult,
  vaultScope: string,
