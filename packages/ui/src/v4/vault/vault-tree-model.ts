import type { ObsidianVaultTreeEntry as VaultTreeEntry } from "@drora/services";

/** Drora 服务面没有独立的 file entry 类型；从树条目联合中取 file 分支（对应 Proma 的 VaultFileEntry）。 */
export type VaultFileEntry = Extract<VaultTreeEntry, { kind: "file" }>;
...
export function buildVaultTree(entries: VaultTreeEntry[]): VaultFolderNode {
  ...
  for (const entry of entries) {
    ...
    const parent =
      separatorIndex < 0 ? root : getOrCreateFolder(entry.relativePath.slice(0, separatorIndex));
 ...
 */
export function hasSameVaultTreeEntries(
  current: VaultTreeEntry[],
  next: VaultTreeEntry[],
): boolean {
  return (
