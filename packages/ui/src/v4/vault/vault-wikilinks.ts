...
import { getLeadingFrontmatterRange } from "../../components/markdown/live-markdown-frontmatter.js";
...
export function resolveVaultWikiLink(
  target: string,
  source: string,
...
export function canRenderVaultWikiLink(state: EditorState, from: number): boolean {
  ...
  for (let node = syntaxTree(state).resolveInner(from, 1); node; node = node.parent!) {
    if (/^(FencedCode|CodeBlock|InlineCode|HTMLBlock|HTMLTag|Link|Image)$/.test(node.name))
      return false;
  }
...
class VaultWikiLinkWidget extends WidgetType {
  ...
  override eq(other: VaultWikiLinkWidget): boolean {
    return (
      this.target === other.target && this.label === other.label && this.onOpen === other.onOpen
    );
  }
...
export function createVaultWikiLinks(onOpen: (target: string) => void): Extension {
  const matcher = new MatchDecorator({
    ...
    decorate: (add, from, to, match, view) => {
      ...
      add(
        ...
        to,
        Decoration.replace({
          widget: new VaultWikiLinkWidget(target, match[2]?.trim() || target, onOpen),
        }),
      );
    },
  });
  return [
    ...
    EditorView.baseTheme({
      ...
      // Proma 原值 hsl(var(--ring)) 依赖 shadcn HSL 变量；Drora 的 ring token 在 @theme 下是完整颜色值。
      ".vault-wikilink:focus-visible": {
