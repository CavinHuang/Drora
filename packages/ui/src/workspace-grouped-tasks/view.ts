...
import type { DroraGroupedTaskView, DroraGroupedTaskViewNode } from "@drora/services";
import type { DroraTaskMeta } from "@drora/shared";
import type { GroupedDraftTaskPlacement } from "@/store/droraSessionStoreTypes.js";
import { taskKey } from "@/workspace-grouped-tasks/ids.js";

function cloneView(view: DroraGroupedTaskView): DroraGroupedTaskView {
  return {
...
function cloneNodes(view: DroraGroupedTaskView): DroraGroupedTaskViewNode[] {
  return [...view.nodes];
}

function cloneGroupNode(
  node: Extract<DroraGroupedTaskViewNode, { type: "group" }>,
): Extract<DroraGroupedTaskViewNode, { type: "group" }> {
...
function filterGroupedViewByTaskKeys(
  ...
  let changed = false;
  const nodes: DroraGroupedTaskView["nodes"] = [];
...
function getGroupedTaskGroupIds(view: DroraGroupedTaskView): string[] {
...
function getTaskGroupIdInView(view: DroraGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function findTopLevelTaskIndex(view: DroraGroupedTaskView, targetTaskKey: string): number {
  return view.nodes.findIndex(
    (node) => node.type === "task" && taskKey(node.task) === targetTaskKey,
  );
}

function findGroupIndex(view: DroraGroupedTaskView, groupId: string): number {
  return view.nodes.findIndex((node) => node.type === "group" && node.group.id === groupId);
}

function findGroupIdByTaskKey(view: DroraGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function insertTaskBeforeTopLevelNode(
  view: DroraGroupedTaskView,
  task: DroraTaskMeta,
  beforeNode: DroraGroupedTaskViewNode | null,
): DroraGroupedTaskView {
  const nodes = cloneNodes(view);
  const insertIndex =
    beforeNode === null
