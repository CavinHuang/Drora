...
import type { DroraGroupedTaskViewNode } from "@drora/services";
...
function getTopLevelNodeKey(node: DroraGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: DroraGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: DroraGroupedTaskViewNode, index: number) => ReactNode;
}) {
