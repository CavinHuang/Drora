...
export class BrowserOAuthCredentialRepo {
  ...
  saveTokenSet(tokenSet: WebZaiTokenSet | OAuthTokenSet, provider: WebOAuthProviderId): void {
    ...
    const droraJwtToken = tokenSet.droraJwtToken;

    this.localStorage.setItem(providerKeys(provider).accessToken, accessToken);
    if (droraJwtToken) {
      this.localStorage.setItem(DRORA_JWT_TOKEN_KEY, droraJwtToken);
    } else {
      this.localStorage.removeItem(DRORA_JWT_TOKEN_KEY);
    }
  }
  ...
  loadCachedSessionState(): OAuthCachedSessionRestoreResult {
    ...
    const droraJwtToken = this.localStorage.getItem(DRORA_JWT_TOKEN_KEY);
    ...
    if (!keys || !hasText(droraJwtToken) || !hasText(accessToken) || !hasText(rawUserInfo)) {
    ...
    if (resolveJwtExpiration(droraJwtToken, this.now()).kind === "expired") {
  ...
  loadDroraJwtToken(): string | null {
    const session = this.loadCachedSessionState();
    if (session.status !== "authenticated") return null;
    return this.localStorage.getItem(DRORA_JWT_TOKEN_KEY)?.trim() || null;
  }

  private hasAnyStoredCredential(): boolean {
    return Boolean(
      this.localStorage.getItem(ACTIVE_PROVIDER_KEY) ||
      this.localStorage.getItem(DRORA_JWT_TOKEN_KEY) ||
