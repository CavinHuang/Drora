...
function createWebZaiOAuthConfig(env: WebImportMetaEnv = {}): WebZaiOAuthConfig {
  ...
  const droraEndpointUrls = buildDroraEndpointUrls(
    env.VITE_DRORA_BASE_URL?.trim() ||
      env.VITE_DRORA_ENDPOINT_ORIGIN?.trim() ||
      DEFAULT_DRORA_ENDPOINT_ORIGIN,
  );
  ...
  return {
    ...
    // BigModel 用 appId 而不是 client_id，且默认值就是桌面端在用的 "drora"。
    bigmodelAppId: env.VITE_BIGMODEL_OAUTH_APP_ID?.trim() || "drora",
    redirectUri: droraEndpointUrls.webShareCallbackUrl,
    shareRedirectUri: droraEndpointUrls.webShareCallbackUrl,
    ...(devOrigin ? { devOrigin } : {}),
