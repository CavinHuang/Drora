...
import { connectViaWebSocket } from "@drora/client";
...
import type { IPlatformService, RemoteTarget, ServerRemoteInfo } from "@drora/shared";
import { WEB_DEFAULT_THEME, resolveWebInitialTheme } from "./webThemeSeed.js";

function resolveWebThemePreference(defaultTheme: Theme = WEB_DEFAULT_THEME): Theme {
  const saved = localStorage.getItem("drora-theme");
...
async function renderConversationSharePage(): Promise<void> {
  ...
  document.title = routeLocale === "zh-CN" ? "Drora 会话分享" : "Drora Conversation Share";
  ...
  const endpointOrigin =
    import.meta.env.VITE_DRORA_BASE_URL?.trim().replace(/\/+$/u, "") || window.location.origin;
  const mockMode =
  ...
  const getMockToken = () =>
    mockMode && window.sessionStorage.getItem("drora:share:mock-auth") === "owner"
      ? "mock-owner-token"
      : null;
  const onLogout = () => {
    if (mockMode) {
      window.sessionStorage.removeItem("drora:share:mock-auth");
  ...
  root.render(
    <ConversationShareLandingLoader
      shareCode={shareCode}
      client={client}
      getAccessToken={() => getMockToken() ?? webAuthService.getDroraJwtToken()}
      onLogin={(provider) => {
        if (mockMode) {
          window.sessionStorage.setItem("drora:share:mock-auth", "owner");
          window.location.reload();
          return;
        }
...
async function bootstrapWebApp() {
  ...
  try {
    ...
    document.title = "Drora - Web + Server";
