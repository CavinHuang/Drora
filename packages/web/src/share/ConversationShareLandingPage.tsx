...
import type { ConversationSharePreview } from "@drora/shared";
import { ConversationShareReadonlyTimeline } from "@drora/ui/conversation-share-readonly";
import { renderOAuthProviderIcon } from "@drora/ui/oauth-provider-icon";
import { applyTheme, resolveTheme, type Theme } from "@drora/ui/useTheme";
...
interface Copy {
  ...
  retry: string;
  continueInDrora: string;
  switchToDarkTheme: string;
  switchToLightTheme: string;
  continueHelp: string;
  downloadDrora: string;
  retryOpen: string;
...
const DRORA_DOWNLOAD_URL = "https://zcode.z.ai";
...
const COPY: Record<ConversationShareLandingLocale, Copy> = {
  "zh-CN": {
    ...
    outdatedTitle: "需要更新 Drora",
    outdatedDescription: "这个分享由更新版本的 Drora 创建，请升级后再查看。",
    unavailableTitle: "分享不可访问",
    unavailableDescription: "当前账号没有权限，或者分享内容已不存在。",
    retry: "重试",
    continueInDrora: "去 Drora 继续",
    switchToDarkTheme: "切换到深色主题",
    switchToLightTheme: "切换到浅色主题",
    continueHelp: "如果没有自动打开 Drora，请先下载客户端，或再次尝试打开。",
    downloadDrora: "下载 Drora",
    artifactCountOne: "{count} 个结果物",
    artifactCountOther: "{count} 个结果物",
    retryOpen: "再次打开",
  },
  "en-US": {
    brand: "Drora Conversation Share",
    ...
    outdatedTitle: "Update Drora to continue",
    outdatedDescription:
      "This share was created by a newer version of Drora. Please update to view it.",
    unavailableTitle: "Share unavailable",
    unavailableDescription:
      "This account is not allowed to view the share, or it no longer exists.",
    retry: "Try again",
    continueInDrora: "Continue in Drora",
    switchToDarkTheme: "Switch to dark theme",
    switchToLightTheme: "Switch to light theme",
    continueHelp: "If Drora did not open, download the app or try opening it again.",
    downloadDrora: "Download Drora",
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {importLink && headerView.continueVisible ? (
                      <a
                        ...
                        href={importLink}
                        aria-label={copy.continueInDrora}
                        title={copy.continueInDrora}
                        onClick={handleContinue}
                      >
                        <span className={headerView.continueCompact ? "sr-only" : "truncate"}>
                          {copy.continueInDrora}
                        </span>
                        <ArrowUpRightIcon
                          className={
                            headerView.continueCompact
                  ...
                  >
                    ...
                    {importLink ? (
                      <span className={SHARE_CONTINUE_MEASURE_CLASS}>
                        <span>{copy.continueInDrora}</span>
                        <ArrowUpRightIcon
        ...
        {importLink && showContinueHelp ? (
          ...
          <ShareContentInset>
            ...
            >
              ...
              <a className="text-brand underline underline-offset-2" href={DRORA_DOWNLOAD_URL}>
                {copy.downloadDrora}
              </a>
            </div>
          </ShareContentInset>
          ...
          <ShareContentInset>
            ...
            >
              <a
                ...
                onClick={handleContinue}
              >
                <span className="truncate">{copy.continueInDrora}</span>
...
export function ConversationShareLandingLoader({
  ...
  const handleThemeChange = useCallback((nextTheme: Theme) => {
    localStorage.setItem("drora-theme", nextTheme);
