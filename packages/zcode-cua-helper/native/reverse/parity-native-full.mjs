...
try {
  // —— 0. 激活(用原版激活一次,给输入类一个稳定前台)——
  {
    ...
    check("activateApplication return shape", JSON.stringify(a) === JSON.stringify(b),
  ...
  {
    const sameShape = (a, b) => {
      const ta = Buffer.isBuffer(a) ? "png" : a === null ? "null" : typeof a === "object" ? JSON.stringify(a) : String(a);
      const tb = Buffer.isBuffer(b) ? "png" : b === null ? "null" : typeof b === "object" ? JSON.stringify(b) : String(b);
    ...
    check("captureApp returns AX snapshot (both)", Boolean(ca?.elements) && Boolean(cb?.elements),
      `orig keys=${Object.keys(ca ?? {})} rest keys=${Object.keys(cb ?? {})}`);
    check("captureApp window fields", JSON.stringify(ca?.window) === JSON.stringify(cb?.window),
      `orig=${JSON.stringify(ca?.window)} rest=${JSON.stringify(cb?.window)}`);
    check("captureApp app fields", JSON.stringify(ca?.app) === JSON.stringify(cb?.app),
      `orig=${JSON.stringify(ca?.app)} rest=${JSON.stringify(cb?.app)}`);
    check("captureApp element shapes",
      ca?.elements?.length === cb?.elements?.length && JSON.stringify(ca?.elements?.[0]) === JSON.stringify(cb?.elements?.[0]),
  ...
  // —— 2. AX 写路径(交叉验证:orig 写 → rest 读;rest 写 → orig 读)——
  {
    ...
    check("AX setValue(orig) cross-read by rest", crossRead1 === mark1,
    ...
    check("AX setValue(rest) cross-read by orig", crossRead2 === mark2,
    ...
    check("AX setFocused return shape", JSON.stringify(sf1) === JSON.stringify(sf2),
    ...
    check("AX selectText return shape", JSON.stringify(st1) === JSON.stringify(st2),
  ...
  // —— 3. 键盘注入(注入到前台记事本,交叉经 AX 读回)——
  {
    ...
    check("typeTextGlobal(orig) visible via rest AX", String(el?.value ?? "").includes(k1),
    ...
    check("typeTextGlobal(rest) injects (cross-read by orig)", injected,
    ...
    check("keyDown/UpGlobal return shapes", JSON.stringify([d1, u1]) === JSON.stringify([d2, u2]),
    ...
    const h2v = await Promise.resolve(h2).then((x) => x).catch(() => x);
    check("holdKey(Async) return shapes", JSON.stringify(h1) === JSON.stringify(h2v),
  ...
  // —— 4. 鼠标注入 ——
  {
    ...
    const tx = npBounds.x + 40, ty = npBounds.y + 120;
    ...
    check("moveTo(orig) moves cursor", Math.abs(p1.x - tx) <= 2 && Math.abs(p1.y - ty) <= 2, `at=${JSON.stringify(p1)} want=${tx},${ty}`);
    check("moveTo(rest) moves cursor", Math.abs(p2.x - (tx + 60)) <= 2 && Math.abs(p2.y - ty) <= 2, `at=${JSON.stringify(p2)} want=${tx + 60},${ty}`);
    check("moveTo return shapes", JSON.stringify(m1) === JSON.stringify(m2), `orig=${JSON.stringify(m1)} rest=${JSON.stringify(m2)}`);
    ...
    check("clickAtPoint return shapes", JSON.stringify(c1) === JSON.stringify(c2), `orig=${JSON.stringify(c1)} rest=${JSON.stringify(c2)}`);
    ...
    check("mouseDown/Up return shapes", JSON.stringify([md1, mu1]) === JSON.stringify([md2, mu2]),
    ...
    check("drag return shapes", JSON.stringify(d1r) === JSON.stringify(d2r), `orig=${JSON.stringify(d1r)} rest=${JSON.stringify(d2r)}`);

    // scrollAt(本机恒 false,与原版一致)
    const s1 = await orig.scrollAt(cx, cy2, -120, "down");
    const s2 = await rest.scrollAt(cx, cy2, -120, "down");
    check("scrollAt return (both false on this machine)", JSON.stringify(s1) === JSON.stringify(s2), `orig=${JSON.stringify(s1)} rest=${JSON.stringify(s2)}`);
  }
  ...
  // —— 5. 图标与 epoch ——
  {
    ...
    check("applicationIconPngAsync shapes", JSON.stringify(pngInfo(i1)) === JSON.stringify(pngInfo(i2)),
  ...
  // —— 6. 深层能力 ——
  {
    ...
    if (zc) {
      ...
      check("captureApp(ZCode) elements count equal (BFS cap)",
        sa?.elements?.length === sb?.elements?.length,
        `orig=${sa?.elements?.length} rest=${sb?.elements?.length}`);
      check("captureApp(ZCode) first element equal",
    ...
    const menuBar = await orig.elementAtPoint(npBounds.x + Math.round(npBounds.width * 0.08), npBounds.y + 60);
    if (menuBar?.actions?.includes("AXPress") || menuBar?.role === "MenuItem") {
      ...
      await sleep(150);
      const menuBar2 = await rest.elementAtPoint(npBounds.x + Math.round(npBounds.width * 0.08), npBounds.y + 60);
      const pa2 = menuBar2 ? await rest.performAction(menuBar2.ref, "AXPress") : null;
      await sleep(250);
      await rest.pressKeyGlobal("escape");
      check("performAction(AXPress menu) shape", JSON.stringify(pa1) === JSON.stringify(pa2),
    ...
    check("performAction(unknown) → action_unsupported", JSON.stringify(u1) === JSON.stringify(u2) && u1?.axError === "action_unsupported",
      `orig=${JSON.stringify(u1)} rest=${JSON.stringify(u2)}`);
    const nonEdit = orig.captureApp(npPid).elements.find((e) => e.editable === false && e.role !== "Document");
    if (nonEdit) {
      const s1 = await orig.setValue(nonEdit.ref, "x");
      const s2 = await rest.setValue(nonEdit.ref, "x");
      check("setValue(non-edit) shape", JSON.stringify(s1) === JSON.stringify(s2),
    ...
    check("clickAtPoint(clicks=2)", JSON.stringify(dbl1) === JSON.stringify(dbl2), `orig=${dbl1} rest=${dbl2}`);
    check("cancelInputHoldsForSession → false (both)",
    ...
    const tryCall = async (m, name, ...args) => {
      try { const r = await m[name](...args); return { thrown: false, value: JSON.stringify(r)?.slice(0, 60) }; }
      catch (e) { return { thrown: true, message: String(e?.message) }; }
    };
    for (const [name, args] of [
      ...
      check(`validation2: ${name}(${JSON.stringify(args).slice(1, -1)})`,
  ...
  // —— 7. AUMID 面(notepad 在 Win11 为 packaged;有 AUMID 则测,无则记录跳过)——
  {
    ...
    if (aumid) {
      ...
      check("applicationInfoByAumid(notepad)", JSON.stringify(info1) === JSON.stringify(info2),
    ...
    check("applicationInfoByAumid(invalid)→null", JSON.stringify(bad1) === JSON.stringify(bad2),
      `orig=${JSON.stringify(bad1)} rest=${JSON.stringify(bad2)}`);
  }
} finally {
  console.log("[teardown] closing notepad…");
  notepad.kill();
  try { process.kill(npPid); } catch {}
}
