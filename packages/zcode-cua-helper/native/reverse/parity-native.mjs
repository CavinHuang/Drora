...
// —— 1. 导出面 ——
{
  ...
  check("export names", JSON.stringify(on) === JSON.stringify(rn),
    `orig-only:[${on.filter((n) => !rn.includes(n))}] rest-only:[${rn.filter((n) => !on.includes(n))}]`);
  let typeOk = true;
  const typeDiff = [];
  for (const n of on) {
    if (typeof orig[n] !== typeof rest[n]) { typeOk = false; typeDiff.push(n); }
  }
...
// —— 2. 参数校验 ——
const badCalls = {
  applicationInfo: ["x"], applicationInfoByAumid: [42], captureApp: ["x"],
  readElement: [42], elementAtPoint: ["x", "y"], moveTo: ["x", 1],
  scrollAt: [1, 2, "x"], mouseDown: [42], mouseUp: [42], drag: [1, 2, 3],
  clickAtPoint: [1, 2, 3], typeTextGlobal: [42], pressKeyGlobal: [42],
  keyDownGlobal: [42], keyUpGlobal: [42], holdKeyGlobal: [42],
  holdKeyGlobalAsync: [42], writeClipboardTextAsync: [42],
};
...
// —— 3. 只读能力 ——
const shape = (v) => JSON.stringify(v, (k, val) =>
...

{
  ...
  check("probeAccessibilityStatus", JSON.stringify(orig.probeAccessibilityStatus()) === JSON.stringify(rest.probeAccessibilityStatus()));
  check("isInteractiveSession", orig.isInteractiveSession() === rest.isInteractiveSession());
  check("isScreenCaptureSupported", orig.isScreenCaptureSupported() === rest.isScreenCaptureSupported());
  check("screenCaptureStatus", orig.screenCaptureStatus() === rest.screenCaptureStatus());
  check("dpiAwarenessInfo", JSON.stringify(orig.dpiAwarenessInfo()) === JSON.stringify(rest.dpiAwarenessInfo()));
  check("startupError", JSON.stringify(orig.startupError()) === JSON.stringify(rest.startupError()));
...
// displays:结构全等 + 值容差
{
  const a = orig.displays(), b = rest.displays();
  const sameShape = shape(a.map((d) => ({ id: typeof d.id, bounds: d.bounds.map(Math.round), main: d.main })))
    === shape(b.map((d) => ({ id: typeof d.id, bounds: d.bounds.map(Math.round), main: d.main })));
  const scaleOk = a.length === b.length && a.every((d, i) => Math.abs(d.scale_factor - b[i].scale_factor) < 0.01);
  check("displays (bounds/main/id-shape)", sameShape, `orig=${shape(a)} rest=${shape(b)}`);
  check("displays scale_factor", scaleOk, `orig=${a.map((d) => d.scale_factor)} rest=${b.map((d) => d.scale_factor)}`);
}

// cursorPoint:数值容差(探测间隙鼠标可能移动)
{
  const a = orig.cursorPoint(), b = rest.cursorPoint();
  check("cursorPoint",
...
{
  const a = orig.listApplications(), b = rest.listApplications();
  const ka = new Set(a.flatMap((x) => Object.keys(x)));
  const kb = new Set(b.flatMap((x) => Object.keys(x)));
  check("listApplications keys", JSON.stringify([...ka].sort()) === JSON.stringify([...kb].sort()),
    `orig=[${[...ka]}] rest=[${[...kb]}]`);
  ...
  check("listApplications pid:exe set", onlyOrig.length === 0 && onlyRest.length === 0,
    `orig-only=${onlyOrig.slice(0, 3)} rest-only=${onlyRest.slice(0, 3)}`);
}

// listWindows
check("listWindows", JSON.stringify(orig.listWindows()) === JSON.stringify(rest.listWindows()),
  `orig=${JSON.stringify(orig.listWindows())} rest=${JSON.stringify(rest.listWindows())}`);

// screenCaptureProbeWindows:窗口集合 + 键形状
{
  const a = orig.screenCaptureProbeWindows(), b = rest.screenCaptureProbeWindows();
  const ka = a.length ? Object.keys(a[0]).sort() : [];
  const kb = b.length ? Object.keys(b[0]).sort() : [];
  check("probeWindows element keys", JSON.stringify(ka) === JSON.stringify(kb),
    `orig=[${ka}] rest=[${kb}]`);
  ...
  check("probeWindows window set", onlyO.length === 0 && onlyR.length === 0,
  ...
  if (a.length && b.length) {
    ...
    check("probeWindows bounds shape", boundsShape, `orig=${shape([fa.bounds])} rest=${shape([fb.bounds])}`);
  }
}

// applicationInfo(node pid → null both)
check("applicationInfo(self)→null", JSON.stringify(orig.applicationInfo(process.pid)) === JSON.stringify(rest.applicationInfo(process.pid)));

// —— 4. AX 面错误路径与元素形状 ——
{
  const a = await orig.performAction("bogus-ref", "press");
  const b = await rest.performAction("bogus-ref", "press");
  check("performAction(bogus)", JSON.stringify(a) === JSON.stringify(b), `orig=${JSON.stringify(a)} rest=${JSON.stringify(b)}`);
  const sa = await orig.selectText("bogus-ref");
  const sb = await rest.selectText("bogus-ref");
  check("selectText(bogus)", JSON.stringify(sa) === JSON.stringify(sb), `orig=${JSON.stringify(sa)} rest=${JSON.stringify(sb)}`);
  const va = await orig.setValue("bogus-ref", "x");
  const vb = await rest.setValue("bogus-ref", "x");
  check("setValue(bogus)", JSON.stringify(va) === JSON.stringify(vb), `orig=${JSON.stringify(va)} rest=${JSON.stringify(vb)}`);
  const fa = await orig.setFocused("bogus-ref");
  const fb = await rest.setFocused("bogus-ref");
  check("setFocused(bogus)", JSON.stringify(fa) === JSON.stringify(fb), `orig=${JSON.stringify(fa)} rest=${JSON.stringify(fb)}`);
  const ra = await orig.readElement("bogus-ref");
  const rb = await rest.readElement("bogus-ref");
  check("readElement(bogus)", JSON.stringify(ra) === JSON.stringify(rb), `orig=${JSON.stringify(ra)} rest=${JSON.stringify(rb)}`);
  ...
  check("elementAtPoint keys", JSON.stringify(Object.keys(ea ?? {}).sort()) === JSON.stringify(Object.keys(eb ?? {}).sort()),
    `orig=${Object.keys(ea ?? {})} rest=${Object.keys(eb ?? {})}`);
  if (ea && eb) {
    check("elementAtPoint role", ea.role === eb.role, `orig=${ea.role} rest=${eb.role}`);
    check("elementAtPoint bounds", JSON.stringify(ea.bounds) === JSON.stringify(eb.bounds),
      `orig=${JSON.stringify(ea.bounds)} rest=${JSON.stringify(eb.bounds)}`);
    check("elementAtPoint ref-format", /^rt:\d+:/.test(ea.ref ?? "") === /^rt:\d+:/.test(eb.ref ?? ""),
...
// —— 5. 剪贴板 round-trip(备份/恢复)——
{
  ...
  try {
    ...
    check("clipboard rest-write→orig-read", readFromOrig?.text === mark, `orig read=${readFromOrig?.text}`);
    check("clipboard orig-write→rest-read", readFromRest?.text === mark + "-2", `rest read=${readFromRest?.text}`);
  } finally {
    // 恢复用户剪贴板
...
check("captureApp(invalid)→null", JSON.stringify(await orig.captureApp(999999999)) === JSON.stringify(await rest.captureApp(999999999)),
...
{
  for (const [label, m] of [["orig", orig], ["rest", rest]]) {
    const el = await m.elementAtPoint(1280, 700);
    const rt = el ? await m.readElement(el.ref) : null;
    check(`readElement round-trip (${label})`, Boolean(rt), `role=${el?.role} → ${rt ? rt.role : "null"}`);
  }
}
