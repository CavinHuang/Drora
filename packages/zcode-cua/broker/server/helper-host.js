...
import { CUA_DEV_TIMEOUT_MS, CUA_HEALTH_POLL_MS, CUA_PIP_NO_ACTIVE_SESSION_PENDING, DEV_CUA_HELPER_BUNDLE_ID_VALUE, dn, } from "./region-constants.js";
import { chmod as HW, chmod as UW, lstat, mkdir as Ese, mkdir as kse, mkdir as zse, rename as Pse, rename as WW, stat as Tse, unlink as Mse, writeFile as Cse, } from "node:fs/promises";
...
export var MW = /^[A-Za-z0-9.-]+$/, qn = class extends CuaHelperError {
    static { }
...
export async function NW(e, t = wse) {
    ...
    let c = [...new Set(a.map(sb))], d = s.filter((l) => cb(l, n, c, e.socketPath)).map((l) => l.pid);
    if (d.length > 0) {
        let l = new Set(r);
        return {
            state: "observed",
...
export async function LW(e, t = yse) {
    ...
    if (!s.some((l) => {
        try {
            let p = t.canonicalize(l), u = mse(a, p);
            return (u.length > 0 &&
                u !== ".." &&
...
export var bse = "ZCODE_CUA_DEV_EXPOSE_BROKER", Ise = "broker-credentials.json", _se = Ib("computer-use", "run");
export function _b(e) {
    if (!bn(e ?? {}))
        return !1;
    let t = e?.[bse], n = typeof t == "string" ? t.trim().toLowerCase() : "";
...
`, s = `${t}.${process.pid}.tmp`;
    (await Cse(s, o, {
        ...
        await UW(s, 384),
        await Pse(s, t),
        e.logger?.warn(void 0, `ZCODE_CUA_DEV_EXPOSE_BROKER=1: wrote broker socket path to ${t} for the external test harness. NEVER enable this in production.`));
}
...
export async function KW(e) {
    let { socketPath: t } = e, n = e.logger ?? NC;
    if (ma(t))
        return null;
    let r = `${t}${Dse}`, o = `${t}${CUA_PIP_NO_ACTIVE_SESSION_PENDING}`;
    if (r.length > CUA_HEALTH_POLL_MS)
        return (n.warn(void 0, `cua transport rendezvous disabled: socket path budget exceeded (${r.length} > ${CUA_HEALTH_POLL_MS})`),
            null);
    let s = null;
    try {
        (await Fse(t),
        ...
        return (s && (await Rb(s).catch(() => { })),
            await Gh(r),
            n.warn(void 0, `cua transport rendezvous disabled: reservation failed (${p instanceof Error ? p.message : String(p)})`),
            null);
    }
    let a = s, c = "reserved", d = null, l = (p) => {
...
export var $se = Ss, GW = jU + 2e3, Zse = 5e3, Hse = ".screen-recording-preflight", CuaHelperHost = class {
    static { }
    options;
    handle = null;
    ...
    resolveResolverPluginAuthority() {
        if (this.resolverPluginAuthority !== void 0)
            return this.resolverPluginAuthority;
        if (this.pluginAuthorityMintError)
            throw this.pluginAuthorityMintError;
        try {
            return ((this.resolverPluginAuthority = (this.options.mintPluginAuthority ?? BW)()),
    ...
    async start() {
        ...
        if (this.restartAfterCurrentStartInFlight)
            return this.restartAfterCurrentStartInFlight;
        if (this.startInFlight)
            return this.startInFlight;
        if (this.handle) {
            let o = this.stopGeneration, s = await this.retryCompletedRefreshMarkerForLiveHandle();
        ...
        if ((await this.finishPendingTermination("before start"),
    ...
    async doStart(t, n, r = "restart") {
        let o = n?.socketPath ??
            (this.options.mintSocketPath ??
                ...
                })))(), s = this.resolveResolverPluginAuthority(), a = r === "cold-start" && !n
        ...
        (await this.options.helperInstaller?.verifyInstalled(p, {
            ...
            u && bb(p, u));
        let g = this.options.ghostCursorOverlay === !0, v = this.options.pipMode === !0, S = this.options.pipLiveProbe === !0, k = YC(this.options.env ?? process.env), C = this.options.healthTimeoutMs ?? 5e3, A = this.options.failedLaunchCleanupNow ?? Date.now, D = Math.max(0, this.options.brokerLaunchDeadlineSlackMs ?? 5e3), U = await (this.options.prepareBrokerLaunchGuard ?? Ez)({
            socketPath: c,
        ...
        try {
            ...
            let M = W instanceof CuaHelperError
                ? W
                : new CuaHelperError("launch_failed", `Failed to launch ${p}: ${W instanceof Error ? W.message : String(W)}`, {
                    cause: W,
                });
        ...
        try {
            ...
            if (((B = (await (this.options.verifyLiveProcessIdentity ?? LW)({
                ...
                M && j.bundleId !== M))
                throw (await this.terminateHelperPid(B, "unexpected bundle id", {
                    socketPath: c,
                    helperAppPath: p,
                }),
        ...
        return (this.options.logger?.info(void 0, `cua helper ready at ${p} pid=${j.pid ?? "?"}`),
    ...
    async stop() {
        let t = ++this.stopGeneration, n = this.startInFlight;
        if (n)
            try {
                await n;
            }
        ...
        t === this.stopGeneration &&
            (await this.stopCurrentHandle("stop", {
                allowSigkill: !0,
            }, !0),
    ...
    beginFreshRestart(t) {
        let r = (async () => (this.assertNotExternallyStopped(t),
            ...
            await this.completeRefreshMarkerAfterTransportRetired(),
            this.assertNotExternallyStopped(t),
            this.doStart(t, void 0, "restart")))().finally(() => {
            this.startInFlight === r && (this.startInFlight = null);
        });
        return ((this.startInFlight = r), r);
    }
    ...
    async retryCompletedRefreshMarkerForLiveHandle() {
        let t = this.pendingRefreshMarker, n = this.handle;
        if (!t || !t.oldTransportRetired || !n)
            return null;
        if (n.socketPath !== t.socketPath)
            throw new CuaHelperError("termination_failed", "Cannot complete CUA permission refresh marker: live Helper transport no longer matches the pending marker");
        return (await t.handle.complete(),
            this.pendingRefreshMarker === t && (this.pendingRefreshMarker = null),
            {
                handle: n,
                reused: !0,
        ...
        let o = (async () => {
            ...
            let a = s?.handle ??
                (await XU(n.socketPath, {
                    deadlineMs: (this.options.terminationTermGraceMs ?? GW) +
                        CUA_DEV_TIMEOUT_MS +
    ...
    async runFailedLaunchCleanup(t, n) {
        let r = this.options.discoverLaunchedHelperProcesses ?? NW, o = Math.max(1, this.options.failedLaunchCleanupPollIntervalMs ?? 100), s = Math.max(1, Math.ceil(Math.max(0, this.options.failedLaunchCleanupQuietMs ?? 500) / o)), a = this.options.failedLaunchCleanupDelay ?? VW, c = this.options.failedLaunchCleanupNow ?? Date.now, d = 0, l = t.cancellationPublished ? 0 : Math.ceil(Math.max(0, t.guard.deadlineEpochMs - c()) / o), p = Math.max(32, s + 8, l + s + 8);
        for (let u = 0; u < p; u += 1) {
            let f = await r({
    ...
    async waitForHelperDeparture(t, n, r, o) {
        let s = Math.max(1, this.options.terminationPollIntervalMs ?? 100), a = Math.max(1, Math.ceil(Math.max(0, n) / s)), c = this.options.terminationDelay ?? VW;
        for (let d = 0; d < a; d += 1) {
            await c(s);
    ...
    async restartAfterCurrentStart() {
        if (this.restartAfterCurrentStartInFlight)
            return this.restartAfterCurrentStartInFlight;
        let t = this.stopGeneration, r = (async () => {
            for (;;) {
                this.assertNotExternallyStopped(t);
    ...
    async restartAfterCurrentStartPreservingTransport(t = {}) {
        if (this.restartPreservingTransportInFlight)
            return this.restartPreservingTransportInFlight;
        let n = this.stopGeneration, o = (async () => {
            for (;;) {
    ...
    async queryPermissionStatus(t = 3e3) {
        if (!this.handle)
            throw new Error("CuaHelperHost.queryPermissionStatus called before start(); start the helper first");
        return (this.options.permissionStatusProbe ??
            ((r, o) => callBrokerMethod({
                socketPath: r,
    ...
    async queryScreenRecordingPreflight(t) {
        if (!this.handle)
            return null;
        let n = this.options.screenRecordingPreflight ?? Fz, r = Wse(qW(this.handle.socketPath), Hse, `.screen-recording-preflight-${randomUUID()}.json`);
        try {
            await zse(qW(r), {
                recursive: !0,
                mode: 448,
    ...
    async queryScreenCaptureProbe(t = 6500) {
        if (!this.handle)
            throw new Error("CuaHelperHost.queryScreenCaptureProbe called before start(); start the helper first");
        return (this.options.screenCaptureProbe ??
            ((r, o) => callBrokerMethod({
                socketPath: r,
