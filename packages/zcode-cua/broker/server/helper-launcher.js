...
import { CUA_DEV_TIMEOUT_MS, dn } from "./region-constants.js";
import { basename as Bre, dirname as kz, isAbsolute as Pz, join as KC, join as ks, resolve as Cz, } from "node:path";
import { chmod as yz, mkdir as wz, open as Tre, realpath as vz, rm as Fre, } from "node:fs/promises";
import { execFile as Nz } from "node:child_process";
import { randomBytes } from "node:crypto";
import { existsSync as Lz, existsSync as Nre, readFileSync as jre, realpathSync as Sz, rmSync as Kre, writeFileSync as Lre, } from "node:fs";
...
export var uz = 4096, pz = 4277009102, fz = 4277009103, mz = 3405691582, jC = 3405691583, Mre = 4278190080, xre = new Map([
    ["7:3", "i386"],
    ["16777223:3", "x86_64"],
    ["16777223:8", "x86_64h"],
    ["12:6", "armv6"],
...
export function gz(e, t) {
    let n = (t & ~Mre) >>> 0, r = xre.get(`${e >>> 0}:${n}`);
    return (r ||
        `unknown(0x${(e >>> 0).toString(16).padStart(8, "0")},0x${n.toString(16).padStart(8, "0")})`);
}
export function Ore(e) {
    if (e.length < 8)
        throw new Error("not a Mach-O file: header shorter than 8 bytes");
    let t = e.readUInt32BE(0), n = e.readUInt32LE(0), r = t === mz || t === jC;
    if (r || n === mz || n === jC) {
        let c = r ? t : n, d = (g) => (r ? e.readUInt32BE(g) : e.readUInt32LE(g)), l = d(4), p = c === jC ? 32 : 20, u = 8 + l * p;
        if (u > e.length)
            throw new Error(`not a Mach-O file: relative header declares ${l} slices (needs ${u} bytes, have ${e.length})`);
        let f = [];
        for (let g = 0; g < l; g += 1) {
            let v = 8 + g * p;
            f.push(gz(d(v), d(v + 4)));
...
export async function hz(e) {
    let t = await Tre(e, "r");
    try {
        let n = Buffer.alloc(uz), { bytesRead: r } = await t.read(n, 0, uz, 0);
...
export var bz = "--broker-launch-deadline-epoch-ms", Iz = "--broker-launch-cancel-file", _z = ".launch-cancel", Ure = /^\.broker-launch-cancel-[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.sentinel$/u, zre = 3e5, Wre = 5e3;
...
export function vh(e, t) {
    if (!Number.isSafeInteger(t.deadlineEpochMs) ||
        t.deadlineEpochMs <= 0 ||
        !Pz(t.cancelFilePath) ||
...
export async function Ez(e) {
    ...
    if (!Az(e.socketPath) ||
        ...
        throw new CuaHelperError("launch_failed", "Refusing to prepare an unsafe Computer Use Helper broker launch guard");
    let n;
    try {
        ...
        throw new CuaHelperError("launch_failed", `Failed to resolve the Computer Use Helper broker runtime directory: ${s instanceof Error ? s.message : String(s)}`, {
            cause: s,
        });
    }
    let r = KC(n, _z);
    if ((await wz(r, {
        ...
        await yz(r, 448),
        (await vz(r)) !== r))
        throw new CuaHelperError("launch_failed", "Refusing to use a redirected Computer Use Helper launch-cancel directory");
    let o = e.mintId ?? Dre;
    for (let s = 0; s < 5; s += 1) {
        let a = o().trim().toLowerCase(), c = {
            cancelFilePath: KC(r, `.broker-launch-cancel-${a}.sentinel`),
            deadlineEpochMs: e.deadlineEpochMs,
        };
        if (vh(e.socketPath, c) && !Nre(c.cancelFilePath))
            return c;
    }
    throw new CuaHelperError("launch_failed", "Failed to allocate a unique Computer Use Helper broker launch guard");
}
export function Tz(e, t) {
    if (!vh(e, t))
        throw new CuaHelperError("termination_failed", "Refusing to publish an unsafe Computer Use Helper broker launch cancellation sentinel");
    try {
...
export function Mz(e, t = {}) {
    let n = t.now ?? Date.now, r = t.setTimer ?? setTimeout, o = Math.max(1e3, e.deadlineEpochMs - n() + Wre);
    r(() => {
        qC(e).catch(() => { });
    }, o).unref?.();
}
...
export var Ph = "--allow-unsigned-launcher-local-dev", Ch = "--allow-external-broker-client-local-dev", bh = "--ghost-cursor-overlay", Ih = "--background-mode", _h = "--pip-mode", Rh = "--pip-live-probe", HELPER_ADDON_ENV_VALUE = "ZCODE_CUA_HELPER_ADDON", Ah = "--ghost-cursor-capture";
export var Eh = "--permission-preflight", Th = "--permission-preflight-result-file";
export var VC = class extends CuaHelperError {
    static { }
    constructor(t) {
        let n = t instanceof CuaHelperError
            ? t
...
export function JC(e, t) {
    let n = ["-n", "-g", e.appPath, "--args", "--socket", e.socketPath], r = e.version?.trim();
    ...
    if ((o && n.push("--expected-app-bundle-path", o),
        ...
        if (!vh(e.socketPath, e.brokerLaunchGuard))
            throw new CuaHelperError("launch_failed", "Refusing to pass an unsafe Computer Use Helper broker launch guard to LaunchServices");
        n.push(bz, String(e.brokerLaunchGuard.deadlineEpochMs), Iz, e.brokerLaunchGuard.cancelFilePath);
    }
    return (n.push("--exit-log", e.exitLogPath?.trim() || `${e.socketPath}.exit.log`),
...
export var Jre = 8e3, Yre = new Set(["granted", "denied", "unknown"]);
export function Xre(e) {
    ...
    if (!t || typeof t != "object" || Array.isArray(t))
        return null;
    let n = t;
    if (n.permission !== "screen_recording")
        return null;
    let r = n.state;
    return typeof r == "string" && Yre.has(r) ? r : null;
}
export var Qre = {
    launchOpen: (e, t, n, r) => Nz(dn.open, e, {
        env: t,
        timeout: n,
    }, (o) => r(o)),
};
...
export async function Fz(e) {
    let t = e.dependencies ?? Qre, n = t.readResult ?? eoe, r = t.removeResult ?? toe, o = e.timeoutMs ?? Jre;
    try {
...
export async function writeOneShotHelperTokenFile(e) {
    ...
    for (let n = 0; n < 5; n += 1) {
        ...
        try {
            ...
            if (r.code === "EEXIST")
                continue;
            throw new CuaHelperError("launch_failed", `Failed to write one-shot Computer Use Helper token file: ${r instanceof Error ? r.message : String(r)}`, { cause: r });
        }
    }
    throw new CuaHelperError("launch_failed", "Failed to write one-shot Computer Use Helper token file: exhausted unique filename retries");
}
...
export function Uz(e) {
    let t = e ?? process.env, n = {};
    for (let [r, o] of Object.entries(t))
        o !== void 0 && roe(r) && (n[r] = o);
    return ((n.PATH = lz), n);
}
        ...
        try {
            (e.assertHelperUnchangedBeforeLaunch?.(),
                Nz(dn.open, JC(e, Gre()), {
                    ...
                    if (r) {
                        n(new CuaHelperError("launch_failed", ioe(e.appPath, r), {
                            cause: r,
                        }));
                        return;
                    }
...
export function ioe(e, t) {
    let n = t instanceof Error ? t.message : String(t), r = `Failed to launch ${e} via LaunchServices (open): ${n}`;
    return soe(t, n)
        ? `${r}
...
export function soe(e, t) {
    if (/(?:_LSOpenURLsWithCompletionHandler|LaunchServices|AppleEvent).*(?:-1712|timed out)/iu.test(t))
        return !0;
...
export function XC(e) {
    ...
    if (!t)
        return null;
    let n = ks(t, "computer-use"), r = YC(e);
    return r
