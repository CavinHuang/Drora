...
import { z } from "zod";
import { CUA_PIP_NO_ACTIVE_SESSION_V2, DEV_CUA_HELPER_BUNDLE_ID_VALUE, dn, } from "./region-constants.js";
import { appendFileSync as Qoe, mkdirSync as eie, readdirSync as tie, realpathSync as Qz, unlinkSync as nie, } from "node:fs";
import { dirname as sW, join as Foe, join as Lh, join as rie, } from "node:path";
import { execFileSync as Boe, } from "node:child_process";
...
export var Yz = new Set(["textfield", "textarea", "securefield", "combobox"]), Doe = new Set([...Yz, "slider", "stepper"]);
export function Nh(e = process.env) {
    let t = bn(e), n = t ? e.ZCODE_CUA_HELPER_BUNDLE_ID?.trim() : void 0;
...
export var Uoe = 32, zoe = ["-awwxo", "pid=,ppid=,uid=,command="];
export function eW(e = process.env) {
    let t = xt.replace(/\.app$/u, ""), n = QC(e), r = zz(e).flatMap((o) => [Lh(o, xt), Lh(o, DEV_HELPER_APP_NAME_APP)]);
...
export function ab(e, t) {
    let n = e.split(/\s+/u), r = n.indexOf(t);
...
export function Zoe(e) {
    if (!e.startsWith("/"))
        return !1;
    let n = `/Contents/MacOS/${xt.replace(/\.app$/u, "")}`, r = e.indexOf(n);
    for (; r > 0;) {
        let o = r + n.length;
        if (o === e.length || /\s/u.test(e[o]))
            return !0;
        r = e.indexOf(n, r + 1);
    }
    return !1;
...
export function ob(e, t, n) {
    let r = ` ${t} ${n}`, o = e.indexOf(r);
    for (; o >= 0;) {
        let s = o + r.length;
        if (s === e.length || /\s/u.test(e[s]))
            return !0;
        o = e.indexOf(r, o + 1);
    }
    return !1;
...
export function Xz(e, t) {
    if (e.ppid !== 1 || e.pid === t.selfPid || !cb(e, t.currentUid, t.executablePaths))
        return !1;
    let n = ab(e.command, "--socket");
    if (!n || !Woe(n, t.runtimeDir))
        return !1;
    let r = $oe(e.command);
    if (r === null)
        return !1;
...
export function lb(e = {}) {
    let t = e.platform ?? process.platform, n = e.currentUid ?? (typeof process.getuid == "function" ? process.getuid() : null), r = e.listProcesses ?? rW, o = e.isProcessAlive ?? tW, s = e.canonicalizePath ?? Qz.native, a = ib(eW(e.env ?? process.env), s), c = e.logger;
    return (d, l) => {
        if (t !== "darwin" || n === null)
            return {
                state: "helper",
        ...
        let f = l?.helperAppPath ? ib([sb(l.helperAppPath), ...a], s) : a, g = f.find((S) => nW(u.command, S));
...
export function oW(e = {}) {
    let t = e.env ?? process.env, n = e.platform ?? process.platform, r = e.logger, o = {
        scanned: 0,
        reaped: [],
    };
    if (n !== "darwin")
    ...
    if (s == null)
        return o;
    let a = e.selfPid ?? process.pid, c = e.maxReap ?? Uoe, d = e.listProcesses ?? rW, l = e.killProcess ??
        ((C) => {
            process.kill(C, "SIGTERM");
        }), p = e.isProcessAlive ?? tW, u = e.canonicalizePath ?? Qz.native, f;
    try {
        f = d();
    }
    catch (C) {
    ...
    o.scanned = f.length;
    let g = new Set(), v = f, S = e.ownerEvidenceProvider ??
        Hoe({
            getRows: () => v,
            currentUid: s,
            selfPid: a,
    ...
    return (o.reaped.length > 0 &&
        r?.info(void 0, `cua helper reaper: SIGTERM'd ${o.reaped.length} orphaned Helper(s): ${o.reaped.join(", ")}`),
        o);
}
...
export var ni = z
    ...
    .refine((e) => e !== CUA_PIP_NO_ACTIVE_SESSION_V2, "identifier is reserved by the PiP session runtime"), Bh = z.number().int().nonnegative().safe(), Wh = z.discriminatedUnion("kind", [
    z
        .object({
...
export var jJe = z
    ...
    .strict();
(() => {
    let e = process.argv.indexOf("--exit-log"), t = e >= 0 && e + 1 < process.argv.length ? process.argv[e + 1] : null;
    if (!t)
        return;
    try {
        eie(sW(t), {
...
`)) {
        ...
        if (a && typeof a == "object" && !Array.isArray(a)) {
            let c = a, { scope: d, level: l, event: p, evidence: u, ...f } = c;
            s = {
                timestamp: t,
