...
import { randomBytes as Aae } from "node:crypto";
...
export var iy = lW, p$ = HELPER_ADDON_ENV_VALUE, f$ = {
    fork(e, t, n) {
        let [r, ...o] = t;
        if (!r)
            throw new Error("Windows Computer Use Helper entry path is required");
        return Rae(r, o, {
            ...n,
            execPath: e,
        });
    },
}, m$ = Uu, g$ = (e, t) => probeHelperHealth(e, {
    timeoutMs: t,
}), sy = class {
    constructor(t) {
        this.logger = t;
...
export var Eae = () => Aae(16).toString("hex"), w$ = 3e4, v$ = 1e3, ep = class {
    constructor(t) {
        this.options = t;
    ...
    start() {
        ...
        if (this.startInFlight && this.startInFlightEpoch === t)
            return this.startInFlight;
        this.createTransportReadyPromise();
        let n = this.transportReadyInFlight, r = this.lastAgentVisibleTransport ?? void 0, o = this.enqueue(() => this.startNow(t, r))
            .catch((s) => {
    ...
    stop() {
        let t = this.current, n = this.invalidateForExternalStop(t);
    ...
    restart() {
        ...
        let t = this.externalStopEpoch, n = this.enqueue(async () => (await this.stopNow("restart"), this.startNow(t))).finally(() => {
            this.restartInFlight === n && (this.restartInFlight = null);
        });
        return ((this.restartInFlight = n), n);
    }
    restartAfterCurrentStart() {
        if (this.restartAfterStartInFlight)
            return this.restartAfterStartInFlight;
        let t = this.externalStopEpoch, n = this.enqueue(async () => (await this.stopNow("permission-restart"), this.startNow(t))).finally(() => {
    ...
    restartAfterCurrentStartPreservingTransport(t = {}) {
        ...
        let n = this.externalStopEpoch, r = this.enqueue(async () => {
    ...
    async checkHealth(t = v$) {
        let n = this.handle;
        if (!n)
            throw new Error("Windows Computer Use Helper is not running");
        return this.healthProbe(n.socketPath, t);
    }
    enqueue(t) {
        let n = this.lifecycleTail.then(async () => {
            if (this.terminationBlocker)
                throw this.terminationBlocker;
            return t();
        });
        return ((this.lifecycleTail = n.then(() => { }, () => { })),
            n);
    }
    startNow(t, n) {
    ...
    startGeneration(t, n) {
        let r = ++this.nextGeneration, o = n?.socketPath ?? this.mintSocketPath(), s = [this.options.runtime.entryPath, "--socket", o, "--parent-pid", String(process.pid)], a;
        try {
            ...
            let d = ay(c);
            return (this.rejectTransportReady(d),
        ...
        return new Promise((c, d) => {
            let l = !1, p, u, f = (C, A) => {
                ...
                ((this.handle = null),
                    ...
                    this.terminateGeneration(u, A).then(() => d(D), (U) => {
                ...
                if (u.stopped || t !== this.externalStopEpoch || this.current?.id !== r)
                    return;
                let A = h$(C);
                if (A !== "ignore") {
                    if (!A)
                        return f(new Error("Invalid Windows Computer Use Helper control message"), "malformed-message");
                    if (A.type === "error")
                        return f(new Error(A.message), "helper-reported-error");
                    if (A.socketPath === o) {
                        if (A.pid !== a.pid)
                        ...
                        (this.resolveTransportReady({
                            ...
                            A.type !== "transport_ready" &&
                                this.healthProbe(o, this.startupTimeoutMs).then((D) => {
                                    if (l ||
                                        u.stopped ||
            ...
            ((p = setTimeout(() => f(new Error("Windows Computer Use Helper startup timed out"), "startup-timeout"), this.startupTimeoutMs)),
                p.unref?.());
        });
    }
    terminateGeneration(t, n) {
        if (t.terminationPromise)
            return t.terminationPromise;
    ...
    invalidateForExternalStop(t) {
        if (((this.externalStopEpoch += 1), (this.lastAgentVisibleTransport = null), !t))
            return (this.rejectTransportReady(new Error("Windows Computer Use Helper startup stopped")),
                Promise.resolve());
        ((this.handle = null),
            (t.stopped = !0),
...
var identifierSchema = z
    ...
    .refine((value) => value !== CUA_PIP_NO_ACTIVE_SESSION_V2, "identifier is reserved by the PiP session runtime");
...
export function S$(e) {
    let t = new Qu(e.socketPath, {
    ...
    }), n = Math.max(0, e.reconnectAttempts ?? 2), r = Math.max(0, e.reconnectDelayMs ?? 50), o = !1, s = !1, a = null, c = Promise.resolve(), d = !1, l = (v) => {
        if (!d) {
            d = !0;
            try {
                e.onDiagnostic?.(v);
        ...
        for (let C = 0; C <= n; C += 1)
            try {
                ...
                if (((k = A), A instanceof Ft && A.code === "version_mismatch"))
                    throw (p(A), A);
                if (!(A instanceof Ft) || A.code !== "broker_unavailable" || C === n)
                    throw (A instanceof Ft &&
        ...
        if (o)
            throw new Error("PiP session client is closed");
        if (a)
            throw a;
        if (s)
            return;
