...
var PermissionBrokerClient = class {
  ...
  constructor(socketPath, options) {
    ...
    this.token = typeof options.token === "string" && options.token.trim().length > 0 ? options.token.trim() : null;
   ...
   */
  async call(method, params, _opts = {}) {
...
`;
    const authLine = this.token !== null ? `${JSON.stringify({ id: 0, method: "authenticate", params: { token: this.token } })}
` : null;
    ...
    const settlement = new Promise((resolve22) => {
      let settled = false;
...
function wrapTransportError(error51, sanitize, state, errorType) {
  ...
  const errorTypeResolved = errorType ?? (error51 instanceof Error ? error51.name : typeof error51 === "string" ? "string" : "unknown");
...
var PermissionBrokerRefreshGate = class {
  ...
  cancelSignal;
  reader;
  constructor(markerPath, options = {}) {
    const normalized = normalizeRefreshMarkerPath(markerPath);
    if (normalized === null) {
      throw new Error("permission broker refresh marker path is required");
    }
    this.markerPath = normalized;
    const base = options.backoffBaseMs ?? DEFAULT_BACKOFF_BASE * 1e3;
    const cap = options.backoffCapMs ?? DEFAULT_BACKOFF_CAP * 1e3;
    if (!(base > 0) || !(cap > 0)) {
    ...
    this.nowMs = options.nowMs ?? Date.now;
    this.sleep = options.sleep ?? interruptibleBrokerSleep;
    this.cancelSignal = options.cancelSignal;
    this.reader = options.reader ?? ((p) => readPermissionRefreshMarker(p, this.nowMs));
  }
  /**
  ...
  async waitUntilClear(absoluteDeadlineMs, callCancelSignal) {
    ...
    while (true) {
      ...
      markerDeadlineMs = markerDeadlineMs === null ? marker.deadlineEpochMs : Math.min(markerDeadlineMs, marker.deadlineEpochMs);
...
function computeAbsoluteDeadline(startedAtMs, method, params, options = {}) {
  const capMs = options.rpcDeadlineCapMs ?? DEFAULT_RPC_DEADLINE_CAP * 1e3;
  const holdMs = boundedHoldDuration(method, params ?? {});
  const local = startedAtMs + capMs + holdMs;
  return options.outerDeadlineMs !== void 0 ? Math.min(local, options.outerDeadlineMs) : local;
}
async function callWithWarmupRetry(client, method, params, options) {
  ...
  while (true) {
    ...
    try {
      ...
      const stopAt = refreshObserved ? options.absoluteDeadlineMs : Math.min(options.absoluteDeadlineMs, options.retryDeadlineMs);
      if (monotonic() >= stopAt) {
        throw exc;
      }
...
function resolvePositiveFloat(value, defaultValue, field, allowZero) {
  ...
  const bad = typeof value !== "number" || !Number.isFinite(value) || value < 0 || !allowZero && value <= 0;
  if (bad) {
...
var cuaApplicationIconSchema = z.object({
  mimeType: z.literal("image/png"),
  data: z.string().min(1).max(Math.ceil(CUA_APP_ASSOCIATIONS_MAX_ICON_BYTES * 4 / 3) + 8)
}).strict();
var cuaApplicationPresentationSchema = z.object({
...
var cuaAppAssociationsSchema = z.object({
  ...
  ).max(CUA_APP_ASSOCIATIONS_MAX_ITEMS).optional()
}).strict();
...
function deriveApplicationKey(evidence, platform = process.platform) {
  ...
  if (platform === "win32") {
    ...
    else if (executablePath && win32.isAbsolute(executablePath) && basename(executablePath).toLowerCase() !== "applicationframehost.exe") {
...
function beginTargetAppDisplayLookup(evidence) {
  ...
  if (!state || !key || state.conflicted || state.primary?.icon) return false;
  if (state.lookupAttempts.has(key)) return false;
  state.lookupAttempts.add(key);
  return true;
}
function recordTargetAppDisplayEvidence(evidence) {
  const state = invocationStorage.getStore();
  const presentation = presentationFromEvidence(evidence);
  if (!state || !presentation || state.conflicted) return;
  if (state.primaryIdentity && state.primaryIdentity !== presentation.appKey) {
    state.conflicted = true;
    state.primary = void 0;
    return;
  }
  state.primaryIdentity ??= presentation.appKey;
  state.primary = {
    appKey: presentation.appKey,
    ...state.primary?.displayName || presentation.displayName ? { displayName: state.primary?.displayName ?? presentation.displayName } : {},
    ...state.primary?.icon || presentation.icon ? { icon: state.primary?.icon ?? presentation.icon } : {}
  };
}
function recordAppAssociationItems(items) {
  ...
  for (const item of items.slice(0, CUA_APP_ASSOCIATIONS_MAX_ITEMS)) {
    ...
    if (presentation) {
      state.items.set(item.resultIndex, {
...
function fitItemsToBudgets(items) {
  const iconPriority = [...items].filter((item) => item.application.icon).sort((left, right) => Number(right.active) - Number(left.active));
  ...
  for (const candidate of [...iconPriority].reverse()) {
    if (Buffer.byteLength(JSON.stringify({ schemaVersion: 1, items: projected }), "utf8") <= CUA_APP_ASSOCIATIONS_MAX_META_BYTES) {
      break;
    }
...
function readAppAssociations(mode) {
  ...
  const value = mode === "items" ? {
    schemaVersion: 1,
    items: fitItemsToBudgets(
      [...state.items.entries()].map(([resultIndex, item]) => ({
        resultIndex,
        ...item
      }))
    )
    ...
    schemaVersion: 1,
    ...state.primary ? { primary: state.primary } : {}
  };
  if (mode === "primary" && !value.primary || mode === "items" && !value.items?.length) {
    return void 0;
  }
  const parsed = cuaAppAssociationsSchema.safeParse(value);
  if (!parsed.success) return void 0;
  return Buffer.byteLength(JSON.stringify(parsed.data), "utf8") <= CUA_APP_ASSOCIATIONS_MAX_META_BYTES ? parsed.data : void 0;
}
...
function parsePipToolRequestContext(value) {
  ...
  const valid = (identifier) => identifier.length > 0 && identifier.length <= 255 && !identifier.includes("\0");
...
var BrokerClient = class {
  ...
  async screenshot(options = {}) {
    ...
    const result = await this.call(
      "screenshot",
      { display_id: options.displayId ?? null },
      { kind: "read", timeoutMs }
    );
    ...
    return {
      ...
      ...result?.coordinate_contract === void 0 ? {} : { coordinate_contract: result.coordinate_contract },
...
async function callBroker(deps, method, params, opts) {
  ...
  const displayLookup = method !== "application_info" && appRef2 ? startTargetAppDisplayLookup(deps, appRef2) : void 0;
  if (opts.kind === "read") {
...
function readAppRef(value) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return null;
  const record2 = value;
  return ["pid", "bundle_id", "name", "window_id"].some((key) => record2[key] != null) ? record2 : null;
}
...
function recordBrokerPresentation(value) {
  ...
  recordAppAssociationItems(
    presentation.items.flatMap((item) => {
      ...
      const record2 = item;
      if (typeof record2.resultIndex !== "number" || !Number.isInteger(record2.resultIndex) || typeof record2.application !== "object" || record2.application === null || Array.isArray(record2.application)) {
        return [];
      }
      return [
        {
...
function startTargetAppDisplayLookup(deps, appRef2) {
  ...
  return deps.broker.resolveTargetAppDisplay(appRef2).then(recordBrokerTargetApp).catch(() => {
  });
}
async function recordBrokerResultTargetApp(deps, method, value) {
  ...
  if (method !== "capture_app" && method !== "open_application" && method !== "activate_application")
    return;
  if (typeof value !== "object" || value === null || Array.isArray(value)) return;
  const record2 = value;
  ...
  const app = typeof evidence === "object" && evidence !== null && !Array.isArray(evidence) ? evidence : void 0;
