...
function legacyHelperMessageProvesActionNotSent(message) {
  return /\braw foreground event\b.*\brefused because\b/i.test(message) || /\bmodifiers must be a '\+'-separated chord\b/i.test(message) || /\belement \([^)]*\) is not settable; refuse set_value\b/i.test(message) || /\belement \([^)]*\) is not selectable; refuse select_text\b/i.test(message);
}
var PermissionBrokerError = class extends Error {
  code;
  details;
  permissionError;
  constructor(message = "unknown broker error", init = {}) {
    const baseMessage = (message && message.length > 0 ? message : init.code && init.code.length > 0 ? init.code : "unknown broker error") || "unknown broker error";
    const actionProvenNotSent = init.details?.action_sent === false || legacyHelperMessageProvesActionNotSent(baseMessage);
    const normalizedMessage = actionProvenNotSent && !/\baction_sent\s*=\s*false\b/i.test(baseMessage) ? `${baseMessage.replace(/\s+$/u, "")} action_sent=false.` : baseMessage;
    super(
      normalizedMessage,
      init.cause !== void 0 ? { cause: init.cause } : void 0
    );
    this.name = "PermissionBrokerError";
    this.code = typeof init.code === "string" && init.code.trim().length > 0 ? init.code.trim() : null;
    this.details = init.details ? { ...init.details } : {};
    this.permissionError = init.permissionError === true;
  }
};
function permissionBrokerPermissionError(message, init = {}) {
...
function cuaNotReadyPayload(opts = {}) {
  const reasonCode = opts.reasonCode ?? REASON_BROKER_NOT_ACCEPTING;
  const message = opts.message ?? NOT_READY_MESSAGE;
...
function isValidErrorEnvelope(error51) {
  ...
  if (details !== void 0 && (details === null || typeof details !== "object" || Array.isArray(details))) {
    return false;
  }
  return typeof message === "string" && message.trim().length > 0 || typeof code === "string" && code.trim().length > 0;
}
function parseErrorPayload(error51) {
  if (error51 !== null && typeof error51 === "object" && !Array.isArray(error51)) {
    ...
    const message = (typeof rawMessage === "string" && rawMessage.trim().length > 0 ? String(rawMessage) : null) ?? code ?? "unknown broker error";
    const details = env.details !== null && typeof env.details === "object" && !Array.isArray(env.details) ? { ...env.details } : {};
...
var interruptibleBrokerSleep = async (ms, cancelSignal) => {
  ...
  while (Date.now() - startedAt < ms) {
    ...
    await new Promise((resolve22) => setTimeout(resolve22, Math.min(slice, remaining)));
  }
};
