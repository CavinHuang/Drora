...
function registerAllTools(server, deps) {
  ...
  for (const name of TOOL_NAMES) {
    ...
    server.registerTool(
      ...
      withKillSwitchPreflight(name, def.buildHandler(deps), deps)
    );
...
function createStdioTransport(beforeClose, stdin, stdout) {
  ...
  const closeOnEnd = () => {
    if (closing) return;
    closing = true;
    void (beforeClose?.() ?? Promise.resolve()).catch((error51) => {
...
async function startStreamableHttpServer(options) {
  ...
  app.all("/mcp", async (req, res) => {
    if (httpAuthToken) {
      ...
      const authenticated = expected.length === received.length && timingSafeEqual(expected, received);
      if (!authenticated) {
  ...
  const httpServer = await new Promise((resolve22, reject) => {
    ...
    async close() {
      ...
      try {
        await new Promise((resolve22, reject) => {
          httpServer.close(
            (error51) => error51 ? reject(error51) : resolve22()
          );
        });
      } catch (error51) {
        errors.push(error51);
...
var BROKER_TOKEN_ENV = "DRORA_CUA_PERMISSION_BROKER_TOKEN";
var BROKER_SOCKET_ENV = "DRORA_CUA_PERMISSION_BROKER_SOCKET";
...
var COMPILED_LOCAL_DEVELOPMENT_RUNTIME = typeof __ZCODE_LOCAL_DEVELOPMENT_RUNTIME__ !== "undefined" ? __ZCODE_LOCAL_DEVELOPMENT_RUNTIME__ : true;
...
var SkyshotDiffer = class {
  ...
  render(appKey, roots, opts) {
    ...
    const removed = prevLines.filter((l) => !currSigs.has(l.sig)).map((l) => l.index).sort((a, b) => a - b);
...
var identifierSchema = z.string().trim().min(1).max(255).refine((value) => !value.includes("\0"), "identifier cannot contain NUL").refine(
...
var pipCaptureTargetSchema = z.object({
...
var configSchema = z.object({
  ...
  socketPath: z.string().refine((path) => isWindowsNamedPipePath(path) && path.length > "\\\\.\\pipe\\".length),
...
var BROKER_REFRESH_MARKER_ENV = "DRORA_CUA_PERMISSION_BROKER_REFRESH_MARKER";
...
async function main(opts = {}) {
  if (opts.transport === "streamable-http") {
    ...
    await new Promise((resolve22) => {
      ...
      const shutdown = () => {
        if (closing) return;
        closing = true;
        void running.close().catch((error51) => {
          console.error(
  ...
  const stdioShutdown = () => {
    void runStdioCleanup().catch((error51) => {
      console.error(
