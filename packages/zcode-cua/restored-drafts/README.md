...
`@zcode/zcode-cua` 的真实实现已从草稿落地为 `src/` 源码（tsc 0 错误、根级 typecheck+lint+fmt 全绿、mock broker 冒烟通过）。本目录保留为还原过程档案，`.txt` 草稿不再参与任何流程；后续语义化（混淆名 → 语义名）直接在 `src/broker/server/*.ts` 上继续。
...
- 构建：`pnpm --filter @zcode/zcode-cua build`（tsc，outDir "." 镜像发射到包根 `broker/`，产物随包分发并已加入 oxlint ignorePatterns）；根 facade（broker.js / broker-server.js / broker-helper-health.js / broker-socket-path.js）只做 re-export。
