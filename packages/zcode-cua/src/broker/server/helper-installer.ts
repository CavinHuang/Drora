...
import { createServer as xse } from "node:net";
...
export function mb(e = fW()) {
  let t: any = e.ZCODE_DEPS_BASE_URL?.trim();
  return t ? t.replace(/\/+$/, "") : `http://${ub(e)}:${fb}/zcode/deps`;
}

export var Cie = ".zcode-cua-helper-meta.json",
  wW = 3e4,
...
export function Mie(e: any = {}) {
  ...
  if (r !== "darwin")
    throw new CuaHelperError(
      "install_failed",
      `ZCode Computer Use auto-install is only supported on macOS, got ${r}`,
    );
  let o = Jie(
      (n ? (e.targetArch ?? t.ZCODE_TARGET_ARCH ?? t.npm_config_arch) : void 0) ?? process.arch,
    ),
    s =
      xie({
        env: t,
  ...
  let l: any = e.bundledAppPath?.trim() || null;
  if (!n && !l)
    throw new CuaHelperError(
      "install_failed",
  ...
  let p: any = l
      ...
      : (() => {
          let f: any = `ZCode-CUA-Helper-${s}-mac-${o}.zip`;
          return {
            kind: "download",
            fileName: f,
  ...
  if (!u)
    throw new CuaHelperError(
      "install_failed",
      `Cannot resolve a safe Computer Use Helper install root from ${HELPER_INSTALL_VARIANT_ENV}.`,
    );
  return {
    version: s,
    platform: r,
...
export function Die(e, t) {
  ...
  if (
    ...
    throw new CuaHelperError(
      "install_failed",
      `Refusing insecure ZCode Computer Use install-lock directory ${e}; it must be a real owner-controlled directory without group/other write access`,
    );
}
...
export function Gc(e, t) {
  ...
  if (
    ...
    throw new CuaHelperError(
      "install_failed",
      `Refusing insecure ZCode Computer Use install lock ${e}; it must be a real owner-only regular file with exactly one link`,
    );
}
...
export async function Lie(e) {
  let t = await yb(e).catch((s) => {
    throw new CuaHelperError(
      "install_failed",
      `Cannot resolve ZCode Computer Use install-lock directory ${e}: ${jn(s)}`,
      {
        cause: s,
      },
    );
  ...
  try {
    ...
    if (s.code !== "EEXIST")
      throw new CuaHelperError(
        "install_failed",
        `Cannot create ZCode Computer Use install lock ${n}: ${jn(s)}`,
        {
          cause: s,
        },
      );
    try {
      ...
      throw Nie(a)
        ...
        : new CuaHelperError(
            "install_failed",
            `Refusing unsafe ZCode Computer Use install lock ${n}: ${jn(a)}`,
            {
              cause: a,
            },
          );
  ...
  try {
    ...
    if ((Gc(n, s), Gc(n, a), c !== Kn(t, SW) || s.dev !== a.dev || s.ino !== a.ino))
      throw new CuaHelperError(
        "install_failed",
        `ZCode Computer Use install lock changed while it was being opened: ${n}`,
      );
    return {
      lockPath: n,
      identity: {
...
export function Fie(e, t) {
  ...
  try {
    ...
    if (
      ...
      throw new CuaHelperError(
        "install_failed",
        `ZCode Computer Use install lock changed while its lease was held: ${e.lockPath}`,
      );
  } catch (n) {
    throw n instanceof CuaHelperError
      ...
      : new CuaHelperError(
          "install_failed",
          `ZCode Computer Use install lease is no longer verifiable: ${jn(n)}`,
          {
            cause: n,
          },
        );
  ...
  for (;;) {
    ...
    try {
      ...
      if (Date.now() >= r)
        throw new CuaHelperError(
          "install_failed",
          `Failed to acquire ZCode Computer Use install lease: timed out after ${t}ms; ${o.message}`,
          {
            cause: o,
          },
        );
...
export async function zie(e: any, t: any = {}) {
  ...
  let n =
      typeof t.waitTimeoutMs == "number" && Number.isFinite(t.waitTimeoutMs) && t.waitTimeoutMs > 0
        ? t.waitTimeoutMs
        : _ie,
  ...
  try {
    ...
    if ((Gc(r.lockPath, a), a.dev !== r.identity.device || a.ino !== r.identity.inode))
      throw new CuaHelperError(
        "install_failed",
        `ZCode Computer Use install lock changed before its lease was used: ${r.lockPath}`,
      );
  } catch (a) {
    throw (
      ...
      a instanceof CuaHelperError
        ...
        : new CuaHelperError(
            "install_failed",
            `Failed to acquire ZCode Computer Use install lease: ${jn(a)}`,
            {
              cause: a,
            },
          )
...
export function Wie(e) {
  ...
  let o = (async () => {
    ...
    let s: any = await e.dependencies.acquireInstallLease(e.plan);
    try {
...
export async function $ie(e, t) {
    ...
    try {
      ...
      if (await Hie(n, a.bundleInfo.executableName))
        ...
        try {
          ...
          r?.warn(
            void 0,
            `verified installed cua helper but failed to refresh install metadata: ${jn(d)}`,
          );
        }
        return (
          r?.info(
            void 0,
            n.allowUnsignedLocalDev
      ...
      if (n.allowUnsignedLocalDev && n.source.kind !== "bundled")
        throw new CuaHelperError(
          "verification_failed",
          `Local unsigned ZCode Computer Use.app failed dev verification: ${jn(a)}. Rebuild and reinstall it at ${n.appPath}, or unset ZCODE_CUA_HELPER_ALLOW_UNSIGNED_LOCAL for the signed release path.`,
          {
            cause: a,
          },
        );
  ...
  if (n.allowUnsignedLocalDev && n.source.kind !== "bundled")
    throw new CuaHelperError(
      "helper_missing",
      `Local unsigned ZCode Computer Use.app is not installed at ${n.appPath}. Build the Helper locally and copy it there, or unset ZCODE_CUA_HELPER_ALLOW_UNSIGNED_LOCAL for the signed release installer.`,
    );
  let s = await gie(Kn(n.installRoot, ".cua-helper-install-"));
  try {
    let a =
        n.source.kind === "bundled" ? await jie(n.source.appPath, s) : await Kie(e, s, n.source),
      c = await Dh(a, n, o);
...
export async function jie(e, t) {
  ...
  return (
    await mie(e, n, {
      ...
      throw new CuaHelperError(
        "install_failed",
        `Failed to stage bundled ZCode Computer Use.app: ${jn(r)}`,
        {
          cause: r,
        },
      );
...
export async function Kie(e, t, n) {
  ...
  o?.info(
    void 0,
    `installing local-development cua helper ${r.version} ${r.platformKey} from ${qh(n.url)}`,
  );
  ...
  (await s.downloadFile(n.url, a).catch((l) => {
    ...
    throw new CuaHelperError(
      "download_failed",
      `Failed to download ZCode Computer Use from ${p}: ${u}`,
      {
        cause: l,
      },
    );
    ...
    await s.extractZip(a, c).catch((l) => {
      throw new CuaHelperError(
        "install_failed",
        `Failed to extract ZCode Computer Use archive ${a}: ${jn(l)}`,
        {
          cause: l,
        },
      );
...
export function Yie(e) {
  ...
  try {
    ...
    throw new CuaHelperError(
      "download_failed",
      `invalid ZCode Computer Use download URL: ${qh(e)}`,
    );
  }
  if (t.protocol !== "https:" && t.protocol !== "http:")
    throw new CuaHelperError(
      "download_failed",
      `refusing to download ZCode Computer Use over unsupported scheme "${t.protocol}" (only http/https are allowed)`,
    );
}
...
export async function tse(e) {
  let { stdout: t } = await Hr(dn.unzip, ["-Z1", e]),
    n = t
      .split(`
...
`)
  ...
  for (let r of n)
    if (ese(r))
      throw new CuaHelperError(
        "install_failed",
        `refusing to extract Computer Use Helper archive: unsafe zip entry path ${JSON.stringify(r)}`,
      );
}

export async function nse(e) {
  let t = await yb(e),
    n = (o) => o === t || o.startsWith(`${t}${vie}`),
    r = [e];
  for (; r.length > 0; ) {
    let o = r.pop(),
      s = await Sb(o, {
    ...
    for (let a of s) {
      ...
      if (a.isSymbolicLink()) {
        ...
        if (!n(l))
          throw new CuaHelperError(
            "install_failed",
            `refusing extracted Computer Use Helper: symlink escapes staging (${c} -> ${d})`,
          );
...
export async function kW(e, t) {
  await t.dependencies.clearQuarantine(e).catch((n) => {
    t.logger?.warn(
      void 0,
      `cua helper installed and verified, but quarantine cleanup failed: ${jn(n)}`,
    );
  });
}
...
export async function PW(e, t) {
  await hie(
    Kn(e.installRoot, Cie),
    `${JSON.stringify({ provider: "zcode-cua-helper", version: e.version, buildId: t.bundleInfo.buildId, platform: e.platformKey, source: e.source.kind === "bundled" ? "bundled:zcode-app" : qh(e.source.url), bundleId: t.bundleInfo.bundleId, displayName: yie(e.appPath, ".app"), teamIdentifier: t.codesign?.teamIdentifier ?? null, verificationMode: t.mode, releaseEligible: t.releaseEligible }, null, 2)}
  ...
  for (; r.length > 0; ) {
    ...
    if (a.isSymbolicLink()) {
      ...
      if (l === ".." || l.startsWith(`..${process.platform === "win32" ? "\\" : "/"}`) || dse(l))
        throw new Error(
          `ZCode Computer Use contains an out-of-bundle symlink: ${s.relativePath} -> ${c}`,
        );
...
export function bb(e, t) {
  ...
  try {
    ...
    throw new Error(
      `ZCode Computer Use changed or became unreadable after verification: ${r instanceof Error ? r.message : String(r)}`,
    );
  }
  if (n !== t) throw new Error("ZCode Computer Use changed after signature verification");
}

export var Dse = ".host-reservation",
  CUA_PIP_NO_ACTIVE_SESSION_PENDING = ".pending",
  CUA_HEALTH_POLL_MS = 100;
