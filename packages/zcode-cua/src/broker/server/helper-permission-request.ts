...
function requestRuntimeDir(env: NodeJS.ProcessEnv): string {
 ...
 */
export async function requestHelperPermissionViaLaunchServices(
  ...
  const cancelFilePath = join(
    requestRuntimeDir(env),
    `.permission-request-${randomUUID()}.sentinel`,
  );
  const args = [
    "-W",
    "-n",
