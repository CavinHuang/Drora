...
import { homedir as sae } from "node:os";
      ...
      new Promise((r, o) => {
        n = setTimeout(() => {
          o(
            new CuaHelperError(
              "caller_timeout",
              `ZCode Computer Use is still starting after ${t}ms; retry the task shortly`,
            ),
          );
        }, t);
      }),
...
export function QW(e, t: { hasActiveTurn?: () => boolean } = {}) {
  let n = null,
    r = null,
    o = null,
    s = null,
  ...
  function u() {
    ...
    let M = o,
      O = (async () => (
        M && (await M.catch(() => {})), (await Eb(e, () => zi(e))).handle
      ))().finally(() => {
        r === O && (r = null);
      });
    return ((r = O), O);
...
export var Qse = ["zcode.json", Tb(".zcode", "config.json")],
  eae = ".git";
