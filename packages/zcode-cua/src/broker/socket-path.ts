 ...
 */
export function pruneStaleBrokerSockets(dir: string): void {
  try {
    const cutoffMs = Date.now() - STALE_SOCKET_MAX_AGE_MS;
    for (const entry of readdirSync(dir)) {
      if (!/^broker-[0-9a-f]{16}\.sock$/u.test(entry)) continue;
      const socketPath = join(dir, entry);
      try {
        if (statSync(socketPath).mtimeMs >= cutoffMs) continue;
 ...
 * - 其余：~/.zcode/cua-broker
 */
export function brokerRuntimeDir(env: NodeJS.ProcessEnv = process.env): string {
...
export function mintBrokerSocketPath(
...
export function resolveBrokerSocketPath(
  ...
  if (process.platform === "win32") return `${WINDOWS_PIPE_PREFIX}default`;
  if (options.dir) return join(options.dir, DEFAULT_SOCKET_FILE_NAME);
...
export {
  mintBrokerSocketPath as Uu,
  brokerRuntimeDir as fa,
  isWindowsNamedPipePath as ma,
  BROKER_SOCKET_ENV_NAME as oo,
  BROKER_UNAVAILABLE_ENV_NAME as ti,
};
