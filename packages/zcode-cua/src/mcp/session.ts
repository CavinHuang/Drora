...
function createPointerHoldGuard(options: any = {}) {
...
var InputHoldRegistry = class {
  constructor(sessionKey) {
    (this as any).sessionKey = sessionKey;
  }
  nextId = 0;
  active = /* @__PURE__ */ new Set();
  cancellationClaimed = /* @__PURE__ */ new Set();
  begin() {
    const key = `${(this as any).sessionKey}:hold:${++this.nextId}`;
    this.active.add(key);
    let ended = false;
    return {
      key,
...
var AccessibilitySession = class {
  ...
  beginMutation(appRef2, preStateId) {
    const preActionState =
      preStateId === null
        ? null
        : preStateId === void 0
          ? this.latestStateForAppRef(appRef2)
          : (this.entries.get(preStateId) ?? null);
   ...
   */
  latestStateForAppRef(appRef2, options: any = {}) {
    ...
    for (const state of [...this.entries.values()].reverse()) {
      ...
      if (
        !(options as any).ignoreWindow &&
        typeof ref.window_id === "number" &&
        state.window.window_id !== ref.window_id
      )
        continue;
      matches.push(state);
    }
    if (matches.length === 0) return null;
    if (!(options as any).ignoreWindow && typeof ref.window_id !== "number") {
  ...
  claimEmptyTreeWarmup(app, window2) {
    const appKey =
      typeof app.pid === "number"
        ? `pid:${app.pid}`
        : typeof app.bundle_id === "string" && app.bundle_id
          ? `bundle:${app.bundle_id.toLowerCase()}`
          : `name:${app.name ?? ""}`;
    const windowKey =
      typeof window2.window_id === "number"
        ? `wid:${window2.window_id}`
        : `geometry:${window2.title ?? ""}:${window2.bounds?.join(",") ?? ""}`;
...
function mutationScopeFrom(appRef2, fallback) {
  ...
  return {
    ...
    bundle_id:
      typeof ref.bundle_id === "string" ? ref.bundle_id : (fallback?.app.bundle_id ?? null),
    name: typeof ref.name === "string" ? ref.name : (fallback?.app.name ?? null),
    window_id:
      typeof ref.window_id === "number" ? ref.window_id : (fallback?.window.window_id ?? null),
  };
}
...
function sameWindow(cached2, observed) {
  ...
  return (
    cached2.title === observed.title &&
    cached2.bounds.length === observed.bounds.length &&
    cached2.bounds.every((value, index) => value === observed.bounds?.[index])
  );
}
...
async function probeScreenshotSize(screenshot) {
  ...
  try {
    ...
    if (
      typeof meta3.width === "number" &&
      typeof meta3.height === "number" &&
      meta3.width > 0 &&
      meta3.height > 0
    ) {
...
async function issueCaptureFrame(raw, app, window2, session) {
  ...
  const geometryVerified =
    raw.coordinate_contract === "frame_pixel_projection_v1" &&
    pointer !== null &&
    pointer !== void 0 &&
    [pointer.x, pointer.y, pointer.width, pointer.height].every(
      (part) => typeof part === "number" && Number.isFinite(part),
    ) &&
    (pointer.width ?? 0) > 0 &&
    (pointer.height ?? 0) > 0 &&
    typeof pointer.unit === "string" &&
    typeof raw.display?.topology_fingerprint === "string" &&
    raw.display.topology_fingerprint.length > 0;
  ...
  const malformedCaptureSurfaces =
    raw.capture_surfaces !== void 0 &&
    raw.capture_surfaces !== null &&
    parsedCaptureSurfaces === void 0;
  const surfaceStatus = raw.capture_surfaces_status;
  const nonActionableReason = screenshotBlank
    ? "screenshot_blank"
    : surfaceStatus === "capture_surfaces_changed"
      ? "capture_surfaces_changed"
      : surfaceStatus === "capture_surfaces_unavailable"
        ? "capture_surfaces_unavailable"
        : malformedCaptureSurfaces
          ? "capture_surfaces_unavailable"
          : raw.capture_surfaces &&
              raw.capture_surfaces.before_fingerprint !== raw.capture_surfaces.after_fingerprint
            ? "capture_surfaces_changed"
            : geometryVerified
              ? null
              : "capture_geometry_unverified";
...
async function ingestCaptureAppResult(raw, options) {
  ...
  const window2 = {
    ...
    bounds: raw.window.bounds
      ? asBounds(raw.window.bounds, "capture_app window.bounds")
      : [0, 0, 0, 0],
    window_id: typeof raw.window.window_id === "number" ? raw.window.window_id : null,
    window_id_fallback: raw.window.window_id_fallback === true || void 0,
    empty_capture_reason:
      typeof raw.window.empty_capture_reason === "string" &&
      raw.window.empty_capture_reason.length > 0
        ? raw.window.empty_capture_reason
        : void 0,
    note:
      raw.window.window_id_fallback === true && typeof raw.window.note === "string"
        ? raw.window.note
        : void 0,
    actual_window_id:
      typeof raw.window.actual_window_id === "number"
        ? raw.window.actual_window_id
        : typeof raw.window.window_id === "number"
          ? raw.window.window_id
          : null,
    presentation_window_id:
      typeof raw.window.presentation_window_id === "number"
        ? raw.window.presentation_window_id
        : typeof raw.window.window_id === "number"
          ? raw.window.window_id
          : null,
    surface_kind: asSurfaceKind2(raw.window.surface_kind) ?? "window",
    surface_lifecycle:
      raw.window.surface_lifecycle === "replaced" || raw.window.surface_lifecycle === "closed"
        ? raw.window.surface_lifecycle
        : "stable",
  };
  ...
  if (imageRef) {
    (state as any).image_ref = imageRef;
  }
  session.remember(state);
  return state;
}
var GLOBAL_SCREEN_COORDINATE_CONTRACT =
  "coordinate_contract: bounds=[x,y,width,height] in global_screen_points; screen_rect=[x0,y0,x1,y1]; zoom.region uses exact image pixels from the current raster";
var EMPTY_CAPTURE_GUIDANCE = {
  // 此时菜单栏通常是完整可读的（元素表里的 menubar 项就来自它），且 File > New … /
  // open_app 都能把窗口开回来。所以这条路径是"换动作"，不是"读失败"。
  "cg-only-no-ax-window":
    "This app has closed all of its real windows \u2014 the window_id above is a WindowServer leftover that is neither visible to the user nor readable through Accessibility. Do NOT retry this read. Either drive the app's menu bar (its items are listed below, e.g. File > New) or reopen the app with open_app to get a real window back.",
  // 窗口确实解析到了，但整棵树是空的：多为 app 无响应 / 树尚未构建。重试或换窗口有意义。
  "ax-window-empty-tree":
    "A window resolved but exposed no Accessibility children \u2014 the app may be busy or still building its tree. Take a screenshot to see what is actually on screen, or call list_windows to pick a different window before reading again.",
};
...
function formatAppStateTree(state, detail = "compact") {
  ...
  const fallbackNote = win.window_id_fallback
    ? `[window_id_fallback] ${win.note ?? "The requested window could not be resolved; a different live window was returned. Call list_windows before acting."}`
    : "";
  const emptyCaptureNote = win.empty_capture_reason
    ? `[${win.empty_capture_reason}] ${EMPTY_CAPTURE_GUIDANCE[win.empty_capture_reason] ?? "This app exposed no window tree. Retrying the same read is unlikely to help; re-resolve the app with list_windows first."}`
    : "";
  const surfaceLine = win.surface_kind
    ? `surface: kind=${win.surface_kind} actual_window_id=${win.actual_window_id ?? win.window_id ?? "unknown"} presentation_window_id=${win.presentation_window_id ?? win.window_id ?? "unknown"} lifecycle=${win.surface_lifecycle ?? "stable"}`
    : "";
  ...
  const truncatedNote =
    truncatedCount > 0
      ? `(showing top ${entries.length} of ${state.elements.length} by priority; ${truncatedCount} low-priority elements hidden)`
      : "";
  return [
...
function bindFramePixelTarget(target, context) {
  ...
  const frame =
    typeof explicitFrameId === "string" && explicitFrameId.length > 0
      ? context.frame_registry.resolveForAction(explicitFrameId)
      : context.frame_registry.resolveLatestForAction();
  const bound = {
    type: "coordinate",
...
function parseAppRef(value) {
  ...
  if (present(bundleIdRaw, "bundle_id", obj)) {
    ...
    (result as any).bundle_id = bundleIdRaw;
  }
  ...
  if (present(pidRaw, "pid", obj)) {
    ...
    (result as any).pid = pidRaw;
  }
  const windowIdRaw = obj.window_id;
  if (present(windowIdRaw, "window_id", obj)) {
    if (
      typeof windowIdRaw === "boolean" ||
      typeof windowIdRaw !== "number" ||
      !Number.isInteger(windowIdRaw)
    ) {
      throw new Error("app_ref.window_id must be an integer");
    }
    (result as any).window_id = windowIdRaw;
  }
...
var elementTargetJSONSchema = {
  ...
  description:
    'Element target (PREFERRED): accessibility-driven, works on a BACKGROUND app (no focus steal) -- {"type":"element","state_id":"s-1","index":4}. state_id/index come from the most recent get_app_state.',
  properties: {
    type: { const: "element" },
    state_id: {
      type: "string",
...
var coordinateTargetJSONSchema = {
  ...
  required: ["type", "x", "y"],
  description:
    'Image-pixel target {"type":"coordinate","x":496,"y":331}. x/y are integer pixel indices in the latest actionable raster returned by this transport. Retina, DPI, crop, resize, display origin and pointer units are resolved internally.',
  properties: {
    ...
    frame_id: {
      ...
      minLength: 1,
      description:
        "optional explicit frame_id for backward compatibility; omit it to use CUA's transport-local current raster binding",
    },
...
var targetArgJSONSchema = {
  description:
    "Choose exactly one semantic path. Use an element target only when selecting an accessibility-tree element. When the action is derived from a delivered image, use the coordinate target and copy integer pixels from that exact raster; CUA binds its frame internally.",
...
var OPEN_APPLICATION_APP_PROPERTIES = {
  ...
  name: {
    type: "string",
    description:
      'Copy the user-provided application name character-for-character. For example, use {"name":"\u7F51\u6613\u4E91\u97F3\u4E50app"}, not {"name":"\u7F51\u6613\u4E91\u97F3\u4E50"}; use {"name":"\u65E5\u5386"}, not {"name":"Calendar"}.',
  },
...
var openApplicationAppArgJSONSchema = {
  description:
    'Application to launch/resolve. Put fields directly inside app: {"name":"Google Chrome","url":"https://..."} or {"bundle_id":"com.google.Chrome","urls":["https://..."]} or {"pid":12345,"name":"Google Chrome"} for an already-running app. Do not use {"app":"Google Chrome"} inside this object.',
...
function parseTarget(value) {
  ...
  if (kind === "coordinate") {
    ...
    if (
      obj.frame_id !== void 0 &&
      (typeof obj.frame_id !== "string" || obj.frame_id.length === 0)
    ) {
...
function findFingerprintMatch(stored, candidates, options: any = {}) {
  if (!stored.title) {
    return null;
  }
  const valueSignal = stored.editable && stored.value.length > 0;
  const highNearnessWaived = (options as any).singleWindowApp === true;
  const high = [];
  const medium = [];
  ...
  const highNear = highNearnessWaived
    ? high
    : high.filter((el) => boundsNear(el.bounds, stored.bounds));
  let match;
  ...
  return {
    reason:
      criteria === "role+title+value"
        ? "element fingerprint changed; re-resolved by role+title+value in a fresh capture (the original handle was gone or its identity changed)"
        : "element fingerprint changed; re-resolved by role+title in a fresh capture (the original handle was gone or its identity changed)",
...
function findSameCaptureVisualMatchInRawElements(stored, storedNative, rawElements) {
  ...
  const exactGeometry = candidates.filter(
    (candidate) =>
      ...
      candidate.bounds.every((value, index) => Math.abs(value - stored.bounds[index]) <= 0.5) &&
      (stored.value.length === 0 || normalizeValue(candidate.value) === stored.value),
  );
...
async function captureFreshElements(deps, appRef2) {
  ...
  const windowRecord =
    window2 && typeof window2 === "object" && !Array.isArray(window2) ? window2 : {};
  return {
    elements: elements.map((entry) => normalizeCandidate(entry)),
    actual_window_id:
      typeof windowRecord.actual_window_id === "number"
        ? windowRecord.actual_window_id
        : typeof windowRecord.window_id === "number"
          ? windowRecord.window_id
          : null,
    presentation_window_id:
      typeof windowRecord.presentation_window_id === "number"
        ? windowRecord.presentation_window_id
        : typeof windowRecord.window_id === "number"
          ? windowRecord.window_id
          : null,
  };
...
function freshSurfaceMatches(stored, fresh) {
  const storedIsAttachedSurface =
    stored.actual_window_id !== null &&
    stored.actual_window_id !== void 0 &&
    stored.presentation_window_id !== null &&
    stored.presentation_window_id !== void 0 &&
    stored.actual_window_id !== stored.presentation_window_id;
  if (
    !storedIsAttachedSurface &&
    fresh.actual_window_id === null &&
    fresh.presentation_window_id === null
  ) {
    return true;
  }
  return (
    (stored.actual_window_id === null ||
      stored.actual_window_id === void 0 ||
      stored.actual_window_id === fresh.actual_window_id) &&
    (stored.presentation_window_id === null ||
      stored.presentation_window_id === void 0 ||
      stored.presentation_window_id === fresh.presentation_window_id)
  );
}
...
function appRefFromState(state) {
  const app = state.app;
  const ref = {};
  if (app.bundle_id) (ref as any).bundle_id = app.bundle_id;
  if (typeof app.pid === "number") (ref as any).pid = app.pid;
  if (typeof app.window_id === "number") (ref as any).window_id = app.window_id;
  if (app.name) (ref as any).name = app.name;
  return ref;
}
...
function buildFromElement(element, state) {
  ...
  if (typeof element.native === "string" && element.native) {
    (resolution as any).native = element.native;
  }
  const [, , bw, bh] = element.bounds;
  if (bw > 0 && bh > 0) {
    (resolution as any).point = visibleElementCenter(element.bounds, state.window.bounds);
  }
  if (!(resolution as any).point && !(resolution as any).native) {
    throw new TargetResolveError(
  ...
  (resolution as any).fingerprint = captureFingerprint(element, state.app.bundle_id, {
...
function resolveTarget(target, ctx) {
  ...
  const bindingKind =
    ...
    originalBindingKind === "implicit" &&
    typeof target.frame_id === "string"
      ? originalBindingKind
      : frameBindingKind(parsed);
...
function displayTopologyFingerprint(displays) {
  return [...displays]
    ...
    .map(
      ({ id, bounds, main: main22, scale_factor }) =>
        `${id}:${bounds.join(",")}:${main22 ? 1 : 0}:${scale_factor}`,
    )
    .join("|");
}
...
async function resolveCapturedDisplay(_deps, reply) {
  ...
  const matching = directTopology.filter(
    (candidate) =>
      ...
      candidate.index === direct.index &&
      candidate.scale_factor === direct.scale_factor &&
      candidate.bounds.every((part, index) => part === direct.bounds[index]),
  );
...
var InvalidTextRangeError = class extends Error {
  constructor(reason) {
    super(
      reason === "not_array_of_2"
        ? "text_range must be a 2-element array [start, length]"
        : "text_range start and length must be >= 0",
    );
...
var zoomSchema = z
  .object({
    ...
    region: stringifiedJsonArray(z.array(numInt).length(4))
      .nullish()
      .describe(
...
var ZOOM_ERR_NEITHER =
  'zoom: provide a region [x0,y0,x1,y1] or a target {"type":"element","state_id":"...","index":N}.';
...
async function composeZoom(deps, args, options) {
  ...
  if (args.target) {
    ...
    if (!(resolved as any).point) {
      throw new Error(
        "zoom target element has no on-screen bounds; call get_app_state again and pick a visible element.",
      );
    }
    targetFingerprint = (resolved as any).fingerprint ?? null;
    targetNative = (resolved as any).native ?? null;
    ...
    sourceFrame = args.frame_id
      ? session.frameRegistry.resolveForAction(args.frame_id)
      : session.frameRegistry.resolveLatestForAction();
  ...
  if (retainedFrame) {
    ...
    const liveMatch = targetFingerprint
      ? findSameCaptureVisualMatchInRawElements(targetFingerprint, targetNative, raw.elements)
      : null;
    if (!liveMatch) {
      throw new Error(
  ...
  if (!imageSize) {
    throw new Error(
      retainedFrame
        ? "zoom: retained source frame could not be decoded."
        : "zoom: capture_app screenshot could not be decoded.",
    );
  }
  const { width: imgW, height: imgH } = imageSize;
  if (
    retainedFrame &&
    (imgW !== retainedFrame.delivered.width || imgH !== retainedFrame.delivered.height)
  ) {
    throw new Error(
  ...
  const preBudgetWidth =
    maxEdge > DEFAULT_MAX_EDGE
      ? Math.max(1, Math.round(cropWidth * (DEFAULT_MAX_EDGE / maxEdge)))
      : cropWidth;
  const preBudgetHeight =
    maxEdge > DEFAULT_MAX_EDGE
      ? Math.max(1, Math.round(cropHeight * (DEFAULT_MAX_EDGE / maxEdge)))
      : cropHeight;
  ...
  const windowGeometryVerified =
    retainedFrame !== null ||
    (captureAppRaw?.coordinate_contract === "frame_pixel_projection_v1" &&
      captureAppRaw.screenshot_blank !== true &&
      captureAppRaw.pointer !== null &&
      captureAppRaw.pointer !== void 0 &&
      captureAppRaw.pointer.unit !== void 0 &&
      framePointer.width > 0 &&
      framePointer.height > 0 &&
      typeof captureAppRaw.display?.topology_fingerprint === "string");
  const frameDescriptor = session.issueFrame({
    ...
    source: {
      ...
      crop: retainedFrame
        ? composeSourceCrop(retainedFrame, {
            left: px0,
            top: py0,
            right: px1,
            bottom: py1,
    ...
    app: retainedFrame
      ...
      : {
          ...
          executable_path:
            typeof capturedApp?.executable_path === "string" ? capturedApp.executable_path : null,
          icon_png: typeof capturedApp?.icon_png === "string" ? capturedApp.icon_png : null,
        },
    window: {
      window_id: retainedFrame
        ? retainedFrame.window.window_id
        : typeof captureAppRaw?.window?.window_id === "number"
          ? captureAppRaw.window.window_id
          : null,
    ...
    display: {
      display_id: retainedFrame
        ? retainedFrame.display.display_id
        : (captureAppRaw?.display?.id ?? null),
      topology_fingerprint: retainedFrame
        ? retainedFrame.display.topology_fingerprint
        : (captureAppRaw?.display?.topology_fingerprint ?? "unverified"),
      origin: frameOrigin,
    },
    ...(retainedFrame?.capture_windows !== void 0
      ? { capture_windows: retainedFrame.capture_windows }
      : {}),
    actionable: windowGeometryVerified,
    non_actionable_reason: windowGeometryVerified
      ? null
      : captureAppRaw?.screenshot_blank === true
        ? "screenshot_blank"
        : "capture_geometry_unverified",
...
function findAppInstruction(input) {
  ...
  const title =
    typeof input.window_title === "string" && input.window_title.trim()
      ? norm(input.window_title)
      : "";
  if (title) {
    for (const entry of APP_INSTRUCTIONS) {
      for (const pattern of entry.name_patterns) {
...
async function loadRgbaRawFromBuffer(png) {
  ...
  const sharp = await loadSharp();
  const { data, info } = await sharp(stripped)
    .ensureAlpha()
    .toColorspace("srgb")
    .raw({ depth: "uchar" })
...
async function encodeScreenshotRaw(rgb, opts: any = {}) {
  const maxEdge = (opts as any).maxEdge ?? MAX_EDGE;
  const quality = (opts as any).quality ?? 75;
  const maxB64 = (opts as any).maxAgentB64 ?? AGENT_B64_BUDGET;
  const W = rgb.width;
  const H = rgb.height;
  ...
  const assign = (targetIndex, visited) => {
    for (const window2 of candidateWindows) {
      ...
      const existingWindowNavigated =
        baselineWindowIds.has(window2.window_id) &&
        !titleMatches(baselineTitle, target) &&
        titleMatches(window2.title, target);
      if (
        (!isFresh && !existingWindowNavigated) ||
        visited.has(window2.window_id) ||
        !titleMatches(window2.title, target)
      ) {
        continue;
      }
    ...
    return {
      ...
      windowTitles: new Map(
        windows
          .filter(
            (window2) =>
              typeof window2.window_id === "number" && Number.isInteger(window2.window_id),
          )
...
async function verifyFileUrlWindowEffect(listWindows, baseline, options: any = {}) {
  ...
  const attempts = (options as any).attempts ?? FILE_URL_WINDOW_VERIFY_ATTEMPTS;
  const delay =
    (options as any).delay ??
    ((milliseconds) => new Promise<void>((resolve22) => setTimeout(resolve22, milliseconds)));
  let lastWindows = [];
...
function withOfficialFrameIntegrity(result, imageRef, data, format, session) {
  ...
  if (
    ...
    !frame ||
    frame.image_ref.width !== imageRef.width ||
    frame.image_ref.height !== imageRef.height ||
    normalizeImageFormat(frame.delivered.format) !== normalizeImageFormat(format) ||
    frame.delivered.digest !== digest
  ) {
    return toolError(
...
function toolError(text, details?: unknown) {
  return details === void 0
    ? { isError: true, content: [textBlock(text)] }
    : { isError: true, content: [textBlock(text)], structuredContent: { ...(details as any) } };
}
...
function mapAppStateResult(result) {
  ...
  const r = result;
  const unboundImageWarning =
    "unbound_image_suppressed: the Helper returned image bytes without an immutable image_ref/frame transform. Capture again with the current Helper; no image was delivered to the model.";
...
async function mapRawCaptureAppResult(result, detail, includeTree, options) {
  ...
  if (!screenshotBlank && typeof screenshot?.data === "string" && screenshot.data) {
    if ((state as any).image_ref) {
      const format = screenshot.format ?? "jpeg";
      content.push(...imageWithRef((state as any).image_ref, screenshot.data, format));
      if ((state as any).image_ref.actionable) {
        emittedFrame = { imageRef: (state as any).image_ref, data: screenshot.data, format };
      }
    }
  }
  if (screenshotBlank) {
...
function actionReceiptBlock(receipt) {
  ...
  if (receipt.ax_error) (action_receipt as any).ax_error = receipt.ax_error;
  if (receipt.target_verification_status) {
    (action_receipt as any).target_verification_status = receipt.target_verification_status;
  }
...
function normalizeActionDispatchReceipt(result) {
  if (result && typeof result === "object") {
    const value = result;
    if (
      value.action_sent === true &&
      (value.dispatch_status === "accepted" || value.dispatch_status === "possibly_sent")
    ) {
      return {
        action_sent: true,
        dispatch_status: value.dispatch_status,
        ...(typeof value.ax_error === "string" ? { ax_error: value.ax_error } : {}),
        ...(value.target_verification_status === "matched" ||
        value.target_verification_status === "mismatched" ||
        value.target_verification_status === "unavailable"
          ? { target_verification_status: value.target_verification_status }
          : {}),
      };
    }
  }
...
async function postActionObserve(deps, opts) {
  ...
  const dispatchRecordBlock =
    opts.dispatchRecord && typeof opts.dispatchRecord === "object"
      ? textBlock(JSON.stringify(opts.dispatchRecord))
      : null;
  if (consistency) {
  ...
  if (opts.return_state === "none") {
    opts.onEffect?.("unknown");
    return {
      content: dispatchRecordBlock
        ? [textBlock(note), dispatchRecordBlock, actionReceiptBlock(receipt)]
        : [textBlock(note), actionReceiptBlock(receipt)],
      isError,
    };
  }
  if (!hasAppRef) {
        ...
        textBlock(
          JSON.stringify({
            state_observation: {
              ...
              message:
                "No app_ref was available for an optional post-action observation. Call get_app_state only if the next step needs UI state.",
            },
          }),
        ),
      ],
...
var listAppsTool = {
  ...
  title: "List Apps",
  description:
    "List running applications only: name, bundle_id (macOS bundle id / Windows AUMID / often empty on Linux), pid, active. A target missing from this list does not mean it is not installed. Do not substitute a different running application for the user-requested app. On Linux/Windows prefer name or pid. Read-only.",
...
On Linux, keyboard input (type/key without an element target) additionally needs the target window frontmost -- Mutter ignores programmatic focus.`,
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      if (args.new_instance === true) (params as any).new_instance = true;
      if (app.bundle_id !== void 0) (params as any).bundle_id = app.bundle_id;
      if (app.name !== void 0) (params as any).name = app.name;
      if (app.pid !== void 0) (params as any).pid = app.pid;
      if (app.window_id !== void 0) (params as any).window_id = app.window_id;
      if (app.urls !== void 0) (params as any).urls = app.urls;
      if (app.url !== void 0) (params as any).url = app.url;
      ...
      if (!requestedUrls && args.activate !== true && args.new_instance !== true) {
        ...
        if (Array.isArray(liveApps)) {
          const matches = liveApps.filter((candidate) => {
            ...
            if (
              app.bundle_id !== void 0 &&
              String(live.bundle_id ?? "").toLocaleLowerCase() !== app.bundle_id.toLocaleLowerCase()
            ) {
              return false;
            }
            if (
              ...
              String(live.name ?? "")
                .trim()
                .toLocaleLowerCase() !== app.name.trim().toLocaleLowerCase()
            ) {
              return false;
            }
            return true;
      ...
      const helperVerification =
        opened.file_window_verification &&
        typeof opened.file_window_verification === "object" &&
        !Array.isArray(opened.file_window_verification)
          ? opened.file_window_verification
          : null;
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      if (args.include_screenshot !== void 0) {
        (params as any).include_screenshot = args.include_screenshot;
      }
      ...
      if (captureWindows === null) {
        return toolError(
          JSON.stringify({
            ...
            message:
              "Full-display screenshot withheld because capture_windows ownership provenance is unavailable. No raster, local artifact, or frame authority was exposed. action_sent=false.",
            actionable: false,
            action_sent: false,
...
var zoomTool = {
  ...
  title: "Zoom",
  description:
    "Exceptional close-up for a target that is too small or ambiguous in the current raster. Crop the latest returned raster with region=[x0,y0,x1,y1]; an optional explicit frame_id is accepted for compatibility, and it never takes a replacement screenshot. Alternatively zoom an accessibility element target; the modes are mutually exclusive. The result is a new raster. Choose x/y only by looking at that child raster and submit them unchanged; frame binding is internal.",
...
var listDisplaysTool = {
  ...
  title: "List Displays",
  description:
    "List displays: index (1-based), bounds, main. Display bounds=[x,y,width,height] use global screen points. Read-only.",
...
var switchDisplayTool = {
  ...
  description:
    "Choose which display a future full-screen `screenshot` captures (1-based index from list_displays). This does not capture an image or create a frame. Previously returned immutable frame_ids keep their original pixel meaning; the next captured image receives a new adjacent image_ref.",
...
function parseScreenshotCaptureWindows(value) {
  ...
  const parsed = windows.map((entry) => {
    ...
    if (
      !Number.isSafeInteger(window2.window_id) ||
      window2.window_id <= 0 ||
      !Number.isSafeInteger(window2.owner_pid) ||
      window2.owner_pid <= 0 ||
      !Number.isSafeInteger(window2.layer) ||
      !(window2.owner_bundle_id === null || typeof window2.owner_bundle_id === "string") ||
      !Array.isArray(bounds) ||
      bounds.length !== 4 ||
      !bounds.every((part) => typeof part === "number" && Number.isFinite(part)) || // additive 字段：缺失/null 视为未知；出现了却不是 boolean 说明上游契约不一致，fail-closed。
      !(
        window2.accepts_left_mouse_down === void 0 ||
        window2.accepts_left_mouse_down === null ||
        typeof window2.accepts_left_mouse_down === "boolean"
      )
    ) {
      return null;
    }
    return {
      ...
      bounds,
      ...(typeof window2.accepts_left_mouse_down === "boolean"
        ? { accepts_left_mouse_down: window2.accepts_left_mouse_down }
        : {}),
    };
  });
...
var cursorPositionTool = {
  ...
  buildHandler: (deps) => async () => {
    try {
      ...
      const selected = Array.isArray(rawDisplays)
        ? rawDisplays.find((display) => display.id === selectedId)
        : void 0;
      if (!selected) {
...
function toolError2(text, details?, _cause?: unknown) {
  return details === void 0
    ? { isError: true, content: [textBlock2(text)] }
    : { isError: true, content: [textBlock2(text)], structuredContent: { ...(details as any) } };
}
...
function pointerActionError(error51, method) {
  ...
  const message = error51 instanceof Error ? error51.message : String(error51);
  const attributed = /\braw foreground event\b.*\brefused because\b/i.test(message)
    ? message.replace(/^click:/i, `${method}:`)
    : message;
...
function resolvedActionAppRef(result, fallback) {
  ...
  if (
    ...
    record2.pid <= 0 ||
    typeof record2.bundle_id !== "string" ||
    record2.bundle_id.length === 0
  ) {
    return fallback;
  }
  return {
    ...
    ...(Number.isSafeInteger(record2.window_id) && record2.window_id > 0
      ? { window_id: record2.window_id }
      : {}),
  };
}
async function resolveTargetForClick(deps, target) {
  try {
    ...
    if (!(resolved as any).point && !((resolved as any).native && (resolved as any).fingerprint)) {
      return toolError2(
        "target did not resolve to a screen point; use an element target with on-screen bounds or a coordinate target.",
      );
    }
    return {
      point: (resolved as any).point,
      app_ref: resolved.app_ref ?? null,
      fingerprint: (resolved as any).fingerprint,
      native: (resolved as any).native,
    };
  } catch (e) {
...
async function resolveExpectedPid(deps, appRef2, alreadyVerified = false) {
  ...
  const record2 =
    appRef2 && typeof appRef2 === "object" && !Array.isArray(appRef2) ? appRef2 : null;
  const hasAdditionalIdentityConstraint =
    typeof appRef2 === "string" ||
    (record2 !== null &&
      ((typeof record2.bundle_id === "string" && record2.bundle_id.trim().length > 0) ||
        (typeof record2.name === "string" && record2.name.trim().length > 0) ||
        (typeof record2.window_id === "number" && Number.isSafeInteger(record2.window_id))));
  ...
  const app =
    captured && typeof captured === "object" && !Array.isArray(captured) ? captured.app : void 0;
  ...
  const capturedWindow =
    captured && typeof captured === "object" && !Array.isArray(captured) ? captured.window : null;
  const capturedWindowRecord =
    capturedWindow && typeof capturedWindow === "object" && !Array.isArray(capturedWindow)
      ? capturedWindow
      : null;
  const requestedBundle =
    record2 && typeof record2.bundle_id === "string" ? record2.bundle_id.trim() : "";
  const requestedName = record2 && typeof record2.name === "string" ? record2.name.trim() : "";
  const requestedWindowId =
    record2 && typeof record2.window_id === "number" && Number.isSafeInteger(record2.window_id)
      ? record2.window_id
      : void 0;
  const capturedBundle =
    capturedApp && typeof capturedApp.bundle_id === "string" ? capturedApp.bundle_id : "";
  const capturedName = capturedApp && typeof capturedApp.name === "string" ? capturedApp.name : "";
  const capturedWindowId =
    capturedWindowRecord && typeof capturedWindowRecord.window_id === "number"
      ? capturedWindowRecord.window_id
      : capturedWindowRecord && typeof capturedWindowRecord.id === "number"
        ? capturedWindowRecord.id
        : void 0;
  if (
    (directPid !== void 0 && pid !== directPid) ||
    (requestedBundle && capturedBundle !== requestedBundle) ||
    (requestedName && capturedName !== requestedName) ||
    (requestedWindowId !== void 0 && capturedWindowId !== requestedWindowId)
  ) {
    throw unsentPointerIdentityError(
...
function frameActionProvenance(deps, ...targets) {
  ...
  return targets.flatMap((rawTarget, endpointIndex) => {
    ...
    if (!(resolved as any).point) return [];
    return [
      {
        ...
        pixel: { x: (target as any).x, y: (target as any).y },
        projected_point: (resolved as any).point,
...
function appRefMatchesFrame(rawAppRef, frame) {
  ...
  if (
    ...
    appRef2.pid !== null &&
    appRef2.pid !== void 0 &&
    appRef2.pid !== frame.app.pid
  ) {
    return false;
  }
  if (
    frame.app.bundle_id !== null &&
    appRef2.bundle_id !== null &&
    appRef2.bundle_id !== void 0 &&
    appRef2.bundle_id.toLocaleLowerCase() !== frame.app.bundle_id.toLocaleLowerCase()
  ) {
    return false;
  }
  if (
    frame.window.window_id !== null &&
    appRef2.window_id !== null &&
    appRef2.window_id !== void 0 &&
    appRef2.window_id !== frame.window.window_id
  ) {
    return false;
  }
  return (
    (frame.app.pid !== null && appRef2.pid === frame.app.pid) ||
    (frame.app.bundle_id !== null &&
      appRef2.bundle_id?.toLocaleLowerCase() === frame.app.bundle_id.toLocaleLowerCase()) ||
    (frame.window.window_id !== null && appRef2.window_id === frame.window.window_id)
  );
}
...
function frameDispatchIdentityError(deps, appRef2, ...targets) {
  ...
  try {
    for (const rawTarget of targets) {
      ...
      const frame = resolveCoordinateFrame(deps, target, bindingKind);
      if (
        (frame.app.pid !== null || frame.app.bundle_id !== null) &&
        !appRefMatchesFrame(appRef2, frame)
      ) {
        return toolError2(
...
async function runClick(deps, input, button, clicks) {
  ...
  try {
    ...
    const expectedPid = await resolveExpectedPid(
      deps,
      actionAppRef,
      frameProvenance.length > 0 ||
        (elementTarget && (args.app_ref === void 0 || args.app_ref === null)),
    );
    const elementStateId = elementTarget ? parsedClickTarget.state_id : void 0;
    const elementMutation = elementTarget
      ? toolSession(deps).beginMutation(
          actionAppRef,
    ...
    if (semanticAxPress) {
      ...
      try {
        ...
        return postActionObserve(deps, {
          note:
            (axResult === null || axResult === void 0 ? "Dispatched AXPress." : toText2(axResult)) +
            " (semantic AXPress on the element token; no coordinate hit-test.)",
        ...
        const actionProvenUnsent =
          details !== null &&
          typeof details === "object" &&
          !Array.isArray(details) &&
          details.action_sent === false;
    ...
    const effStrategy =
      elementTarget && (button === "left" || clicks > 1 || fellThroughActionUnavailable)
        ? "event"
        : strategy;
    try {
      if (resolved.fingerprint && resolved.native) {
        result = await callPointerAction(
          ...
          "click",
          {
            ...
            expected_window_id: (resolved as any).app_ref?.window_id,
            expected_bundle_id: (resolved as any).app_ref?.bundle_id,
          },
          [target],
        );
      } else {
      ...
      return postActionObserve(deps, {
        note: elementMutation
          ? "Element click may already have reached the target window."
          : "Coordinate click may already have reached the target window.",
        app_ref: actionAppRef,
        return_state: args.return_state,
        ...(elementMutation
          ? { stateConsistency: { mutation: elementMutation, dispatch } }
          : { dispatch }),
      });
    }
    if (elementMutation) {
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      const expectedPid = await resolveExpectedPid(
        ...
        scopedAppRef,
        frameProvenance.length > 0 ||
          (targetIsElement && (args.app_ref === void 0 || args.app_ref === null)),
      );
...
var leftClickDragTool = {
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      if (!(start as any).point || !(end as any).point) {
      ...
      const result = await callPointerAction(
        ...
        "drag",
        {
          start: (start as any).point,
          end: (end as any).point,
          modifiers,
          ...(frameProvenance.length > 0 ? { frame_provenance: frameProvenance } : {}),
          ...(expectedPid !== void 0
            ? { app_ref: verifiedPointerAppRef(scopedAppRef, expectedPid) }
...
var mouseMoveTool = {
  ...
  title: "Mouse Move",
  description:
    'Move the virtual pointer to a coordinate target without clicking (hover). ADVANCED/RAW. On macOS this is a background/window-scoped move and leaves the physical cursor unchanged; Linux/Windows retain their native pointer semantics. Coordinate only; an element target is refused (use left_click to actuate an element). Use integer pixels from the latest returned raster. Example: mouse_move(coordinate={"type":"coordinate","x":100,"y":100}). ' +
    SKILL_REF,
...
var leftMouseDownTool = {
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      const heldAppRef =
        verifiedPointerAppRef(scopedAppRef, expectedPid) ?? scopedAppRef ?? targetAppRef;
      if (deps.inputState) {
      ...
      const deliveryState =
        e !== null && typeof e === "object" && "details" in e && e.details?.request_delivery_state;
...
var leftMouseUpTool = {
  ...
  title: "Left Mouse Up",
  description:
    "Release the left mouse button previously pressed by this MCP session. CLEANUP RELEASE ONLY: it refuses when this session has not successfully called left_mouse_down, so an accidental call cannot release the user's own mouse press. return_state defaults to none; request compact/full only when fresh UI state is needed. The app scope is remembered from the matching left_mouse_down. Example: left_mouse_up(). " +
    SKILL_REF,
...
var cuaRequestAccessStatusSchema = z
  .object({
...
function keyboardIdentityConstraints(appRef2) {
  ...
  const constrained = ["pid", "bundle_id", "name", "window_id"].filter((field) => {
    ...
    return (
      (field === "pid" && typeof value === "number" && Number.isSafeInteger(value) && value > 0) ||
      (field === "window_id" &&
        typeof value === "number" &&
        Number.isSafeInteger(value) &&
        value !== 0) ||
      ((field === "bundle_id" || field === "name") &&
        typeof value === "string" &&
        value.trim().length > 0)
    );
  });
...
async function verifyCompoundKeyboardAppRef(deps, appRef2, method) {
  ...
  const app =
    captured && typeof captured === "object" && !Array.isArray(captured) ? captured.app : null;
  const live = app && typeof app === "object" && !Array.isArray(app) ? app : null;
  const window2 =
    captured && typeof captured === "object" && !Array.isArray(captured) ? captured.window : null;
  const liveWindow =
    window2 && typeof window2 === "object" && !Array.isArray(window2) ? window2 : null;
  const appMismatch =
    !live ||
    (typeof expected.pid === "number" && live.pid !== expected.pid) ||
    (typeof expected.bundle_id === "string" &&
      (typeof live.bundle_id !== "string" ||
        live.bundle_id.trim().toLowerCase() !== expected.bundle_id.trim().toLowerCase())) ||
    (typeof expected.name === "string" &&
      (typeof live.name !== "string" ||
        live.name.trim().toLowerCase() !== expected.name.trim().toLowerCase()));
  const windowMismatch =
    typeof expected.window_id === "number" &&
    (!liveWindow || liveWindow.window_id !== expected.window_id);
...
function toolError3(text, details?: unknown, _cause?: unknown, _meta?: unknown) {
  return details === void 0
    ? { isError: true, content: [textBlock3(text)] }
    : { isError: true, content: [textBlock3(text)], structuredContent: { ...(details as any) } };
}
...
function appRefHasIdentity(appRef2) {
  ...
  return (
    ...
    (typeof r.bundle_id === "string" && r.bundle_id.trim() !== "") ||
...
function typedTextNote(text) {
  ...
  return codePoints === utf16Units
    ? `Typed ${codePoints} character(s).`
    : `Typed ${codePoints} Unicode code point(s) (${utf16Units} UTF-16 code unit(s)).`;
}
async function defaultSleep2(seconds) {
  ...
  if ((cancelSignal as any)?.cancelled) {
...
async function typeViaAppOrFocus(deps, text, returnState, appRef2, strategy) {
  ...
  const params = { text };
  if (hasRef) (params as any).app_ref = processScopedKeyboardAppRef(appRef2);
  if (strategy === "event") (params as any).strategy = strategy;
  try {
    ...
    try {
      ...
      if (hasTarget) {
        ...
        if (nativeToken && args.strategy !== "event") {
          const fingerprint = (r.resolved as any).fingerprint ?? null;
          ...
          if (fingerprint) {
            try {
              ...
              return postActionObserve(deps, {
                note:
                  args.text.length === 0
                    ? "Cleared the target element value by typing an empty string."
                    : typedTextNote(args.text),
          ...
          try {
            ...
            return postActionObserve(deps, {
              note:
                (args.text.length === 0
                  ? "Cleared the target element value by typing an empty string."
                  : typedTextNote(args.text)) + hitNote,
...
` + SKILL_REF2,
  ...
  buildHandler: (deps) => async (input) => {
    ...
    const observedFingerprint = (r.resolved as any).fingerprint ?? null;
    const targetStateId = explicitElementStateId(args.target);
    const mutation = toolSession(deps).beginMutation(
      nativeAppRef,
      typeof targetStateId === "string" ? targetStateId : null,
...
var selectTextTool = {
  ...
  title: "Select Text",
  description:
    'Select a text range [start, length] in a text element, or place the caret. target must an element target ({"type":"element","state_id","index"}); a coordinate target is accepted. Background-safe via accessibility. Omit text_range to place the caret.\nExample: select_text(target={"type":"element","state_id":"s-1","index":1}, text_range=[0,3]).\n' +
    SKILL_REF2,
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      if (!observedFingerprint) {
        ...
        const selRange = (textRangeParam as any).text_range;
        const selIsCaret = !selRange || selRange[1] === 0;
        return postActionObserve(deps, {
          note:
            (selIsCaret
              ? `Caret placed at ${selRange ? selRange[0] : "current position"} (no text selected).`
              : `Text selected.`) + hitNote,
          app_ref: nativeAppRef,
          return_state: args.return_state,
          stateConsistency: {
            mutation,
      ...
      const fingerprint = (r.resolved as any).fingerprint;
      const requestedRange = (textRangeParam as any).text_range;
      if (
        requestedRange &&
        typeof (r.resolved as any).observedValue === "string" &&
        textRangeSplitsUtf16SurrogatePair((r.resolved as any).observedValue, requestedRange)
      ) {
        ...
        if (
          ...
          textRangeSplitsUtf16SurrogatePair(fresh.value, requestedRange)
        ) {
          return toolError3(
      ...
      const range = (textRangeParam as any).text_range;
      const isCaret = !range || range[1] === 0;
      const note = isCaret
        ? `Caret placed at ${range ? range[0] : "current position"} (no text selected).`
        : `Text selected.`;
      return postActionObserve(deps, {
        note,
        app_ref: r.resolved.app_ref ?? null,
...
var keyTool = {
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      if (hasAppRef) (params as any).app_ref = processScopedKeyboardAppRef(args.app_ref);
      if (args.strategy) (params as any).strategy = args.strategy;
      const note = clamped.clamped
        ? `Key pressed ${clamped.value} time(s) (requested ${requested}, clamped to 1..100).`
        : `Key pressed ${clamped.value} time(s).`;
      let dispatch;
...
var holdKeyTool = {
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      if (args.strategy) (params as any).strategy = args.strategy;
      const cancelSignal = currentBrokerCancelSignal();
      const unsubscribe = (cancelSignal as any)?.onCancel?.(() => {
      ...
      try {
        try {
          ...
          const detail = error51 instanceof Error ? error51.message : String(error51);
          const legacyCancelledHold =
            method === "hold_key_to_app" &&
            /hold_key_to_app: verified pid-scoped input(?: to \d+)? failed \(rejected\)/u.test(
              detail,
            );
      ...
      const note =
        bounded !== duration3
          ? `Key held for ${bounded}s (requested ${duration3}, clamped to 0..30).`
          : `Key held for ${bounded}s.`;
      return postActionObserve(deps, {
        note,
        app_ref: args.app_ref,
        return_state: args.return_state,
...
function resolveActionAlias(requested, allowed) {
  ...
  return (
    allowed.find((action) => family.includes(action.trim().toLowerCase().replaceAll("-", "_"))) ??
    requested
  );
}
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      return postActionObserve(deps, {
        note:
          `Dispatched ${action}.` +
          (action !== args.action ? ` (normalized from ${args.action})` : ""),
        app_ref: r.resolved.app_ref ?? null,
        return_state: args.return_state,
        stateConsistency: {
          mutation,
  ...
  buildHandler: (deps) => async (input) => {
    ...
    try {
      ...
      if (args.capabilities !== void 0 && args.capabilities !== null) {
        (params as any).capabilities = args.capabilities;
      }
      ...
      const result =
        brokerInfo.platform === "darwin"
          ? await callBroker(deps, "permission_status", {}, { kind: "read" })
          : await callBroker(deps, "request_access", params, { kind: "read" });
      if (!result || typeof result !== "object" || Array.isArray(result)) {
...
var stopComputerControlTool = {
  ...
  description:
    "Stop computer control for this session: refuses all further actions and releases a left mouse button held by this session (or recovers an orphaned button no session owns \u2014 e.g. a drag interrupted mid-gesture). Enforced, not just cooperative.",
