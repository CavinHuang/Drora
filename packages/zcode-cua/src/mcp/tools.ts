...
var TOOL_REGISTRY_MAP = new Map(TOOL_REGISTRY.map((t: any): [string, any] => [t.name, t]));
var TOOL_DEF_BY_NAME = new Map([
  ...observationTools.map((t: any): [string, any] => [t.name, t]),
  ...displayActionTools.map((t: any): [string, any] => [t.name, t]),
  ...pointerTools.map((t: any): [string, any] => [t.name, t]),
  ...keyboardRuntimeTools.map((t: any): [string, any] => [t.name, t]),
]);
...
function withKillSwitchPreflight(name, handler, deps) {
  return (input, extra) =>
    runWithTargetAppDisplayContext(async () => {
      ...
      const runInSession = () =>
        runWithPipToolRequestContext(requestContext, () =>
          deps.session ? runWithSession(deps.session, invoke) : invoke(),
        );
      const lifecycleSignal = deps.requestLifecycle?.signal;
      const signal =
        extra?.signal && lifecycleSignal
          ? AbortSignal.any([extra.signal, lifecycleSignal])
          : (extra?.signal ?? lifecycleSignal);
      const operation = signal
        ? runWithBrokerCancelSignal(brokerCancelSignalFromAbortSignal(signal), runInSession)
        : runInSession();
