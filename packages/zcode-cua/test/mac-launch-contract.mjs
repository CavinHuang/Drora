...
const cancelSentinel = join(cancelDir, `.broker-launch-cancel-${randomUUID()}.sentinel`);
const args = mod.JC(
  {
    appPath: helperApp,
    socketPath,
...
const expectedPrefix = [
  ...
  "--token-file",
  tokenFile,
  "--presentation-token-file",
  "/tmp/presentation-token",
  "--version",
  "3.11.2",
  "--expected-app-bundle-path",
  helperApp,
  "--broker-launch-deadline-epoch-ms",
  argsDeadline,
  "--broker-launch-cancel-file",
  cancelSentinel,
  "--exit-log",
  `${socketPath}.exit.log`,
  "--launcher-pid",
...
const runE2E =
  process.env.MAC_LAUNCH_CONTRACT_E2E !== "0" &&
  existsSync(join(helperApp, "Contents", "MacOS", "ZCode Computer Use"));
const launcherPid = (() => {
  let pid = process.pid;
...
if (runE2E) {
  ...
  assert.ok(
    pinged === "unknown" || typeof pinged === "string",
    "callBrokerMethod with token works",
  );
  let rejected = false;
  try {
  ...
  console.log(
    "C2. callBrokerMethod token passthrough (screen_capture_status): ok / wrong-token auth-rejected",
  );
  console.log("F. presentation role: pip gate passes with presentation token / tool role rejected");
  console.log(
    existsSync(exitLogPath)
      ? "   exit-log file written"
      : "   exit-log not yet flushed (non-fatal)",
  );
  ...
  if (process.env.MAC_LAUNCH_CONTRACT_OPEN === "1" && runE2E) {
    ...
    const pollAuth = async () => {
      try {
        const replies = await new Promise((res, rej) => {
          ...
          sock.on("connect", () => {
            sock.write(
              JSON.stringify({ id: 1, method: "authenticate", params: { token: openToken } }) +
                "\n",
            );
    ...
    assert.ok(
      openReplies?.[0]?.ok === true,
      "LS launch: token file delivered, authenticate passes",
    );
    assert.equal(
      openReplies[1]?.result?.bundle_id,
      "dev.zcode.cua-helper",
      "LS launch broker_info claims",
    );
    const helperPid = openReplies[1].result.pid;
    console.log(
      `G. real LaunchServices open: LS accepted args, token file delivered (helper pid=${helperPid})`,
    );
    process.kill(helperPid);
    setTimeout(() => {
      try {
        process.kill(helperPid, 9);
      } catch {}
    }, 2000);
...
const services = await import(resolve(packageRoot, "../../services/dist/node.js")).catch(
  () => null,
);
