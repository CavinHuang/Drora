...
const defaultEntryPath = join(repoRoot, "apps/drora-cli/packages/cli/dist/drora.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.DRORA_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "drora.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `drora.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `drora.bytecode-runtime-${runtimeHash}.cjs`;
