...
import {
  ...
  stageTuiRuntime,
} from "./drora-distribution/assets.mjs";
import { installScriptSource } from "./drora-distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "drora");
const defaultBaseUrl = (await loadEndpointEnv()).DRORA_DIST_BASE_URL?.trim() || "";
const packageDirName = "drora";
const usage = `Usage:
  pnpm build:drora
  node scripts/build-drora.mjs --skip-build
  node scripts/build-drora.mjs --version 3.3.3-dev.1
  node scripts/build-drora.mjs --out-dir dist/drora
  node scripts/build-drora.mjs --base-url http://host/drora/deps/drora/
...
Options:
  ...
  --out-dir <path>    Output directory. Defaults to dist/drora.
...
function run(command, args, options = {}) {
  console.log(`[drora] ${commandText(command, args)}`);
...
async function buildOutputs(skipBuild) {
  if (skipBuild) {
    console.log("[drora] skipping build; reusing existing outputs");
    return;
  }

  run("pnpm", ["--filter", "@drora/cli...", "build"]);
  await rm(resolve(root, "packages", "server", "dist"), {
    force: true,
    recursive: true,
  });
  run("pnpm", ["--filter", "@drora/server", "build"]);
  run("pnpm", ["--filter", "@drora/web", "build"]);
}

async function stageDroraPackage({ packageRoot, version }) {
  const webDist = resolve(root, "packages", "web", "dist");
  const serverDist = resolve(root, "packages", "server", "dist");
  const agentBundle = resolve(root, "apps", "drora-cli", "packages", "cli", "dist", "drora.cjs");
  const agentProvider = resolve(root, "apps/drora-cli/packages/cli/dist/provider");
  ...
  await assertFile(resolve(agentProvider, "drora-builtin.json"), "Agent provider config");
  ...
  await cp(agentBundle, resolve(packageRoot, "agent", "drora.cjs"));
  // TUI 入口通过真正的 CLI 路径定位伴随配置；只复制 JS 会在仓库外启动失败。
  await cp(agentProvider, resolve(packageRoot, "agent/provider"), { recursive: true });
  await cp(
    resolve(root, "apps/drora-cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
    resolve(packageRoot, "agent/THIRD-PARTY-NOTICES.md"),
  );
  await chmod(resolve(packageRoot, "agent", "drora.cjs"), 0o755);
  ...
  const runner = resolve(packageRoot, "bin", "drora.mjs");
  await cp(resolve(root, "scripts/drora-distribution/runner.mjs"), runner);
...
async function main() {
  ...
  if (!options.help && !options.baseUrl)
    throw new Error("Configure DRORA_DIST_BASE_URL in .env or pass --base-url");
  if (options.help) {
    console.log(usage);
    return;
  }
  ...
  if (!version || typeof version !== "string") {
    throw new Error("Unable to resolve Drora version.");
  }
  ...
  console.log(`[drora] release directory: ${outDir}`);
  console.log(`[drora] tarball: ${tarball}`);
  console.log(`[drora] sha256: ${sha256}`);
}
