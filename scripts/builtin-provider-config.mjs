...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.DRORA_ENV !== undefined) value = parsed.DRORA_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid DRORA_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.DRORA_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/drora-builtin.json",
  );
  try {
    ...
    const { decodeDroraBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/drora-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeDroraBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "drora-builtin.json"), config.content, "utf8");
  return config;
}
