  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    const child = spawn(command, spawnArgs, {
      ...
      env: withPinnedNodePath(
        {
          ...process.env,
          DRORA_ENV: requestedEnv,
          DRORA_DESKTOP_AGENT_BYTECODE: agentBytecode ? "1" : "0",
        },
        process.execPath,
      ),
      stdio: "inherit",
...
try {
  ...
  await run(pnpmCommand, ["--filter", "@drora/desktop", "pre-dev"]);
  ...
  await run(pnpmCommand, ["--filter", "@drora/desktop", "dev:runtime"]);
} catch (error) {
