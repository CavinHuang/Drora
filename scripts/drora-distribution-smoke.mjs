...
assert.ok(archive, "Usage: node scripts/drora-distribution-smoke.mjs <archive.tar.gz>");
const directory = await realpath(await mkdtemp(join(tmpdir(), "drora-release-smoke-")));
const root = join(directory, "drora");
const runner = join(root, "bin/drora.mjs");
const workspace = join(directory, "workspace");
const env = {
  ...process.env,
  DRORA_DATA_BASE_DIR: join(directory, "data"),
  NODE_PATH: "",
  NODE_OPTIONS: "",
  TERM: "xterm-256color",
};
...
try {
  ...
  await writeFile(
    runtimeCheck,
    'import { runTui } from "@drora/tui"; if (typeof runTui !== "function") throw new Error("Missing TUI export"); console.log("tui-runtime-ok");',
  );
  ...
  await until(
    () => /Drora/.test(screen) && /(?:登录|\/login|输入提示词|Type a prompt)/i.test(screen),
