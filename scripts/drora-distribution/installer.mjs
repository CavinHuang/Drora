...
BASE_URL="\${DRORA_DIST_BASE_URL:-${baseUrl}}"
INSTALL_DIR="\${DRORA_DIST_HOME:-$HOME/.drora/runtime}"
BIN_DIR="\${DRORA_DIST_BIN_DIR:-$HOME/.local/bin}"

need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "drora install requires $1" >&2
    exit 1
  fi
}
...
cat > "$BIN_DIR/drora" <<SH
#!/usr/bin/env sh
exec node "$INSTALL_DIR/current/bin/drora.mjs" "\\$@"
SH
chmod +x "$BIN_DIR/drora"

echo "Drora $VERSION installed."
echo "Run: drora (TUI) or drora --web (Web)"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
