...
const agentEntry = join(root, "agent", "drora.cjs");

function usage() {
  return `Usage:
  drora --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  drora --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      DRORA_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      DRORA_AGENT_SERVER_COMMAND: process.execPath,
      DRORA_SERVER_HOST: options.host,
      DRORA_SERVER_WORKSPACE: options.workspace,
      DRORA_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      DRORA_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("Drora Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: drora --web [options] (drora --web --help for details)\n");
    }
