...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "drora-native-search-build-"));
  ...
  console.log("==> Drora native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DDRORA_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DDRORA_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DDRORA_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DDRORA_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DDRORA_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DDRORA_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DDRORA_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DDRORA_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(
