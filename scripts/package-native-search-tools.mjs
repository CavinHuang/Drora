...
export async function packageNativeSearchTools({
  platform = process.env.DRORA_TARGET_OS || process.platform,
  arch = process.env.DRORA_TARGET_ARCH || process.arch,
...
async function main() {
  await packageNativeSearchTools({
    platform: readOption("platform") ?? process.env.DRORA_TARGET_OS ?? process.platform,
    arch: readOption("arch") ?? process.env.DRORA_TARGET_ARCH ?? process.arch,
