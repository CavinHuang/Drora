...
const DRORA_AGENT_RUNTIME = {
  glm: {
    version: readDroraAgentRuntimeVersion(),
  },
};
...
/**
 * Node dist 下载源。默认走国内镜像，`DRORA_NODE_DIST_MIRROR` 可覆盖（与
 * `.github/workflows` 的同名 CI 变量、`scripts/cua-helper-sea-base.mjs` 同一约定）。
 *
...
export function nodeDistBase(env = process.env) {
  const mirror = env.DRORA_NODE_DIST_MIRROR?.trim();
  return (mirror || DEFAULT_NODE_DIST_BASE).replace(/\/+$/u, "");
}
const BROWSER_USE_PLUGIN_PACKAGE_NAME = "@drora/browser-use-plugin";
// node_repl 宿主抽成独立包 @drora/node-repl-host 之后，browser-use
// 不再产出 dist/mcp/server.js，CUA 资产也已归 @drora/drora-cua-plugin。这是**第三份**平行清单
...
const remoteOfficialPluginPackages = [
  ...
  // 清单、packages/server/src/remote/droraAgentOfficialPluginAssets.ts 的远端合同保持一致。
  {
    // 远端 shared-host 必须部署 node_repl runtime，否则只剩 skill 而没有 mcp__node_repl__js ——
    // 该 runtime 现由 @drora/node-repl-host 提供（见下一个条目），browser-use 只带自己的
...
const remoteBundledSkillPack = {
  ...
  topLevelPaths: ["skills"], // 远端 bundle 不带 README（desktop 版含）；两处清单不同是有意的,
};
...
function readDroraAgentRuntimeVersion() {
  const runtimeSourcePath = join(rootDir, "packages/shared/src/drora-agent-runtime.ts");
  const runtimeSource = readFileSync(runtimeSourcePath, "utf8");
  const match = runtimeSource.match(/version:\s*["']([^"']+)["']/);
  if (!match?.[1]) {
    throw new Error("Unable to parse Drora Agent runtime version");
  }
  return match[1];
}
...
async function download(url, destinationPath) {
  ...
  const response = await fetch(url, {
    redirect: "follow",
    signal: AbortSignal.timeout(Number(process.env.DRORA_PREBUILD_DOWNLOAD_TIMEOUT_MS) || 300_000),
  });
  if (!response.ok) {
...
function downloadViaCurl(url, destinationPath) {
  const timeoutSec = Math.ceil(
    (Number(process.env.DRORA_PREBUILD_DOWNLOAD_TIMEOUT_MS) || 300_000) / 1000,
  );
  const curl = process.platform === "win32" ? "curl.exe" : "curl";
  let curlAvailable = false;
  try {
    curlAvailable =
      spawnSync(curl, ["--version"], { stdio: "ignore", timeout: 10_000 }).status === 0;
  } catch {
    curlAvailable = false;
  }
...
async function extractArchiveMember(url, destinationDir, archiveMember) {
  const tempDir = mkdtempSync(join(tmpdir(), "drora-node-dist-"));
...
async function prepareNodeBinaries() {
  for (const platformKey of remotePlatforms) {
    ...
    try {
      ...
      console.error(
        `  [error] 请检查 CI runner 的外网访问、tar/xz 依赖，或用 DRORA_NODE_DIST_MIRROR 覆盖下载源（当前 ${nodeDistBase()}）`,
      );
      throw error;
    }
  }
...
function runBootstrapServerRemoteBuild() {
  ...
  // 这里不能复用已有 drora-server.cjs：开发时 package version 常不变，旧 bundle 会把缺少新 RPC 的
...
function copyServerBundle() {
  ...
  copyFileSync(
    join(rootDir, "packages/server/dist/remote/drora-server.cjs"),
    join(serverDir, "drora-server.cjs"),
  );
  console.log("  [ok] mock-cdn server/drora-server.cjs");
}
...
function buildRemoteOfficialPluginRuntimes() {
  for (const plugin of remoteOfficialPluginPackages) {
    ...
    runCommand(
      pnpmCommand,
      ["--dir", join(rootDir, "apps/drora-cli"), "--filter", plugin.packageName, "build"],
      {
        cwd: rootDir,
        env: process.env,
      },
...
async function stageRemoteAgentBundles() {
  ...
  // browser-use runtime 的 tsc 依赖 @drora/core/dist。远端资产也必须先构建
  // agent CLI 依赖，避免 CI 干净检出时被开发机缓存掩盖的 TS2307。
  buildRemoteOfficialPluginRuntimes();
  const cliBundlePath = join(rootDir, "apps/drora-cli/packages/cli/dist/drora.cjs");
  ...
  for (const platformKey of remotePlatforms) {
    ...
    // 干净重建：glm 组件现在只含 drora.cjs，清掉历史遗留的原生二进制 / 旧 meta，
    // 避免被打进组件 tar 把远端资源撑大。
...
// glm 承载 drora-cli app-server 协议 schema。即使 runtime 版本未变化，
// drora.cjs 也可能随 app 代码变更；跨 release 复用旧 glm 会让远端 agent 拒绝新协议字段。
...
function buildReusableComponentRequiredPaths(componentId, platformKey) {
  switch (componentId) {
    ...
    case "glm":
      // GLM 现在是编译产物 drora.cjs（跨平台同一份），远端用已部署的 node 执行它。
      // 复用时还要确认官方插件 seed 资源完整，否则旧 release 会继续产出 0 builtin plugin 的远端资源包。
      return ["drora.cjs", ...remoteOfficialPluginRequiredPaths];
    case "bfs":
      return ["bfs"];
    case "ripgrep":
...
export function buildRemoteComponentDefinitions(platformKey) {
  const baseComponents = [
    ...
    },
    {
      ...
      // 这里复用 DRORA_AGENT_RUNTIME.glm.version，保持 manifest 版本与运行时描述一致。
      semanticPrefix: DRORA_AGENT_RUNTIME.glm.version,
